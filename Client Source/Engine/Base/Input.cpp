...
//0105
	...
	m_bTcpIp = FALSE;//TRUE;////ÀÌ°Ç TCP¿Í »ó°ü¾øÀÌ ¸¶¿ì½º È¸Àü¿¡ °üÇØ¼­. ¼öÁ¤·çÆ¾ÀÌ TRUE
	m_WheelPos = 0;
			...
			// if snooping messages
				...
				{ 
					if(!_pInput->m_bPressedRMouse){
						...
						_pInput->m_bPressedRMouse = TRUE; //press »óÅÂÀÌ´Ù.
						if(!_pInput->IsRMousePressed()){	//Åä±Û			
							_pInput->SetRMousePressed(TRUE);						
						}else{
							_pInput->SetRMousePressed(FALSE);	
						}					  
					}else //°è¼Ó press»óÅÂÀÌ´Ù.
					{					  
					}			  
				}
				else{ //unPress»óÅÂ°¡ µÇ¾ú´Ù.
				...
				{ 
					if(!_pInput->m_bPressedLMouse){
						...
						_pInput->m_bPressedLMouse = TRUE; //press »óÅÂÀÌ´Ù.
						if(!_pInput->IsLMousePressed()){	//Åä±Û			
				...
				{ 
					if(!_pInput->m_bPressedHome){
						...
						_pInput->m_bPressedHome = TRUE; //press »óÅÂÀÌ´Ù.
						if(!_pInput->IsHomePressed()){	//Åä±Û			
				...
				if(_abKeysPressed[KID_1]){//
					if(!_pInput->m_bPressed1Key){
						...
						if(!_pInput->IsNum1KeyPressed()){	//Åä±Û
				...
				if(_abKeysPressed[KID_2]){
					if(!_pInput->m_bPressed2Key){
						...
						if(!_pInput->IsNum2KeyPressed()){	//Åä±Û
				...
				if(_abKeysPressed[KID_3]){
					if(!_pInput->m_bPressed3Key){
						...
						if(!_pInput->IsNum3KeyPressed()){	//Åä±Û
				...
				if(_pNetwork->m_ubGMLevel > 1)
				{
					...
					if (_abKeysPressed[KID_LCONTROL]) { //LÄÁÆ®·Ñ Å°°¡ ´­·¯Á®ÀÖ´Ù¸é,
						if(!_pInput->m_bPressedLControlKey ){//unPress»óÅÂ¿¡¼­ Ã³À½ PressµÈ°Å¶ó¸é,
							//_pInput->m_bLControlKey = TRUE; 
							_pInput->m_bPressedLControlKey = TRUE; //press »óÅÂÀÌ´Ù.
							if(!_pInput->IsLControlKeyPressed()){	//Åä±Û			
					...
					if(_abKeysPressed[KID_R]){
						if(!_pInput->m_bPressedRKey){//unPress»óÅÂ¿¡¼­ Ã³À½ PressµÈ°Å¶ó¸é,
							_pInput->m_bPressedRKey = TRUE; //press »óÅÂÀÌ´Ù.
							if(!_pInput->m_bRKey){	//Åä±Û
					...
					if (_abKeysPressed[KID_LCONTROL]) { //LÄÁÆ®·Ñ Å°°¡ ´­·¯Á®ÀÖ´Ù¸é,
						...
						if(_abKeysPressed[KID_4]){
							if(!_pInput->m_bPressed4Key){
								...
								if(!_pInput->IsNum4KeyPressed()){	//Åä±Û
					...
					if(_abKeysPressed[KID_I]){
						if(!_pInput->m_bPressedIKey){//unPress»óÅÂ¿¡¼­ Ã³À½ PressµÈ°Å¶ó¸é,
							_pInput->m_bPressedIKey = TRUE; //press »óÅÂÀÌ´Ù.
							if(!_pInput->m_bIKey){	//Åä±Û
					...
					if(_abKeysPressed[KID_L]){
						if(!_pInput->m_bPressedLKey){//unPress»óÅÂ¿¡¼­ Ã³À½ PressµÈ°Å¶ó¸é,
							_pInput->m_bPressedLKey = TRUE; //press »óÅÂÀÌ´Ù.
							if(!_pInput->m_bLKey){	//Åä±Û
					...
					if(_abKeysPressed[KID_J]){
						if(!_pInput->m_bPressedJKey){//unPress»óÅÂ¿¡¼­ Ã³À½ PressµÈ°Å¶ó¸é,
							_pInput->m_bPressedJKey = TRUE; //press »óÅÂÀÌ´Ù.
							if(!_pInput->m_bJKey){	//Åä±Û
					...
					if(_abKeysPressed[KID_K]){
						if(!_pInput->m_bPressedKKey){//unPress»óÅÂ¿¡¼­ Ã³À½ PressµÈ°Å¶ó¸é,
							_pInput->m_bPressedKKey = TRUE; //press »óÅÂÀÌ´Ù.
							if(!_pInput->m_bKKey){	//Åä±Û
					...
					if(_abKeysPressed[KID_P]){
						if(!_pInput->m_bPressedPKey){//unPress»óÅÂ¿¡¼­ Ã³À½ PressµÈ°Å¶ó¸é,
							_pInput->m_bPressedPKey = TRUE; //press »óÅÂÀÌ´Ù.
							if(!_pInput->m_bPKey){	//Åä±Û
					...
					if(_abKeysPressed[KID_TAB]){
						if(!_pInput->m_bPressedTabKey){//unPress»óÅÂ¿¡¼­ Ã³À½ PressµÈ°Å¶ó¸é,
							//_pInput->m_bLControlKey = TRUE;
							_pInput->m_bPressedTabKey = TRUE; //press »óÅÂÀÌ´Ù.
							if(!_pInput->m_bTabKey){	//Åä±Û
...
static void ReadMouseDevice(CInputDevice &idDevice, BOOL bPreScan)
{
	...
	// start seo
	extern BOOL _bInTestGame;
	if (!_bWorldEditorApp || _bInTestGame)		//Å×½ºÆ® °ÔÀÓÀÏ ¶§ ¾Æ·¡ ·çÆ¾À» µé¾î°¡°Ô ¼öÁ¤ :Su-won
	//if (0) 
	{
	...
	//..
		...
		//0105
				if(!_pInput->m_bTcpIp || _pInput->inp_bFreeMode ||
					(_pInput->m_bTcpIp &&_abKeysPressed[KID_MOUSE2] && !_pInput->inp_bFreeMode)){//¸¶¿ì½º ÈÙ¹öÆ° DOWN //1211
	...
	//0105 ÈÙ¸¶¿ì½º ´©¸¥»óÅÂ¿¡¼­´Â ¸¶¿ì½º°¡ Áß¾ÓÀ¸·Î...
		if(_pInput->m_bTcpIp){
			if(_abKeysPressed[KID_MOUSE2]){//¸¶¿ì½º ÈÙ¹öÆ° DOWN	
...
// end seo
	...
	//..
			...
			FLOAT fMouseRelZ = _iMouseZ;
	//0105
			if(!_pInput->m_bTcpIp || (_pInput->m_bTcpIp &&_abKeysPressed[KID_MOUSE2])){//¸¶¿ì½º ÈÙ¹öÆ° DOWN //1211
			idDevice.id_aicControls[0].ic_fValue =  Max(0.0f,fMouseRelX*3/4);
			idDevice.id_aicControls[1].ic_fValue = -Min(0.0f,fMouseRelX*3/4);
			idDevice.id_aicControls[2].ic_fValue =  Max(0.0f,fMouseRelY*3/4);
			idDevice.id_aicControls[3].ic_fValue = -Min(0.0f,fMouseRelY*3/4);
	...
	//0105 ¿ìÃø ¸¶¿ì½º ´©¸¥»óÅÂ¿¡¼­´Â ¸¶¿ì½º°¡ Áß¾ÓÀ¸·Î...
		if(_pInput->m_bTcpIp){
			if(_abKeysPressed[KID_MOUSE2]){//¸¶¿ì½º ¿ìÃø ¹öÆ° DOWN	
