...
extern FLOAT _bCheckAllAllocations = FALSE;
 //new2022
static void DefaultErrorInsufficientMemoryCallback(SLONG slMemoryRequired, DWORD error);
static void (*_pErrorInsufficientMemoryCallback)(SLONG slMemoryRequired, DWORD error) = DefaultErrorInsufficientMemoryCallback;
...
void *MEM_MAlloc( size_t size )
{
	...
	if (pmem==NULL) {
		...
		if(_pErrorInsufficientMemoryCallback!=NULL) {
			_pErrorInsufficientMemoryCallback(size, errno); //new2022
		}
...
extern void SetErrorInsufficientMemoryCallback(void(*pErrorInsufficientMemoryCallback)(SLONG slMemoryRequired, DWORD error))
{
	_pErrorInsufficientMemoryCallback = pErrorInsufficientMemoryCallback;
}

// Default function that will be caller on out of memory error //new2022
static void DefaultErrorInsufficientMemoryCallback(SLONG slMemoryRequired, DWORD error)
{
	FatalError(TRANS("Not enough memory (%d bytes needed)! Error: %s(code %d)[DefaultErrorInsufficientMemoryCallback]"), slMemoryRequired, strerror(error), error);
}
