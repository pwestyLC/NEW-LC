...
{
	// Use direct file I/O instead of CTFileStream to avoid virtual memory paging issues
	CTFileName fnmFull;
	ExpandFilePath(EFP_READ, fnmPS, fnmFull);

	FILE *fp = fopen(fnmFull.str_String, "rb");
	if (fp == NULL) {
		ThrowF_t("Cannot open persistent symbol file '%s'", (const char*)fnmPS);
	}

	INDEX len = -1;
	if (fread(&len, sizeof(INDEX), 1, fp) != 1 || len <= 0 || len > 10 * 1024 * 1024) {
		fclose(fp);
		ThrowF_t("Invalid length in persistent symbol file '%s'", (const char*)fnmPS);
	}
