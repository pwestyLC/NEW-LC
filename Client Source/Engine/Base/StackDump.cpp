...
static void IntelStackWalk(HANDLE hFile, PCONTEXT pContext)
{
	...
	size_t pc = pContext->Rip;
	PDWORD_PTR pFrame, pPrevFrame;
	pFrame = (PDWORD_PTR)pContext->Rbp;
#else
	...
	for(INDEX iDepth=0; iDepth<100; iDepth++) {
		...
		if((size_t)pFrame&3) { // Frame pointer must be aligned on a
...
static void SendErrorInfo( HANDLE hFile, CTString &csContents )
{
	...
	hConnect = InternetConnect( hOpen, "https://www.darkagelc.com/", INTERNET_DEFAULT_HTTP_PORT,
...
extern BOOL GetLogicalAddress(void *pAddr, char *szModule, ULONG ulLen, ULONG &ulSection, ULONG &ulOffset)
{
	...
	size_t hMod = (size_t)mbi.AllocationBase;
	...
	size_t rva = (size_t)pAddr - hMod; // RVA is offset from module load address
	
	// Iterate through the section table, looking for the one that encompasses
	// the linear address.
	for ( unsigned int i=0;i<pNtHdr->FileHeader.NumberOfSections;i++,pSection++) {
		size_t sectionStart = pSection->VirtualAddress;
		size_t sectionEnd = sectionStart + max(pSection->SizeOfRawData, pSection->Misc.VirtualSize);
