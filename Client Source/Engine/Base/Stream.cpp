...
// [070531: Su-won] ������ �������� 128MB�� �޸𸮸� ��û�Ͽ� ������ �۾��� �޸� ���� ���� �߻�.
// �ִ� ���� ũ�⸦ 128MB�� �� �ʿ䰡 ���� �� �Ͽ� 64MB�� ����. 
//ULONG _ulMaxLengthOfSavingFile = (1UL<<20)*128;  //pwesty
ULONG _ulMaxLengthOfSavingFile = (1UL<<20)*64;
  ...
  {for(INDEX iFileName=0; iFileName<ctFileNames; iFileName++) {
    ...
    try {
	  ...
	  /* else if (strExt==".bm") {
		...
		FatalError(TRANS("[%s][%s] File is not exist!!! Error: %s\n"), strExt, (CTString&)fnm, strError);
    }
  }}
}
...
void CTFileStream::Open_t(const CTFileName &fnFileName, CTStream::OpenMode om/*=OM_READ*/)
{
	...
	else if( om == OM_WRITE)
	{
		...
		// technique)
		AllocateVirtualMemory( _ulMaxLengthOfSavingFile+_ulPageSize);
	}
	// if unknown mode
	else
	{
...
CTMemoryStream::CTMemoryStream(void)
{
  ...
  // allocate amount of memory needed to hold maximum allowed file lenght (when saving)
  AllocateVirtualMemory( _ulMaxLengthOfSavingFile);
  mstrm_ctLocked = 0;
