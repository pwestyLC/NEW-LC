...
static CTCriticalSection& _GetNullCriticalSection(void)
{
    static CTCriticalSection s_NullCriticalSection;
    return s_NullCriticalSection;
}
...
CTSingleLock::CTSingleLock(CTCriticalSection *pcs, BOOL bLock) : sl_cs(pcs ? *pcs : _GetNullCriticalSection())
{
...
{
#ifdef UI_TOOL
    if (&sl_cs==&_GetNullCriticalSection()) {
        sl_bLocked = FALSE;
        return;
    }
#endif
