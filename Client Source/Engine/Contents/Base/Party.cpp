...
void Party::RecvPartyDungeonRecall(SBYTE sbType, CNetworkMessage* istr)
{
	CUIManager* pUIMgr = CUIManager::getSingleton();

	switch (sbType)
	{
	case MSG_PARTYDUNGEON_RECALL_RECV:
	{
		CTString BossName;
		int zoneidx;
		...
		strzonename = CZoneInfo::getSingleton()->GetZoneName(zoneidx);

		CUIMsgBox_Info	MsgBoxInfo;
		MsgBoxInfo.SetMsgBoxInfo(_S(97, "파티"), UMBS_YESNO | UMBS_USE_TIMER, UI_PARTY, MSGCMD_CLOUD_PARTY_DUNGEON_RECALL);

		CTString strMessage;

		strMessage.PrintF(_S(RECALL_STRING_IDX, "Party Leader [%s] has started an Expedition, Do you want to move to the %s Expedition?"), BossName, strzonename);

		MsgBoxInfo.AddString(strMessage);
		MsgBoxInfo.SetMsgBoxTimer(60, FALSE);			// 30 seconds.
		CUIManager::getSingleton()->CreateMessageBox(MsgBoxInfo);
...
void Party::RecvDungeonDetail(SBYTE sbType, CNetworkMessage* istr)
{
	CUIManager* pUIMgr = CUIManager::getSingleton();
	...
	case MSG_PARTYDUNGEON_SEND:
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			pUIMgr->GetExpeditionPartyDungeon()->SetDetailInfo(i, nCount, zone, difficulty, level, cp, needitem, needcount, partymember, time, rewardindex, rewardindex1, rewardindex2, rewardindex3, rewardindex4, title, box);
		}
		if (pUIMgr->GetExpeditionPartyDungeon()->IsVisible())
			pUIMgr->GetExpeditionPartyDungeon()->UpdateDungeonInfo();
		else
			pUIMgr->GetExpeditionPartyDungeon()->OpenUI();
	}
	break;
	}
}
