...
UINT MsgBoxMgr::CreateMsgBoxNEW(const char* strTitle, const char* strContent, eMSG_TYPE eType, bool bDestroy, _ui_func pCmdOk, _ui_func pCmdNo, _ui_func pCmdClose, CTString* oktxt, CTString* closetxt)
{
	...
	case eMSG_OK:
	{
		...
		if (pMsgBox != NULL)
		{
			...
			pMsgBox->initialize();
			SetMsgBoxInfoNEW(pMsgBox, m_IDGenerator, nX, nY, strTitle, strContent, pCmdOk, pCmdNo, pCmdClose, bDestroy);
	...
	case eMSG_YESNO:
	{
		...
		if (pMsgBox != NULL)
		{
			...
			pMsgBox->initialize();
			SetMsgBoxInfoNEW((CUIMsgBox*)pMsgBox, m_IDGenerator, nX, nY, strTitle, strContent, pCmdOk, pCmdNo, pCmdClose, bDestroy);

			_map.insert(std::make_pair(m_IDGenerator, (CUIMsgBox*)pMsgBox));
		}
		if (oktxt != NULL)
		{
			CUIButton* okbtn = (CUIButton*)pMsgBox->findUI("btn_ok");
			okbtn->SetText(*oktxt);
		}
		if (closetxt != NULL)
		{
			CUIButton* cancelbtn = (CUIButton*)pMsgBox->findUI("btn_cancle");
			cancelbtn->SetText(*closetxt);
		}
		CUIButton* closebtn = (CUIButton*)pMsgBox->findUI("btn_close");
...
void MsgBoxMgr::SetMsgBoxInfoNEW(CUIMsgBox* pMsgBox, UINT nID, int x, int y, const char* strTitle, const char* strContent, _ui_func pCmdOk /*= NULL*/, _ui_func pCmdNo /*= NULL*/, _ui_func pCmdClose /*= NULL*/, bool bDestroy /*= TRUE*/)
{
	if (pMsgBox == NULL)
		return;
	...
	if (pCmdOk != NULL)
		pMsgBox->SetOkFCommand(pCmdOk);

	CmdMsgEvent* pCmdOkEvent = new CmdMsgEvent;
	pCmdOkEvent->setData(nID, bDestroy);
	pMsgBox->SetOkCommand(pCmdOkEvent);


	if (pCmdClose != NULL)
		pMsgBox->SetCloseFCommand(pCmdClose);

	CmdMsgEvent* pCmdCloseEvent = new CmdMsgEvent;
	pCmdCloseEvent->setData(nID, bDestroy);
	pMsgBox->SetCloseCommand(pCmdCloseEvent);

	if (pCmdNo != NULL)
		pMsgBox->SetNoFCommand(pCmdNo);
	else if (pCmdClose != NULL)
		pMsgBox->SetNoFCommand(pCmdClose);

	CmdMsgEvent* pCmdNoEvent = new CmdMsgEvent;
	pCmdNoEvent->setData(nID, bDestroy);
	pMsgBox->SetNoCommand(pCmdNoEvent);
}
