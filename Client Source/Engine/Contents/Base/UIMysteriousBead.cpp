...
int CUIMysteriousBead::SetPutOnItemData()
{
	...
	if (pDrag->getIndex() == pUIManager->GetInventory()->GetItemIndex(m_UseBeadInfo.tab, m_UseBeadInfo.invenIndex))
		return eERR_SUB_USING_ITEM;	// ���� ����ϰ� �ִ� ���� �������� �߽� �Ұ�.

	if (pItems->InvenIndex < 0)
		return eERR_SUB_NOT_IN_THE_INVEN;	// �κ��丮�� ���� �������� �߽� �Ұ�.

	if (pDrag->getBtnType() != UBET_ITEM )
		return eERR_SUB_ITEM_IS_NOT;	// �������� �ƴ϶�� �߽� �Ұ�.

	if (pItems->IsFlag(FLAG_ITEM_LENT))
		return eERR_SUB_ITEM_IS_LENT;	// ��Ʈ ������ �߽� �Ұ�.
	...
	if ( !(pItemData->GetFlag() & ITEM_FLAG_EXCHANGE) )
		return eERR_SUB_ITEM_IS_UNTRADEABLE;	// ��ȯǰ���� �ƴ϶�� �߽� �Ұ�.
	...
	if( pItemData->GetFlag() & ITEM_FLAG_COUNT )	// ���� �ִ� ������
	{
		...
		pCmd->setData(this);
		UIMGR()->GetMsgBoxNumOnly()->SetInfo(pCmd, _S(191,"Ȯ��"), strMessage, 1, pItems->Item_Sum);
	}
	else	// �׿�
	{
		m_pSendItems = new CItems;
...
void CUIMysteriousBead::SendItemMsgBox()
{
	...
	MsgBoxInfo.SetMsgBoxInfo( _S( 191, "Ȯ��" ), UMBS_OKCANCEL, UI_MYSTERYOUSBEAD, MSGCMD_MYSTERIOUS_BEAD_SEND_ITEM );
	strCount = pUIManager->IntegerToCommaString(nCount);
	strMessage.PrintF( _S(6046, "%s %s���� %s�� �ź��� �������� ���� �Ͻðڽ��ϱ�?"), szItemName, strCount, strName);
...
void CUIMysteriousBead::Success()
{
	...
	strCount = pUIManager->IntegerToCommaString(nCount);
	strMessage.PrintF(_S(6047, "%s %s���� %s�� �ź��� �������� ���� �Ͽ����ϴ�."), szItemName, strCount, strName);
	MsgBoxInfo.AddString( strMessage );
	pUIManager->CreateMessageBox( MsgBoxInfo );
	CloseUI();
}
...
void CUIMysteriousBead::ErrMessage(int type)
{
	...
	case eERR_INVALID_NAME:
		{
			strMessage.PrintF( _S( 2500, "�߸��� �̸��Դϴ�.") );
		}
		break;
	...
	case eERR_ITEM_COUNT_OVER:
		{
			strMessage.PrintF( _S( 6109, "������ ���Կ� �����ϴ� �������� �ִ������ �ѱ� �� �����ϴ�.") );
		}
		break;
	}
