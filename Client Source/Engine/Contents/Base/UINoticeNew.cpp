...
void CUINoticeNew::PressOK(Notice::eNoticeType iNoticeType, int nIndex)
{
	...
	case Notice::NOTICE_AFFINITY:	// 친화도 보상
		...
		break;
#ifdef SHOW_PROMO_KEY
	case Notice::NOTICE_PROMO_KEY:	// 일반 공지
		{
			pData->SetNoticeBlink(iNoticeType, false);
			btnType = UBET_QUEST;
		}
...
#ifdef SHOW_PROMO_KEY
			else if (iNoticeIndex == NEWS_PROMO)
			{
				pUIManager->GetPromoKeyInputUI()->ToggleVisible();
				break;
			}
#endif
...
void CUINoticeNew::UpdateBtn()
{
	...
	CUIText* pText;
#ifdef SHOW_PROMO_KEY
	pVec = pData->GetNoticeList(Notice::NOTICE_PROMO_KEY);
	if (pVec != NULL)
	{
		...
		if (pIcon != NULL)
		{
			...
			pIcon->SetHighLightMode(pData->GetNoticeBlink(Notice::NOTICE_PROMO_KEY));

			CTString strTooltip;
			strTooltip.PrintF(_s("Promo key"));
			...
			pCmd->setData(this, Notice::NOTICE_PROMO_KEY, NEWS_PROMO);
...
void CUINoticeNew::ShowRoyalRumbleWinner(const INDEX& iLevel, const CTString& strNickName, const INDEX& iZone)
{
	CTString strTemp;
	CTString strType;
	...
	{
		strType = CZoneInfo::getSingleton()->GetZoneName(iZone);//_S(20223, "Black castle");
		strDifficulty = _S(8725, "Normal");
	}
	else if (iLevel == 61)
	{
		strType = CZoneInfo::getSingleton()->GetZoneName(iZone);
		strDifficulty = _S(8726, "Hard");
	}
	else if (iLevel == 62)
	{
		strType = CZoneInfo::getSingleton()->GetZoneName(iZone);
		strDifficulty = _S(8727, "Hell");
	}

	if (iLevel >= 60)
		strTemp.PrintF(_S(9382, "%s has successfully completed the %s(%s) expedition."), strNickName, strType, strDifficulty);
	else
#endif
