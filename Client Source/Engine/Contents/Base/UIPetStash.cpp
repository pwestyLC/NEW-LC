...
//ken//new2022
#ifdef PET_BUFF_ALL_SYSTEM
	m_pbtnBuffAll = (CUIButton*)findUI("btn_buff_all");
	if ( m_pbtnBuffAll != NULL )
	{
		CmdPetBuffAll* pCmd = new CmdPetBuffAll;
		pCmd->setData(this);
		m_pbtnBuffAll->SetCommandUp( pCmd );
	}

	m_pIconBuffAll = (CUIIcon*)findUI("icon_buff_all");
	if (m_pIconBuffAll != NULL)
	{
		m_pIconBuffAll->clearIconData();
	}
#endif
//new2022// //TITLE_DELETE
...
//ken//new2022
	...
	if (pUIManager->GetDragIcon() != NULL)
	{
		...
		{
			if (m_pIconBuffAll != NULL && 
				m_pIconBuffAll->IsInside(x, y) == TRUE)
			{
				...
				if (pItemData->getindex() == 20654)
				{
					m_pIconBuffAll->copyItem(pDrag);
				}
				else
...
//ken//new2022
#ifdef PET_BUFF_ALL_SYSTEM
void CUIPetStash::UsePetAllBuffs()
{
	CUIManager* pUIManager = CUIManager::getSingleton();

	if (m_pIconBuffAll->IsEmpty())
	{
		pUIManager->GetChattingUI()->AddSysMessage( _S( 9383, "pet stash inventory empty, nothing to use" ), SYSMSG_ERROR );
		return;
	}

	CItems* pItem = m_pIconBuffAll->getItems();
	if (pItem != NULL)
		_pNetwork->UsePetAllBuffs( pItem->Item_Tab, pItem->InvenIndex );
}
#endif
//new2022//