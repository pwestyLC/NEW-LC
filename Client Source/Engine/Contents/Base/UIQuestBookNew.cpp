...
extern INDEX g_iQuestAutoDelivery;//Cloud auto quest
#endif
...
#ifdef CLOUD_AUTO_QUEST_DELIVERY
class CmdQuestBookAutoDelivery : public Command//Cloud auto quest
{
public:
	CmdQuestBookAutoDelivery() : pUI_(NULL) {}
	void setData(CUIQuestBook* pUI)	{ pUI_ = pUI; }
	void execute() {
		if (pUI_)
			pUI_->PressCheckAutoDelivery();
	}
private:
	CUIQuestBook* pUI_;
};
...
CUIQuestBook::CUIQuestBook()
	...
	, m_pbtnGiveUp(NULL)
#ifdef CLOUD_AUTO_QUEST_DELIVERY
	, m_pBtnAutoDelivery(NULL)//Cloud auto quest
#endif
	, m_pDesign(NULL)
	, m_plistScroll(NULL)
	, m_pstrQuestCnt(NULL)
...
#ifdef CLOUD_AUTO_QUEST_DELIVERY
	, m_bAutoSave(FALSE)//Cloud auto quest
#endif
{
	int i;
...
void CUIQuestBook::RefreshQuestListNew()
{
	...
	if (pText != NULL)
	{
		strTemp.PrintF(_S( 1707, "�������� ����Ʈ (%d)" ), nProceedCount);
	...
	if (pText != NULL)
	{
		strTemp.PrintF(_S( 1708, "�Ϸ�� ����Ʈ (%d)" ), nCompleteCount);
		pText->InitPos(20, 0, 260, 16);
		pText->SetText(strTemp);
	}
	...
	if (pText != NULL)
	{
		strTemp.PrintF(_S( 4429, "���̵� ���� �޽��� (%d)" ), nRaidCount);
		pText->InitPos(20, 0, 260, 16);
		pText->SetText(strTemp);
	}
...
void CUIQuestBook::UpdateQuestContent( int nQuestIdx )
{
	...
	{		
		if(m_pImageArr[i] != NULL)
		{
			...
			strTooltip.PrintF(_S(nToolTipIdx[i], "Ÿ�� ����"));
			...
			else
			{
				...
				{	
					if (pQuestDD->GetPrizeType(i-1) == CUIImageBox::IT_CORPS)
						pImgBox->SetImageByType(CUIImageBox::IT_EXP, pQuestDD->GetPrizeIndex(i-1), pQuestDD->GetSyndicateType());
					else
	...
	if (pQuestDD->GetPrizeNPCIndex() != 0 && GAMEDATAMGR()->GetQuest()->IsCompleteQuest(pQuestDD->GetQuestIndex()))
	{
		if(GetSaveAutoQuest())//for optionprize quests or normal quests when get inven full
		{
			if(m_nSelectQuestIdx)
			{
				if (m_pbtnOk != NULL)
...
#ifdef CLOUD_AUTO_QUEST_DELIVERY
	{//Cloud auto quest
		if(GetSaveAutoQuest() /*&& pDD->IsOptionPrizeExist()*/)//for optionprize quests or normal quests when get inven full
		GAMEDATAMGR()->GetQuest()->SendQuestPrize(m_nSelectQuestIdx, -1, iItemIndex, iItemPlus);
		else
...
#ifdef CLOUD_AUTO_QUEST_DELIVERY
	m_pBtnAutoDelivery = (CUICheckButton*)findUI("btn_autodelivery");//Cloud auto quest

	if (m_pBtnAutoDelivery != NULL)
	{
		CmdQuestBookAutoDelivery* pCmd = new CmdQuestBookAutoDelivery;
		pCmd->setData(this);
		m_pBtnAutoDelivery->SetCommand(pCmd);
		m_bAutoSave = g_iQuestAutoDelivery;
	    m_pBtnAutoDelivery->SetCheck( GetSaveAutoQuest() );
	}
#endif
...
void CUIQuestBook::GiveUpPress()
{
	...
	CTString	strMessage;
	CUIMsgBox_Info	MsgBoxInfo;	
	MsgBoxInfo.SetMsgBoxInfo( _S( 99, "����Ʈ" ), UMBS_OKCANCEL, UI_QUESTBOOK_LIST, MSGCMD_QUEST_GIVEUP);	
	strMessage.PrintF( "%s", pQuestDD->GetTitle() );
	MsgBoxInfo.AddString( strMessage );
	strMessage.PrintF( _S( 7002, "����Ʈ�� ���� �Ͻðڽ��ϱ�?" ));
	MsgBoxInfo.AddString( strMessage );
	pUIManager->CreateMessageBox( MsgBoxInfo );
...
#ifdef CLOUD_AUTO_QUEST_DELIVERY
void CUIQuestBook::PressCheckAutoDelivery()//Cloud auto quest
{
	
	if (m_pBtnAutoDelivery != NULL)
	{
		BOOL bChecked =	m_pBtnAutoDelivery->IsChecked() ? TRUE : FALSE;
		
		g_iQuestAutoDelivery = bChecked;
		
		m_bAutoSave = bChecked;
		m_pBtnAutoDelivery->SetCheck( bChecked );
	}
}
#endif