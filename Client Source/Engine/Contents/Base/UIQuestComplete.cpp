...
#include <Engine/Contents/Base/UIQuestBookNew.h>//Cloud auto quest
#endif
//--------------------------------------------------------------
// �Ϸ�â �ݱ�
...
void CUIQuestComplete::PrizeQuest()
{
	...
	{//Cloud auto quest
		...
		if(pUIManager->GetQuestBookList()->GetSaveAutoQuest() /*&& pDD->IsOptionPrizeExist()*/)//for optionprize quests or normal quests when get inven full
		GAMEDATAMGR()->GetQuest()->SendQuestPrize(m_nQuestIdx, -1, iItemIndex, iItemPlus);
		else
