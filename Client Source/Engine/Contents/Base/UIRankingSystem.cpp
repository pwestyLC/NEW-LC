...
CTString CUIRankingSystem::GetJobStringByType(UBYTE ubClass)
{
	...
	case NIGHTSHADOW:
		cstrTemp	= _S(4410, "����Ʈ�ε���");
		break;

#ifdef CHAR_EX_ROGUE
	case EX_ROGUE:	// [2012/08/27 : Sora] EX�α� �߰�
		cstrTemp	= _S( 5732, "EX�α�" );
		break;
#endif
#ifdef  CHAR_EX_MAGE
...
CTString CUIRankingSystem::GetRankStringByType(ERANKINGEX_SUB_TYPE eSubType)
{
	...
	case ERANKINGEX_SUB_TYPE_LEVELCLASS_NIGHTSHADOW:
		return _S(5196, "����Ʈ�ε��� ��ŷ");

#ifdef CHAR_EX_ROGUE
	case ERANKINGEX_SUB_TYPE_LEVELCLASS_EX_ROGUE:	// [2012/08/27 : Sora] EX�α� �߰�
		return _S( 5737,"EX�α� ��ŷ");
#endif
#ifdef CHAR_EX_MAGE
	case ERANKINGEX_SUB_TYPE_LEVELCLASS_EX_MAGE:	// 2013/01/08 jeil EX������ �߰� ���� ��Ʈ�� ������ �߰� ���� �ʿ� 
		return _S( 5825,"��ũ������ ��ŷ");
#endif
	}
...
#endif
#ifdef CHAR_EX_MAGE
	case ERANKINGEX_SUB_TYPE_LEVELCLASS_EX_MAGE:	// 2013/01/08 jeil EX������ �߰� 
#endif
		return 1;
	}
...
void CUIRankingSystem::SetRankingListUnSelect( RANK_LIST_TYPE ListType, COLOR col )
{
	...
	if(oRankData.cstrCharacterName == _pNetwork->MyCharacterInfo.name) // �ڽ��� ���̵�� ���ٸ� ���̶���Ʈ
		colorText	= 0xF22020FF;
	else if(oRankData.cstrCharacterName == m_cstrSearch && ListType == RANKING_LIST_BOTTOM)	// �˻� ���̵�� ���ٸ� ���̶���Ʈ
...
void CUIRankingSystem::CloseUI()
{
	...
	CUIFocus::getSingleton()->setUI(NULL);	// â�� ���� �� ��Ŀ���� �ִٸ�, �ʱ�ȭ
...
void CUIRankingSystem::UpdateBottomRanking()
{
	...
	if(m_eSubType != ERANKINGEX_SUB_TYPE_GUILD)
	{
		...
		for(int i = 0; i < nTotal; i++)
		{
			...
			if(oRankData.cstrCharacterName == _pNetwork->MyCharacterInfo.name)		// �ڽ��� ���̵�� ���ٸ� ���̶���Ʈ
				colorText	= 0xF22020FF;
			else if(oRankData.cstrCharacterName == m_cstrSearch)					// �˻� ���̵�� ���ٸ� ���̶���Ʈ
				colorText	= 0x20F220FF;
	...
	case MSG_EX_RANKING_ERROR:		// (s->c) (uc)type
		{
			...
			case MSG_EX_RANKING_SYSTEM_ERROR:
				{
					SetTopListErrorMsgString( _S(5581, "��ŷ ���� �����Դϴ�."), FALSE );
			...
			case MSG_EX_RANKING_SYSTEM_ERROR_NOT_EXIST:
				{
					SetBottomListErrorMsgString( _S(5574, "�ش� ĳ���Ϳ� ���� ��ŷ ������ �����ϴ�."), FALSE );
				}				
				break;

			case MSG_EX_RANKING_SYSTEM_ERROR_REFRESH_TIME:
				{
					SetTopListErrorMsgString( _S(5575, "��ŷ�� ���ŵ� ���Ŀ� �ٽ� Ȯ�� ���ֽʽÿ�."), FALSE );
