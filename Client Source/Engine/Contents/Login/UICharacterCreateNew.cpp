...
CUICharacterCreateNew::CUICharacterCreateNew(void)
	...
	,	m_pMouseWheel(NULL)
	,	m_fXSpeed(0.00f)
#if defined(LOGIN_EP1) || defined(LOGIN_EP2)
	,	m_fZSpeed(0.00f)
	,	m_fYSpeed(0.00f)
	...
	,	m_fMinDistanceX(0.0f)
	,	m_fMaxDistanceX(0.0f)
	,	m_fMinDistanceY(0.0f)
	,	m_fMaxDistanceY(0.0f)
	,	m_fMinDistanceZ(0.0f)
	,	m_fMaxDistanceZ(0.0f)
	,	m_fMinDistance(0.0f)
	,	m_fMaxDistance(0.0f)
	,	m_fMaxHeight(0.0f)
	,	m_fMinHeight(0.0f)
...
void CUICharacterCreateNew::initializeCenterUI()
{
	...
	#ifndef ENABLE_NIGHTSHADOW
	m_pbtnSelJob_left->Hide(TRUE);
	m_pImgSelJob_Arrow_highlight_left->Hide(TRUE);
	#endif
...
#ifndef ENABLE_NIGHTSHADOW
	if (nIdx + 1 == NIGHTSHADOW && BTN_TYPE_NEXT == eType) {
		nIdx++;
	}
#endif
...
void CUICharacterCreateNew::SetSelectJobInfo()
{
	...
	CTString strDualJob = _S(1224, "��������");

	if (nJob == NIGHTSHADOW) // ����Ʈ �������� ��� ���� ������ ���⿡ �ѹ��� �ѷ��ش�. (����Ʈ�������� ���� ���� ���� ���)
	{
		strDualJob = _S(6105, "����Ʈ �ε��� ���� ����");
	}
	else
	{
		strJob2 += "\\n\\n" + strJob2Name2 + "\\n" + strJob2Desc2;
...
void CUICharacterCreateNew::Zoom(eCreateBtnType eType)
{
	...
	UpdateDistance(static_cast<eJob>(nJob));
	UpdateZoomSpeed(static_cast<eJob>(nJob));

	if (eType == BTN_TYPE_PREV)
	{
		pModel->en_plPlacement.pl_PositionVector(3) -= m_fZSpeed;
		pModel->en_plPlacement.pl_PositionVector(1) -= m_fXSpeed;

		if ( pModel->en_plPlacement.pl_PositionVector(1) > m_fMaxDistanceX)
		{
			pModel->en_plPlacement.pl_PositionVector(1) = m_fMaxDistanceX;
			bRefresh = FALSE;
		}
		if (pModel->en_plPlacement.pl_PositionVector(1) < m_fMinDistanceX)
		{
			pModel->en_plPlacement.pl_PositionVector(1) = m_fMinDistanceX;
			bRefresh = FALSE;
		}

		if (pModel->en_plPlacement.pl_PositionVector(3) > m_fMaxDistanceZ)
		{
			pModel->en_plPlacement.pl_PositionVector(3) = m_fMaxDistanceZ;
			bRefresh = FALSE;
		}
		if (pModel->en_plPlacement.pl_PositionVector(3) < m_fMinDistanceZ)
		{
			pModel->en_plPlacement.pl_PositionVector(3) = m_fMinDistanceZ;
			bRefresh = FALSE;
		}
	}
...
void CUICharacterCreateNew::UpdateZoomSpeed(eJob eClass)
{
	switch (eClass)
	{
	case TITAN:
	...
	case EX_MAGE:
		...
		break;
#endif
	case TOTAL_JOB: break;
	default: ;
	}
}
void CUICharacterCreateNew::UpdateDistance(eJob eClass)
{
	switch (eClass)
	{
	case TITAN:
		m_fMaxDistanceX = 1110.04f;
		m_fMaxDistanceY = 163.44f;
		m_fMaxDistanceZ = 435.08f;
		m_fMinDistanceX = 1109.36f;
		m_fMinDistanceY = 163.44f;
		m_fMinDistanceZ = 433.72f;
		break;
	case KNIGHT:
		m_fMaxDistanceX = 1106.4f;
		m_fMaxDistanceY = 163.56f;
		m_fMaxDistanceZ = 436.85f;
		m_fMinDistanceX = 1106.09f;
		m_fMinDistanceY = 163.56f;
		m_fMinDistanceZ = 435.61f;
		break;
	case HEALER:
		m_fMaxDistanceX = 1103.25f;
		m_fMaxDistanceY = 163.625f;
		m_fMaxDistanceZ = 437.55f;
		m_fMinDistanceX = 1103.25f;
		m_fMinDistanceY = 163.625f;
		m_fMinDistanceZ = 436.25f;
		break;
	case MAGE:
		m_fMinDistanceX = 1099.96f;
		m_fMaxDistanceY = 163.69f;
		m_fMaxDistanceZ = 438.07f;
		m_fMaxDistanceX = 1100.37f;
		m_fMinDistanceY = 163.69f;
		m_fMinDistanceZ = 436.44f;
		break;
	case ROGUE:
		m_fMinDistanceX = 1097.09f;
		m_fMaxDistanceY = 163.75f;
		m_fMaxDistanceZ = 436.43f;
		m_fMaxDistanceX = 1097.56f;
		m_fMinDistanceY = 163.75f;
		m_fMinDistanceZ = 435.495f;
		break;
	case SORCERER:
		m_fMinDistanceX = 1094.02f;
		m_fMaxDistanceY = 163.75f;
		m_fMaxDistanceZ = 434.465f;
		m_fMaxDistanceX = 1094.68f;
		m_fMinDistanceY = 163.75f;
		m_fMinDistanceZ = 434.02f;
		break;
	case NIGHTSHADOW:
		m_fMinDistanceX = 1092.41f;
		m_fMaxDistanceY = 163.656f;
		m_fMaxDistanceZ = 431.445f;
		m_fMaxDistanceX = 1093.48f;
		m_fMinDistanceY = 163.656f;
		m_fMinDistanceZ = 431.02f;
		break;
#ifdef CHAR_EX_ROGUE
	case EX_ROGUE:
		m_fMinDistanceX = 1091.97f;
		m_fMaxDistanceY = 163.343f;
		m_fMaxDistanceZ = 428.22f;
		m_fMaxDistanceX = 1093.45f;
		m_fMinDistanceY = 163.343f;
		m_fMinDistanceZ = 428.22f;
		break;
#endif
#ifdef CHAR_EX_MAGE
	case EX_MAGE:
		m_fMinDistanceX = 1091.9f;
		m_fMaxDistanceY = 163.09f;
		m_fMinDistanceZ = 425.18f;
		m_fMaxDistanceX = 1093.65f;
		m_fMinDistanceY = 163.09f;
		m_fMaxDistanceZ = 425.615f;
		break;
#endif
	case TOTAL_JOB: break;
	default: ;
...
void CUICharacterCreateNew::CreateCharacter()
{
	...
	if (IsBila(g_iCountry) == TRUE || g_iCountry == KOREA)
	{
		...
		else if ( strlen(pCharCreate->GetName()) > MAX_NAME_LEN  )
		{
			...
			strMessage.PrintF(_S( 434, "ĳ���� �̸��� �ʹ� ��ϴ�.\n(�ѱ� 4~8��, ���� 2~16��)" ), MAX_NAME_LEN);
			MsgBoxInfo.AddString(strMessage);
	...
	else
	{
		...
		if (!pUIManager->checkName(strMessage, eCHECK_TYPE_PLAYER_NAME)) 
			return;
	}
...
#ifdef LOGIN_EP4
	float fDist = 2.0f;//pBackMgr->GetCharDistance(false);	
	m_fMinDistance = 2.0f;//pBackMgr->GetZoomDisValue(false); // min
...
CTString CUICharacterCreateNew::GetClassDesc( eJob eClass )
{
	...
	retStr1 = _S(nDesc1[eClass], "�⺻ Ŭ���� ���� 1");
	retStr2 = _S(nDesc2[eClass], "�⺻ Ŭ���� ���� 2");
...
CTString CUICharacterCreateNew::GetClass2Desc( eJob eClass, eJob2 eClass2 )
{
	...
	{
		if (_pGameState->IsCreatableNightShadow() == TRUE)// ����Ʈ �������� ��� ���� ������ ���⿡ ����Ʈ�������� ���� ���� ���� ���
			retStr = _S(4794, "����Ʈ������ ��������");
		else
			retStr = _S(4700, "����Ʈ������ �����Ұ�");
	}
	...
	{
		retStr = _S(nDesc[eClass][eClass2 - 1], "���� Ŭ���� ����");
	}
