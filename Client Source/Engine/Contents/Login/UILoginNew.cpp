...
{
	eEFFECT_STEP_READY = 0, // �غ�
	eEFFECT_STEP_FINISH,	// ��� ������ ��ħ
};
...
void CUILoginNew::PressChangeLang(std::string language)
{
//#ifdef MULTI_LANGUAGE
	//	if (m_pBtnLangEN != NULL && m_pBtnLangES != NULL &&
	//m_pBtnLangPT != NULL && m_pBtnLangDT != NULL /*m_pBtnLangFR != NULL*/  )//note if LANGUAGE_FR_DETH04072021 is turned off this might throw an error

	if (m_pBtnLangEN != NULL &&
		m_pBtnLangPT != NULL /*m_pBtnLangFR != NULL*/  )//note if LANGUAGE_FR_DETH04072021 is turned off this might throw an error
	{
		extern CTString g_language;
		extern INDEX g_iCountry;
		g_language = language.c_str();
		g_iCountry = DefHelp::getNationByLang(g_language);
	}
...
#ifdef LOGIN_EP2
	_pGameState->SetCameraById(43565, TRUE);
...
#ifdef MULTI_LANGUAGE
	if (m_pBtnLangES != NULL)
		m_pBtnLangES->SetEnable(TRUE);
	if (m_pBtnLangDT != NULL)
		m_pBtnLangDT->SetEnable(TRUE);
#endif
	//if (m_pBtnLangEN != NULL)
		//m_pBtnLangEN->SetEnable(TRUE);

	//if (m_pBtnLangPT != NULL)
		//m_pBtnLangPT->SetEnable(TRUE);

#ifdef LANGUAGE_FR_DETH04072021
	if (m_pBtnLangFR != NULL)
		m_pBtnLangFR->SetEnable(TRUE);
#endif
...
void CUILoginNew::initialize()
{
...
#ifdef MULTI_LANGUAGE
	m_pBtnLangDT = (CUIButton*)findUI("btn_lang_dt");

	if (m_pBtnLangDT != NULL)
	{
		CmdChangeLang* pCmd = new CmdChangeLang;
		pCmd->setData(this, "dt");
		m_pBtnLangDT->SetCommandUp(pCmd);
		m_pBtnLangDT->CopyUV(UBS_IDLE, UBS_CLICK);
	}
	m_pBtnLangES = (CUIButton*)findUI("btn_lang_es");

	if (m_pBtnLangES != NULL)
	{
		CmdChangeLang* pCmd = new CmdChangeLang;
		pCmd->setData(this, "es");
		m_pBtnLangES->SetCommandUp(pCmd);
		m_pBtnLangES->CopyUV(UBS_IDLE, UBS_CLICK);
	}
#endif
#ifdef LANGUAGE_FR_DETH04072021
	m_pBtnLangFR = (CUIButton*)findUI("btn_lang_fr");

	if (m_pBtnLangFR != NULL)
	{
		CmdChangeLang* pCmd = new CmdChangeLang;
		pCmd->setData(this, "fr");
		m_pBtnLangFR->SetCommandUp(pCmd);
		m_pBtnLangFR->CopyUV(UBS_IDLE, UBS_CLICK);
	}

#endif

	
	/*m_pBtnLangEN = (CUIButton*)findUI("btn_lang_en");

	if (m_pBtnLangEN != NULL)
	{
		CmdChangeLang* pCmd = new CmdChangeLang;
		pCmd->setData(this, "en");
		m_pBtnLangEN->SetCommandUp(pCmd);
		m_pBtnLangEN->CopyUV(UBS_IDLE, UBS_CLICK);
	}

	m_pBtnLangPT = (CUIButton*)findUI("btn_lang_pt");

	if (m_pBtnLangPT != NULL)
	{
		CmdChangeLang* pCmd = new CmdChangeLang;
		pCmd->setData(this, "pt");
		m_pBtnLangPT->SetCommandUp(pCmd);
		m_pBtnLangPT->CopyUV(UBS_IDLE, UBS_CLICK);
	}*/
	...
	if (m_pCbSelServer != NULL)
	{
		m_pCbSelServer->AddString( _S(6084, "���̰� ������ ����"));
		m_pCbSelServer->SetCurSel(DEF_BASIC_SERVER);
		m_pCbSelServer->SetEnable(FALSE);
		...
		if (IsGamigo(g_iCountry) == TRUE)
		{
			m_pCbSelServer->AddString( _S(5777, "�����Ǿ� ������ ����"));
...
void CUILoginNew::ReadPreviousId()
{
	...
	if (strlen(g_strSaveID.str_String) > 0)
	{
		...
		m_pEbPw->SetString(g_strSavePW.str_String);
...
#ifdef DEV_LOGIN
void CUILoginNew::ReadTextDevPreviousIdPw() // Login.txt �� ����� ���̵� �н����� ������
{
...
void CUILoginNew::updateLogoXmas( float fDelta )
{
	...
	static bool	bUp = true; // ��������, ��������
	static bool	bEffect1 = true; // ù��° ���� �ι�° ����
	static int nEffectStep = eEFFECT_STEP_READY; // ���� ����Ʈ�� �Ѿ������ ���� 
