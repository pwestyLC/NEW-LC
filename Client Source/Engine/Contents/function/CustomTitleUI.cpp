...
class CmdSealPutOnItem : public Command
{
public:
	CmdSealPutOnItem() : m_pWnd(NULL), m_bOk(false) {}
	void setData(CCustomTitleUI* pWnd, bool bOk)	{	m_pWnd = pWnd; m_bOk = bOk;	}
	void execute()
	{
		if (m_pWnd != NULL)
		{
			if (m_bOk == true)
				m_pWnd->SealPutOnCallBack();
			else
				m_pWnd->SealPutOnCancelCallBack();
		}
	}
private:
	CCustomTitleUI* m_pWnd;
	bool m_bOk;
};
...
void CCustomTitleUI::MakeTitle()
{
	...
	std::vector<int> pItemSeals;
	std::vector<int> pItemSealsInvenIndex;
	std::vector<int> pItemSealsTab;

#ifdef ENABLE_CUSTOM_TITLE
	for(int i = 0; i < MAX_TITLE_SEAL; i++) {
		if (m_pIconSealItem[i] != NULL && !m_pIconSealItem[i]->IsEmpty()) {
			CItems* pItemSeal = m_pIconSealItem[i]->getItems();
			pItemSealsTab.push_back(pItemSeal->Item_Tab);
			pItemSealsInvenIndex.push_back(pItemSeal->InvenIndex);
			pItemSeals.push_back(pItemSeal->Item_UniIndex);
		}
	}
#endif
	...
	pTitle->SendTitleMake(pItem->Item_Tab, pItem->InvenIndex, pItem->Item_UniIndex, pItemSealsTab, pItemSealsInvenIndex, pItemSeals);
	m_nMsgID = 0;
}
...
void CCustomTitleUI::_resetUI()
{
	...
	for (int i = 0; i < MAX_TITLE_SEAL; i++)
	{
		if (m_pIconSealItem[i] != NULL)
			m_pIconSealItem[i]->clearIconData();
	}
#endif
...
void CCustomTitleUI::initialize()
{
	...
	for (int i = 0; i < MAX_TITLE_SEAL; i++)
	{
		sealIcon.PrintF("icon_seal%d", i + 1);
		m_pIconSealItem.push_back((CUIIcon*)findUI(sealIcon));
	}
#endif
...
WMSG_RESULT CCustomTitleUI::OnLButtonUp( UINT16 x, UINT16 y )
{
	...
	for (int i = 0; i < MAX_TITLE_SEAL; i++)
	{
		if (m_pIconSealItem[i] != NULL && m_pIconSealItem[i]->IsInside(x, y)) {

			if (pDrag != NULL && pDrag->IsEmpty() != TRUE)
			{
				CItems* pItem = pDrag->getItems();

				if (pItem->ItemData->GetSubType() == CItemData::ITEM_SUB_SEAL) {

					CUIMsgBoxNumericOnly* pMsgBoxNumOnly = pUIManager->GetMsgBoxNumOnly();

					if (pMsgBoxNumOnly->IsOpen())
						pMsgBoxNumOnly->CloseBox();

					int nInitCount = pItem->Item_Sum;
					if (pItem->Item_Sum > 1)
						nInitCount = 1;
					...
					CTString strMessage;
					strMessage.PrintF(_S(8412, "You can only add 1 seal."));

					CmdSealPutOnItem* pCmdOk = new CmdSealPutOnItem;
					pCmdOk->setData(this, true);

					CmdSealPutOnItem* pCmdCancel = new CmdSealPutOnItem;
					pCmdCancel->setData(this, false);

					pMsgBoxNumOnly->SetInfo(pCmdOk, _S(8410, "Custom title"), strMessage, 1, (int)nInitCount, pCmdCancel);
					pMsgBoxNumOnly->InitData((int)nInitCount);

					{
						m_pIconSealItem[i]->cloneItem(pDrag);
						m_pIconSealItem[i]->setCount(nInitCount);
					}
					...
					MSGBOXMGR()->CreateMsgBox(_S(8410, "Custom title"), _S(8411, "Sorry. Only seal items are allowed"), eMSG_OK);
				}
			}
		}
	}
...
void CCustomTitleUI::ShowErrorMessage( int nErrorCode )
{
	...
	case eMAKE_ERROR_SAME_UNIQUE_TITLE:
		strSysMessage = _S(7089, "���� ȣĪ�� ���� �� �� �����ϴ�. �ٸ� ȣĪ���� ������ �ּ���.");
		break;
	case eMAKE_ERROR_FILTERING_NOPASS:
		strSysMessage = _S(7094, "�������� ȣĪ�� ������ �� �����ϴ�.");
		break;
	case eMAKE_ERROR_FULL_INVEN:
		strSysMessage = _S(7090, "�κ��丮 ������ �����Ͽ� ȣĪ�� ���� �� �� �����ϴ�.");
		break;
	default:
		return;
	}
...
#ifdef CUSTOM_TITLE_SEAL_REQUIRED
	bool sealNotEmpty = false;
	for(int i = 0; i < MAX_TITLE_SEAL; i++) {
		if (m_pIconSealItem[i] != NULL && !m_pIconSealItem[i]->IsEmpty())
			sealNotEmpty = true;
	}
...
void CCustomTitleUI::PressMakeBtn()
{
	...
	m_nMsgID = MSGBOXMGR()->CreateMsgBox(_S(7088, "ȣĪ ����"),
		_S(7091, "���� �Ͻ� �������� ȣĪ�� ���� �մϴ�. Ȯ�� Ŭ�� �� �κ��丮 ȣĪ�� ���� �˴ϴ�."),
...
void CCustomTitleUI::SealPutOnCallBack()
{
#ifdef ENABLE_CUSTOM_TITLE
	SQUAD llData = UIMGR()->GetMsgBoxNumOnly()->GetData();

	if(llData && llData <= 1 && llData > 0)
	{
		if (llData <= m_pIconSealItem[m_SlotPosNumeric]->getItems()->Item_Sum)
		{
			m_pIconSealItem[m_SlotPosNumeric]->setCount(llData);
		}
	}
	else
	{
		UIMGR()->GetChattingUI()->AddSysMessage( _S(8412,"You can only add 1 seal."), SYSMSG_ERROR );
		m_pIconSealItem[m_SlotPosNumeric]->clearIconData();
		return;
	}

	if (m_pIconSealItem[m_SlotPosNumeric]->IsEmpty())
		m_pIconSealItem[m_SlotPosNumeric]->setCount(llData);

#endif
}

void CCustomTitleUI::SealPutOnCancelCallBack()
{
#ifdef ENABLE_CUSTOM_TITLE
	if (m_pIconSealItem[m_SlotPosNumeric]->IsEmpty() == true)
		return;
