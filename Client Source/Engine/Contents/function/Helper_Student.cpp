...
void CHelper_StudentUI::BeMyTeacher()
{
	...
	//Mario fix for guardian index mapping START
	CHelperManager::_vec_member h_list = hm->GetHelperList(); // Ensure the selected index matches the sorted guardian list

	std::sort(h_list.begin(), h_list.end()); // Sort the helper list (same sorting used when displaying the list)

	if (iSelMember >= h_list.size()) // Ensure the selected index is within the bounds of the sorted list
		return;

	CHelperManager::sHelperMember* TempMember = &h_list[iSelMember];// Get the selected guardian based on the sorted list

	if (TempMember == NULL)
		return;
	//Mario fix for guardian index mapping END
	m_slIndex = TempMember->lIndex;
	m_strTeacherName = TempMember->strName;    // Store the selected guardian's index and name
	...
	strMessage.PrintF(_S(1151, "님을 후견인으로 신청하시겠습니까?"), TempMember->strName);
