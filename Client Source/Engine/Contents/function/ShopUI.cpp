...
{ //dethunter13 here cloud
	// Buy nothing
	if( m_nSendTotalPrice < 0 )
		return;
	int nMyItemCount11343 = UIMGR()->GetInventory()->GetItemCount(11343);
	int nShopID = UIMGR()->GetShopUI()->GetShopID();//Cloud shop
	if (nShopID == 1654 && nMyItemCount11343 < m_nTotalPrice)
	{
		// Add system message
		CUIManager::getSingleton()->GetChattingUI()->AddSysMessage( _S( 8503, "¼ÒÁö±ÝÀÌ ºÎÁ·ÇÕ´Ï´Ù." ), SYSMSG_ERROR );
		return;
	}
...
void CShopUI::ShopToTrade( SQUAD llCount )
{
	...
	if( bBuyShop )
	{
		...
		int nMyItemCount11343 = UIMGR()->GetInventory()->GetItemCount(11343);//Cloud shop
		if(m_nSelectShopID == 1654 && nMyItemCount11343  < llPrice)
		{
			// Add system message
			CUIManager::getSingleton()->GetChattingUI()->AddSysMessage( _S( 8503, "¼ÒÁö±ÝÀÌ ºÎÁ·ÇÕ´Ï´Ù." ), SYSMSG_ERROR );
			return;
		}
		int nMyHonorCount = _pNetwork->MyCharacterInfo.a_honor;//Cloud shop
		if (m_nSelectShopID == 1665 && nMyHonorCount < llPrice)
		{
			// Add system message
			CUIManager::getSingleton()->GetChattingUI()->AddSysMessage(_S(8551, "¼ÒÁö±ÝÀÌ ºÎÁ·ÇÕ´Ï´Ù."), SYSMSG_ERROR);
			return;
		}
		if( m_nSelectShopID != 1654 && m_nSelectShopID != 1665)
		{// Not enough money
...
void CShopUI::updateUI()
{
	// Set Money
	int nMyItemCount11343 = UIMGR()->GetInventory()->GetItemCount(11343);
	int nMyHonorCount = _pNetwork->MyCharacterInfo.a_honor;
	int nShopID = UIMGR()->GetShopUI()->GetShopID();//Cloud shop
	if (nShopID == 1654 && nMyItemCount11343 >= 0)
	{
		_setNas(eTEXT_MY_NAS, nMyItemCount11343);
	}
	// if not the condition above execute the condition below.
	else if (nShopID == 1665 && nMyHonorCount >= 0)
	{
		_setNas(eTEXT_MY_NAS, nMyHonorCount);
	}

	if (nShopID != 1654 && nShopID != 1665)
	{
