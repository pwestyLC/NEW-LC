...
#ifndef ENABLE_IGNITION
	m_pImgEPBack->Hide(TRUE);
	m_pImgEpGage->Hide(TRUE);
#endif
...
#ifndef ENABLE_DUNGEON_BONUS
	m_pAniDungeonTime->Hide(TRUE);
#endif
int		i;
#ifdef ENABLE_SOUL_NS
	CTString strTemp;
...
void CSimplePlayerInfoUI::UpdateEP()
{
	...
	pUIMgr->GetReZaBotUI()->UpdateEP(nEpLv);
...
void CSimplePlayerInfoUI::SetDungeonList(SLONG slExp)
{
	...
	/* int */size_t nMax = m_pListDungeon->getListItemCount();

	CUIListItem* pItem = NULL;
	CUIText* pText = NULL;

	int nStrIndex[20] = { 343/*1 ���ǽ�Ʈ ����*/, 345/*3 ����Ű�� ����*/, 1201/*9 ��������*/, 2368/*12 ������ ����*/, 4946/*21 �׿����� ����*/,
		4947/*17, 18, 19, 20 �罺Ʈ ����*/, 5146/*37 Ʈ����� ���*/, 4319/*29 ������ź��*/, 4317/*30 �̽�Ƽ����*/, 4395/*31 �÷ζ��� ����*/,
		4509/*34 �����*/, 4510/*35 ������ ����*/, 5065/*36 ��ĭ���*/, 5817/*41 ������ ���� ������*/,345,345,345,345,345,345
	};
	int nMaxWidth = 0;

	CDrawPort* pDrawPort = UIMGR()->GetDrawPort();

	for (/* int */size_t i = 0; i < nMax; ++i)
	{
		...
		if (i == 0)
		{
			...
			if (slExp > 0)
				strTemp.PrintF(CTString("%s(%d%%)"), _S(4959, "���� ���� ����ġ�� �߰��� ȹ��"), slExp);
			else
				strTemp = _S(4959, "���� ���� ����ġ�� �߰��� ȹ��");
		...
		{
			pText->SetText(_S(4960, "���� Ÿ��!"));
			pText->setFontColor(0xA3A1A3FF);
		}
		else
		{
			pText->SetText(_S(nStrIndex[i-2], "���� �̸�"));
