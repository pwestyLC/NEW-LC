...
void TitleNetwork::RecvTitleMessage( CNetworkMessage* istr )
{
	...
	case MSG_EX_TITLE_SYSTEM_ALREADY_HAVE_TITLE:
		...
		break;
	case MSG_EX_TITLE_SYSTEM_CANNOT_DUPLICATE_SEAL:
		{
			strSysMessage	=_S(8413, "You cannot duplicate the seal Absorb. PvP");
		}
		break;
	case MSG_EX_TITLE_SYSTEM_INSERT_SUCCESS:
		{
			...
			if (CustomTitleData::IsKeepTitleInfo(pRecv->custom_title_index) == true)
				strSysMessage.PrintF(_S(4828, "%s ȣĪ�� �߰��Ͽ����ϴ�."), CustomTitleData::m_mapCustomTitleItemInfo[pRecv->custom_title_index]->name);
			else
				strSysMessage.PrintF(_S(4828, "%s ȣĪ�� �߰��Ͽ����ϴ�."), pUIManager->GetNickName()->GetName(iItemIndex));
		}
		break;

	case MSG_EX_TITLE_SYSTEM_NOT_HAVE_TITLE:
		{
			strSysMessage	=_S(4829, "�������� �ʴ� ȣĪ �Դϴ�.");
		}
		break;
			...
			if ( pRecv->char_index == _pNetwork->MyCharacterInfo.index )
			{
				...
				if( !pUIManager->GetHelp()->IsBeginner() )
				{
					if (bCustomEffect == true)
						strSysMessage.PrintF(_S(4830,"%s ȣĪ���� ���� �Ϸ��Ͽ����ϴ�."), CustomTitleData::m_mapCustomTitleItemInfo[pRecv->custom_title_index]->name);
					else
						strSysMessage.PrintF(_S(4830,"%s ȣĪ���� ���� �Ϸ��Ͽ����ϴ�."), pUIManager->GetNickName()->GetName(iItemIndex));					
				}
				else
	...
	case MSG_EX_TITLE_SYSTEM_TITLE_DELETE_SUCCESS:
		{
			...
			if (CustomTitleData::IsKeepTitleInfo(pRecv->custom_title_index) == true)
				strSysMessage.PrintF(_S(4832, "���������� %s ȣĪ�� �����Ͽ����ϴ�."), CustomTitleData::m_mapCustomTitleItemInfo[pRecv->custom_title_index]->name);
			else
				strSysMessage.PrintF(_S(4832, "���������� %s ȣĪ�� �����Ͽ����ϴ�."), pUIManager->GetNickName()->GetName(iItemIndex));
		}
		break;
...
std::vector<int> indexSealsTab, std::vector<int> indexSealsInvenIdx, std::vector<int> indexSeals)
{
	CNetworkMessage nm;
	...
	packet->total_seals = indexSeals.size();

	for(int i = 0; i < indexSeals.size(); i++) {
		packet->sealsTab[i] = indexSealsTab[i];
		packet->sealsInvenIdx[i] = indexSealsInvenIdx[i];
		packet->seals[i] = indexSeals[i];
	}
...
bool TitleStaticData::loadEx(const char* FileName)
{
	...
	m_dummy = new TitleStaticData; // ���̵���Ÿ ����
...
bool CustomTitleData::LoadCustomTitleDataFromFile( const char* fileName )
{
	...
	{
		{ CTString msg; msg.PrintF("customTitle File is not Exist: %s", fileName); MessageBox(NULL, msg, "error!", MB_OK); }
		return false;
	}
	fflush(fp);
