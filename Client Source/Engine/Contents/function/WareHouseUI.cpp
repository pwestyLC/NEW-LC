...
#define WAREHOUSE_WAREHOUSE_SLOT_COL		20
#define	WAREHOUSE_TRADE_SLOT_TOTAL			40
#define WAREHOUSE_TRADE_SLOT_COL			20

#define WAREHOUSE_DEFUALT_MAX_SLOT			220
...
void CWareHouseUI::AddWareHouseItem( int nIndex )
{
		...
		(!m_bStorageWareHouse && (pItemData->GetFlag() & ITEM_FLAG_COUNT) && nCount > 1))
	{
			...
			pItemData->GetSubType() == CItemData::ITEM_ETC_MONEY )
		{
			...
			MsgBoxInfo.SetMsgBoxInfo( _S( 823, "â��" ), UMBS_OKCANCEL | UMBS_INPUTBOX,	
		...
		else
		{
			...
			pCmd->setData(this);
			UIMGR()->GetMsgBoxNumOnly()->SetInfo(pCmd, _S( 823, "â��" ), strMessage, 1, nCount);
		}
	}
		...
		(pItemData->GetSubType() == CItemData::ITEM_ETC_MONSTER_MERCENARY_CARD || nItemIndex == 6941))))
	{
		CTString	strMessage;
		CUIMsgBox_Info	MsgBoxInfo;
		MsgBoxInfo.SetMsgBoxInfo( _S( 823, "â��" ), UMBS_YESNO,		
			UI_WARE_HOUSE, MSGCMD_WAREHOUSE_ADD_PLUSITEM );
		const char*	szItemName = _pNetwork->GetItemName(nItemIndex);
		strMessage.PrintF(_S2(264, szItemName, "[%s +%d]<��> �ű�ðڽ��ϱ�?"), szItemName, nItemPlus);
		MsgBoxInfo.AddString( strMessage );
		pUIManager->CreateMessageBox( MsgBoxInfo );
	}
	else
...
void CWareHouseUI::DelWareHouseItem()
{
	...
	if (m_pItemsTemp->Item_Sum > 1)
	{
			...
			pItemData->GetSubType() != CItemData::ITEM_ETC_MONEY)
		{
			...
			pCmd->setData(this);
			UIMGR()->GetMsgBoxNumOnly()->SetInfo(pCmd, _S( 823, "â��" ), strMessage, 1, m_pItemsTemp->Item_Sum);
		}
	}
	else
	{
...
void CWareHouseUI::WareIconMouseLDownEvent( int nSelIdx )
{
	...
	{
		m_nSelTradeItemID = -1;
		m_nSelWareHouseItemID = nSelIdx;	// ���õ� ������ ������ ID
	}	
}
...
void CWareHouseUI::KeepItems()
{
...
#endif
		{
			tv_str.PrintF(_S(2578, "������ �� ���� ������[%s] �Դϴ�."), pItemData->GetName());
	...
	{
		// Add system message
		pUIManager->GetChattingUI()->AddSysMessage( _S( 827, "������ �����Ͽ� ��ǰ�� ������ �� �����ϴ�." ), SYSMSG_ERROR );	
		return;
	}
...
void CWareHouseUI::AddWareHouseItemFromInventory()
{
		...
		( !m_bStorageWareHouse && ( pItemData->GetFlag() & ITEM_FLAG_COUNT ) && m_nTempMax > 1 ) )
	{
			...
			pItemData->GetSubType() == CItemData::ITEM_ETC_MONEY )
		{
			MsgBoxInfo.SetMsgBoxInfo( _S( 823, "â��" ), UMBS_OKCANCEL | UMBS_INPUTBOX,	
				UI_WARE_HOUSE, MSGCMD_WAREHOUSE_ADD_MONEY );
			strMessage.PrintF(  _S( 960, "�����Ͻ� ������ �Է��� �ֽʽÿ�." )  );		
			MsgBoxInfo.AddString( strMessage );
			pUIManager->CreateMessageBox( MsgBoxInfo );
		}
		else
		{
			...
			CmdWareHouseAddItem* pCmd = new CmdWareHouseAddItem;
			pCmd->setData(this);
			UIMGR()->GetMsgBoxNumOnly()->SetInfo(pCmd, _S( 823, "â��" ), strMessage, 1, m_nTempMax);
		}		
	}
...
void CWareHouseUI::OpenWareHouse( SBYTE sbSetPW, bool bCashRemoteOpen /*= false */ )
{
	...
	pUIManager->CreateMessageBoxL( _S( 814, "������" ) , UI_WARE_HOUSE, MSGLCMD_WAREHOUSE_REQ );	
	...
	pUIManager->AddMessageBoxLString( MSGLCMD_WAREHOUSE_REQ, TRUE, _S( 815, "��ǰ ������ ���Ͻʴϱ�?" ) , -1, 0xA3A1A3FF );		
	pUIManager->AddMessageBoxLString( MSGLCMD_WAREHOUSE_REQ, TRUE, _S( 816, "���� ��ǰ �����ҿ����� �����ڴ��� ��ǰ�� �����ϰ� �����ϰ� ���� ��𼭵��� ���� ��ǰ �����Ұ� �ִ� ���̶�� ��ǰ�� ã���� �� �ֽ��ϴ�." ) , -1, 0xA3A1A3FF );		

	CTString strMessage;	
	strMessage.PrintF( _S( 817, "��ǰ�� �ñ��." ) );			
	pUIManager->AddMessageBoxLString( MSGLCMD_WAREHOUSE_REQ, FALSE, strMessage, WAREHOUSE_IN );

	strMessage.PrintF( _S( 818, "��ǰ�� ã�´�." ) );			
	pUIManager->AddMessageBoxLString( MSGLCMD_WAREHOUSE_REQ, FALSE, strMessage, WAREHOUSE_OUT );

#ifdef	STASH_PASSWORD
	pUIManager->AddMessageBoxLString( MSGLCMD_WAREHOUSE_REQ, FALSE, _S( 819, "��ȣ�� �����Ѵ�." ), WAREHOUSE_CHANGEPW );
...
void CWareHouseUI::MsgBoxCommand( int nCommandCode, BOOL bOK, CTString &strInput )
{
	...
	case MSGCMD_WAREHOUSE_ADD_MONEY:	//2013/04/04 jeil ���� ������ ���� 
		{
			char	*pcInput	= strInput.str_String;
			int		nLength		= strInput.Length();
			int		iChar;
	...
	case MSGCMD_WAREHOUSE_DEL_ITEM:
		...
		break;
	case MSGCMD_WAREHOUSE_TAKE_MONEY:	//2013/04/04 jeil ���� ������ ���� 
		{
			char	*pcInput	= strInput.str_String;
			int		nLength		= strInput.Length();
			int		iChar;
...
void CWareHouseUI::MsgBoxLCommand( int nCommandCode, int nResult )
{
	...
	case WAREHOUSE_IN: // ��ǰ�� �ñ��
		{
			...
			if(g_iShowHelp1Icon)
			{
				...
				pUIManager->GetHelp3()->AddHelpString(_S(3313, "ĳ������ �κ��丮 â���� â���� ���� �ϰ��� �ϴ� ��ǰ�� Ŭ�� �� â���� �ϴ� ��ĭ�� �巡�� �Ͽ� �÷� �����ϴ�."));
				pUIManager->GetHelp3()->AddHelpString(_S(3314, "������ư�� ������ �����Ḹŭ �ڽ��� �κ��丮���� ������ ���������� ��ǰ�� â���� ���� �˴ϴ�."));
				pUIManager->GetHelp3()->AddHelpString(_S(3315, "�� ������� �ñ���� ��ǰ 1���� 100���� �Դϴ�."));
				pUIManager->GetHelp3()->AddHelpString(_S(3316, "�� ��� �׵θ��� �ѷ��� �������� â���� ������ �Ұ����մϴ�."));
				pUIManager->GetHelp3()->AddHelpString(_S(3317, "�� â���� ���� ������ ������ �⺻ 50ĭ�̸�, â��Ȯ��ī�带 ����ϸ� 150ĭ���� Ȯ���� �� �ֽ��ϴ�."));
				pUIManager->GetHelp3()->OpenHelp(this);
			}		
		}
		break;

	case WAREHOUSE_OUT: // ��ǰ�� ã�´�
		{
			if (pUIManager->GetInventory()->IsLocked() == TRUE ||
				pUIManager->GetInventory()->IsLockedArrange() == TRUE)
			{
			...
			{
				pUIManager->GetHelp3()->ClearHelpString();
				pUIManager->GetHelp3()->AddHelpString(_S(3318, "â���� ������ ��ǰ �� ã������ ��ǰ�� â�� â �Ʒ��� ��ĭ���� �ű�� ã�� ��ư�� Ŭ�� �մϴ�."));
				pUIManager->GetHelp3()->AddHelpString(_S(3319, "ã�� ��ǰ�� �ڽ��� �κ��丮�� �Ű����� �˴ϴ�."));
				pUIManager->GetHelp3()->AddHelpString(_S(3320, "�� �ִ� 10���� �������� �ѹ��� ã�� �� �ֽ��ϴ�."));
				pUIManager->GetHelp3()->OpenHelp(this);
			}
		}
		break;
	...
	case WAREHOUSE_CHANGEPW : // ��ȣ�� �����Ѵ�.
		{
			pUIManager->CloseMessageBoxL( MSGLCMD_SECURITY_REQ );

			pUIManager->CreateMessageBoxL( _S( 1743, "â�� ���ȼ���" ) , UI_SECURITY, MSGLCMD_SECURITY_REQ );		

			pUIManager->AddMessageBoxLString( MSGLCMD_SECURITY_REQ, TRUE, _S( 1778, "â�� ������ ���Ͽ� ��ȣ�� ���� �Ͻ� �� �ֽ��ϴ�." ), -1, 0xA3A1A3FF );	
			pUIManager->AddMessageBoxLString( MSGLCMD_SECURITY_REQ, TRUE, CTString("  "), -1, 0xA3A1A3FF );
			pUIManager->AddMessageBoxLString( MSGLCMD_SECURITY_REQ, TRUE, _S( 1779, "��ȣ�� �����ϸ� â���� �̿��� �� �����Ͻ� ��ȣ�� �Է��Ͽ��߸� â���� �̿��� �� �ֽ��ϴ�." ), -1, 0xA3A1A3FF );	

			pUIManager->AddMessageBoxLString( MSGLCMD_SECURITY_REQ, FALSE, _S( 1780, "��ȣ ����" ), CUISecurity::SET_PASSWORD );
			if( m_bHasPassword )
			{
				pUIManager->AddMessageBoxLString( MSGLCMD_SECURITY_REQ, FALSE, _S( 1781, "��ȣ �н�" ), CUISecurity::UNSET_PASSWORD );
			}
			pUIManager->AddMessageBoxLString( MSGLCMD_SECURITY_REQ, FALSE, _S( 1220, "����Ѵ�." ) );
		}
		break;	
...
void CWareHouseUI::ReceiveNas( CNetworkMessage *istr )
{
	...
	if (pPack->subType == MSG_SUB_UPDATE_MONENY)
	{
		...
		if (delta_nas > 0)
		{
			...
			strOutNas.PrintF( _S( 1346, "������ %s�� �����Ͽ����ϴ�." ), strTotalNas);		
...
void CWareHouseUI::PrepareWareHouse()
{
	...
	{
		pUIManager->SetCSFlagOff( CSF_WAREHOUSE );
		pUIManager->GetChattingUI()->AddSysMessage( _S( 821, "��ȯ�߿��� â���� ����� �� �����ϴ�." ), SYSMSG_ERROR );		
		return;
	}
	...
	{
		pUIManager->SetCSFlagOff( CSF_WAREHOUSE );
		pUIManager->GetChattingUI()->AddSysMessage( _S( 822, "���� ���� ����߿��� â���� ����� �� �����ϴ�." ), SYSMSG_ERROR );		
		return;
	}
	...
	{
		pUIManager->SetCSFlagOff( CSF_WAREHOUSE );
		pUIManager->GetChattingUI()->AddSysMessage( _S( 807, "���� �ŷ��߿��� â���� ����� �� �����ϴ�." ), SYSMSG_ERROR );		
		return;
	}
...
void CWareHouseUI::WareHouseToTrade( SQUAD llCount /*= 1*/ )
{
	...
	else
	{
		...
		if( iItem == WAREHOUSE_TRADE_SLOT_TOTAL )
		{
			...
			// Add system message
			if( m_bStorageWareHouse )
				strMessage.PrintF( _S( 831, "�ѹ��� �ִ� %d���� �������� �����Ͻ� �� �ֽ��ϴ�." ), WAREHOUSE_TRADE_SLOT_TOTAL );	
			else
				strMessage.PrintF( _S( 832, "�ѹ��� �ִ� %d���� �������� ã���� �� �ֽ��ϴ�." ), WAREHOUSE_TRADE_SLOT_TOTAL );		
...
void CWareHouseUI::SendWareHouseTakeReq()
{
	...
	// ã�� ��û				: password(str) takecount(n) [item_idx(n) count(ll)]:takecount
	CNetworkMessage nmMessage;
...
void CWareHouseUI::updateUseTime()
{
	...
	if (m_useTime > 0)
	{
		...
		tv_str.PrintF(  _S( 6070,"���� : %d��%d��%d��%d��%d��"),pTime->tm_year + 1900
...
void CWareHouseUI::SetUI()
{
	...
	if (m_bStorageWareHouse == TRUE)
	{
		strNasTitle1.PrintF(_S( 5908, "�Ա� �ݾ�" ));
		strNasTitle2.PrintF(_S( 824, "������" ));
		strBtn1.PrintF(_S( 5906, "�Ա�" ));
		strBtn2.PrintF(_S( 812, "����" ));		
	}
	else
	{
		strNasTitle1.PrintF(_S( 5909, "���� �ݾ�" ));
		strNasTitle2.PrintF(_S( 5910, "��� �ݾ�" ));
		strBtn1.PrintF(_S( 5907, "���" ));
		strBtn2.PrintF(_S( 813, "ã��" ));	
	}
	// Nas Title
