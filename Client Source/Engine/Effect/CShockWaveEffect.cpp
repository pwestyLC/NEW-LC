...
LPDIRECT3DPIXELSHADER9 CShockWaveEffect::m_ulPixelProgramNoTex = NULL;
ULONG CShockWaveEffect::m_ulVertexProgramTex = 0;
LPDIRECT3DPIXELSHADER9 CShockWaveEffect::m_ulPixelProgramTex = NULL;
...
void CShockWaveEffect::Read(CTStream *pIS)
{
	...
	{
		is >> m_strTextureFileName;	//textureÀÇ ÆÄÀÏ ÀÌ¸§
		is >> m_bInnerBasis;
		is >> m_iSplitCount;
		ssReadFromStream(is, m_ssRadius, FLOAT);
		ssReadFromStream(is, m_ssWidth, FLOAT);
		ssReadFromStream(is, m_ssHeight, FLOAT);
		is >> (INDEX&)m_eRotation;			//tagÀÇ È¸Àü¿¡ ¾ó¸¸Å­ ¿µÇâÀ» ¹ÞÀ» °ÍÀÎ°¡?
		is >> (INDEX&)m_ePosition;			//tagÀÇ ÀÌµ¿¿¡ ¾ó¸¸Å­ ¿µÇâÀ» ¹ÞÀ» °ÍÀÎ°¡?
...
void CShockWaveEffect::Write(CTStream *pOS)
{
	...
	os << m_strTextureFileName;	//textureÀÇ ÆÄÀÏ ÀÌ¸§
	os << m_bInnerBasis;
	os << m_iSplitCount;
	ssWriteToStream(os, m_ssRadius);
	ssWriteToStream(os, m_ssWidth);
	ssWriteToStream(os, m_ssHeight);
	os << (INDEX)m_eRotation;			//tagÀÇ È¸Àü¿¡ ¾ó¸¸Å­ ¿µÇâÀ» ¹ÞÀ» °ÍÀÎ°¡?
	os << (INDEX)m_ePosition;			//tagÀÇ ÀÌµ¿¿¡ ¾ó¸¸Å­ ¿µÇâÀ» ¹ÞÀ» °ÍÀÎ°¡?
...
{
	//Rednering¿¡ »ç¿ëÇÒ Shader¸¦ »ý¼ºÇÑ´Ù.
