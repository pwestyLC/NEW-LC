...
LPDIRECT3DPIXELSHADER9 CTraceEffect::m_ulPixelProgramNoTex = NULL;
ULONG CTraceEffect::m_ulVertexProgramTex = 0;
LPDIRECT3DPIXELSHADER9 CTraceEffect::m_ulPixelProgramTex = NULL;
...
void CTraceEffect::Render()
{
	...
	if( (m_vectorPos.size() > 2 && m_iTagCount == 2) || (m_vectorPos.size() > 3 && m_iTagCount == 3) )
	{
		...
		if(bAddSpare)	//¿©ºÐÀ¸·Î Ãß°¡µÇ¾ú´ø vertex»èÁ¦.
		{
			if(m_iTagCount == 2)
			{
