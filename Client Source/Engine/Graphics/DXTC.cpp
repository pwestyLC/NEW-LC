...
inline void DecodeColorBlock(size_t *pImPos, DXTColBlock *pColorBlock, int width,  // width is width of image in pixels
                              size_t *col_0, size_t *col_1, size_t *col_2, size_t *col_3)
{
...
inline void DecodeAlphaExplicit(size_t *pImPos, DXTAlphaBlockExplicit *pAlphaBlock, int width)
{
  ...
  for( int row=0; row<4; row++) {
    ...
    for( int pixel=0; pixel<4; pixel++) {
      ...
      *pImPos &= 0x00FFFFFF;
      ...
      col.a = col.a | (col.a<<4);
      *pImPos |= *((size_t*)&col); // or the bits into the prev. nulled alpha
...
inline void DecodeAlpha3BitLinear(size_t*pImPos, DXTAlphaBlock3BitLinear *pAlphaBlock, int width)
{
  ...
  size_t bits = *((size_t*)&(pAlphaBlock->stuff[0]));
  ...
  bits = *((size_t*)&(pAlphaBlock->stuff[3]));   // last 3 bytes
  ...
  for( row=0; row<4; row++) {
    ...
    for( pixel=0; pixel<4; pixel++) {
      // zero the alpha bits of image pixel
      *pImPos &= 0x00FFFFFF;
      *pImPos |= *((size_t*)&(gACol[row][pixel])); // or the bits into the prev. nulled alpha
      pImPos++;
    }
    // next row
    pImPos += width-4;
...
extern void DecompressDXT1( UBYTE *pubCompressed, ULONG *pulExpanded, PIX pixWidth, PIX pixHeight)
{
  ...
  for( int j=0; j<yblocks; j++) {
    // 8 bytes per block
    DXTColBlock *pBlock = (DXTColBlock*)((size_t)pubCompressed + j*xblocks *8);
    for( int i=0; i<xblocks; i++) {
      ...
      size_t *pImPos = (size_t*)((size_t)pulExpanded + i*16 + (j*4) * pixWidth*4);
...
extern void DecompressDXT3( UBYTE *pubCompressed, ULONG *pulExpanded, PIX pixWidth, PIX pixHeight)
{
  ...
  for( int j=0; j<yblocks; j++) {
    ...
    DXTColBlock *pBlock = (DXTColBlock*)((size_t)pubCompressed + j*xblocks *16);
    for( int i=0; i<xblocks; i++) {
      // Get alpha block
