                                         ...
                                         DepthInfo *pdi, const INDEX ctCount)
{
  ...
  if( gap_bUseOcclusionCulling>=10)
  {
    ...
    for (INDEX idi = 0; idi < _adiDelayed.sa_UsedCount && idi < 20; idi++) {//liquid marvin test
  ...
  if( eAPI==GAT_D3D)
  {
    ...
    if(bFullScreen) {
      ...
      hr = pra->ra_pvpViewPort->vp9_pSwapChain->GetBackBuffer( 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
    }
    ...
    if(ctPrimitives>0) {
      // draw a bunch
      hr = _pGfx->gl_pd3d9Device->DrawPrimitiveUP( D3DPT_TRIANGLELIST, ctPrimitives, &_avtxDelayed[0], sizeof(CTVERTEX));
      D3D_CHECKERROR(hr);
    }
