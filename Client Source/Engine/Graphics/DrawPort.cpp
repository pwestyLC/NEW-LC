...
void CDrawPort::DrawPoint3D( FLOAT3D v, COLOR col, FLOAT fRadius/*=1.0f*/) const
{
	...
	if( eAPI==GAT_OGL) {
		...
		hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_POINTSIZE, *((DWORD*)&fRadius));
		D3D_CHECKERROR(hr);
		// set vertex shader and draw
		d3dSetVertexShader(D3DFVF_CTVERTEX);
		hr = _pGfx->gl_pd3d9Device->DrawPrimitiveUP( D3DPT_POINTLIST, 1, &avtx, sizeof(CTVERTEX));
...
void CDrawPort::Fill( COLOR col) const
{
	...
	{
		const ULONG d3dColor = rgba2argb(col);
		HRESULT hr = _pGfx->gl_pd3d9Device->Clear( 0, NULL, D3DCLEAR_TARGET, d3dColor,0,0);
		D3D_CHECKERROR(hr);
	}
}
...
void CDrawPort::GrabScreen( class CImageInfo &iiGrabbedImage, INDEX iGrabZBuffer/*=0*/) const
{
	...
	else if( eAPI==GAT_D3D)
	{
		...
		if( bFullScreen) hr = _pGfx->gl_pd3d9Device->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
		else hr = dp_Raster->ra_pvpViewPort->vp9_pSwapChain->GetBackBuffer( 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
		D3D_CHECKERROR(hr);
...
{
	// guard against null this / null font
	volatile CDrawPort* vThis = this;
	if (vThis == nullptr) return;
	...
	// 
	// ###
	// ### when create russian character in menu - segfault here (x64)
	// ### only create russian name with error (eng character with x64 ver is OK)
	// ###
	dp_FontData = pfd;
	dp_pixTextCharSpacing = pfd->fd_pixCharSpacing;
#if defined(PLATFORM_64BIT)
	if (g_iCountry == RUSSIA) { // ### fix fo Russian text on 64 bit mode (Create character name)
		if (this != nullptr) dp_pixTextLineSpacing = pfd->fd_pixLineSpacing;
		if (this != nullptr) dp_fTextScaling = 1.0f;
		if (this != nullptr) dp_fTextAspect = 1.0f;
		if (this != nullptr) dp_ulTextFlags = NONE;
		if (this != nullptr) dp_fTextShadow = 0;
		if (this != nullptr) dp_ulTextBlendingType = PBT_BLEND;
	}
	else {
...
ULONG CDrawPort::GetTextWidth2(const CTString &strText)
{
	...
	if (g_bDrawportRus == true)
	{
		...
		if (_pfdDefaultFont == NULL || strText.Length() <= 0)
			return 0;

		// Compute width directly from the global default font without
		// requiring a valid 'this' — many callers invoke GetTextWidth2
		// on a drawport pointer that may be NULL at UI-build time.
		ULONG pixWidth = 0;
		const char* pStr = strText.str_String;
		for (INDEX i = 0; i < strText.Length(); i++)
		{
			unsigned char ch = (unsigned char)pStr[i];
			PIX pixStart = _pfdDefaultFont->fd_fcdFontCharData[ch].fcd_pixStart;
			PIX pixEnd   = _pfdDefaultFont->fd_fcdFontCharData[ch].fcd_pixEnd;
			pixWidth += (pixEnd - pixStart) + _pfdDefaultFont->fd_pixCharSpacing;
		}
		return pixWidth;
	}
...
{
	// guard against null dp_FontData (SetFont was never called on this drawport)
	if (dp_FontData == NULL) return;
							 ...
							 FLOAT fZ, BOOL bShadow, const COLOR colShadow ) const
{
	...
	if (strText.Length() == NULL)//liquid marvin test
	{
		return;
	}
...
void CDrawPort::PutTextExRus(const CTString &strText, PIX pixX0, PIX pixY0, const COLOR colBlend,
							 FLOAT fZ, BOOL bShadow, const COLOR colShadow ) const
{
	// guard against null dp_FontData
	if (dp_FontData == NULL) return;
...
void CDrawPort::EndTextEx( BOOL bDepthTest )
{
	...
	if (g_bDrawportRus == true && dp_FontData != NULL)
	{
...
{
	// guard against being invoked on a null CDrawPort* (some callers do this).
	// Use volatile cast to prevent MSVC from optimizing away the null-this check (UB).
	volatile CDrawPort* vThis = this;
	if (vThis == nullptr) return -1;
	if (strString == NULL) return -1;
	if(nSectionEnd > (int)strlen(strString))
		return -1;
	...
	// _pfdDefaultFont may be NULL if Russia.fnt failed to load; bail out safely.
	if (_pfdDefaultFont == NULL) return -1;
...
#if defined(PLATFORM_64BIT)
		if (g_iCountry == RUSSIA) { // ### fix fo Russian text on 64 bit mode (Create character name)
			dp_FontData = _pfdDefaultFont;
			dp_pixTextCharSpacing = _pfdDefaultFont->fd_pixCharSpacing;
			dp_pixTextLineSpacing = _pfdDefaultFont->fd_pixLineSpacing;
			dp_fTextScaling = 1.0f;
			dp_fTextAspect = 1.0f;
			dp_ulTextFlags = NONE;
			dp_fTextShadow = 0;
