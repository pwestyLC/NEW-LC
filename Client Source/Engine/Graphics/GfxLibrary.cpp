...
extern INDEX ska_iMaxWeightsPerVertex  = 4;
	// ### Temporarily disable because function D3DXAssembleShader
	// ### have bugs on Debug preset (DX8.1) and have bug on Release preset (DX9) // (segfault)
	// ### The implementation is initially buggy
extern INDEX ska_bUseHardwareShaders   = TRUE; //FALSE; // TRUE;
	// ###
...
// terrain controls
	...
	// ### have bugs on Debug preset (DX8.1) and have bug on Release preset (DX9) // (segfault)
	// ### The implementation is initially buggy
extern INDEX ter_bUseHardwareShaders = TRUE; // FALSE; // TRUE;
	// ###
...
static void GAPInfo(void)
{
	...
	if( (_pGfx->go_hglRC==NULL && _pGfx->gl_pd3d9Device==NULL) || eAPI==GAT_NONE) {
		// be brief, be quick
...
ULONG64 CGfxLibrary::SavePixelShader(LPDIRECT3DPIXELSHADER9 ps)
{
	curPS++;
	_ps[curPS] = ps;
	//mapPS.insert(std::pair<ULONG, LPDIRECT3DPIXELSHADER9>(curPS, ps));
	return curPS;
}
...
LPDIRECT3DPIXELSHADER9 CGfxLibrary::GetPixelShader(ULONG64 ulHandle)
{
	if (curPS >= ulHandle && ulHandle > 0)
		return _ps[ulHandle];

	/*
	auto it = mapPS.find(ulHandle);
	if (it != mapPS.end())
		return it->second;
	*/
	return NULL;
...
void CGfxLibrary::DeletePixelShader(ULONG64 ulHandle)
{
	LPDIRECT3DPIXELSHADER9 pInfo = GetPixelShader(ulHandle);
	if (pInfo)
	{
		pInfo->Release();
		if (ulHandle < 1000) {
...
BOOL CGfxLibrary::StartDisplayMode( enum GfxAPIType eAPI, INDEX iAdapter, PIX pixSizeI, PIX pixSizeJ,
																		enum DisplayDepth eColorDepth)
{
 D3DRES_LOG_MARKER("StartDisplayMode BEGIN");
	// reinit gamma table
   ...
   GfxAPIType eInternalAPI = eAPI;
	if (eAPI == GAT_VK) {
		sam_bGfxAPIVulkan = TRUE;
		eInternalAPI = GAT_D3D;
	} else if (eAPI == GAT_D3D) {
		sam_bGfxAPIVulkan = FALSE;
	}
	const int iLoggedApi = (eAPI == GAT_D3D && sam_bGfxAPIVulkan) ? (int)GAT_VK : (int)eAPI;
	D3DRES_LOG("  API=%d internal=%d adapter=%d size=%dx%d depth=%d vkFlag=%d", iLoggedApi, (int)eInternalAPI, (int)iAdapter, (int)pixSizeI, (int)pixSizeJ, (int)eColorDepth, (int)sam_bGfxAPIVulkan);

	// OpenGL driver ?
	if( eInternalAPI==GAT_OGL)
	{
		...
		ska_bUseHardwareShaders = FALSE;
		ter_bUseHardwareShaders = FALSE;
	...
	// DirectX driver ?
 else if( eInternalAPI==GAT_D3D)
	{
		// ### Use directx byte code
		ska_bUseHardwareShaders = TRUE;
		ter_bUseHardwareShaders = TRUE;
 ...
 if (gl_eCurrentAPI == GAT_D3D && gl_pd3d9Device) {
		D3DRES_LOG_REFCOUNT("gl_pd3d9Device", gl_pd3d9Device);
	}
	D3DRES_LOG_MARKER("StartDisplayMode END");
	// all done
	return TRUE;
}
...
{
	D3DRES_LOG_MARKER("StopDisplayMode BEGIN");
	...
	UncacheShadows();
	D3DRES_LOG("  After Models_ClearVertexArrays + UncacheShadows");
	if (gl_eCurrentAPI == GAT_D3D && gl_pd3d9Device) {
		D3DRES_LOG_REFCOUNT("gl_pd3d9Device", gl_pd3d9Device);
	}
	...
	D3DRES_LOG("  After ReleaseShadowTexture");
	...
	ReleaseBloomTexture();
	// sehan end
	D3DRES_LOG("  After ReleaseRefraction/Reflection/BloomTexture");
	if (gl_eCurrentAPI == GAT_D3D && gl_pd3d9Device) {
		D3DRES_LOG_REFCOUNT("gl_pd3d9Device", gl_pd3d9Device);
	}
	...
	D3DRES_LOG("  After TR_UnbindTerrainTextures");
	...
	D3DRES_LOG("  After unbind %d stock textures", textureCount);
	if (gl_eCurrentAPI == GAT_D3D && gl_pd3d9Device) {
		D3DRES_LOG_REFCOUNT("gl_pd3d9Device", gl_pd3d9Device);
	}
	...
	_ptdFlat->td_tpLocal.Clear();
	_ptdFlat->Unbind();
	D3DRES_LOG("  After unbind fog/haze/flat textures");
	...
	D3DRES_LOG("  After unbind %d mesh VBs", meshCount);

	// also delete morph buffers used by SKA models
	extern void DeleteMorphedDynamicBuffer(void);
	DeleteMorphedDynamicBuffer();
	D3DRES_LOG("  After DeleteMorphedDynamicBuffer");
	if (gl_eCurrentAPI == GAT_D3D && gl_pd3d9Device) {
		D3DRES_LOG_REFCOUNT("gl_pd3d9Device (pre EndDriver)", gl_pd3d9Device);
	}

	// release cached DX9 shader handles (safety cleanup)
	int savedVsCount = 0;
	int savedPsCount = 0;
	for (INDEX i = 0; i < 1000; i++) {
		stVertexShaderInfo* pInfo = _vs[i];
		if (pInfo != NULL) {
			if (pInfo->vertexDeclaration) {
				pInfo->vertexDeclaration->Release();
				pInfo->vertexDeclaration = NULL;
			}
			if (pInfo->vertexShader) {
				pInfo->vertexShader->Release();
				pInfo->vertexShader = NULL;
			}
			delete pInfo;
			_vs[i] = NULL;
			savedVsCount++;
	...
	if (m_defaultDeclaration != NULL) {
		m_defaultDeclaration->Release();
		m_defaultDeclaration = NULL;
	}
	if (m_defaultProgrammable != NULL) {
		m_defaultProgrammable->Release();
		m_defaultProgrammable = NULL;
	}
	if (m_defaultFixed != NULL) {
		m_defaultFixed->Release();
		m_defaultFixed = NULL;
	}
	D3DRES_LOG("  Released saved DX9 shaders: VS=%d PS=%d", savedVsCount, savedPsCount);
	D3D9_DEVREF_DUMP("StopDisplayMode (after shader cleanup, before EndDriver)");
...
//¾ÈÅÂÈÆ ¼öÁ¤ ³¡	//(Add & Modify SSSE Effect)(0.1)
	...
	D3DRES_LOG_MARKER("StopDisplayMode END");
}
...
BOOL CGfxLibrary::LockDrawPort( CDrawPort *pdpToLock)
{
	...
	{ 
		...
		//D3DVIEWPORT9 d3dViewPort = { pixMinSI, pixMinSJ, pixMaxSI-pixMinSI, pixMaxSJ-pixMinSJ, 0,1 };
		D3DVIEWPORT9 d3dViewPort = {
			static_cast<DWORD>(pixMinSI),
			static_cast<DWORD>(pixMinSJ),
			static_cast<DWORD>(pixMaxSI - pixMinSI),
			static_cast<DWORD>(pixMaxSJ - pixMinSJ),
			0, 1
		};
...
{
	// ±è¿µÈ¯ : Å¬¶óÀÌ¾ðÆ®¿¡¼­ »ç¿ë¾ÈÇÔ ¾Æ¸¶ Åø¿¡¼­ ÇÏ¿ëÇÒ µí.
...
void CGfxLibrary::DestroyWorkCanvas(CDrawPort *pdpOld)
{
	...
	//	±è¿µÈ¯ Å¬¶óÀÌ¾ðÆ®¿¡¼­ »ç¿ë¾ÈÇÏ´Â ÇÔ¼ö Åø¿¡¼­ »ç¿ëÇÒ µí
	if(_bClientApp)
		return; 
	::DestroyWindow(hwnd);
}
			...
			{ // windowed mode
				if( pvp->vp9_pSwapChain != NULL)
				{
					if (g_web.IsWebHandle())
					{
						...
						hr = pvp->vp9_pSwapChain->Present(&rcTop, &rcTop, _hwndMain, NULL, NULL);
						if (!FAILED(hr)) {
							pvp->vp9_pSwapChain->Present(&rcBottom, &rcBottom, _hwndMain, NULL, NULL);
						}
						if (!FAILED(hr)) {
							pvp->vp9_pSwapChain->Present(&rcLeft, &rcLeft, _hwndMain, NULL, NULL);
						}
						if (!FAILED(hr)) {
							pvp->vp9_pSwapChain->Present(&rcRight, &rcRight, _hwndMain, NULL, NULL);
						}
					}
					else
					{
						hr = pvp->vp9_pSwapChain->Present( NULL, NULL, NULL, NULL, NULL);
					}
				}
			} else
			{ // full screen mode
				if (g_web.IsWebHandle())
				{
					...
					hr = gl_pd3d9Device->Present(&rcTop, &rcTop, _hwndMain, NULL);  
					if (!FAILED(hr)) {
						hr = gl_pd3d9Device->Present(&rcBottom, &rcBottom, _hwndMain, NULL);  
					}
					if (!FAILED(hr)) {
						hr = gl_pd3d9Device->Present(&rcLeft, &rcLeft, _hwndMain, NULL);
					}
					if (!FAILED(hr)) {
						hr = gl_pd3d9Device->Present(&rcRight, &rcRight, _hwndMain, NULL);
					}
				}
				else
				{
					hr = gl_pd3d9Device->Present( NULL, NULL, NULL, NULL);
				}
			} // done swapping
			...
			// ÀÓÀÇ °Ë»ç¸¦ À§ÇØ¼­´Â IDirect3DDevice::TestCooperativeLevel()¸¦ È£ÃâÇØ¾ßÇÑ´Ù.
 			if (FAILED(hr))
 			{
				if (D3DERR_DEVICELOST == hr)
				{
					...
					{
						if (D3DERR_DEVICELOST == hr)
						{ // µð¹ÙÀÌ½º¸¦ ¼Ò½ÇÇß°í, º¹±¸°¡ ºÒ°¡´É »óÅÂ
							Sleep(1); // ### -> (500) old
							hr = gl_pd3d9Device->TestCooperativeLevel(); // º¹±¸ °¡´É »óÅÂ±îÁö ´ë±â ÇÑ´Ù.
						}
						else if (D3DERR_DEVICENOTRESET == hr)
						{ // µð¹ÙÀÌ½º¸¦ ¼Ò½ÇÇßÁö¸¸, º¹±¸°¡ °¡´ÉÇÑ °æ¿ì
...
static void AdjustGammaRamp( CViewPort *pvp)
{
	...
	if( _pGfx->gl_eCurrentAPI==GAT_OGL) {
		...
		// Direct3D
		_pGfx->gl_pd3d9Device->SetGammaRamp(/* #### [in] UINT iSwapChain */ 0, D3DSGR_NO_CALIBRATION, (D3DGAMMARAMP*)&_auwGammaTable[0]);
	}
