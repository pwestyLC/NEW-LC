...
#ifndef NDEBUG
      // paranoid!
      hr = pd3dDev->SetSamplerState(iTU, D3DSAMP_MIPFILTER, (ULONG*)&eMipFilter);
      D3D_CHECKERROR(hr);
      ASSERT( eMipFilter==D3DTEXF_POINT || eMipFilter==D3DTEXF_LINEAR);
#endif // set it
      hr = pd3dDev->SetSamplerState(iTU, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
    }
    // yes mipmaps?
    else {
      switch( iMipFilter) {
      ...
      hr = pd3dDev->SetSamplerState(iTU, D3DSAMP_MIPFILTER, eMipFilter);
    } 
...
extern BOOL UploadCompressedTexture_D3D( LPDIRECT3DTEXTURE9 *ppd3dTexture, UBYTE *pubTexture, PIX pixSizeU,
