...
void CGfxLibrary::InitContext_OGL(void)
{
  ...
  if( pwglGetExtensionsStringARB != NULL) {
	...
	CPrintF( TRANS("OpenGL: Found WGL_ARB_extensions_string\n"));
  }
  ...
  if( HasExtension( go_strExtensions, "GL_ARB_multitexture")) {
    ...
    if( gl_ctRealTextureUnits>1 && HasExtension( go_strExtensions, "GL_EXT_texture_env_combine")) {
	  ...
	  CPrintF( TRANS("OpenGL: Found GL_ARB_multitexture\n"));
      CPrintF( TRANS("OpenGL: Found GL_EXT_texture_env_combine\n"));
    } else {
  ...
  if( HasExtension( go_strExtensions, "GL_ARB_texture_compression")) {
	...
	CPrintF( TRANS("OpenGL: Found GL_ARB_texture_compression\n"));
  }
  ...
  if( HasExtension( go_strExtensions, "GL_EXT_texture_lod_bias")) {
    ...
    OGL_CHECKERROR;
	CPrintF( TRANS("OpenGL: Found GL_EXT_texture_lod_bias\n"));
  }
  ...
  if( HasExtension( go_strExtensions, "GL_EXT_texture_filter_anisotropic")) {
    ...
    OGL_CHECKERROR;
	CPrintF( TRANS("OpenGL: Found GL_EXT_texture_filter_anisotropic\n"));
  }
  ...
  if( HasExtension( go_strExtensions, "GL_EXT_compiled_vertex_array")) {
	...
	CPrintF( TRANS("OpenGL: Found GL_EXT_compiled_vertex_array\n"));
  }
  ...
  if( HasExtension( go_strExtensions, "WGL_EXT_swap_control")) {
	...
	CPrintF( TRANS("OpenGL: Found WGL_EXT_swap_control\n"));
  }
  ...
  if( HasExtension( go_strExtensions, "GL_ATI_pn_triangles")) {
    ...
    OGL_CHECKERROR;
	CPrintF( TRANS("OpenGL: Found GL_ATI_pn_triangles\n"));
  } 
  ...
  { // prepare extension's functions
    ...
    ASSERT( pglGenOcclusionQueriesNV!=NULL && pglDeleteOcclusionQueriesNV!=NULL
         ...
         && pglGetOcclusionQueryivNV!=NULL && pglGetOcclusionQueryuivNV!=NULL
         && pglIsOcclusionQueryNV!=NULL);
		 CPrintF( TRANS("OpenGL: Found GL_NV_occlusion_query\n"));
  }
  ...
  // init vertex buffers
  CPrintF( TRANS("OpenGL: Init vertex buffers\n"));
  ...
  CPrintF( TRANS("OpenGL: Generate occlusion query IDs\n"));
  ...
  UpdateGfxSysCVars();
  CPrintF( TRANS("OpenGL: Update Gfx Sys CVars\n"));
  ...
  CPrintF( TRANS("OpenGL: Try Reload Textures ...\n"));
  ReloadTextures();
  CPrintF( TRANS("OpenGL: Try Reload Meshes ...\n"));
  ReloadMeshes();
  if( shd_bCacheAll) {
    CPrintF( TRANS("OpenGL: Try Cache Shadows ...\n"));
    CacheShadows();
  }
  CPrintF( TRANS("OpenGL: Init Context OGL done\n"));
}
