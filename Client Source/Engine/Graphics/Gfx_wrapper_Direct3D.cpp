...
static void UpdateClipPlane_D3D(void)
{
	...
	if( _pGfx->gl_ulFlags&GLF_D3D_CLIPPLANE) {
		HRESULT hr = _pGfx->gl_pd3d9Device->SetClipPlane( 0, &afObjectClipPlane[0]);
		D3D_CHECKERROR(hr);
	} 
	// keep it as current
	pulLast[0] = pulThis[0];
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetTextureStageState( GFX_iActiveTexUnit, D3DTSS_COLOROP, (DWORD*)&bRes);
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_ZENABLE, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
...
static void d3d_EnableDepthBias(void)
{
	...
	HRESULT hr = _pGfx->gl_pd3d9Device->SetRenderState(D3DRS_DEPTHBIAS, 2);
	D3D_CHECKERROR(hr);
...
static void d3d_DisableDepthBias(void)
{
	...
	HRESULT hr = _pGfx->gl_pd3d9Device->SetRenderState(D3DRS_DEPTHBIAS, 0);
	D3D_CHECKERROR(hr);
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_ZWRITEENABLE, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
...
static void d3d_EnableDither(void)
{
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_DITHERENABLE, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_DITHERENABLE, TRUE);
	D3D_CHECKERROR(hr);
...
static void d3d_DisableDither(void)
{
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_DITHERENABLE, FALSE);
	D3D_CHECKERROR(hr);
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_ALPHATESTENABLE, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_ALPHABLENDENABLE, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
...
static void d3d_SetVertexProgram(const ULONG ulHandle)
{
	...
	if(_pGfx->gl_dwVertexShader!=ulHandle) {
		//CPrintF("d3d_SetVertexProgram: ulHandle = %u\n", ulHandle);
		// see if required shader has already been created
		for (iVS = 0; iVS < ctProgramShaders; iVS++) {
			if (_avsVertexShaderProgram[iVS].vp_ulProgramHandle == ulHandle) {
				//CPrintF("d3d_SetVertexProgram: Handle = %u, Found = %u,  Count = %u\n", ulHandle, iVS, ctProgramShaders);
				break;
			}
		}
		HRESULT hr = _pGfx->gl_pd3d9Device->SetVertexShader(_avsVertexShaderProgram[iVS].Shader); // ###
		D3D_CHECKERROR(hr);
		...
		_currentVS_Declaration	= _avsVertexShaderProgram[iVS].Declaration;
...
static void d3d_SetPixelProgram(LPDIRECT3DPIXELSHADER9 ulHandle)
{
	ASSERT( _pGfx->gl_eCurrentAPI==GAT_D3D);

	extern LPDIRECT3DPIXELSHADER9 _dwCurrentPS;
...
static void d3d_DeleteVertexProgram(ULONG ulHandle)
{
	...
	HRESULT hr = dxDeleteVertexShader(ulHandle, 0);
	//HRESULT hr = _pGfx->gl_pd3dDevice->DeleteVertexShader(ulHandle, 0);
	D3D_CHECKERROR(hr);
}


static void d3d_DeletePixelProgram(LPDIRECT3DPIXELSHADER9 ulHandle)
{
	ASSERT(_pGfx->gl_eCurrentAPI==GAT_D3D);
	ASSERT(ulHandle!=NONE);
...
static ULONG d3d_CreateVertexProgram(const char *strVertexProgram, ULONG ulStreamFlags)
{
	...
	//else {
	//	CPrintF("D3D wrapper: Create Vertex Shader Done!\n"); //return NULL;
	//}
	hr = _pGfx->gl_pd3d9Device->CreateVertexDeclaration(ulDeclaration, &vsp.Declaration);
	if (FAILED(hr)) {
		CPrintF("D3D wrapper: Create Vertex Declaration Error!!! Code: %u\n", hr); //return NULL;
	}
	//else {
	//	CPrintF("D3D wrapper: Create Vertex Declaration Done!"); //return NULL;
	//}
	...
	return  (ULONG)_avsVertexShaderProgram.Count();;;
}
...
#if _DEBUG
	FLOAT faData[96*4];
	hr = _pGfx->gl_pd3d9Device->GetVertexShaderConstantF(iRegister, static_cast<float *>(&faData), ctRegisters);
	D3D_CHECKERROR(hr);
...
static void d3d_EnableLighting(void)
{
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_LIGHTING, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_LIGHTING, TRUE);
	D3D_CHECKERROR(hr);
...
static void d3d_DisableLighting(void)
{
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_LIGHTING, FALSE);
	D3D_CHECKERROR(hr);
...
static void d3d_EnableClipping(void)
{
	...
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_CLIPPING, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
...
static void d3d_EnableClipPlane(void)
{
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_CLIPPLANEENABLE, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_CLIPPLANEENABLE, D3DCLIPPLANE0);
	D3D_CHECKERROR(hr);
...
static void d3d_DisableClipPlane(void)
{
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_CLIPPLANEENABLE, FALSE);
	D3D_CHECKERROR(hr);
...
extern void d3d_EnableColorArray(void)
{
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_COLORVERTEX, (DWORD*)&bRes);
	D3D_CHECKERROR(hr);
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_COLORVERTEX, TRUE);
	D3D_CHECKERROR(hr);
...
static void d3d_DisableColorArray(void)
{
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_COLORVERTEX, FALSE);
	D3D_CHECKERROR(hr);
...
static void d3d_EnableTruform(void)
{
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState(((D3DRENDERSTATETYPE)164), (DWORD*)&fSegments);
	D3D_CHECKERROR(hr);
	...
	hr = _pGfx->gl_pd3d9Device->SetRenderState((D3DRENDERSTATETYPE)164, *((DWORD*)&fSegments));
	D3D_CHECKERROR(hr);
...
static void d3d_BlendFunc( GfxBlend eSrc, GfxBlend eDst)
{
	...
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_SRCBLEND,  (DWORD*)&d3dSrc);  D3D_CHECKERROR(hr);
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_DESTBLEND, (DWORD*)&d3dDst);  D3D_CHECKERROR(hr);
...
#ifndef NDEBUG
	GfxComp gfxFunc; 
	hr = _pGfx->gl_pd3d9Device->GetRenderState( D3DRS_ZFUNC, (DWORD*)&d3dcFunc);
	D3D_CHECKERROR(hr);
...
#ifndef NDEBUG
//	05.31
//	hr = _pGfx->gl_pd3d9Device->GetViewport( &d3dViewport);										// \BF\F8\BA\BB.
//	ASSERT( d3dViewport.MinZ==GFX_fMinDepthRange && d3dViewport.MaxZ==GFX_fMaxDepthRange);		// \BF\F8\BA\BB.
//	05.31
#endif
	...
	// get viewport
	hr = _pGfx->gl_pd3d9Device->GetViewport( &d3dViewport);
	D3D_CHECKERROR(hr);
...
static void d3d_PolygonMode( GfxPolyMode ePolyMode)
{
	...
	HRESULT hr;
	switch(ePolyMode) {
	case GFX_POINT:  hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_FILLMODE, D3DFILL_POINT);      break;
	case GFX_LINE:   hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_FILLMODE, D3DFILL_WIREFRAME);  break;
	case GFX_FILL:   hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_FILLMODE, D3DFILL_SOLID);      break;
...
static void d3d_EnableLight( INDEX iLight)
{
...
#ifndef NDEBUG
	BOOL bRes; 
	hr = _pGfx->gl_pd3d9Device->GetLightEnable( iLight, &bRes);
	D3D_CHECKERROR(hr);
	...
	hr = _pGfx->gl_pd3d9Device->LightEnable( iLight, TRUE);
	D3D_CHECKERROR(hr);
...
static void d3d_DisableLight( INDEX iLight)
{
	...
	hr = _pGfx->gl_pd3d9Device->LightEnable( iLight, FALSE);
	D3D_CHECKERROR(hr);
...
static void d3d_SetTextureWrapping( enum GfxWrap eWrapU, enum GfxWrap eWrapV)
{
	...
	hr = _pGfx->gl_pd3d9Device->GetTextureStageState( GFX_iActiveTexUnit, D3DTSS_ADDRESSU, (ULONG*)&d3dtaU);  D3D_CHECKERROR(hr);
	hr = _pGfx->gl_pd3d9Device->GetTextureStageState( GFX_iActiveTexUnit, D3DTSS_ADDRESSV, (ULONG*)&d3dtaV);  D3D_CHECKERROR(hr);
...
/*
	INDEX iRet;
	hr = _pGfx->gl_pd3d9Device->GetTextureStageState( GFX_iActiveTexUnit, D3DTSS_COLOROP, (DWORD*)&iRet);
	D3D_CHECKERROR(hr);
...
static void d3d_GenerateTexture( ULONG64 &ulTexObject)
{
	...
	D3D_CHECKERROR(hr);
	D3DRES_LOG_CREATE(LPDIRECT3DTEXTURE9, (void*)ulTexObject, "1x1 dummy");
...
static void d3d_DeleteTexture( ULONG64 &ulTexObject)
{
   ...
   D3DRES_LOG_TEXTURE("d3d_DeleteTexture ptr=0x%p", (void*)ulTexObject);
...
static void d3d_SetSubBuffer(const INDEX iID, const INDEX iUnit/*=-1*/, const INDEX tedt, const INDEX tedt2)
{
...
static void d3d_Finish( BOOL bReadBack)
{
	...
	if( bFullScreen) {
		...
		hr = _pGfx->gl_pd3d9Device->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
	} else if( _pGfx->gl_pvpActive!=NULL) {
		// thru swapchain in window
		hr = _pGfx->gl_pvpActive->vp9_pSwapChain->GetBackBuffer( 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
	}
...
// ### Only fixed functions here (not vertex shaders), 
// ### i see this on debug mode, put 'SetFVF'
