...
//°­µ¿¹Î ¼öÁ¤ ³¡ Å×½ºÆ® Å¬¶óÀÌ¾ðÆ® ÀÛ¾÷		06.16
			HRESULT hr;
			hr = _pGfx->gl_pd3d9Device->SetClipPlane( 0, &_plOld[0]);				D3D_CHECKERROR(hr);
			//gfxDisableClipPlane();
//°­µ¿¹Î ¼öÁ¤ ½ÃÀÛ Å×½ºÆ® Å¬¶óÀÌ¾ðÆ® ÀÛ¾÷	06.16
			}
//°­µ¿¹Î ¼öÁ¤ ³¡ Å×½ºÆ® Å¬¶óÀÌ¾ðÆ® ÀÛ¾÷		06.16
...
//	{
		...
		shaSetVertexProgramConst_EyePos();			//c19					// Ä«¸Þ¶ó À§Ä¡.

		// c10 -> LightPos
		shaSetVertexProgramConst_Fixup();			//c11					// Å×ÀÏ·¯ ±Þ¼ö Àü°³ÀÇ ºÎÁ¤È®¼ºÀ» º¸Á¤ÇÏ±â À§ÇÑ °è¼ö c11		
...
extern void shaSetEMBM()
{
	...
	//{
		...
		*/
		...
		// EMBM ¼³Á¤.
		hr = _pGfx->gl_pd3d9Device->SetTextureStageState( 1, D3DTSS_BUMPENVMAT00,   F2DW(matBumpMat._11));	D3D_CHECKERROR(hr);
...
{
	_pGfx->gl_pd3d9Device->GetVertexShaderConstantF(iRegister, static_cast<float *>((void*)(&pData)), 1);
}
extern void shaGetVertexProgramConst(INDEX iRegister, Matrix12 &pmData)
{
	_pGfx->gl_pd3d9Device->GetVertexShaderConstantF(iRegister, static_cast<float *>((void*)(pmData)), 3);
}
extern void shaGetVertexProgramConst(INDEX iRegister, Matrix16 &pmData)
{
	_pGfx->gl_pd3d9Device->GetVertexShaderConstantF(iRegister, static_cast<float *>((void*)(pmData)), 4);
}
// Set const pixel program register
extern void shaGetPixelProgramConst(INDEX iRegister, ShaderRegister &pData)
{
	_pGfx->gl_pd3d9Device->GetPixelShaderConstantF(iRegister, static_cast<float *>((void*)(&pData)), 1);
}
//¾ÈÅÂÈÆ ¼öÁ¤ ³¡	//(Add Tagent-space Normal Map)(0.1)
...
//¾ÈÅÂÈÆ ¼öÁ¤ ½ÃÀÛ	//(Add Tagent-space Normal Map)(0.1)
//¾ÈÅÂÈÆÀÌ ÀÌ ºÎºÐÀÇ ÄÚµå¸¦ ShaderCode.hÂÊÀ¸·Î »©¼­ Á¤¸®ÇßÀ½.
...
extern void shaDoFogAndHazePass(void)
{
	...
	if(_ulRenFlags&SHA_RMF_HAZE) {
		...
		if (sam_iGfxAPI == GAT_OGL){
			...
			_pGfx->gl_pd3d9Device->GetTextureStageState(GFX_iActiveTexUnit, D3DTSS_COLOROP, &op);
	...
	if(_ulRenFlags&SHA_RMF_FOG) {
		...
		if ((_ulRenFlags&SHA_RMF_HAZE) && (sam_iGfxAPI == GAT_OGL))
		{
			// ### TODO:
			// ### opengl
			// ###
		} else if ((_ulRenFlags&SHA_RMF_HAZE) && (sam_iGfxAPI == GAT_D3D))
		{
			_pGfx->gl_pd3d9Device->GetTextureStageState(GFX_iActiveTexUnit, D3DTSS_COLOROP, &op);
			_pGfx->gl_pd3d9Device->GetTextureStageState(GFX_iActiveTexUnit, D3DTSS_COLORARG1, &arg1);
			_pGfx->gl_pd3d9Device->SetTextureStageState(GFX_iActiveTexUnit, D3DTSS_COLOROP, D3DTOP_SELECTARG1);
...
extern void shaSetNormalBufferID(INDEX iNormalBufferID, INDEX iFirstVertex, INDEX ctVertices)
{
	ASSERT(_bUseHWVertexBuffers); // must use hardware shaders
	...
	gfxSetNormalSubBuffer(_iNormalBufferID, -1, _iFirstVertex, ctVertices);
}

//¾ÈÅÂÈÆ ¼öÁ¤ ½ÃÀÛ	//(Add Tagent-space Normal Map)(0.1)
extern void shaSetTangentBufferID(INDEX iTagentBufferID, INDEX iFirstVertex, INDEX ctVertices)
{
	ASSERT(_bUseHWVertexBuffers); // must use hardware shaders
	ASSERT(_iVertexBufferID >= 0); // vertex buffer must be set before normal buffer
	...
	gfxSetTangentSubBuffer(_iTangentBufferID, -1, _iFirstVertex, ctVertices);
}
//¾ÈÅÂÈÆ ¼öÁ¤ ³¡	//(Add Tagent-space Normal Map)(0.1)

extern void shaSetWeightBufferID(INDEX iWeightBufferID, INDEX iFirstVertex, INDEX ctVertices)
{
	ASSERT(_bUseHWVertexBuffers); // must use hardware shaders
	...
	gfxSetWeightSubBuffer(_iWeightBufferID, -1, _iFirstVertex, ctVertices);
}
...
extern void shaSetTexCoordBufferID(INDEX iTexCoordBufferID, INDEX iTextureUnit/*=0*/)
{
	...
	gfxSetTexCoordSubBuffer(iTexCoordBufferID, iTextureUnit, _iFirstVertex, _ctVertices);
}
...
{
	ASSERT(FALSE && "¾ÆÁ÷ ±¸ÇöµÇÁö ¾ÊÀº ºÎºÐÀÓ");
}
...
static void shaSetVertexProgram_NMMTanSpace(INDEX iVertexProgram)
{
	...
	//--- Block A Begin ---//
	INDEX ivp = iVertexProgram;
	//NotYet : ¾ÆÁ÷ Æ÷±×¿Í ¿¬µ¿µÇ´Â ºÎºÐÀº ÀÛ¼ºÇÏÁö ¾Ê¾ÒÀ½.
	const BOOL bHasFog = (_ulRenFlags & SHA_RMF_HAZE || _ulRenFlags & SHA_RMF_FOG) && _iFogTexUnit >= 0;
	// if in fog or haze
	if(bHasFog)
	{
	...
	//--- Block A End ---//
	//--- Block A ÇØ¼³ ---//
	//CShader¿¡¼­ GetDesc·Î ShaderÀÛ¼ºÀÚ°¡ ¼³Á¤ÇÑ VPÀÇ °³¼ö¸¦ ¹Þ¾Æ *2¸¦ ÇÑÈÄ sha_aVPHandles¸¦ ¸¸µç´Ù.
	//±×¸®°í Æ÷±×³ª ÇìÀÌÁî°¡ ÀÖÀ¸¸é ¿ø·¡ÀÇ VPÀÇ index¿¡ °³¼ö¸¦ ´õÇØ *2·Î ÀÎÇØ Ãß°¡µÈ ºÎºÐÀÇ VP¸¦ »ç¿ëÇÑ´Ù.
	//±×°ÍÀº ÀÌ µÞºÎºÐ¿¡ AssembleShaderProgramµÚ¿¡ Ã³¸®µÈ´Ù.
	...
	if(ulHandle==NONE) {
		...
		//TODO : ¾ÆÁ÷ Æ÷±×¿Í ¿¬µ¿µÇ´Â ºÎºÐÀº ÀÛ¼ºÇÏÁö ¾Ê¾ÒÀ½.
...
extern void shaSetPixelProgram(INDEX iPixelProgram)
{
	...
	LPDIRECT3DPIXELSHADER9 &ulHandle = pph.pph_ulHandle;
...
void CShader::DeleteHandles(void)
{
	...
	for(INDEX ipp=0;ipp<ctpp;ipp++) {
		...
		LPDIRECT3DPIXELSHADER9 &ulHandle = pp.pph_ulHandle;
		if(ulHandle!=NULL) {
