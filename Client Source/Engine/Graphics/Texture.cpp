...
#include <../include/libpng-1.6.3/include/lpng/png.h>
...
static void RemoveOversizedMipmaps( CTextureData *pTD)
{
	...
	if( pTD->td_ulFlags & TEX_COMPRESSED) 
	{
		...
		slNewFrameSize = slOldFrameSize - ((SLONG)(size_t)pubOldFrame - (SLONG)(size_t)pTD->td_pulFrames);
...
BOOL CTextureData::UploadCompressed(void)
{
	...
	if( td_ulObject==NONE) 
	{
		...
		if( td_ctFrames>1) 
		{ // animation textures
			td_pulObjects = (ULONG64*)AllocMemory( td_ctFrames *sizeof(td_ulObject));
	...
	ULONG64 *pulTexObject = (td_ctFrames>1) ? td_pulObjects : &td_ulObject;
	for( INDEX iFr=0; iFr<td_ctFrames; iFr++)
	{ // determine frame offset and upload texture frame
...
//안태훈 수정 끝	//(Create Effect Texture Bug Fix)(0.1)
	...
	if( td_ctFrames>1 && !td_tpLocal.IsEqual(_tpGlobal[0])) 
	{
		...
		{
			td_tpLocal.Clear();
			gfxSetTexture( ((ULONG64*)td_ulObject)[iFr], td_tpLocal);
		}
	} 
	
	// set corresponding probe or texture frame as current
	ULONG64 ulTexObject = td_ulObject; // single-frame
	if( td_ctFrames>1) ulTexObject = ((ULONG64*)td_ulObject)[iFrameNo]; // animation
	if( bUseProbe) 
	{
...
SLONG CTextureData::GetUsedMemory(void)
{
	...
	ULONG64 ulTexObject = td_ulObject;
