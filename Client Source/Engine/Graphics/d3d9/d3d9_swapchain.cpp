...
namespace dxvk {
  ...
  bool D3D9SwapChainEx::UpdateWindowCtx() {
    ...
    if (entry == m_presenters.end()) {
      // Clean up presenters for windows that no longer exist
      // to avoid leaking Vulkan swapchain resources
      for (auto it = m_presenters.begin(); it != m_presenters.end(); ) {
        if (!wsi::isWindow(it->first)) {
          if (it->second.presenter) {
            it->second.presenter->destroyResources();
            it->second.presenter = nullptr;

            if (m_presentParams.hDeviceWindow == it->first)
              DestroyLatencyTracker();
          }
