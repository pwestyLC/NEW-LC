...
void CUIBattlePass::OpenUI()
{
	...
	for (int i = 0; i < 10 ; i++)
	{
		...
		else if (m_pImgPremiumLock[i] && !m_nPremium)
			m_pImgPremiumLock[i]->Hide(FALSE);
	}

	//fuck is how many are checked
	int fuck = m_nLevel  - (m_nCurrentPage * 10);
	if (fuck < 0)
		fuck = 0;

	for (int i = 0; i < 10 && i < fuck ; i++)
	{
...
WMSG_RESULT CUIBattlePass::OnMouseWheel( UINT16 x, UINT16 y, int wheel )
{
	if (m_bHide)
		return WMSG_FAIL;
