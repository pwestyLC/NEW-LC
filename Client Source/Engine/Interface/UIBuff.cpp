...
void CUIBuff::RenderToolTip(int nWhichBuff, __int64 llCurTime)
{
	...
	if (m_nCurInfoLines > 1)
	{
		...
		BOOL bUpSkill; // IsUpSkill()���� ������ �ε����� �ϵ��ڵ� �Ǿ� ����.
		...
		// Date : 2005-09-09(���� 4:12:22), By Lee Ki-hwan
		if (bUpSkill)
		{
			m_strBuffInfo[m_nRemainTimeLine].PrintF(_S(1885, "���� �ð� : ������"), llRemainTime);
		}
		else
		{
			if (llRemainTime < 0 ||
			...
			//wooss 050903
				...
				m_strBuffInfo[m_nRemainTimeLine].PrintF(_S(2510, "���� �Ⱓ : "));

				if (tv_day > 0) {
					tv_str.PrintF(_S(2511, "%d�� "), tv_day);
					m_strBuffInfo[m_nRemainTimeLine] += tv_str;
				}
				if (tv_hour > 0) {
					tv_str.PrintF(_S(2512, "%d�ð� "), tv_hour);
					m_strBuffInfo[m_nRemainTimeLine] += tv_str;
				}
				if (tv_min > 0) {
...
void CUIBuff::RenderMyBuff()
{
	...
	if (m_ubMyGoodCount > BUFF_GOOD_COUNT) {
		m_ubMyGoodCount = BUFF_GOOD_COUNT; // Clamp to maximum allowed
	}

	if (m_ubMyBadCount > BUFF_BAD_COUNT) {
		m_ubMyBadCount = BUFF_BAD_COUNT; // Clamp to maximum allowed
	}
...
void CUIBuff::RenderPartyBuff()
{
	...
	if (m_ubPartyGoodCount > BUFF_GOOD_COUNT) {
		m_ubPartyGoodCount = BUFF_GOOD_COUNT; // Clamp to maximum allowed
	}

	if (m_ubPartyBadCount > BUFF_BAD_COUNT) {
		m_ubPartyBadCount = BUFF_BAD_COUNT; // Clamp to maximum allowed
	}
...
void CUIBuff::RenderTargetBuff()
{
	...
	if (m_ubTargetGoodCount > BUFF_GOOD_COUNT) {
		m_ubTargetGoodCount = BUFF_GOOD_COUNT; // Clamp to maximum allowed
	}

	if (m_ubTargetBadCount > BUFF_BAD_COUNT) {
		m_ubTargetBadCount = BUFF_BAD_COUNT; // Clamp to maximum allowed
	}
...
void CUIBuff::RemoveMyBuff(SLONG slItemIndex, SLONG slSkillIndex)
{
	...
	if (rSkillData.GetFlag() & SF_FORHELP)
	{
		...
		if (slSkillIndex == 1760)
		{
			...
			m_nMSGID = MSGBOX_OK(_S(6281, "��ġ ���� ����"), _S(6205, "��ġ ���� �������� ��� �ð��� ���� �Ǿ� ��ġ�� ���� �˴ϴ�."));
		}
	}
	// Bad
	else
			...
			nY > m_rcMyGoodBuff.Top && nY < m_rcMyGoodBuff.Bottom)
		{
			for (int i = 0; i < MAX_BUFF_ROW; i++) {
					...
					nY > m_rcMyGoodBuffRow[i].Top && nY < m_rcMyGoodBuffRow[i].Bottom)
				{
					...
					case 590:
					case 465:
						UIMGR()->GetChattingUI()->AddSysMessage(_S(8406, "You cannot remove this buff or item"), SYSMSG_ERROR);
						return WMSG_FAIL;
						break;
					default:
						break;
					...
					MsgBox.SetMsgBoxInfo(_S(8366, ""), UMBS_OKCANCEL, UI_QUEST, MSGLCMD_REMOVE_SELECTED_BUFF);
					MsgBox.AddString(_S(8367, "Do you want to remove this buff?"), 0xa3a1a3ff);
					...
					MsgBox.AddString(CTString(" "), 0xa3a1a3ff);
					MsgBox.AddString(_S(8368, "Warning: A wrongly remove buff can not be restored"), 0xa3a1a3ff);
...
BuffInfo* CUIBuff::GetBuffInfoSkill(SLONG skillindex) {

	for (int i = 0; i < MAX_BUFF_COUNT; i++)
	{
		if ((m_aMyBuffInfo[i].m_slSkillIndex) == skillindex)
		{
			return &m_aMyBuffInfo[i];
		}
	}
	return NULL;

}
/*CBuffIcon* CUIBuff::GetBuffIconSkill(SLONG skillindex) {
