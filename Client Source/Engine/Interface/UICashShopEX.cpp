...
#include <Engine/Contents/function/DonationTierUIMini.h>
#include <Engine/Contents/function/DonationTierUI.h>
...
const INDEX _RankCount = 5; // ��õ ������ ��ũ ����
const INDEX _GoodsListCount = 8; // ��ǰ ��� ǥ�� ����(��������)
const INDEX _PurchaseListCount = 25;
const INDEX _PurchaseGetCount = 10; // ������ ��ǰ �� �ѹ��� �κ����� ������ �� �ִ� �ִ� ����
extern INDEX g_iCountry;

const INDEX _PreviewX = 140, _PreviewY = 226, _RTWidth = 366, _RTHeight = 226; // ĳ���͸� ǥ���ϴ� Preivew Screen ��ġ
...
//276,69
	...
	m_btnConfirm.Create(this, _S(2380, "����"), 0,0,108,21);
	...
	m_btnCancel.Create(this, _S(139, "���"), 0,0,108,21);
	...
	case CSP_Gift:
		{
			...
			m_btnConfirm.SetText(_S(3090, "������"));
			m_btnCancel.SetPos(185,174);
			m_btnCancel.SetText(_S(139, "���"));
			m_btnClose.SetPos(260,4);
	...
	case CSP_Package:
		{
			...
			pDrawPort->PutTextExCX(_S(5307, "��Ű�� ����"), m_nPosX+150, m_nPosY+4, 0xFFFFFFFF);
	...
	case CSP_Gift:
		{
			...
			pDrawPort->PutTextExCX(_S(3104, "���� ���� ģ��"), m_nPosX+150, m_nPosY+4, 0xFFFFFFFF);
			pDrawPort->PutTextExRX(_S(5214, "ĳ���� ��"), m_nPosX+142,m_nPosY+28,0xFFFFFFFF);
			pDrawPort->PutTextExRX(_S(5215, "ģ�����"), m_nPosX+142,m_nPosY+50,0xFFFFFFFF);
			pDrawPort->PutTextEx(_S(3101, "�����޽���"), m_nPosX+17,m_nPosY+79,0xFFFFFFFF);
			...
			// 100 byte�������� ���� MultiEditBox���� IME�ϼ��� �ȵǼ� ���� ����(�Է� ���� �������� ����)
			CTString temp;
	...
	case CSP_Purchase:
		{
			...
			pDrawPort->PutTextExCX(_S(5304, "���� ���� â"), m_nPosX+150, m_nPosY+4, 0xFFFFFFFF);
			pDrawPort->PutTextExCX(_S(2392, "������ ��ǰ"), m_nPosX+103,m_nPosY+40,0xFFFFFFFF);
			pDrawPort->PutTextExCX(_S(2397, "����"), m_nPosX+252,m_nPosY+40,0xFFFFFFFF);
			...
			pDrawPort->PutTextEx(_S(5216, "�� ���� �ݾ�"), m_nPosX+20,m_nPosY+279, 0xFFFFFFFF);
...
//	m_ModelRenderTarget->Init(_PreviewX,_PreviewY);
	...
	m_SelectedCount = 0; // �ִ� _PurchaseGetCount ����
	m_SelectedGoods = -1;
...
{
	/*if( m_nPosX < pixMinI || m_nPosX + GetWidth() > pixMaxI ||
		m_nPosY < pixMinJ || m_nPosY + GetHeight() > pixMaxJ )*/ // CashShop UI�� ���, ������ ���ͷ� �������� �Ѵ�.
		ResetPosition( pixMinI, pixMinJ, pixMaxI, pixMaxJ );
...
void CUICashShopEX::OpenCashShopEX()
{
	...
	int nX = GetAbsPosX() + 9;
	int nY = GetAbsPosY() + 138;
	pUIManager->GetDonationTierMini()->SetPos(nX, nY);
	pUIManager->GetDonationTierMini()->ToggleUI();
...
void CUICashShopEX::CloseCashShopEX( void )
{
	...
	UIMGR()->GetDonationTierMini()->ToggleUI();
		...
		/*		else if (m_ViewFrameType == VFT_GoodsList && m_cbCategoryType[CST_Second].MouseMessage(pMsg) != WMSG_FAIL)
				...
				else if (IsInsideRect(nX, nY, m_CashShopFrame[CSF_View]))
				{
					...
					else if (m_ViewFrameType == VFT_PurchaseList)
					{
						...
						if (m_BtnMove.MouseMessage(pMsg) != WMSG_FAIL) // ��� �ű��
						{
							MovePurchaseGoods();
							return WMSG_SUCCESS;
						}
							...
							case BIT_CashCharge: // ĳ�� ����
								{
									if (IsBila(g_iCountry) == TRUE || g_iCountry == USA || g_iCountry == RUSSIA)
									{
										...
										#ifdef MALL_CLASSIC
											ShellExecute(NULL, "open", "https://www.darkagelc.com/", NULL, NULL, SW_SHOWNORMAL);
										#else
											ShellExecute(NULL, "open", "https://www.darkagelc.com/", NULL, NULL, SW_SHOWNORMAL);
										#endif
									...
									else
									{
										...
										CTString strCallUrl;
										#ifdef MALL_CLASSIC
										strCallUrl = _S(5318, "https://www.darkagelc.com/");
										#endif
										strCallUrl = _S(5318, "https://www.darkagelc.com/");
...
//	m_BtnSearch.Render();
	...
	if (UIMGR()->GetDonationTierMini()->IsVisible() && !UIMGR()->GetDonationTierMini()->GetHide())
	{
		UIMGR()->GetDonationTierMini()->Render(pDrawPort);
	}
	pDrawPort->InitTextureData(m_ptdBaseTexture);
				...
				else
				{
					...
					else
					{
						if (m_GoodsBoxList[i].GoodsData->m_itemListCnt > 0)
						{
							...
							pDrawPort->PutTextEx(strCount, m_nPosX+rtTmpBox.Left+45, m_nPosY+rtTmpBox.Top+57, 0xFFFFFFFF); // ������ ����
						}
					}

					if (m_GoodsBoxList[i].GoodsData->IsNew()) // �Ż�ǰ
					{
					...
					if (m_GoodsBoxList[i].GoodsData->IsPopular()) // �α� ��ǰ
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
					...
					if (m_GoodsBoxList[i].GoodsData->IsDiscount()) // Ư�� ��ǰ
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
					...
					pDrawPort->PutTextEx(m_GoodsBoxList[i].GoodsData->m_itemName, 
						m_nPosX+rtTmpBox.Left+21, m_nPosY+rtTmpBox.Top+10, 0xFFFFFFFF); // ������ �̸�
					...
					strPrice.PrintF(_S(255, "����: %s"), strCash);

					if (m_GoodsBoxList[i].GoodsData->m_limitCnt > 0)
					{
						strRemain.PrintF(_S(2890, "���� ����: %d"), m_GoodsBoxList[i].GoodsData->m_limitCnt);
					}
					...
					pDrawPort->PutTextExRX(strPrice, m_nPosX+rtTmpBox.Left+203, m_nPosY+rtTmpBox.Top+41, 0xFFFFFFFF); // ��ǰ ����
					pDrawPort->PutTextExRX(strRemain, m_nPosX+rtTmpBox.Left+203, m_nPosY+rtTmpBox.Top+54, 0xFFFFFFFF); // ���� ����
				}
			}
	...
	case VFT_PurchaseList:
		{
			...
			m_BtnGift.Render();
//345
			pDrawPort->PutTextExCX(_S(2392, "������ ��ǰ"), m_nPosX+124, m_nPosY+218, 0xFFFFFFFF);
			pDrawPort->PutTextExCX(_S(2393, "�ű� ��ǰ"), m_nPosX+345, m_nPosY+218, 0xFFFFFFFF);
	...
	case VFT_GiftList:
		{
			...
			m_BtnGet.Render();
			m_GiftScroll.Render();
			pDrawPort->PutTextExCX(_S(3098, "���� ������"), m_nPosX+124, m_nPosY+218, 0xFFFFFFFF);
			pDrawPort->PutTextExCX(_S(3101, "���� �޽���"), m_nPosX+345, m_nPosY+218, 0xFFFFFFFF);
			...
			for (i=0; i<5; ++i)
			{
				if (!m_pIconRevGift[i]->IsEmpty())
				{
					...
					nTemp.PrintF(_S( 3091, "%d�� %d�� %d��" ), tmDate->tm_year+1900, tmDate->tm_mon+1, tmDate->tm_mday);
	...
	case VFT_WishList:
		{
			...
			for (i=0; i<8; ++i)
			{
				...
				else
				{
					...
					if (m_WishBoxList[i].GoodsData->IsNew()) // �Ż�ǰ
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
							m_uvFlag[CFI_New].U0, m_uvFlag[CFI_New].V0, m_uvFlag[CFI_New].U1, m_uvFlag[CFI_New].V1, 0xFFFFFFFF);
						nTmpX += (22+2);
					...
					if (m_WishBoxList[i].GoodsData->IsPopular()) // �α� ��ǰ
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
							m_uvFlag[CFI_Popular].U0, m_uvFlag[CFI_Popular].V0, m_uvFlag[CFI_Popular].U1, m_uvFlag[CFI_Popular].V1, 0xFFFFFFFF);
						nTmpX += (22+2);
					...
					if (m_WishBoxList[i].GoodsData->IsDiscount()) // Ư�� ��ǰ
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
							m_uvFlag[CFI_Discount].U0, m_uvFlag[CFI_Discount].V0, m_uvFlag[CFI_Discount].U1, m_uvFlag[CFI_Discount].V1, 0xFFFFFFFF);
						nTmpX += (22+2);
					...
					pUIManager->InsertCommaToString(strCash);
					strPrice.PrintF(_S(255, "����: %s"), strCash);
					strRemain.PrintF(_S(2890, "���� ����: %d"), m_WishBoxList[i].GoodsData->m_limitCnt);
					pDrawPort->PutTextExRX(strPrice, m_nPosX+rtTmpBox.Left+203, m_nPosY+rtTmpBox.Top+46, 0xFFFFFFFF); // ��ǰ ����
				}
			}
	...
	if (UIMGR()->GetDonationTier()->IsVisible() && !UIMGR()->GetDonationTier()->GetHide())
	{
		UIMGR()->GetDonationTier()->Render(pDrawPort);
	}
...
/// CashShop Frame Iventory begin() //////////////////////////////////////////////////////////////////////////////////////
	pDrawPort->PutTextExRX(_S(2389, "��ǰ �ݾ�"),  m_nPosX+630,m_nPosY+392, 0xFFFFFFFF);
	pDrawPort->PutTextExRX(_S(2390, "���� ĳ��"), m_nPosX+630,m_nPosY+418, 0xFFFFFFFF);
...
void CUICashShopEX::MainTabFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_BtnMainTab[CS_IrisShop].Create(this,_S(2572, "���̸��� ����"), 7,36,113,22);
	m_BtnMainTab[CS_PurchaseInfo].Create(this,_S(2386, "���� ����"), 121,36,113,22);
	m_BtnMainTab[CS_GotTheGift].Create(this,_S(3088, "���� ����"), 235,36,113,22);
	m_BtnMainTab[CS_WishPage].Create(this, _S(5218, "�Ҹ�����"), 349,36,113,22);
...
void CUICashShopEX::ViewFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_BtnMove.Create(this, _S(5219, "��� �ű��"), 71,463,96,21);
	...
	m_BtnGet.Create(this, _S(2383, "��������"), 299,363,96,21);
	...
	m_BtnGift.Create(this, _S(3089, "�����ϱ�"), 299,394,96,21);
...
void CUICashShopEX::PreviewFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_btnRestore.Create(this, _S(5220, "������� ������"), 603,268,129,21);
...
void CUICashShopEX::InvenFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_btnInven[BIT_Clear].Create(this, _S(5221, "��ٱ��� ����"), 564,439,164,21);
	m_btnInven[BIT_PurchaseItem].Create(this, _S(2385, "������ ����"), 564,464,164,21);
	m_btnInven[BIT_CashCharge].Create(this, _S(2387, "ĳ�� ����"), 564,488,164,21);
	m_btnInven[BIT_Exit].Create(this, _S(4117, "����"), 564,511,164,21);
...
{
	ULONG ulSearchType; // �˻� ����
	UBYTE ubCatalogCount; // ��ǰ ���� 0 ~ 8
	ULONG ulCTID; // ��ǰ ���� ����
	SLONG slRemainCount; // ���� ��ǰ ���� ����
	ULONG ulPrice; // ��ǰ ����
	ULONG ulCurrentPage; // ���� ������
	ULONG ulEndPage; // ��ü ������ ��
...
void CUICashShopEX::ClearCashType(void)
{
	...
	for (i=0; i<CSC_Total; ++i)
	{
		...
		if (i==0)
		{
			CS_FirstSubType& tmpCTTypeInfo1 = m_CashSubTypeInfo[i].FirstSubTypeIndexList.Push(); // �Ż�ǰ
			tmpCTTypeInfo1.iFirstSubType = 1;
			CS_FirstSubType& tmpCTTypeInfo2 = m_CashSubTypeInfo[i].FirstSubTypeIndexList.Push(); // Ư��
			tmpCTTypeInfo2.iFirstSubType = 2;
			CS_FirstSubType& tmpCTTypeInfo3 = m_CashSubTypeInfo[i].FirstSubTypeIndexList.Push(); // �α��ǰ
...
void CUICashShopEX::SetCashType(CNetworkMessage *istr)
{
	...
	for (i=0; i<ulSubTypeCount; ++i)
	{
		...
		//ASSERT(iFirstSubType > 1 && "is not first subtype"); // 1�� �������� none type���� ���� �Ǿ� �ִ�.
		if (iFirstSubType == 0) continue; // none type�� ������ �ʴ´�.
	...
	for (i=0; i<ulSubTypeCount; ++i)
	{
		...
		if (iSecondSubType == 0) continue; // none type�� ������ �ʴ´�.
		//ASSERT(iSecondSubType > 1 && "is not second subtype"); // 1�� �������� none type���� ���� �Ǿ� �ִ�.
...
void CUICashShopEX::SetRClickMenu(INDEX iItem)
{
	...
	m_tpCommand.AddMenuList(_S(5224, "������ �����ϱ�"), 0xc2bac5FF, CSC_ItemPurchase);
	if (tmpData->m_itemListCnt == 1) // ��Ű�� ��ǰ�� ��Ű�� ��ǰ �ȿ��� �Դ´�.
	{
		...
		{ // �ѹ� �ǻ�� ������ ���� �� ����.
			m_tpCommand.AddMenuList(_S(5225, "�ڽ�Ƭ �Ա�"), 0xc2bac5FF, CSC_CostumeWear);
		}
	}

	m_tpCommand.AddMenuList(_S(5226, "��ٱ��Ͽ� ���"), 0xc2bac5FF, CSC_AddInven);
	if (m_ViewFrameType == VFT_GoodsList)
	{
		m_tpCommand.AddMenuList(_S(5227, "�Ҹ� ���ڿ� ���"), 0xc2bac5FF, CSC_AddWishBox);
	}
	else if (m_ViewFrameType == VFT_WishList)
	{
		m_tpCommand.AddMenuList(_S(5228, "�Ҹ� ���ڿ��� ����"), 0xc2bac5FF, CSC_AddWishBox);
	}
	m_tpCommand.AddMenuList(_S(139, "���"), 0xc2bac5FF, CSC_Cancel);
	m_tpCommand.Show();
}
	...
	if (pItemData->GetType() == CItemData::ITEM_WEAPON || pItemData->GetType() == CItemData::ITEM_SHIELD)
	{
		...
		else
		{
			...
			if (pItemData->GetSubType() == CItemData::ITEM_SHIELD_HEAD) {
				...
				iType = WEAR_SHIELD;
				m_iEquipCashIndex[6] = iItemIndex; // ���и� dbitemindex�� ����.
			}
...
void CUICashShopEX::SetTakeOffModel(INDEX iRef)
{
	if (iRef > 0)
	{
		...
		{ // ���⸦ ���� ���� ���е� ���´�.
			iRef = 0; // 0���� ����
			iType = WEAR_WEAPON; // ���� �������� ������.//iType = WEAR_SHIELD;
			((CPlayerEntity*)CEntity::GetPlayerEntity(0))->HUD_WearingModel(WEAR_SHIELD,_pNetwork->MyCharacterInfo.job,0,FALSE);
			m_iEquipCashIndex[6] = 0;
		}
...
void CUICashShopEX::SendWishItems(ULONG ulCTID, BOOL bCommit)
{
	if (ulCTID > 0)
	{
		...
		if (bCommit)
		{
			nmItem << (UBYTE)MSG_EX_CASHITEM_WISHLIST_SAVE_REQ; // ���
		}
		else
		{
			nmItem << (UBYTE)MSG_EX_CASHITEM_WISHLIST_DEL_REQ; // ����			
		}
...
void CUICashShopEX::RevWishList(CNetworkMessage* istr, BOOL bCommit)
{
		...
		case 1:
			{
				if (bCommit)
				{
					Message(MSGCMD_CASH_EX_ERROR, _S(5270, "�Ҹ�����"), _S(5271, "�� �̻� �Ҹ� ���ڿ� �������� ���� �� �����ϴ�."), UMBS_OK);
				}
				else
				{
					Message(MSGCMD_CASH_EX_ERROR, _S(5270, "�Ҹ�����"), _S(5272, "�Ҹ� ���ڿ� �������� �ʴ� ��ǰ�Դϴ�."), UMBS_OK);
				}
			}
			break;
		case 2:
			{
				Message(MSGCMD_CASH_EX_ERROR, _S(5270, "�Ҹ�����"), _S(5273, "�ý��� �����Դϴ�. ��ڿ��� �����Ͻʽÿ�."), UMBS_OK);
			}
			break;
		}
	}
	else
	{
		if (bCommit)
		{
			Message(MSGCMD_CASH_EX_ERROR, _S(5270, "�Ҹ�����"), _S(5315, "�Ҹ����� ��⸦ �Ϸ��Ͽ����ϴ�"), UMBS_OK);
		}
