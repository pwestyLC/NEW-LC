...
// Date : 2005-11-18(���� 1:45:04), By Lee Ki-hwan
//#define SHOW_DAMAGE_TEXT	// text�� ǥ�õǴ� ������ 
extern cWeb			g_web;

extern INDEX	g_iCountry;

//added by sam 11/02/24 ���и� ���� ī��Ʈ 
const	int	   SPAM_CHAT_LENGTH				= 300;  //�幮�� �Ǵ��ϴ� ���� ���� 
const   int	   SPAM_CHAT_BLOCK_INPUT_COUNT  = 150;	// 1�е��� �Է� ���� ���� ī��Ʈ 
...
#define AKAN_TEST_COMMAND // ��ĭ��� �׽�Ʈ�� Ŭ���̾�Ʈ ���ɾ�
...
//FRANCE_SPAIN_CLOSEBETA_NA_20081124
	...
	m_colChatTypeColor[CHATMSG_EXPEDITION] = 0x91A7EAFF;	//[sora] ������ ä�� �߰�
	...
	// Date : 2005-04-14(���� 2:32:19), By Lee Ki-hwan
	// �߱� ������ ��쿡�� gm �޼��� ���������� ǥ�� 
	if( g_iCountry == CHINA )
	{
	...
	m_sChatInfo.colSendNameType[CHATMSG_EXPEDITION] = 0x91A7EAFF;	// [sora]������ ä�� �߰�
...
void CUIChatting::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
	...
	***/
	...
	// Option region
	int	nMaxSize = _S( 452, "�Ϲ�" ).Length();

#ifdef GER_MODIFY_PARTYCAHT_ABRIDGE_KEY_NA_20081224
	int nSize = _S( 453, "��Ƽ(?)" ).Length();
#else
	int nSize = _S( 453, "��Ƽ(@)" ).Length();
#endif
	...
	nSize = _S( 454, "���(#)" ).Length();
	if( nMaxSize < nSize ) nMaxSize = nSize;
	nSize = _S( 455, "�Ÿ�($)" ).Length();
	if( nMaxSize < nSize ) nMaxSize = nSize;
	nSize = _S( 456, "�ӼӸ�(%)" ).Length();
	if( nMaxSize < nSize ) nMaxSize = nSize;
	nSize = strlen( _S( 459, "��ġ��(%)" ) );
	if( nMaxSize < nSize ) nMaxSize = nSize;
	nSize = _S( 457, "�ý���" ).Length();
	if( nMaxSize < nSize ) nMaxSize = nSize;
	...
	m_cbtnChatOption[CHATOPT_NORMAL].SetText( _S( 452, "�Ϲ�" ) );

#ifdef GER_MODIFY_PARTYCAHT_ABRIDGE_KEY_NA_20081224
	m_cbtnChatOption[CHATOPT_PARTY].SetText( _S( 453, "��Ƽ(?)" ) );
#else
	m_cbtnChatOption[CHATOPT_PARTY].SetText( _S( 453, "��Ƽ(@)" ) );
#endif
	
	m_cbtnChatOption[CHATOPT_GUILD].SetText( _S( 454, "���(#)" ) );
	m_cbtnChatOption[CHATOPT_TRADE].SetText( _S( 455, "�Ÿ�($)" ) );
	m_cbtnChatOption[CHATOPT_WHISPER].SetText( _S( 456, "�ӼӸ�(%)" ) );
	m_cbtnChatOption[CHATOPT_SHOUT].SetText( _S( 459, "��ġ��(%)" ) );			
	m_cbtnChatOption[CHATOPT_SYSTEM].SetText( _S( 457, "�ý���" ) );
...
void CUIChatting::RenderChatNameMsgString(const int iPosX, const int iPosY, const int iChatLine)
{
	...
	//modified by sam 11/03/04 ���þ� ä�ý� �̸��� �޽����� ��ħ �׷��� ����
...
void CUIChatting::Render()
{
	...
	if( m_bShowOption )
	{
		...
		{
			m_cbtnChatOption[CHATOPT_PARTY].SetText(_S(4492, "������(@)" ));
		}
		else
		{
			m_cbtnChatOption[CHATOPT_PARTY].SetText( _S( 453, "��Ƽ(@)" ) );
	...
	for( int iTab = 0; iTab < CHATTAB_TOTAL; iTab++ )
	{
		...
		else
		{
			...
			{	// �Ϲ� ���
				for(int iMsgType = 0; iMsgType <= CHATOPT_SHOUT; iMsgType++)	// CHATOPT_SHOUT ���� üũ
				{
	...
	pDrawPort->PutTextExCX( _S( 506, "��ü" ), nX, nY,								
...
void CUIChatting::AddChatString( CTString &strChatString, COLOR colChatString, int nCurTab, BOOL bNotice, BYTE Channel)
{
	...
	// channel mode ��ϰ�, normal mode ����� �����Ͽ� �߰��Ѵ�.
	int &nCurChatCount = (Channel > 0) ? m_nChannelCurChatCount[nCurTab] : m_nCurChatCount[nCurTab]; // ���� ä�� ī��Ʈ
	int &nCurChatInsert = (Channel > 0) ? m_nChannelCurChatInsert[nCurTab] : m_nCurChatInsert[nCurTab]; // ���� ä�� Insert ī��Ʈ
	CTString *ArraystrChatString = (Channel > 0) ? &m_ChannelstrChatString[nCurTab][0] : &m_strChatString[nCurTab][0]; // ä�� ����Ʈ
	COLOR *ArraycolChatString = (Channel > 0) ? &m_ChannelcolChatString[nCurTab][0] : &m_colChatString[nCurTab][0];  // ä�� �÷� ����Ʈ
...
void CUIChatting::AddSysString( CTString &strSysString, COLOR colSysString )
{
...
#if defined(G_RUSSIA)
	{
		...
		if(iStrSub != -1)
		{
			...
			// ITS#7563 AddSysString�� ���ȣ��Ǹ鼭 ���������� ������ �ٲ��ִµ� [3/6/2012 rumist]
			// �ι� �ٲ��� �ʿ䰡 ����. �׷��� ���⼭ ���� �߰��ϹǷ� ������ �߻��Ѵ�.
...
// 			int strTempLength = strTemp.Length();
			// ITS#7563 if(strTempLength > nSplitPos)��� ������ �ʿ����.  [3/6/2012 rumist]
			// ��Ʈ���� �������� ���Ʒ� �� �ѷ���� �ϴ°� �´�. �׷��� ���Ǵ�� �ϸ� ���� ��Ʈ���� �ѷ����� ����.
			// �������� �ڵ�.
	...
	case WM_LBUTTONDOWN:
		{
					...
					if( IsInsideRect( nX, nY, m_rcTab[i] ) )
					{
						if( m_nCurSelTab != i )
						{
							...
							else
							{
								if (m_bChannelMode)
								{
									RenderConditionsysMsg(ChattingMsgType(m_nCurSelTab)); // �ý��� �޽����� ��� ���� ���
								}
							}
							...
							if(m_bChannelMode && m_bChatOption[i][CHATOPT_NORMAL])
								m_bRecvMessage[0][CHATMSG_CHANNEL_LEVEL]	= FALSE;		// ä�� ä��
							if(m_bChannelMode && m_bChatOption[i][CHATOPT_TRADE])
								m_bRecvMessage[0][CHATMSG_CHANNEL_TRADE]	= FALSE;		// ä�� ä��
							...
							for( int j = 0; j < CHATOPT_TOTAL; j++ )
							{
								...
								// [091022: selo] ��ü �ǿ����� ��� �ɼ��� Enable ���¿��� �Ѵ�.
								if(0 == m_nCurSelTab)
								{
#if !defined(G_EUROPE2)
									if (i > 0) // ��ü �ǿ����� �Ϲ� ä���� Ȱ��ȭ ���� �ʴ´�.
									{
			...
			else if (!m_bShowOption && (m_nCurSelTab == CHATMSG_NORMAL || m_nCurSelTab == CHATMSG_TRADE))
			{
				...
				else if (m_ChannelModeTab_btn[1].MouseMessage(pMsg) != WMSG_FAIL) // normal mode
				{
					...
					{
						int	iMsgType;
						for( iMsgType = 0; iMsgType <= CHATOPT_SHOUT; iMsgType++)	// CHATOPT_SHOUT ���� üũ
						{	// ��� ä������ ���� �����ֱ� �ɼ��� �ִ� ��� ��� �������� ��ŷ
...
{	
	...
	{
		if(nChatType == -1)	// [sora] ������ ��ü ä���� ���
		{
			strChatString = &pcChatString[1];
		}
		else				// [sora] �׷� ä���� ��� @���� ���ڰ� �׷��̹Ƿ� 3��° ���ں��� ����
		{
	...
	if( _pNetwork->m_ubGMLevel > 1 )
	{
		// Date : 2005-04-08(���� 1:20:42), By Lee Ki-hwan
		/* commnet : �߱� Local GM�� ��� ü�� �� ���� ��û�� ���� ���� */
		// [2012/04/20 : Sora]  ITS 7999 �±� GM��Ʈ ���� ��û ����
#if defined(G_THAI)
		if( ubType == CHATMSG_NORMAL || ubType == CHATMSG_RANKER )
#else 
									...
									CTString &strChatMessage , BYTE channelNumber, SLONG slGroup)
{
	...
	//UIMGR()->GetChattingUI()->AddChatMessage(ubChatType, slSendIndex, strSendName, strChatMessage, channelNumber, slGroup);
	...
	{
		if(ubChatType == CHATMSG_EXPEDITION) // [sora] ������ ä��
		{
			if(slGroup == -1) // ������ ��ü ä��
			{
				m_sChatInfo.strName =  "[" + _S(4493, "������") + "] " + strSendName + CTString(" > ");
				strChatString = "[" + _S(4493, "������") + "] " + strSendName + CTString( " > " ) + strChatMessage;
			}
			else // ������ �׷� ä��
			{
				CTString strTemp;
		...
		else
		{
			...
			{
				CTString strTmp;
				strTmp.PrintF(_s("[%s%d]%s > "), _S(5392, "ä��"), channelNumber, strSendName);
	...
	if( ubChatType == CHATMSG_NOTICE || ubChatType == CHATMSG_LORD || ubChatType == CHATMSG_NOTICE_SERVERDOWN || ubChatType == CHATMSG_PRESSCORPS)
	{
		...
		else if( ubChatType == CHATMSG_NOTICE_SERVERDOWN )
		{
			...
			CTString tv_str;
			tv_str.PrintF(_S(2573,"���� ������� %d�� ���ҽ��ϴ�."),tv_i);
		...
		for( int i = 0; i < CHATTAB_TOTAL; i++ )
			{
				if( m_bChatOption[i][ubChatType] )
				{
					...
					AddChatString( strChatString, colChatString, i, FALSE );//�ӽ� MODIFY_CHATWORD_NOTICE_NA_2009_0219
	...
	else
	{
			...
			&& CeilWritingCut_CompareStr(strChatMessage))
		{
			...
			CTString temStr;
			temStr.PrintF(_S(3006, "%s���� ���ܵǾ����ϴ�."),strSendName);
...
void CUIChatting::AddSysMessage( CTString &strSysMessage, int nSysType ,COLOR colMsg )
{
	...
	//UIMGR()->GetChattingUI()->AddSysMessage(strSysMessage, nSysType, colMsg);
	...
	case CHATMSG_CHANNEL_TRADE:
		{
			...
			{
				strMoney.PrintF(_S(5389, "[%s] ������ �����մϴ�.(%d ����)"), _S(5391, "�Ÿ� ä�θ��"), nNeedNas);
				AddSysMessage(strMoney, SYSMSG_ERROR);
				bResult = FALSE;
			}
		}
...
void CUIChatting::PopupUserNotice()
{
	...
	MsgBoxInfo.SetMsgBoxInfo( _S(5610, "���� ����"), UMBS_USER_12 | UMBS_INPUT_MASK, UI_CHATTING, MSGCMD_USERNOTICE, 300);	// UMBS_USER_12 | UMBS_INPUT_MASK | UMBS_ALIGN_RIGHT
	MsgBoxInfo.SetUserBtnName( _S(191, "Ȯ��"), _S(139, "���") ); 
	MsgBoxInfo.AddString(_S(5609, "���� �ϰ��� �ϴ� ������ �Է��� �ּ���."), 0xF3F3F3FF, TEXT_CENTER);
	MsgBoxInfo.AddString(_S(5630, "(��, �ִ� 30�� ���� �Է��� �����մϴ�.)"), 0xF3F3F3FF, TEXT_CENTER);
#ifdef G_KOR
