...
void CUIChildQuickSlot::SlotRender()
{
	...
	if (*(m_bRotation[m_nCurSlot]) == 0) // 0 ���� �� �� ��ư ��ġ ���� [12/21/2012 Ranma]
	{
		nPosY = 16;
...
void CUIChildQuickSlot::SlotNumberRender()
{
	...
	else if( *(m_bRotation[m_nCurSlot]) == 1 )
	{
		int	nX = m_nPosX;
		int	nY = m_nPosY;
		// ���� ��ȣ �ؽ���
	...
	case UI_QUICKSLOT4:
		{
			...
			{
				m_nStartX = pixMaxI - m_nWidth;
				m_nStartY = pixMaxJ - m_nHeight - 44/*QUICKSLOT_HEIGHT*/ - CHILD_QUICKSLOT_HEIGHT - CHILD_QUICKSLOT_HEIGHT;
				bSave = FALSE;
			}				
		}
		break;
...
void CUIChildQuickSlot::Render()
{
	...
	if( *(m_bRotation[m_nCurSlot]) == 2 )
	{
		...
		pDrawPort->AddTexture( nX, nY
			,nX + CHILD_QUICKSLOT_WIDTH, nY + CHILD_QUICKSLOT_HEIGHT + CHILD_QUICKSLOT_HEIGHT
		...
		m_btnLock.SetPos(CHILD_QUICKSLOT_WIDTH - QSLOT_OUTSIDE_BTN_SIZE - QSLOT_OUTSIDE_BTN_SIZE - 1, 36);
		m_btnUnLock.SetPos(CHILD_QUICKSLOT_WIDTH - QSLOT_OUTSIDE_BTN_SIZE - QSLOT_OUTSIDE_BTN_SIZE - 1, 36);
	}
	else if( *(m_bRotation[m_nCurSlot]) == 1 )
	{
		int	nX = m_nPosX;
...
{
	if( *(m_bRotation[m_nCurSlot]) == 1 ) // 1 ���� 0 ���� [12/21/2012 Ranma]
	{
