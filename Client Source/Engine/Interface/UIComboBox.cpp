...
{
	m_vecString.clear();
	m_vecString.shrink_to_fit();  // Release memory capacity
...
void CUIComboBox::Render()
{
	...
	// Text
	if( m_vecString.size() > 0 && m_nSelectList >= 0)
	...
	case WM_LBUTTONDOWN:
		{
			...
			else if( m_bDropList && IsInsideRect( nX, nY, m_rcDropList ) )
			{
				...
				m_nOverList = ( nY - nAbsY - m_nTextSY + m_nSelectOffsetY ) / m_nLineHeight +
								m_sbScrollBar.GetScrollPos();
				if( m_nOverList >= 0 && m_nOverList < m_vecString.size() && !IsLocked(m_nOverList))
				{
					SetCurSel(m_nOverList);
					return WMSG_COMMAND;
				}
...
void CUIComboBox::OnRender( CDrawPort* pDraw )
{
	...
	if( m_bDropList )
	{
		...
		for( int iString = m_nScrollPos; iString < nRowE; iString++ )
		{
			...
			if (IsLocked(iString))
				pDraw->PutTextEx(m_vecString[iString], nTexX + outX, nTexY, m_colorLock);
			else if( (m_nOverList + m_nScrollPos) == iString )
...
WMSG_RESULT CUIComboBox::OnLButtonDown( UINT16 x, UINT16 y )
{
	...
	if (m_bDropList && IsInsideRect( x, y, m_rcDropArea ))
	{
		...
		if( m_nOverList >= 0 && m_nOverList < m_vecString.size() && !IsLocked(m_nOverList))
		{
			SetCurSel(m_nOverList + m_nScrollPos);
			m_bDropList = !m_bDropList;
