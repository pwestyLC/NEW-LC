...
public:
	...
	{
		int oldSelectedItem = m_nSelectList;

		if (nCurSelection == -1)
		{
			m_nSelectList = -1;
		}
		else if (nCurSelection < m_vecString.size())
		{
			m_nSelectList = nCurSelection;
			if (SelectIndexChanged) SelectIndexChanged(oldSelectedItem, nCurSelection);
		}
	}

	void LockString(int nIdx)
	{
		if (nIdx < 0 || nIdx >= m_vecString.size())
			return;

		if (std::find(m_disableStrings.begin(), m_disableStrings.end(), nIdx) != m_disableStrings.end())
			return;

		m_disableStrings.push_back(nIdx);
	}

	void UnlockString(int nIdx)
	{
		if (nIdx < 0 || nIdx >= m_vecString.size())
			return;

		auto iter = std::find(m_disableStrings.begin(), m_disableStrings.end(), nIdx);
		if (iter == m_disableStrings.end())
			return;

		m_disableStrings.erase(iter);
	}
    bool IsLocked(int nIdx)
	{
		if (nIdx < 0 || nIdx >= m_vecString.size())
			return false;

		return std::find(m_disableStrings.begin(), m_disableStrings.end(), nIdx) != m_disableStrings.end();
	}
...
// 		m_rcBtn.Right = nW; m_rcBtn.Bottom = nH; }
	...
	boost::function<void(int, int)> SelectIndexChanged;
...
protected:
	...
	std::vector<int>		m_disableStrings;
