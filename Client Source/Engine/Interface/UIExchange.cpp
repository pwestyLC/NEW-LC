...
void CUIExchange::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
	...
	// Ready button
	m_btnReady.Create( this, _S( 148, "�غ� �Ϸ�" ), 11, 420, 63, 21 );
	...
	// Exchange button
	m_btnExchange.Create( this, _S( 149, "��ȯ" ), 76, 420, 63, 21 );
	...
	// Cancel button
	m_btnCancel.Create( this, _S( 139, "���" ), 141, 420, 63, 21 );
...
void CUIExchange::PrepareExchange()
{
	...
	{
		pUIManager->GetChattingUI()->AddSysMessage( _S(1890 ,"â�� ����߿��� ��ȯ�� �Ҽ� �����ϴ�." ) );	
...
void CUIExchange::Render()
{
	...
	// Text in exchange
	// Title
	pDrawPort->PutTextEx( _S( 149, "��ȯ" ), m_nPosX + EXCH_TITLE_TEXT_OFFSETX,
	...
	// Date : 2005-11-11(���� 10:48:01), By Lee Ki-hwan 
	// ���� �� �߰� 
	// Money
	COLOR colNas = 0;
	...
	case WM_MOUSEMOVE:
		{
			...
			{
				// My items
				/*if( !m_bExchReady && m_nSelMyItemID >= 0 )		// ������ ���� ����
				{
					// Close message box of exchange
				...
				if( IsInside( nX, nY ) )
				{
						...
						pUIManager->GetDragIcon()->GetWhichUI() == UI_EXCHANGE)
					{
						...
						if( IsInsideRect( nX, nY, m_rcMySlot ) )
						{
							...
							/*if( m_nSelMyItemID < 0 ||
								m_abtnMyItems[m_nSelMyItemID].GetBtnID() != pUIManager->GetHoldBtn().GetBtnID() )*/	// ������ ���� ����
							{
...
void CUIExchange::AddExchItem( int nTab, int inven_idx, int nUniIndex, SQUAD llCount )
{
	...
	if( ( pItemData->GetFlag() & ITEM_FLAG_COUNT ) && llTempCount > 1 )
	{
		...
		pCmd->setData(this);
		UIMGR()->GetMsgBoxNumOnly()->SetInfo(pCmd, _S( 149, "��ȯ" ), strMessage, 1, pItems->Item_Sum);
	}
	else
	{
	...
	case MSGCMD_EXCH_ADD_NAS:
		{
			...
			if( iChar == nLength )
			{
				...
				else
				{
					...
					strInNasError.PrintF( _S( 5902, "���� ������ �ִ� ���� ���� ���� ������ ��ȯ �� �� �����ϴ�." ));		
...
void CUIExchange::SendExchangeReq_Req( CTString &strDestName )
{
	...
	if( m_nSrcIndex != -1 && m_nDestIndex != -1 )
	{
		...
		if( m_bAmISrc )
			strSysMessage.PrintF( _S2( 422, m_strDestName, "�̹� %s<��> ��ȯ���Դϴ�." ), m_strDestName );
		else
			strSysMessage.PrintF( _S2( 422, m_strSrcName, "�̹� %s<��> ��ȯ���Դϴ�." ), m_strSrcName );
	...
	if (pObject != NULL)
	{
		...
		{
			pUIManager->GetChattingUI()->AddSysMessage( _S( 737, "���� �������� ��ȯ�� �� �� �����ϴ�." ), SYSMSG_ERROR );		
			return;
		}
	...
	if( slDestIndex == -1 )
	{
		pUIManager->GetChattingUI()->AddSysMessage( _S( 151, "��ȯ ����� ã�� �� �����ϴ�." ), SYSMSG_ERROR );
		return ;
	}
	...
	MsgBoxInfo.SetMsgBoxInfo( _S( 149, "��ȯ" ), UMBS_CANCEL,
								UI_EXCHANGE, MSGCMD_EXCH_REQ_SRC );
	strMessage.PrintF( _S( 152, "%s���� ��ȯ�� ��û���Դϴ�. ����Ͻðڽ��ϱ�?" ), m_strDestName );
									...
									int nDestIndex, CTString &strDestName )
{
	...
	CTString		strMessage;
	CUIMsgBox_Info	MsgBoxInfo;
	MsgBoxInfo.SetMsgBoxInfo( _S( 149, "��ȯ" ), UMBS_YESNO,
								UI_EXCHANGE, MSGCMD_EXCH_REQ_DEST );
	strMessage.PrintF( _S2( 153, m_strSrcName, "%s<��> ��ȯ�� ��û�߽��ϴ�. �����Ͻðڽ��ϱ�?" ), m_strSrcName );
	MsgBoxInfo.AddString( strMessage );
	pUIManager->CreateMessageBox( MsgBoxInfo );
...
void CUIExchange::ExchangeReq_Rep()
{
	...
	{
		pUIManager->CloseMessageBox( MSGCMD_EXCH_REQ_SRC );
		strMessage.PrintF( _S2( 154, m_strDestName, "%s<��> ��ȯ�� �����մϴ�." ), m_strDestName );
	}
	else
		strMessage.PrintF( _S2( 154, m_strSrcName, "%s<��> ��ȯ�� �����մϴ�." ), m_strSrcName );
...
void CUIExchange::ExchangeReq_Rej( BOOL bSrcReject )
{
	...
	{
		if( bSrcReject )
			strMessage.PrintF( _S2( 155, m_strDestName, "%s<��>�� ��ȯ�� ����ϼ̽��ϴ�." ), m_strDestName );
		else
		{
			pUIManager->CloseMessageBox( MSGCMD_EXCH_REQ_SRC );
			strMessage.PrintF( _S2( 156, m_strDestName, "%s<��> ��ȯ�� ����߽��ϴ�." ), m_strDestName );
		}
	}
	else
	{
		...
		{
			pUIManager->CloseMessageBox( MSGCMD_EXCH_REQ_DEST );
			strMessage.PrintF( _S2( 156, m_strSrcName, "%s<��> ��ȯ�� ����߽��ϴ�." ), m_strSrcName );
		}
		else
			strMessage.PrintF( _S2( 155, m_strSrcName, "%s<��>�� ��ȯ�� ����ϼ̽��ϴ�." ), m_strSrcName );
	}
...
void CUIExchange::ExchangeReq_BothReady()
{
	...
	// Add system message
	CUIManager::getSingleton()->GetChattingUI()->AddSysMessage( _S( 157, "��ȯ �غ� �Ϸ�Ǿ����ϴ�." ), SYSMSG_ERROR );
}
...
void CUIExchange::ExchangeReq_Full( BOOL bSrcFull )
{
	...
	if( m_bAmISrc )
	{
		if( bSrcFull )
			...
			strMessage.PrintF( _S( 158, "%s�� �κ��丮 ������ �����մϴ�." ), m_strDestName );
	}
	else
	{
		if( bSrcFull )
			strMessage.PrintF( _S( 158, "%s�� �κ��丮 ������ �����մϴ�." ), m_strSrcName );
		else
			strMessage = _S( 265, "�κ��丮 ������ �����մϴ�." );
	}
...
void CUIExchange::ExchangeReq_BothOk()
{
	...
	// Add system message
	CUIManager::getSingleton()->GetChattingUI()->AddSysMessage( _S( 159, "��ȯ�� ���������� �̷�������ϴ�." ) );
...
void CUIExchange::ExchangeReq_Pet_Err(BOOL petErr)
{
	...
	if(!petErr){
		// Add system message
		pUIManager->GetChattingUI()->AddSysMessage( _S(2221, "���� �� �̻� ������ �� �����ϴ�." ),SYSMSG_ERROR );
	}
	else{
		CTString	strMessage;
		strMessage.PrintF(_S(2555, "%s���� ���� �� �̻� ������ �� ���� ���� �Դϴ�." ) ,m_strDestName);
									...
									int nIndex, ULONG ulPlus, ULONG ulFlag, LONG ulUsed, LONG ulUsed2, LONG lRareIndex )
{
	...
	if ((m_bAmISrc && bSrcAdd) || (!m_bAmISrc && !bSrcAdd))
	{
		...
		{
			pUIManager->GetChattingUI()->AddSysMessage( _S( 1079, "���ε� �������� ��ü�� �� �����ϴ�." ), SYSMSG_ERROR );		
			m_pIconsMyItems[iItem]->clearIconData();
			return;
		}
...
void CUIExchange::ExchangeNas()
{
	...
	if(pUIManager->DoesMessageBoxExist(MSGCMD_EXCH_ADD_NAS))
		return;
	MsgBoxInfo.SetMsgBoxInfo( _S( 149, "��ȯ" ), UMBS_OKCANCEL | UMBS_INPUTBOX,	
		UI_EXCHANGE, MSGCMD_EXCH_ADD_NAS );
	strMessage.PrintF( _S( 5903, "�� ���� ������ ��ȯ �Ͻðڽ��ϱ�?" ) );		
	MsgBoxInfo.AddString( strMessage );
	pUIManager->CreateMessageBox( MsgBoxInfo );
	return;
