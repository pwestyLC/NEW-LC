...
CUIBase* UIFactory::BuildControlFromElement( TiXmlElement* pElement )
{
	...
	else if (strcmpi(strTmp.c_str(), "uimultiedit") == 0)
		pUI = createMultiEdit(pElement);
...
CUIMultiEditBox* UIFactory::createMultiEdit(TiXmlElement* pElement)
{
	CUIMultiEditBox* pUI = new CUIMultiEditBox;
	getBaseValue(pUI, pElement);
	pUI->Create();
	pUI->InitFont();
	int max = 256, max_line = 20, line_height = 3;
	BOOL bCursorMove = TRUE, bOnlyNum = FALSE;
	if (pElement->Attribute("max", &max))
		pUI->SetMaxCharCount(max);
	if (pElement->Attribute("max_line", &max_line))
		pUI->SetMaxLineCount(max_line);
	if (pElement->Attribute("cursor_move", &bCursorMove))
		pUI->SetCursorMove(bCursorMove);
	if (pElement->Attribute("only_num", &bOnlyNum))
		pUI->SetOnlyNumMode(bOnlyNum);
	if (pElement->Attribute("line_height", &line_height))
		pUI->InitXmlInfo(line_height);
	return pUI;
}
CUICheckButton* UIFactory::createCheck(TiXmlElement* pElement)
{
	...
	for (; element != NULL; element = element->NextSiblingElement())
	{
		...
		// <color ...>
		if (_stricmp(strTmp.c_str(), "color") == 0)
		{
			for (i = 0; i < UCBS_TOTAL; ++i)
			{
				const char* att = element->Attribute(strAtt[i].c_str());
				if (att && *att)
					pUI->SetTextColor((UICheckBtnState)i, strtoul(att, NULL, 16));
			}
		}
		else
		{
			// Match UVs
			for (int u = 0; u < UCBS_TOTAL; ++u)
			{
				if (_stricmp(strTmp.c_str(), strAtt[u].c_str()) == 0)
				{
					getUV(&uv, element);
					pUI->SetUVTex((UICheckBtnState)u, uv);
					break;
				}
			}

			// fallback color_* from parent
			for (i = 0; i <= UCBS_CHECK; ++i)
			{
				strName = "color_" + strColor[i];
				const char* att = pElement->Attribute(strName.c_str());
				if (att && *att)
					pUI->SetTextColor((UICheckBtnState)i, strtoul(att, NULL, 16));
			}
		}
...
CUITextBox* UIFactory::createTextBox( TiXmlElement* pElement )
{
	...
	else if( pElement->Attribute( "space_x", &nSX) && pElement->Attribute( "space_y", &nSY))	// ������ ����ϴ� �̸� ����.
		pUI->SetStartXY( nSX, nSY );
