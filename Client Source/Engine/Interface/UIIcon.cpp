...
void CUIIcon::setData(CItems* pItems, bool bRef, bool isChat)
{
	...
	{
	    if (pItemData->GetType() == CItemData::ITEM_ONCEUSE && !isChat && //dethunter12 test
			(pItemData->GetSubType() == CItemData::ITEM_SUB_SEAL))
	    {
	        int nGrade = pItemData->GetNum2() - 1;
	...
	// Stack
	setStack();
#ifdef ENABLE_UPGRADE_INFO
	// Upgrade
	setUpgrade();
#endif
	// ����
	{
		if (pItems->IsFlag(FLAG_ITEM_TRANSMOGRIFY))
			setImage(eIMG_TYPE_COMPOSITION, false);
		else if (pItems->IsFlag(FLAG_ITEM_COMPOSITION) && pItemData->IsFlag(ITEM_FLAG_COMPOSITE) == false && !isChat) 
...
CUIBase* CUIIcon::Clone()
{
	...
	if (m_pUpgrade != NULL)
 		pIcon->setPointUpgrade(new CUIImageFont(*m_pUpgrade));
	if (m_pAttribute != NULL)
...
WMSG_RESULT CUIIcon::OnLButtonDBLClick( UINT16 x, UINT16 y )
{
	...
	if (m_pToggle != NULL && m_pToggle->GetHide() == FALSE && m_BtnType == UBET_ITEM)
	{
		if (m_eWhichUI != UI_QUICKSLOT &&
			(m_eWhichUI < UI_INVENTORY || m_eWhichUI > UI_QUICKSLOT4))
		{
			UIMGR()->GetChattingUI()->AddSysMessage(_S(7035, "Ȱ��ȭ �� �������� �ű� �� �����ϴ�."));
			return WMSG_FAIL;
		}
	}
...
void CUIIcon::setUpgrade()
{
	...
	if (pItemData != NULL && !(pItemData->GetFlag() & ITEM_FLAG_COUNT) && !(pItemData->GetFlag() & ITEM_FLAG_COSTUME2) && (pItemData->GetType() == CItemData::ITEM_WEAPON || pItemData->GetType() == CItemData::ITEM_SHIELD))
	{
		if (m_pItems->Item_Plus > 0)	
		{
			if (m_pUpgrade == NULL)
			{
				m_pUpgrade = UISUPPORT()->clone_stack();
				addChild(m_pUpgrade);
			}

			CTString strTmp;
			strTmp.PrintF("+%d", m_pItems->Item_Plus);
			m_pUpgrade->setString(strTmp);
			m_pUpgrade->Hide(FALSE);
		} else if (m_pUpgrade != NULL) {
			m_pUpgrade->Hide(TRUE);
		}
	} else {
		if (m_pUpgrade != NULL)
			m_pUpgrade->Hide(TRUE);
