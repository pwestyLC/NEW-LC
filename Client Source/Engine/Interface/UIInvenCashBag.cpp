...
void CUIInvenCashBag::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
	...
	m_rcLockArea.SetRect(30, 39, 30 + 37, 39 + 47);		// �ڹ��� ����
...
void CUIInvenCashBag::SetCashBagExpireTime(int uiType)
{
	...
	if( _pUIBuff->GetMyTimerItemBuffExist(uiType) == true )
	{
		...
		m_strTime.PrintF( _S(6068, "%d��%d��%d��%d��"), pTime->tm_year + 1900, pTime->tm_mon + 1, pTime->tm_mday, pTime->tm_hour );
		m_bLock = false;
	}
	else
	{
...
void CUIInvenCashBag::RenderExpireTime(CDrawPort* pDrawPort)
{
	...
	strMessage.PrintF( _S(6060, "%d�� ���̸��� ����"), m_nIndexBag - INVEN_TAB_CASH1 + 1);
	pDrawPort->PutTextExCX( strMessage, m_nPosX + TITLE_SX, m_nPosY + TITLE_SY, 0xFFFFFFFF );	// �ϴû�
	pDrawPort->EndTextEx();
}
