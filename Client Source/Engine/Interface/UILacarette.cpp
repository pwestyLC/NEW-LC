...
void		CUILacaball::_renderBackground()
{
	...
	pDrawPort->PutTextExCX( _S( 5350, "��ī��"),		m_nPosX + m_nWidth/2,
...
void		CUILacaball::_renderCourseInfo()
{
	...
	szTokenName.PrintF( "%s/%s", _S( 5351, "��������" ), _S( 5352, "�Ѱ���") );
	...
	pDrawPort->PutTextExCX( _S( 1866, "����ǰ" ),		m_nPosX + 226+97,
																	m_nPosY + 170 + 4, 
																	0xDED9A0FF );
	pDrawPort->PutTextExCX( _S( 5351, "��������" ),			m_nPosX + 421+35,
																	m_nPosY + 170 + 4, 
																	0xDED9A0FF );
	pDrawPort->EndTextEx();
...
void		CUILacaball::_renderBallInfo()
{
	...
	pDrawPort->InitTextureData( m_ptdBaseTexture );
	_updateDropPos();
	if( m_animBall.IsAnim() && m_animBall.GetFrame()+1 >= m_animBall.NumberOfFrames ) // base ����.
	{
		m_animBall.EndAnim();
...
void		CUILacaball::_receiveInfo(CNetworkMessage* istr )
{
	...
	if( m_iCurrentCoursePos < 1 )
		m_iShowCoursePos = m_iCurrentCoursePos;		// ���� �տ� ���ý�.
	else if( m_iCurrentCoursePos >= bySubCourseLoopCount-1 )
		m_iShowCoursePos = m_iCurrentCoursePos -2;	// ���� ���ϰ��.
	else
	{
...
void		CUILacaball::_receiveUsedResult(CNetworkMessage* istr )
{
	...
	{
		m_bAutoClick = FALSE;
		MsgBoxInfo.SetMsgBoxInfo( _S( 191, "Ȯ��" ), UMBS_OK, UI_LACABALL, MSGCMD_LACABALL_ERROR );
	...
	else if( byErrorCode == 2 )
	{
		m_bAutoClick = FALSE;
		MsgBoxInfo.SetMsgBoxInfo( _S( 191, "Ȯ��" ), UMBS_OK, UI_LACABALL, MSGCMD_LACABALL_ERROR );
		MsgBoxInfo.AddString( _S( 4860, "��ū�� �����մϴ�." ) );
	}
	else
	{
		;//??
...
{
	CTString strMsg;
	strMsg.PrintF( _S( 5349, "%s���� %s �������� ȹ���Ͽ����ϴ�."), m_strWinner, _pNetwork->GetItemName( m_iWinnerItemIdx ) );
