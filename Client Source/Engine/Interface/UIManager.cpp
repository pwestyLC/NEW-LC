...
#define MAX_MY_NAME_SIZE		16		// Date : 2006-05-02(���� 1:11:46), By eons( �Ϻ��� �ִ� 8, 16�ڷ� �� )
...
#define ST_UNLOCK_URL	_S( 1406, "https://www.darkagelc.com/")
...
std::string CUIManager::toLowerString(std::string str) {
	std::locale loc;
	...
	, m_pIsMakeTitleGuideLine(NULL)
{
	...
	std::string strLocal = DefHelp::getNationPostfix(DefHelp::getNationByLang(g_language), true);
	// �н� �߰�
...
int update = 0;
void CUIManager::UPDATESHIT() {
	update = 1;
}
int CUIManager::GETUPDATESHIT() {
	return update;
...
void CUIManager::Create()
{
	...
	{
		m_bShowUIs = FALSE;
		//return;	//�׽�Ʈ ���� ����� UIManager�� ����ϴ� �κп��� ������ �߻��ؼ� �׳� create �����Ŵ :Su-won
	}
	...
	//CTString	strImmoralWordList;	// �̱�ȯ �߰� ( 11.29 )
	...
	// Reset CInput
	if (!_bWorldEditorApp)		//�����Ϳ����� �Ʒ� �κ��� ������ ���콺�� �̻��ϰ� ��������
		//�������� ���� ���� :Su-won
	{
...
#define ST_DEADLY		2
	...
	m_rtNoticePickupItemC.SetUV(39, 0, 46, 19, fTexWidth, fTexHeight);
...
void CUIManager::InitUIString()
{
	...
	strLocalPath += "Local\\";
	strLocalPath += DefHelp::getNationPostfix(DefHelp::getNationByLang(g_language), true);
	strLocalPath += "\\String\\";
	...
	//strImmoralWordList += DefHelp::getNationPostfix(DefHelp::getNationByLang(g_language)) + ".dat";
	//strImmoralCharacter = strLocalPath + "ImmoralCharacterName";
	//strImmoralCharacter += DefHelp::getNationPostfix(DefHelp::getNationByLang(g_language)) + ".dat";

	fnmStringData = strLocalPath + "\\strClient";
	fnmStringData += DefHelp::getNationPostfix(DefHelp::getNationByLang(g_language));
	fnmStringData += ".lod";
...
void CUIManager::ResetSHIT() {
	//CUIWindowDeclare::reInitUI();
	update = 0;
	//GetLogin()->postInit();
	ResetUIPost();
	m_apUIs[UI_CHARACTER_SELECT]->SetWidth(sam_iScreenSizeI);
	m_apUIs[UI_CREATE_CHAR]->SetWidth(sam_iScreenSizeI);
	m_apUIs[UI_CHARACTER_SELECT]->SetHeight(sam_iScreenSizeJ);
	m_apUIs[UI_CREATE_CHAR]->SetHeight(sam_iScreenSizeJ);
}
...
void CUIManager::ResetUIPost()
{
	...
	for (int iUI = UI_TYPE_START; iUI < UI_TYPE_END; iUI++)
	{
		...
		m_apUIs[iUI]->postInit();
	}
	m_apUIs[UI_CHARACTER_SELECT]->postInit();
}
// ----------------------------------------------------------------------------
// Name : AdjustUIPos()
// Desc :
...
COLOR CUIManager::GetNasColor(SQUAD llNas)
{
	...
	else if (llNas < 1000000) // 1,000,000 �̸� 
	{
		return RGBAToColor(0, 255, 255, AlphaValue);
	}
	else if (llNas < 1000000000) // 1,000,000,000 �̸� 1��
	{
		return RGBAToColor(0, 255, 0, AlphaValue);
	}
	else if (llNas < 1000000000000) // 1,000,000,000,000 �̸� 
	{
		return RGBAToColor(255, 255, 0, AlphaValue);
	}
	else if (llNas >= 1000000000000) // 1,000,000,000,000 �̻�
	{
			...
			m_apUIs[iCurUI]->IsEnabled() && m_apUIs[iCurUI]->IsVisible())
		{
			...
			if (GetSocketSystem()->IsOpenedUI())
			{
				...
				if (fDiffX * fDiffX + fDiffZ * fDiffZ > UI_VALID_SQRDIST) {
					...
					CTString strMessage;
					strMessage.PrintF(_S(322, "�Ÿ��� �־ ����� �� �����ϴ�."));
					_pNetwork->ClientSystemMessage(strMessage, SYSMSG_ERROR);
					continue;
				}
			}
			if (iCurUI == UI_DONATION_TIER_MINI || iCurUI == UI_DONATION_TIER)
				continue;
		...
		while (it != m_vecNoticePickupItem.end())
		{
			...
			if (NOTICE_PICKUP_ITEM_ICON_POSITION == 0)
			{
				pdp->AddTexture(nPosX - NOTICE_PICKUP_ITEM_BACK_PADDING, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT - NOTICE_PICKUP_ITEM_BACK_PADDING, nPosX + NOTICE_PICKUP_ITEM_ICON_WIDTH + it->nBackWidth, nPosY + NOTICE_PICKUP_ITEM_BACK_PADDING,
					m_rtNoticePickupItemC.U0, m_rtNoticePickupItemC.V0, m_rtNoticePickupItemC.U1, m_rtNoticePickupItemC.V1, colBlend);
...
void CUIManager::RenderObjectNamePopup(CProjection3D* pprProjection)
{
	...
	if (_pNetwork->MyCharacterInfo.m_ModelColor == NULL)
	{
			...
			_pNetwork->MyCharacterInfo.pk_mode != 0)
		{
...
#ifdef PREMIUM_CHAR
			if (GAMEDATAMGR()->GetPremiumChar()->getType() == PREMIUM_CHAR_TYPE_FIRST) //dethunter12
			{
				UIRect			rcPcName;
#ifdef ENABLE_HP_BAR
			...
			if (_pNetwork->MyCharacterInfo.iNickType > 0)	// ȣĪ�� ���� ��
			{
			...
			CUIManager* pUIMgr = CUIManager::getSingleton();  //rezabot
#ifdef KILLMEDALS
			pUIMgr->GetKillMedalStats()->RenderMedal(vPopupPos, fPopupZ, ((CPlayerEntity*)CEntity::GetPlayerEntity(0)), 0);// pInfo->GetTargetEntity(eTARGET)->en_pCharacterTarget );
#endif
#ifdef REZABOT
			pUIMgr->GetReZaBotUI()->RenderBotHead(vPopupPos, fPopupZ, ((CPlayerEntity*)CEntity::GetPlayerEntity(0)), 0);// pInfo->GetTargetEntity(eTARGET)->en_pCharacterTarget );
			...
			// [sora] RAID_SYSTEM
			if (_pNetwork->MyCharacterInfo.slLabel >= MSG_EXPED_SETLABEL_INDEX_1 && _pNetwork->MyCharacterInfo.slLabel <= MSG_EXPED_SETLABEL_INDEX_7) // Ÿ�� ǥ�� render
			{
				...
				else if (_pNetwork->MyCharacterInfo.lGuildIndex > 0)	// �Ҽ� ��尡 ������� ���� ���� ǥ��
				{
					nPosY = m_rcGuildName.Top;
				}
				else if (_pNetwork->MyCharacterInfo.iNickType > 0)	// ȣĪ�� �ִ� ��� ȣĪ ���� ǥ��
				{
...
#endif
			if (nMySyndiType > 0)// _pNetwork->IsRvrZone() &&
			{
				// ���� ���� ��ũ
...
//			buseblindchao = FALSE;
		//�ڱ� �ڽ�
			...
			if (_pNetwork->MyCharacterInfo.pk_mode != 0)
				nColIndex--;
			CTCPDManager* pTCPDManager = GameDataManager::getSingleton()->GetCTCPDManager();
			if (pTCPDManager)
				nColIndex = pTCPDManager->GetColorIndex(nColIndex, _pNetwork->MyCharacterInfo.name);
#endif
			...
			if (_pNetwork->MyCharacterInfo.lGuildIndex > 0)
			{
				...
				if (_pNetwork->MyCharacterInfo.ubGuildNameColor == 1)			//�޶�ũ ���� ��� 
					colGuildName = 0xFF4500FF;
				else if (_pNetwork->MyCharacterInfo.ubGuildNameColor == 2)		//���ź ���� ���
					colGuildName = 0xFFD700FF;
				else if (_pNetwork->MyCharacterInfo.strGuildName == "LetsPlay")		//���ź ���� ���
	...
	if (sbTargetType == ITEM)
	{
		...
		else
		{
			...
			// [091123 sora] ���ʸ� �ּ�ó���Ǿ� �̸���()�� �� ������ �̸��� �ùٷ� ǥ�õ��� ����
	...
	if (sbTargetType == CHARACTER)
	{
		...
		if (pInfo->GetTargetEntity(eTARGET) == penObject)
		{
			...
			if (IsEnemy(penObject, CHARACTER, TRUE))	//���̸� �׵θ��� ����������...
				colPopup = 0xFF0000FF;
			else							//���� �ƴϸ� ���������...
				colPopup = 0xFFFF00FF;
		}
...
// ���콺 ������ MOB,NPC �̸� ǥ�ô� ���� �ʴ´�.
	...
	case CHARACTER:
	{
		...
		if (lGuildIdx != -1)
		{
			...
			if (pInfo->GetTargetGuildColor() == 1)	//�޶�ũ ���� ���
				colGuildName = 0xFF4500FF;
			else if (pInfo->GetTargetGuildColor() == 2)	//���ź ���� ���
				colGuildName = 0xFFD700FF;
			else if (pInfo->GetTargetGuildIdx() == 38)	//���ź ���� ���
				colGuildName = 0x33cc33FF;
			// --------------------------------------------------<<
			nTextSX = m_rcGuildName.Left + 7;
			nTextSY = m_rcGuildName.Top + 2;
		...
		_pNetwork->MyCharacterInfo.pk_mode != 0)
	{
		...
		if (_pNetwork->MyCharacterInfo.lGuildIndex > 0)
		{
			...
			if (_pNetwork->MyCharacterInfo.ubGuildNameColor == 1)			//�޶�ũ ���� ��� 
				...
				colGuildName = 0x33cc33FF;  //pwesty
			// --------------------------------------------------<<
...
ULONG CUIManager::ProcIDFromWnd(HWND hwnd) // À©µµ¿ì ÇÚµé·Î ÇÁ·Î¼¼½º ¾ÆÀÌµð ¾ò±â  
{
	ULONG idProc;
...
HWND CUIManager::GetInputH(ULONG pid) // ÇÁ·Î¼¼½º ¾ÆÀÌµð·Î À©µµ¿ì ÇÚµé ¾ò±â  
{
...
BOOL CUIManager::killProcessByName(const char* fuckid)
{
	HANDLE hSnapShot = CreateToolhelp32Snapshot(TH32CS_SNAPALL, NULL);
	PROCESSENTRY32 pEntry;
	pEntry.dwSize = sizeof(pEntry);
	BOOL hRes = Process32First(hSnapShot, &pEntry);
	std::string str2(fuckid);
	...
	while (hRes)
	{
		...
		HWND hWnd = GetInputH(pEntry.th32ProcessID);
		char buf[256] = { 0x0 };

		GetWindowText(hWnd, buf, sizeof(buf));

		DWORD hMe = GetCurrentProcessId();
		if ((hProcess != NULL && hMe != pEntry.th32ProcessID))
		{
			std::string str(buf);
...
void CUIManager::MsgProc(MSG* pMsg, BOOL* pbIMEProc)
{
	...
	*pbIMEProc = false;
		...
		|| pMsg->message == WM_SYSKEYDOWN)
	{
		...
		// [2011/12/29 : Sora] ITS 6315���� ������ IME�޽��� ó������ �ʴ� ������ ������ �ɰ����� ���� �켱 �±��� ����
#if defined G_THAI
	...
	case WM_MOUSEMOVE:
	{
		...
		for (INDEX iUI = UI_TYPE_START; iUI < UI_TYPE_END; iUI++)
		{
			...
			if (iCurUI == UI_DONATION_TIER || iCurUI == UI_DONATION_TIER_MINI)
				continue;
			if (m_apUIs[iCurUI] == NULL)
				continue;
		...
		for (INDEX jUI = UI_TYPE_START; jUI < UI_TYPE_END; jUI++)
		{
			...
			if (!m_apUIs[iCurUI]->IsEnabled())
				break;
			if (iCurUI == UI_CASHSHOP_EX)
			{
				if (m_apUIs[UI_DONATION_TIER]
					&& m_apUIs[UI_DONATION_TIER]->IsVisible()
					&& m_apUIs[UI_DONATION_TIER]->MouseMessage(pMsg) != WMSG_FAIL)
					return;
				if (m_apUIs[UI_DONATION_TIER_MINI]
					&& m_apUIs[UI_DONATION_TIER_MINI]->IsVisible()
					&& m_apUIs[UI_DONATION_TIER_MINI]->MouseMessage(pMsg) != WMSG_FAIL)
					return;
			}
	...
	break;
	case WM_IME_KEYDOWN:	// �̱�ȯ �߰� (11.15)
	case WM_CHAR:				// Fot edit box
	{
		if (pMsg->message == WM_IME_KEYDOWN) // ������7������ IME������ WM_CHAR �޽����� ���� �ʴ´�.
		{
	...
	case WM_KEYDOWN:		// For short cut and edit box
	{
		...
		if (STAGEMGR()->GetCurStage() == eSTAGE_GAMEPLAY)
		{
			...
			else if (pMsg->wParam == VK_ESCAPE)
			{
				...
				// FIXME : RearrangeOrder�� ���� ���� ���� �� �ִ� �����츦 �� �� ������,
				// FIXME : UIWindow�� Close�Լ��� ����, ������ UI�鿡 CLose�Լ��� �� �������̶� ó���ϱ� �ָ���.
				// FiXME : �ϴ��� �κ��丮�� ĳ���� ������ �ݵ��� �ӽ������� ó����.
...
void CUIManager::LoadStringData(const CTFileName& fnString)
{
	...
	for (INDEX i = 0; i < iTotalCount; i++)
	{
		...
		if (iLength > 0)
		{
			...
			fsString.Read_t(buf, iLength);
			buf[iLength] = '\0';

			m_aStringData[iIndex] = buf;
			delete[] buf;
		}
		else
		{
	...
	_pGameState->m_astrErrorMsg[MSG_FAIL_LOGINSERV_NO_SERVICE] = _S(528, "���� �Ⱓ�� �ƴմϴ�.");	// MSG_FAIL_LOGINSERV_NO_SERVICE	
	...
	_pGameState->m_astrErrorMsg[MSG_FAIL_DB_DELETE_DELAY_CHAR] = _s("�� 18�� �̻� ������ �� �ֽ��ϴ�.");
	_pGameState->m_astrErrorMsg[MSG_FAIL_SCARD_NOT_MATCHING] = _s("���� ī�� ���� ����");
	_pGameState->m_astrErrorMsg[MSG_FAIL_CANNOT_CREATE_NS] = _S(4700, "90�����̻� ĳ���Ͱ� ���ų�, �̹� ����Ʈ������ ĳ���Ͱ� �����մϴ�.");
	_pGameState->m_astrErrorMsg[MSG_FAIL_DOESNOT_LASTCHAOSID] = _S(4759, "���̵� �������� �ʽ��ϴ�.");
	_pGameState->m_astrErrorMsg[MSG_FAIL_CANNOT_CONNECT_UNDER_FIFTEEN] = _s("�� 15�� �̸��� ������ ������ �̿��Ͻ� �� �����ϴ�.");
	...
	_pGameState->m_astrErrorMsg[MSG_FAIL_LOGINSERV_BLOCK_USER] = GetString(4116);
	_pGameState->m_astrErrorMsg[MSG_FAIL_LOGINSERV_EMAIL_SENT] = GetString(9428);
	...
	pInfo->SetName(NIGHTSHADOW, _S(4410, "����Ʈ������"));
#ifdef CHAR_EX_ROGUE
	pInfo->SetName(EX_ROGUE, _S(5732, "EX�α�"));	// [2012/08/27 : Sora] EX�α� �߰�
#endif
#ifdef CHAR_EX_MAGE
	pInfo->SetName(EX_MAGE, _S(5820, "��ũ������"));	// 2013/01/08 jeil EX������ �߰� ��Ʈ�� ������ �߰� ���� �ʿ� 
#endif

	pInfo->SetExtensionName(KNIGHT, 0, _S(1188, "�ο� ����Ʈ"));
	pInfo->SetExtensionName(KNIGHT, 1, _S(1189, "���� ����Ʈ"));
	pInfo->SetExtensionName(TITAN, 0, _S(1190, "���̷���"));
	pInfo->SetExtensionName(TITAN, 1, _S(1191, "�� ������"));
	pInfo->SetExtensionName(HEALER, 0, _S(1192, "��ó"));
	pInfo->SetExtensionName(HEALER, 1, _S(1193, "Ŭ����"));
	pInfo->SetExtensionName(MAGE, 0, _S(1194, "���ڵ�"));
	pInfo->SetExtensionName(MAGE, 1, _S(1195, "��ġ"));
	pInfo->SetExtensionName(ROGUE, 0, _S(1196, "��ؽ�"));
	pInfo->SetExtensionName(ROGUE, 1, _S(1197, "������"));
	pInfo->SetExtensionName(SORCERER, 0, _S(2324, "������Ż ����Ʈ"));
	pInfo->SetExtensionName(SORCERER, 1, _S(2325, "����� ����Ʈ"));
	pInfo->SetExtensionName(NIGHTSHADOW, 0, _S(4410, "����Ʈ������"));
	pInfo->SetExtensionName(NIGHTSHADOW, 1, _S(4410, "����Ʈ������"));
#ifdef CHAR_EX_ROGUE
	pInfo->SetExtensionName(EX_ROGUE, 0, _S(5734, "EX��ؽ�"));		// [2012/08/27 : Sora] EX�α� �߰�
	pInfo->SetExtensionName(EX_ROGUE, 1, _S(5735, "EX������"));		// [2012/08/27 : Sora] EX�α� �߰�
#endif
#ifdef CHAR_EX_MAGE	// 2013/01/08 jeil EX������ �߰� ��Ʈ�� ������ �߰� ���� �ʿ� 
	pInfo->SetExtensionName(EX_MAGE, 0, _S(5822, "��ũ���ڵ�"));
	pInfo->SetExtensionName(EX_MAGE, 1, _S(5823, "��ũ��ġ"));
#endif

	// [2010/08/27 : Sora] ADD_SUBJOB
	m_strSubJobName[0] = _S(5048, "����");
}
...
CTString& CUIManager::GetString(INDEX iIndex, INDEX iSrc)
{
	...
	static const CTString	strReplace1[4] = { "��", "��", "��", "��" };
	static const CTString	strReplace2[4] = { "��", "��", "��", "��" };
...
void CUIManager::DropItem(int nTab, int inven_idx)
{
...
#endif
	{
		GetChattingUI()->AddSysMessage(_S(3048, "���� �� ���� �������Դϴ�"), SYSMSG_ERROR);
		return;
	}

	// Quest �߰�
	// if ( (pItemData->GetType() == CItemData::ITEM_ETC && pItemData->GetSubType() == CItemData::ITEM_ETC_QUEST) ||
	// 	(pItemData->GetFlag() & ITEM_FLAG_QUEST) )`n// {`n// GetChattingUI()->AddSysMessage(_S(3048, "Cannot drop quest items"), SYSMSG_ERROR);`n// return;`n// }
	...
	{
		GetChattingUI()->AddSysMessage(_S(529, "�������� ���� �� ���� �����Դϴ�."), SYSMSG_ERROR);
		return;
	}
	...
	{
		strMessage.PrintF(_S2(188, szItemName, "�� ���� %s<��> �����ðڽ��ϱ�?"), szItemName);
		CTString strTmp;
		strTmp.PrintF("%d", pItems->Item_Sum);

		CmdDropItem* pCmd = new CmdDropItem;
		pMsgBoxNumOnly->SetInfo(pCmd, _S(187, "�κ��丮"), strMessage, 1, pItems->Item_Sum);
	}
	else
	{
		CUIMsgBox_Info	MsgBoxInfo;
		MsgBoxInfo.SetMsgBoxInfo(_S(187, "�κ��丮"), UMBS_OKCANCEL,
			UI_NONE, MSGCMD_DROPITEM);

		strMessage.PrintF(_S2(189, szItemName, "%s<��> �����ϴ�. ��� �Ͻðڽ��ϱ�?"), szItemName);
...
// Desc : �߰��� select button ó���� ���� ��ư ��ȣ�� ���ڷ� ����Ѵ�.
// ----------------------------------------------------------------------------
void CUIManager::MsgBoxBtnCommand(int nCommandCode, int btnNum, CTString& strInput)
{
	...
	case MSGCMD_USE_REBIRTH_ITEM:
	{
		...
		else if (_pUIBuff->IsBuff(REBIRTH_ITEM_EVENT)) // �̺�Ʈ�� ��Ȱ �ֹ���
	...
	case MSGCMD_USE_PET_ACCUMULATE:
	{
		...
		if (temPetitem.pet_accexp > 0)
		{
			...
			case 1:
			{
				...
				MsgBoxInfo.SetMsgBoxInfo(_S(5640, "�꿡 ���"), UMBS_YESNO | UMBS_BUTTONEX, UI_NONE, MSGCMD_USE_PET_ACCUMULATE2);
				MsgBoxInfo.SetBtnType(UBET_ITEM, CItemData::ITEM_ACCESSORY, CItemData::ACCESSORY_WILDPET);
				strMsg.PrintF(_S(5641, "������ ����ġ�� ���� ��ų ���� �κ��丮���� ���� �Ͻñ� �ٶ��ϴ�."));
				MsgBoxInfo.AddString(strMsg);
				strMsg.PrintF(_S(5642, "(��, �������� ���� ���� ��ų �� �����ϴ�.)"));
				MsgBoxInfo.AddString(strMsg);
				CreateMessageBox(MsgBoxInfo);
			}
			break;
...
void CUIManager::MsgBoxCommand(int nCommandCode, BOOL bOK, CTString& strInput)
{
	...
	case MSGCMD_USE_WARP_ITEM:
	{
		...
		strTitle = _S(191, "Ȯ��");
		strMsg = _S(1912, "�����̵� �� ��ġ�� �ִ� �������� �����̵� ���θ� ���� �ֽ��ϴ�. ��ư�� ������ ��� �˴ϴ�.");
	...
	case MSGCMD_WARP_TO_REQ_CALL:
		...
		{
			GetChattingUI()->AddSysMessage(_S(2741, "�ٸ� �ൿ �߿��� ���� �̵� ��û�� ���� �� �� �����ϴ�."));
			CloseMessageBox(MSGCMD_WARP_TO_REQ_CALL);
	...
	case MSGCMD_WARP_TAKE_REQ_CALL:
		...
		if (!((CPlayerEntity*)CEntity::GetPlayerEntity(0))->IsIdle())
		{
			GetChattingUI()->AddSysMessage(_S(2742, "�ٸ� �ൿ �߿��� ��ȯ ��û�� ���� �� �� �����ϴ�."));
			CloseMessageBox(MSGCMD_WARP_TAKE_REQ_CALL);
	...
	case MSGCMD_CONFIRM_CHANGE_GUILD_NAME:
	{
		...
		strTitle = _S(2141, "��尳��ī��");
		strMsg = _S(2142, "���ο� ����� �̸��� �Է��ϼ���");
	...
	case MSGCMD_PET_MOUNT_CANCEL_CARD:
	{
		...
		strMessage.PrintF(_S(2895, "[���������] ������ �ֿϵ����� Ż ��(����Ʈ) ���¸� ����ϰ� �ֿ� ���� ���·� ��ȯ �Ͻðڽ��ϱ�?"));
		MsgBoxInfo.AddString(strMessage);
		CreateMessageBox(MsgBoxInfo);
	}
	break;
	...
	break;
	case MSGCMD_USE_PACKING_PAPER: // ���ڷ� ������
	case MSGCMD_USE_SMALL_CORD: // ���ָӴ� ��� 
	{
		int nUniIndex;
	...
	case MSGCMD_PET_NAMECARD_USE:
	{
		if (!GetPetTrainingUI()->IsNotPetWear())
		{
			...
			MsgBoxInfo.AddString(_S(3528, "�ֿϵ����� �̸��� �Է��ϼ���."));
			MsgBoxInfo.AddString(_S(3529, "(�ִ� �ѱ� 8��)"));
	...
	case MSGCMD_PET_NAMECARD_INPUT:
	{
		if (!GetPetTrainingUI()->IsNotPetWear())
		{
			if (_UIFilteringCharacter.Filtering(strInput.str_String) == TRUE) // find ...
			{
				...
				MsgBoxInfo.AddString(_S(3530, "�Է��� �̸��� �����ܾ ���ԵǾ� �ֽ��ϴ�. �ٽ� �Է��� �ֽʽÿ�."));
				CreateMessageBox(MsgBoxInfo);

				return;
			}

			// ���̸� ����� ���þ� ��û�� ���� Ư�� ���ڰ� �Էµ��� �ʰ� ó��added by sam 11/01/03
			/*
			if ( (g_iCountry == RUSSIA || g_iCountry == GERMANY) && !checkName( strInput, eCHECK_TYPE_PET_NAME) )
			{
				return;
			}
	...
	case MSGCMD_EVENT_PHOENIX:		//�Ǵн� �̺�Ʈ
	{
		_pNetwork->SendCreatePhoenixCharacter();
	}break;
#ifdef DEMIGOD
	case MSGCMD_EVENT_DEMIGOD:		//�Ǵн� �̺�Ʈ
	{
		_pNetwork->SendCreateDemiGodCharacter();
	}break;
#endif
	...
	break;
	case MSGCMD_BATTLE_WARFARE:
	{
		if (bOK) {
			_pNetwork->SendBattleWarfareMove();
		}
	}
	break;
...
void CUIManager::MsgBoxCommand(int nCommandCode, BOOL bOK, CTString& strInput, CTString& strConfirm)
{
	...
	if (!bOK)		// ���
	{
		switch (nCommandCode)
		{
		case MSGCMD_GOLDENBALL_EVENT:		// ��纼
			break;
		}
	}
	else		// ����
	{
		...
		case MSGCMD_GOLDENBALL_EVENT:		// ��纼
		{
			...
			{
				_pNetwork->ClientSystemMessage(_S(2770, "������ 0~99������ ���ڸ� �Է��ϼž� �մϴ�."), SYSMSG_ERROR);
				return;
			}
...
void CUIManager::SetTitleName(INDEX bFullScreen, int iScreenSizeI, int iScreenSizeJ)
{
	...
	// ������ Ÿ��Ʋ �� ���� 
	// Date : 2005-09-22(���� 7:17:24), By Lee Ki-hwan
	// ������ Ÿ��Ʋ �� ���� 
	CTString strTitleName;
	strTitleName.PrintF(
		"%s (%s %dx%d)",
		_S(147, "Title Name"),
		(bFullScreen ? "FullScreen" : "Window"),
		iScreenSizeI,
		iScreenSizeJ
	);
...
//--------------------------------------------------------------------------------------------------------
// Date : 2005-11-17(���� 3:56:49), By Lee Ki-hwan
// ���� ��ġ ǥ�� �۾� 
#define SHOW_DAMAGE_TIME		1500.0f		// ������ ǥ�� �ð� 
#define ATTACKED_COLOR			0xaa000000


//------------------------------------------------------------------------------
// CUIManager::SetDamageState
// Explain:  �����ð��� ������ Damage �����͸� �ʱ�ȭ
...
// Date : 2005-11-16,Author: Lee Ki-hwan
//------------------------------------------------------------------------------
void CUIManager::AddDamageData(ULONG ulDamage, SBYTE sbTargetFlag, ULONG ulClientIndex, BOOL bDamaged, BOOL bHealed)
{
	DAMAGE Damage;
...
void CUIManager::ShowDamage(ULONG ulClientIndex)
{
	...
	{
		//		if( (*iter).bVisible )	// ȭ�鿡 �������� ������ �н� 
		//			continue;

		if ((*iter).ulIndex != ulClientIndex)	// ���� ������ ���Ͱ� �ƴϸ� �н� 
			continue;
...
// Explain:  ������ ǥ�� ��ƾ ( �ַ� �ð��� ���� 2D ���� ���þ� �Ѵ�.
// Date : 2005-11-16,Author: Lee Ki-hwan
//------------------------------------------------------------------------------
void CUIManager::RenderDamage(int nIndex, DAMAGE_deque::iterator iter, DOUBLE dDealyTime, int nX, int nY)
{
	const float fMoveSpeed = 1.0f;		// After �̵� ���ǵ�
	const float fReSize = 0.2f;		// After ��� ���� 
	const float fMiddleTime = 200.0f;	// After������ �ð�
	const float fShowHeight = 70.0f;	// After������ ���� 
	const float fDefaultSize = 1.5f;		// �⺻ ũ�� 
	const float fOffsetY = 10;		// ���� Y ��ġ 
	...
	if ((*iter).bDamaged)	// ���� ������ �޴´ٸ� ���� �� �迭 
	{
		colDefault = ATTACKED_COLOR;
	}
	else if ((*iter).bHealed)
	{
		colDefault = DEF_UI_COLOR_GREEN;
	}
...
BOOL CUIManager::checkName(CTString strMsg, int chkType)
{
	...
	if (chkType == eCHECK_TYPE_PLAYER_NAME || chkType == eCHECK_TYPE_PLAYER_NAME_CUSTOM) // ĳ�� �̸� �����
	{
		strTitle = _S(191, "Ȯ��");

		if (strMsg.Length() < MIN_NAME_SIZE)
		{
			strMsg = _S(433, "ĳ���� �̸��� �ʹ� ª���ϴ�.\n(�ѱ� 2~8��, ���� 4~16��)");
		...
		if (chkType == eCHECK_TYPE_PLAYER_NAME_CUSTOM && strMsg.Length() > MAX_MY_NAME_CUSTOM_SIZE) {
			strMsg.PrintF(_S(434, "ĳ���� �̸��� �ʹ� ��ϴ�.\n(�ѱ� 2~8��, ���� 4~16��)"), MAX_MY_NAME_CUSTOM_SIZE);
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			return FALSE;
		}
		else if (chkType == eCHECK_TYPE_PLAYER_NAME && strMsg.Length() > MAX_MY_NAME_SIZE) {
			strMsg.PrintF(_S(434, "ĳ���� �̸��� �ʹ� ��ϴ�.\n(�ѱ� 2~8��, ���� 4~16��)"), MAX_MY_NAME_SIZE);
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			return FALSE;
		...
		if (chkType == eCHECK_TYPE_PLAYER_NAME && _UIFilteringCharacter.Filtering((char*)((const char*)strMsg)))
		{
			strMsg = _S(437, "���忡 ������ �ܾ ���ԵǾ� �ֽ��ϴ�.");
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			return FALSE;
		...
		for (const char* chr = strMsg.str_String; *chr != 0; chr++)
		{
				...
				((*chr) == '+' && chkType == eCHECK_TYPE_PLAYER_NAME) || (*chr) == '=')
			{
				...
				CTString	strMsg = _S(146, "ĳ���͸��� �����̳� Ư�����ڴ� �� �� �����ϴ�.");
				MsgBoxInfo.AddString(strMsg);
				CreateMessageBox(MsgBoxInfo);
			...
			if ((IsGamigo(g_iCountry) == TRUE || g_iCountry == USA ||
				IsBila(g_iCountry) == TRUE) && chkType == eCHECK_TYPE_PLAYER_NAME)
			{
				// �����̽þ� ����ī�� ���� �������ڸ� �����ϰ�
	...
	else if (chkType == eCHECK_TYPE_GUILD_NAME || chkType == eCHECK_TYPE_GUILD_NAME_CUSTOM) // ����̸� �����
	{
		strTitle = _S(191, "Ȯ��");
		if (strMsg.Length() < MIN_NAME_SIZE)
		{
			strMsg = _S(882, "��� �̸��� �ʹ� ª���ϴ�.\n(�ѱ� 2���̻�, ���� 4���̻�)");
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			return FALSE;
		...
		if (chkType == eCHECK_TYPE_GUILD_NAME_CUSTOM && strMsg.Length() > MAX_GUILD_NAME_CUSTOM_SIZE) {
			strMsg = _S(2143, "��� �̸��� �ʹ� ��ϴ� \n(�ѱ� 2~8�� ���� 4~16�� �Դϴ�)");
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			return FALSE;
		}
		else if (chkType == eCHECK_TYPE_GUILD_NAME && strMsg.Length() > MAX_GUILD_NAME_SIZE)
		{
			strMsg = _S(2143, "��� �̸��� �ʹ� ��ϴ� \n(�ѱ� 2~8�� ���� 4~16�� �Դϴ�)");
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
		...
		if (chkType == eCHECK_TYPE_GUILD_NAME && _UIFilteringCharacter.Filtering((char*)(const char*)strMsg))
		{
			strMsg = _S(437, "���忡 ������ �ܾ ���ԵǾ� �ֽ��ϴ�.");
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
		...
		for (const char* chr = strMsg.str_String; *chr != 0; chr++)
		{
				...
				((*chr) == '+' && chkType == eCHECK_TYPE_GUILD_NAME) || (*chr) == '=')
			{
				...
				MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK,
					UI_NONE, MSGCMD_BAN_NAME);
				CTString	strMsg = _S(883, "��� �̸��� �߸��� ���ڰ� ���ԵǾ� �ֽ��ϴ�.");
				MsgBoxInfo.AddString(strMsg);
				CreateMessageBox(MsgBoxInfo);
				...
				IsBila(g_iCountry) == TRUE) && chkType == eCHECK_TYPE_GUILD_NAME)
			{
					...
					)//FRANCE_SPAIN_CLOSEBETA_NA_20081124
				{
					...
					MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK,
						UI_CREATE_CHAR, MSGCMD_CREATE_ERROR);
					CTString	strMessage = _S(3113, "��� �̸��� ������ ���ڸ� ����մϴ�.");
					MsgBoxInfo.AddString(strMessage);
					CreateMessageBox(MsgBoxInfo);
					return FALSE;
				}
	...
	//���̸� ���� added by sam 11/01/03
	else if (chkType == eCHECK_TYPE_PET_NAME)
	{
		strTitle = _S(191, "Ȯ��");
		if (strMsg.Length() < 1)
		{
			strMsg = _S(3257, "��û�� �۾��� ���� �Ͽ����ϴ�.");
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			return FALSE;
		...
		for (const char* chr = strMsg.str_String; *chr != 0; chr++)
		{
				...
				IsBila(g_iCountry) == TRUE)
			{
					...
					)//FRANCE_SPAIN_CLOSEBETA_NA_20081124
				{
					...
					MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK,
						UI_CREATE_CHAR, MSGCMD_CREATE_ERROR);
					CTString	strMessage = _S(3, "�߸��� ���ڰ� ���ԵǾ� �ֽ��ϴ�.");
					MsgBoxInfo.AddString(strMessage);
					CreateMessageBox(MsgBoxInfo);
					return FALSE;
				}
...
BOOL CUIManager::IsEnemy(void* pTarget, TARGET_TYPE eType, BOOL bSearch)
{
	...
	{
		if (bSearch)
		{// ���⼭ CharacterTargetŬ������ �Ҹ��ڰ� ȣ�� �Ǹ� �ȵȴ�.(ItemEffect �Ҹ��ڱ��� ȣ��ǹǷ�)
	...
	//pTarget �� NULLüũ�� ������ ��� �ߴ�. ���� ã�� ���ߴٸ� return FALSE;�� ������ pTarget���� NULL�ƴ��� �����Ѵ�.
...
// Desc : [ADD_SUBJOB] seller üũ�� ������ �� UI���� ��� ���� ���� Ȯ��
// ----------------------------------------------------------------------------
BOOL CUIManager::CheckSellerItem(UI_TYPE uiType, int flag)
{
	...
	{
		// [2010/10/14 : Sora] ��û���� ���ǿ� ITEM_FLAG_COMPOSITE �߰�
		if ((flag & ITEM_FLAG_ABS) || (flag & ITEM_FLAG_COSTUME2) || (flag & ITEM_FLAG_COMPOSITE) || (flag & FLAG_ITEM_TRANSMOGRIFY))
		{
			return FALSE; // �Ⱓ�� �ڽ�Ƭ2 �������� ������ FALSE
		}
		else // �����׸� ������ ������
		{
			if (bMerchant)
				return (uiType == UI_SHOP ? FALSE : TRUE); // ������ �����ǸŸ� �����ϰ��� ��� ���� 
			else
			{
				// [2012/04/09 : Sora]  ITS 8019 ��Ȱ �ֹ��� â�� ������ ������ �������� 
				if (flag & ITEM_FLAG_NO_STASH)
					return FALSE;
				else
					return (uiType == UI_WARE_HOUSE ? TRUE : FALSE); // ������ �ƴϸ� â���� �����ϰ��� ��� �Ұ�
			}
		}
	}
	else //not cash + seller
	{
		return (uiType == UI_SHOP ? FALSE : TRUE);	// ���� �ǸŻ��� ��� ����
	}
...
// Desc : Player.es���� define�� ������� �ʾƼ� UIManager���� define���� ȣ�⿩�� Ȯ��
// ----------------------------------------------------------------------------
...
DWORD CUIManager::IsCSFlagOnElapsed(DWORD dwCSF, TIME tElapsedTime)
{
	...
	if (tTiem > tElapsedTime) // Ư�� ��� �ð��� ������ Reset�Ѵ�.
	...
	if (pIcon->getBtnType() == UBET_ITEM)
	{
		...
		if (pItems != NULL)
		{
			if (pItems->Item_Toggle == true)
			{
				...
				{
					GetChattingUI()->AddSysMessage(_S(7035, "Ȱ��ȭ �� �������� �ű� �� �����ϴ�."));
					return;
				}
				else if (pIcon->GetWhichUI() != UI_QUICKSLOT &&
					(pIcon->GetWhichUI() < UI_INVENTORY ||
						pIcon->GetWhichUI() > UI_QUICKSLOT4))
				{
					GetChattingUI()->AddSysMessage(_S(7035, "Ȱ��ȭ �� �������� �ű� �� �����ϴ�."));
					return;
				}
