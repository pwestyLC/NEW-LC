...
// const value				
#define MAX_SIGNAL					3	// ȭ�鿡 ǥ�õǴ� �ִ� Signal �� 
...
//		m_rtExpedition[i].SetUV( 108, 67, 119, 78, fTexWidth, fTexHeight );
	...
	m_btnGoTop.Create( this, _S( 5784, "������ ����"), m_nWidth - 98, 26, 83, 23 );
...
#ifndef NEW_WORLD_MAP		// [2012/10/11 : Sora] ����� ����
	CZoneInfo::getSingleton()->SetZoneName( 17, _S(3050, "������") ); // ������ ����
#endif	// NEW_WORLD_MAP
...
void CUIMap::ShowToolTip( BOOL bShow, CTString &strName, int nX, int nY )
{
	...
	if( m_bShowWorldMap )
	{
		...
		// revision map size. [10/20/2009 rumist]
		// �̹� nX, nY�� m_fMapSizeRevision�� �ݿ��� ��ġ���̹Ƿ� m_fMapSizeRevision�� �ݿ����� �ʴ´�.
		nX *= m_fZoomWorld;
		nY *= m_fZoomWorld;
...
void CUIMap::SetCurrentWorldMap( int nWorld, SBYTE sbLayer )
{
	...
	case 0:
		...
		break;
	case 4:
		_pNetwork->MyCharacterInfo.fWaterHeight = -1.0f;	// WSS_DRATAN_SIEGEWARFARE 2007/10/16 �������� ������� ���� ���� 99.5f -> -1.0f
		break;
	}
	// ********************************************************* //
...
void CUIMap::Render()
{
	...
	if( m_bMapBlendingForDetail || m_bShowWorldMap )
	{
		if( IsCastleMap( m_nCurMapData ) )
		{
			...
			for( i = 0; i < m_vecMapData[m_nCurMapData].vecCastleData.size(); i++ )
			{
				...
				if( nType == CASTLE_TOWER ) // ��ȣž�� ��쿡�� hp�� 0�̸� �׸��� �ʴ´�.
				{
	...
	// connie [2009/9/18] - NPC ã�� Render Mob Blinking 
	...
	// [2011/04/28 : Sora] m_nCurMapData���� ���� ������ �ƴ�
...
// 		OutputDebugString(temp);
	...
	if( _pNetwork->MyCharacterInfo.zoneNo == m_vecMapData[m_nCurMapData].World.nZoneIndex )
	{
		...
		if (pOption->IsCheck(eNT_PARTY) == true)
		{
			...
			for( i = 0; i < pParty->GetMemberCount(); i++ )
			{
				...
				if(pParty->GetLevel( i ) == 0)	// [sora] ������ 0�� ����� �������� �����̹Ƿ� �׸��� �ʴ´�.
					continue;
		...
		if(pUIManager->IsCSFlagOn(CSF_EXPEDITION) && pOption->IsCheck(eNT_FELLOWSHIP) == true) // [sora] ������� ��ǥ��
		{
			...
			for(i=0; i<EXPEDITION_GROUP_MAX; i++)
			{
				for(int j = 0; j < EXPEDITION_MEMBER_PER_GROUP; j++ )
				{
					...
					if(!pParty->GetExpeditionMemberOnline(i,j)) // �¶��� ���°� �ƴϸ� �׸��� ����
						continue;
	...
	if( m_bShowFrame || m_bFrameBlending )
	{
		...
		// Text in map
		pDrawPort->PutTextEx( _S( 190, "����" ), m_nPosX + MAP_TITLE_TEXT_OFFSETX,
		...
		if( fBackRatio >= 1.0f )
		{
			...
			pDrawPort->PutTextEx( _S( 5798, "������"), m_slbOpacity.GetAbsPosX() - 10 - (CTString(_S( 5798, "������")).Length() * (_pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing())), m_nPosY + MAP_SLIDEINFO_Y);
#endif
...
#else
	m_strOpacitySlideInfo.PrintF( _S( 1035, "���� %d%%" ), ( 25 + m_slbOpacity.GetCurPos() ) );
...
void CUIMap::UpdateZoomSlideInfo( BOOL bWorldMap )
{
	...
	if( bWorldMap )
	{
		if( m_slbZoomWorld.GetCurPos() < MAP_ZOOM_SLIDE_ORIGIN )
			m_strZoomSlideInfo.PrintF( _S( 1036, "ũ�� %d%%" ), 100 - ( MAP_ZOOM_SLIDE_ORIGIN - m_slbZoomWorld.GetCurPos() ) * 5 );
		else
			m_strZoomSlideInfo.PrintF( _S( 1036, "ũ�� %d%%" ), 100 + ( m_slbZoomWorld.GetCurPos() - MAP_ZOOM_SLIDE_ORIGIN ) * 25 );
	}
	else
	{
		if( m_slbZoomDetail.GetCurPos() < MAP_ZOOM_SLIDE_ORIGIN )
			m_strZoomSlideInfo.PrintF( _S( 1036, "ũ�� %d%%" ), 100 - ( MAP_ZOOM_SLIDE_ORIGIN - m_slbZoomDetail.GetCurPos() ) * 5 );
		else
			m_strZoomSlideInfo.PrintF( _S( 1036, "ũ�� %d%%" ), 100 + ( m_slbZoomDetail.GetCurPos() - MAP_ZOOM_SLIDE_ORIGIN ) * 25 );
	}
			...
			if( m_bShowFrame )
			{
				...
				if( !pUIManager->IsInsideUpperUIs( UI_MAP, nX, nY ) )
				{
					...
					for( int i = 0; i < m_vecMapData[m_nCurMapData].vecNpc.size(); i++ )
					{
						...
						{
							if (m_nCurDetail >= 0 && m_nCurDetail < m_vecMapData[m_nCurMapData].vecDetail.size() )	// ����ڵ� �߰�.
							{
...
void CUIMap::AddSignal( FLOAT fX, FLOAT fY, LONG lSenderFlag, LONG lSenderIndex )
{
	...
	// Date : 2005-11-10(���� 1:30:55), By Lee Ki-hwan 
	// �ñ׳� ���� �߰� 
...
// �ñ׳� ǥ�� ����
#define SIGNAL_SHOWTIME		5.0f							// 5�� ����
#define SIGNAL_COUNT		5.0f							// 5�� 
#define SIGNAL_UNITTIME		(SIGNAL_SHOWTIME/SIGNAL_COUNT)  // 1�� ǥ���Ҷ� �ɸ��� �ð�.
...
void CUIMap::ReSetData()
{
	...
	for( int i = 0; i < m_vecMapData.size(); i++ )
	{
		if( IsCastleMap( i ) ) // ������ ���̶�� 
		{
			if( IsPossibleCastleMap() ) // �������� ������ ��Ȳ���� �ľ��ؼ� �߰�
			{
...
BOOL CUIMap::IsNpcInField(INDEX nZoneNum, INDEX nNpcIndex)
{
	...
	if (m_vecMapData[m_nCurMapData].vecDetail.size() == 0)
		return FALSE;
...
CTString CUIMap::GetZoneName( int zone, int layer )
{
	...
	else
	{
		if( layer > 0 )
		{
			strReturn.PrintF( _S( 3083, "%s ��������" ), _pUISWDoc->GetCastleName( zone ) );
		}
		else
		{
...
bool CUIMap::Goto(int x, int y)
{
	...
	if (_bDetail == false || m_vecMapData[m_nCurMapData].vecDetail.size() == 0)
	{
		nMX = (x * m_fZoomDetail / m_vecMapData[m_nCurMapData].World.fRatio 
			+ m_vecMapData[m_nCurMapData].World.lOffsetX) / m_fMapSizeRevision;

		nMY = (y * m_fZoomDetail / m_vecMapData[m_nCurMapData].World.fRatio 
			+ m_vecMapData[m_nCurMapData].World.lOffsetZ) / m_fMapSizeRevision;
	}
	else
	{
		if (m_nCurDetail >= 0)
...
void CUIMap::UpdateMap( bool bUpdate /*= false*/ )
{
		...
		CZoneInfo::getSingleton()->GetZoneType( g_slZone ) == ZONE_DUNGEON )
	{
		...
		pUIManager->GetChattingUI()->AddSysMessage( _S( 5785, "���� ���������� ������� ��� �� �� �����ϴ�.." ), SYSMSG_ERROR );
		m_eMapState = eMS_NONE;
		return;
	}
