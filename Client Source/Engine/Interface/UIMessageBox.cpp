...
void CUIMsgBox_Info::SetInputBox( int nRow, int nCol, int nMaxChar, int nInputWidth )
{
	...
	m_nInputPosY = MSGBOX_MESSAGE_OFFSETY + nRow * _pUIFontTexMgr->GetLineHeight() -
					( 13 - _pUIFontTexMgr->GetFontHeight() ) / 2;		// 13�� �Է� ���� �̹����� ����
...
void CUIMessageBox::CreateMessageBox( CUIMsgBox_Info &rMsgBoxInfo, int nPosX, int nPosY )
{
	...
	if(m_dwStyle & UMBS_USE_TIMER) // [sora] �����ð��� �޽��� ����
	{
		nHeight += 14;	// Ÿ�̸��� ��� ���� �ð� ǥ�ø� ���� 1���� ��ŭ ���̸� �÷��ش�
	}
	...
	if( m_dwStyle & UMBS_INPUT_MASK )
	{
		...
		if( rMsgBoxInfo.m_nInputMaxChar > 0 )
			...
			m_ebInput.SetMaxChar( 17 ); // �⺻ �ִ� ĳ���� ���� 

		if( m_dwStyle & UMBS_SECOND_INPUTBOX )		// Date : 2006-06-01(���� 4:39:49), By eons
		{
		...
		if( rMsgBoxInfo.m_nInputPosX < 0 )
		{
			...
			if( m_dwStyle & UMBS_CONFIRMPASSWORD )
			{
				...
				// �Է�â�� ��ġ�� �������Ƿ� �� ����ŭ�� �޽��� â�� ũ�⵵ ���ؾ� �Ѵ�. added by sam 10/11/18
		...
		else
		{
			...
			{
				//�Է»����� ũ�Ⱑ �޽��� â�� ũ�⸦ ������ �޽��� â�� ũ�⸦ �ø���. modified by sam 10/11/18
	...
	if(m_dwStyle & UMBS_SELECTBOX)
	{
		switch(m_nCommandCode)
		{
			...
			case MSGCMD_USE_REBIRTH_ITEM_SCROLL2:
				...
				addSelectContext( _S( 2144, "����� ������������ ��Ȱ" ) ); 
				addSelectContext( _S( 2145, "�����ҿ��� ��Ȱ" ) ); 
				break;

			case MSGCMD_USE_SLOT_ITEM:
				addSelectContext(_S( 2112,  "Ȯ�彽�� 1" )); 
				addSelectContext( _S( 2113, "Ȯ�彽�� 2" )); 
				break;
			case MSGCMD_USE_PET_ACCUMULATE:
				addSelectContext(_S(5643,"ĳ���Ϳ� ����"));
				addSelectContext(_S(5640, "�꿡 ����"));
				addSelectContext(_S(139,"���"));
				break;
			default:
				break;
		}
...
void CUIMessageBox::Render()
{
	...
	if( m_dwStyle & UMBS_INPUT_MASK )
	{
		...
		{			
			...
			pDrawPort->PutTextEx( _S(2278, "�Է�" ), m_nPosX + 25, m_nPosY + 64+2 );		
			pDrawPort->PutTextEx( _S(191, "Ȯ��" ), m_nPosX + 25, m_nPosY + 83+2 );
		}
		else 
		{		
	...
	if( m_nWhichUI > 0 )	// m_nWhichUI���� -1�� ��� ���콺 �޽����� ���� �����Ƿ� �׸��� �ʴ´�
	{
	...
	if( m_dwStyle & UMBS_USE_TIMER ) // [sora] �����ð� ǥ��
	{
		if(m_nTime > 0)
		{
			__int64 tmpTime = m_nTime - ((_pTimer->GetHighPrecisionTimer().GetMilliseconds() - m_nStartTime) / 1000);
			int nStrLastNum = iMsg-1;	// ������ ����
			CTString strRemainTime;
			if(tmpTime / 60)
			{
				int RemainMin = tmpTime/60;
				int RemainSec = tmpTime%60;
				strRemainTime.PrintF(_S(4511, "�����ð� : %d�� %d��"), RemainMin, RemainSec);
			}
			else
			{
				strRemainTime.PrintF(_S(4512, "�����ð� : %d��"), tmpTime);
			}
			pDrawPort->PutTextEx(strRemainTime, m_nPosX + m_nStrPosX[nStrLastNum] + 20,
					m_nPosY + m_nStrPosY[nStrLastNum] + 40, m_colMessage[nStrLastNum]);
		}
		else
		{
			// Run Command
			...
			if (pUIManager->GetDragIcon() == NULL)
			{
				...
				if( ( wmsgResult = m_sbtnSpin.MouseMessage( pMsg ) ) != WMSG_FAIL )
				{
					if( m_nCommandCode == MSGCMD_SIEGE_DUNGEON_ENVIRONMENT_CONTROL )
					{
						...
						str.PrintF( _s("%s: %s %s%s"), _S(1058, "�ʿ䳪��"), strNas, _S(1762, "����"), _s("   ") );
...
// wooss Fix - call me later ^^;					
						...
						pUIManager->GetDragIcon()->GetWhichUI() == UI_INVENTORY )
					{
						for(int i=0; i<m_nBtnCount; ++i)
						{
							// ($E_WC2010) [100517: selo] Ȱ��ȭ�� ���� ��ư�� �ƴϸ� ���� �ϴ� ��� �߰�
...
void CUIMessageBox::SetBtnSlot(int nBtn)
{
	...
	{
		pUIManager->GetChattingUI()->AddSysMessage( _S( 2898, "���� ���� ���� �������� �÷� �ּ���" ), SYSMSG_ERROR );		
		return;	
	}
	...
	if( m_nCommandCode == MSGLCMD_EVENT_XMAS_2006_CHANGE ) // 2006 X-Mas event [12/12/2006 Theodoric]
	{
		...
		if( nItemIndex != 1975 ) // ����ũ 1975
		{
			pUIManager->GetChattingUI()->AddSysMessage( _S(3181, "��ȯ ������ �������� �ƴմϴ�." ), SYSMSG_ERROR );
			return;
		}
		...
		if( pUIManager->GetMessageBox(MSGLCMD_EVENT_XMAS_2006_COUNT) == NULL)
		{
			...
			strSysMessage.PrintF( _S(150,"�� ���� %s<��> �ű�ðڽ��ϱ�?"), pItemData->GetName() );
			MsgBoxInfo.AddString( strSysMessage );
			MsgBoxInfo.SetMsgBoxInfo( _S(127, "��ȯ" ), UMBS_OK|UMBS_INPUTBOX , UI_NONE, MSGLCMD_EVENT_XMAS_2006_COUNT);
	...
	{	// �� �Ʒ� ��� ī���� �޽��� �ڽ��� ���
		...
		{// ���� �Ǽ��縮( �� ������ )�� ���� ��ư ���Ը� �˻�
			if( m_nItemSubType == CItemData::ACCESSORY_PET )	// �� ������
			{
		...
		if( iter != _pNetwork->m_vectorPetList.end() )
		{
			...
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S( 2899, "��ų�� �ʱ�ȭ �� Ż ��(����Ʈ) ������ �ֿ� ������ �÷��ּ���." ), SYSMSG_ERROR );
				return;
			}
		}
		m_btnOk.SetEnable( TRUE );
	...
	else if( m_nCommandCode == MSGCMD_RARE_IDENTIFY)
	{
		...
		{
			pUIManager->GetChattingUI()->AddSysMessage( _S(3162, "�ɼ� ������ �� �� ���� �������Դϴ�." ), SYSMSG_ERROR );		
			return;
		}
		else
			m_btnOk.SetEnable(TRUE);
	...
	else if(m_nCommandCode == MSGCMD_USE_PACKING_PAPER || m_nCommandCode == MSGCMD_USE_SMALL_CORD)
	{
		...
		{
			if (nItemIndex < 2043 || nItemIndex > 2045) // ���ڷ� ������ , ���ָӴ� ������)
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(3201, "���ڷ� �������� �÷��ּ���" ), SYSMSG_ERROR );
				return;
			}
		}
		else if (m_nCommandCode == MSGCMD_USE_SMALL_CORD)
		{
			...
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(3995, "���ָӴ� �������� �÷��ּ���" ), SYSMSG_ERROR );
				return;
			}
		}
	...
	else if(m_nCommandCode == MSGCMD_OK_EXCHANGE_CHOCOBOX || m_nCommandCode == MSGCMD_OK_EXCHANGE_LUCKYBOX)
	{
		...
		{
			if (nItemIndex != 2047)		// ���ڷ� ������)
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(3202, "���ڼ������� �������� �÷��ּ���" ), SYSMSG_ERROR );
				return;
			}
		}
		else if (m_nCommandCode == MSGCMD_OK_EXCHANGE_LUCKYBOX )
		{
			if (nItemIndex != 2633)	// ���ؼҸ����� ������
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(4009, "���ؼҸ����� �������� �÷��ּ���" ), SYSMSG_ERROR );
				return;
			}
		}
	...
	else if( m_nCommandCode == MSGCMD_ADULTEVENT_ALCHEMIST)
	{
		...
		if( iLevel<1 ||																		//������ ������ 0�� ��,
			pItemData->IsFlag(ITEM_FLAG_CASH) ||												//ĳ�� �������� ��,
			( 1228 <= pItemData->GetItemIndex() && pItemData->GetItemIndex() <=1278) ||		//�Ϻ� �ǻ� �������� ����
			( 1978 <= pItemData->GetItemIndex() && pItemData->GetItemIndex() <=2031) )		//���� �õ��� �� ����.
		{
		...
		if( iLevel >33)
		{
			...
			MsgBoxInfo.AddString( _S(3645, "������ �� ���� �������Դϴ�. 33���� ������ �� �� ���⸦ �÷��ּ���.") );
			pUIManager->CreateMessageBox( MsgBoxInfo );
		...
		INDEX iReqMaterial[2] ={0,0};		// �ʿ� ���(index)
		INDEX iReqCount[2] ={0,0};			// �ʿ� ����

		if( iLevel>=1 && iLevel<=5 )
		{
			iReqMaterial[0] =113;	iReqCount[0] =2;	//���찡��
			iReqMaterial[1] =114;	iReqCount[1] =2;	//�ڵ��̻�
		}
		else if( iLevel>=6 && iLevel<=10 )
		{
			iReqMaterial[0] =119;	iReqCount[0] =3;	//��Ծ� �̻�
			iReqMaterial[1] =110;	iReqCount[1] =3;	//�罿��
		}
		else if( iLevel>=11 && iLevel<=15 )
		{
			iReqMaterial[0] =127;	iReqCount[0] =5;	//������ ��
			iReqMaterial[1] =123;	iReqCount[1] =3;	//������ �ֹ���

		}
		else if( iLevel>=16 && iLevel<=20 )
		{
			iReqMaterial[0] =116;	iReqCount[0] =5;	//�η��� ��������
			iReqMaterial[1] =133;	iReqCount[1] =5;	//������ ����
		}
		else if( iLevel>=21 && iLevel<=25 )
		{
			iReqMaterial[0] =124;	iReqCount[0] =7;	//����Į��
			iReqMaterial[1] =140;	iReqCount[1] =5;	//���� �ش�����
		}
		else if( iLevel>=26 && iLevel<=30 )
		{
			iReqMaterial[0] =129;	iReqCount[0] =7;	//�η��� Į��
			iReqMaterial[1] =134;	iReqCount[1] =7;	//�콼 ��������
		}
		else if( iLevel>=31 && iLevel<=35 )
		{
			iReqMaterial[0] =118;	iReqCount[0] =9;	//����
			iReqMaterial[1] =128;	iReqCount[1] =9;	//��ȭ���
		}
		...
		strTemp.PrintF( _S(3646, "%s (%s/%s)��"), _pNetwork->GetItemName(iReqMaterial[0]), strCur, strCon);
		...
		strTemp.PrintF( _S(3646, "%s (%s/%s)��"), _pNetwork->GetItemName(iReqMaterial[1]), strCur, strCon);
	...
	// [070807: Su-won] EVENT_ADULT_OPEN
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	else if (m_nCommandCode == MSGCMD_TG2007_1000DAYS_CAP_UPGRADE) // 1000�� ������ ���׷��̵�
	{
		int nItemIndex = pItemData->GetItemIndex();

		if (nItemIndex < 2423 || nItemIndex > 2434) // ������ �������� �ƴ϶��
		{
			pUIManager->GetChattingUI()->AddSysMessage( _S(3831, "��Ʈī���� 1000�� �����ڸ� �ش�˴ϴ� Ȯ���ϰ� �ٽ� �õ��Ͽ� �ֽʽÿ�"), SYSMSG_ERROR );
			return;
		}
	}
	...
	{
		int nItemIndex = pItemData->GetItemIndex();
		// �÷�Ƽ�� �ֻ�����ú�ȣ��(#6218) �߰�. [9/14/2010 rumist]
		...
		if( nItemIndex == 2394 )
		{
			...
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(3663, "+3���� +9������ ��񿡸� ����� �� �ֽ��ϴ�." ), SYSMSG_ERROR );
				return;
			}
		}
		...
		if( nItemIndex == 2395 )
		{
			...
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(3664, "+10���� +19������ ��񿡸� ����� �� �ֽ��ϴ�." ), SYSMSG_ERROR );
				return;
			}
		}
		if( nItemIndex == 11023)
		{
			...
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(6467, "+15���� +19������ ��񿡸� ����� �� �ֽ��ϴ�." ), SYSMSG_ERROR );
				return;
			}
		}
		if( nItemIndex == 6218)
		{
			...
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(5612, "+20���� +25������ ��񿡸� ����� �� �ֽ��ϴ�." ), SYSMSG_ERROR );
				return;
			}
		}
		...
		if ( nItemIndex == 5091 )
		{
			...
			{
				strSysMessage.PrintF(_S(4824, "%d���� %d������ ĳ���͸� ����� �� �ֽ��ϴ�."), 1, 60);
			...
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S(3664, "+10���� +15������ ��񿡸� ����� �� �ֽ��ϴ�." ), SYSMSG_ERROR );
				return;
			}
		}
		...
		if ( nItemIndex == 5092 )
		{
			...
			{
				strSysMessage.PrintF(_S(4824, "%d���� %d������ ĳ���͸� ����� �� �ֽ��ϴ�."), 61, 90);
				pUIManager->GetChattingUI()->AddSysMessage( strSysMessage, SYSMSG_ERROR );
				return;
			}
			// 14->15
	...
	}else if (m_nCommandCode == MSGCMD_PET_EVOLUTION || m_nCommandCode == MSGCMD_USE_PET_ACCUMULATE2)
	{
		...
		if( nItemType != m_nItemType || pItemData->GetSubType() != m_nItemSubType )
		{// ���� �Ǽ��縮( �� ������ )�� ���� ��ư ���Ը� �˻�
			if( m_nItemSubType == CItemData::ACCESSORY_WILDPET )	// �� ������
			{
				pUIManager->GetChattingUI()->AddSysMessage( _S( 2445, "�� �����۸� �����մϴ�." ), SYSMSG_ERROR );
				return;
			}
		...
		if (m_nCommandCode == MSGCMD_USE_PET_ACCUMULATE2)
		{
			...
			strTemp.PrintF( _S(5671,"���� ������ ����ġ: %s"), strExp);
	...
	// ($E_WC2010) [100517: selo] 2010 ���ư� ������ �̺�Ʈ2 ��� ���� �����ϱ�
	else if (m_nCommandCode == MSGCMD_EVENT_WORLDCUP_EVENT2_REQ)
	{
		...
		if( bNationFlag != TRUE )
		{
			pUIManager->GetChattingUI()->AddSysMessage( _S(4931, "���� �����۸� �����մϴ�."), SYSMSG_ERROR );
		}
	}
	...
	else if( m_nCommandCode == MSGCMD_SOCKETSYSTEM_CHANCECARD )
	{
		...
		{
			pUIManager->GetChattingUI()->AddSysMessage( _S( 4998, "������ �������� ���� �������Դϴ�." ), SYSMSG_ERROR );
			return;
		}
		else if (pDrag->IsWearTab() == true)
		{
			pUIManager->GetChattingUI()->AddSysMessage( _S( 5009, "������ �����ۿ��� ����� �� �����ϴ�." ), SYSMSG_ERROR );
			return;
		}
	}
	...
	else if( m_nCommandCode == MSGCMD_PET_STASH_INSTASH )
	{
		...
		if ( pItems->IsFlag(FLAG_ITEM_SEALED) )
		{
			...
			MsgBoxInfo.AddString( _S(5952,"���� �� ���� ���� �Ͻ� �� �����ϴ�.") );
			pUIManager->CreateMessageBox( MsgBoxInfo );
			return;
		}		
	}
