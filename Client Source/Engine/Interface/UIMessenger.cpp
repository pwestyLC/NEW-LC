...
#define RESIZE_RANGE						10		// Windowũ�� �� ���� ����
...
#define	TOP_HEIGHT							25		// Ÿ��Ʋ ���� ����
#define MY_INFO_HEIGHT						19		// �ڽ��� ������ ǥ�� �Ǵ� â�� ���� 
#define BACK_MIDDLE_SCROLL_BOTTOM_HEIGHT	5+5		// ��ũ�ѹٰ� ���ԵǾ� �ִ� �߰� â�� ���� 
#define BACK_BOTTOM_HEIGHT					13		// �Ʒ��� â�� ����
...
#define MAX_MY_NAME_SIZE_EX		24		// ���� �������� ���Ͽ� �ߺ��� ���̵� �ڿ� [K] �� �ٰ� ������ ���Ͽ� 16�ڰ� �Ѿ�� ��찡 �߻�.
										// ���̵� �߰�, ����, ����, �˻� ���̵� üũ�����μ� 20�ڱ��� ���

static CTString s_strReqCharName;
static bool	g_bUseChat[MAX_TALK];							//ä��â ��� Ȯ��
static int	g_nMakeCharIndex[MAX_TALK];						//ä��â ���� ĳ�� �ε���
static int	g_nChatIndex[MAX_TALK];							//ä��â �ε���
...
inline COLOR GetSafeChatColor(int index) {
	if (index >= 0 && index < MAX_COLOR) {
		return ChatColor[index];
	}
	return 0xFFFFFFFF;
}
//------------------------------------------------------------------------------
...
void CUIMessenger::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
	...
	***/
	...
	//m_btnGroup.Create( this, _S( 1612,  "�׷�"  ), BUTTON_GROUP_X, BUTTON_Y, 52, 21 );   ==>
	m_btnGroup.Create( this, _S(2982, "�׷����"  ), BUTTON_GROUP_X, BUTTON_Y, 78, 21 ); 
	...
	m_btnBlock.Create( this, _S(2983, "���ܱ��"  ), BUTTON_BLOCK_X, BUTTON_Y, 78, 21 ); 
	...
	//m_btnMember.Create( this, _S( 1613,  "ģ��"  ), BUTTON_MEMBER_X, BUTTON_Y, 52, 21 ); 
	m_btnInsert.Create( this, _S(550, "�߰�"  ), BUTTON_INSERT_X, BUTTON_Y, 52, 21 ); 
	...
	m_btnDelete.Create( this, _S( 1614,  "����"  ), BUTTON_DELETE_X, BUTTON_Y, 52, 21 ); 
	...
	//m_rtArrow.SetUV(11, 117, 19, 125, fTexWidth, fTexHeight);		//�Ķ� ȭ��ǥ
}
...
void CUIMessenger::CreateGroup( float fTexWidth, float fTexHeight )
{
	...
	m_mgBlock.SetGroupName( _S(2985, "���ܵ� ����" ) ); 
	...
	{
		CTString strName;
		strName.PrintF( "%c%s%d", rand()%25+65, "��������������", i );
...
void CUIMessenger::CreateTrackPopup( float fTexWidth, float fTexHeight )
{
	...
	m_tpSetMyState.AddMenuList( _S( 1615,  "��������"  ) ); 
	m_tpSetMyState.AddMenuList( _S( 1616,  "�¶���"  ) );	
	m_tpSetMyState.AddMenuList( _S( 1617,  "�ڸ����"  ) );
	...
	m_tpGroup.AddMenuList( _S(2987, "���׷� �߰�") +CTString("     "));
	m_tpGroup.AddMenuList( _S(2988, "�׷� ����") +CTString("     "));
	m_tpGroup.AddMenuList( _S(2989, "�׷� �̸� ����" ) +CTString("     "));
	...
	m_tpMemberMenu.AddMenuList( _S(2990, "�׷��̵�") +CTString("     ") );
	m_tpMemberMenu.AddMenuList( _S(2991, "��ȭ�ϱ�") );
	m_tpMemberMenu.AddMenuList( _S(338, "����" ) );
	m_tpMemberMenu.AddMenuList( _S(2992, "����" ) );
	...
	m_tpGroupMenu.AddMenuList( _S(2987, "���׷� �߰�") );
	m_tpGroupMenu.AddMenuList( _S(2988, "�׷� ����") );
	m_tpGroupMenu.AddMenuList( _S(2989, "�׷� �̸� ����") );
	...
	m_tpBlock.AddMenuList( _S(3013, "��������"));
	m_tpBlock.AddMenuList( _S(338, "����"));
}
...
void CUIMessenger::RenderGroup( int nX, int nY )
{
	...
	ResizeScrollBar(); //Group�� ������ ���� ��ũ�� �� ũ�� ����
		
	int nBoardHeight = m_nBoardHeight; // ��� ���� ����
	int nPos		 = m_sbMemberList.GetScrollPos(); // ��ũ�� �� Pos
	...
	case WM_LBUTTONUP:
		{
			...
			if( m_tpBlock.MouseMessage( pMsg ) != WMSG_FAIL )
			{
				...
				*****/
				...
				{
					CTString strMessage;
					strMessage.PrintF(_S(3014, "%s���� ���������Ͻðڽ��ϱ�?"), m_miSelectMember.m_strName );
					Message( MSGCMD_MESSENGER_UNBLOCK, _S(3004, "ĳ���� ����"), strMessage, UMBS_YESNO );
				}
				else  if( m_tpBlock.GetCurSel() ==1)
			...
			if( IsInside( nX, nY ) )
			{
				...
				{
					Message( MSGCMD_MESSENGER_BLOCK_REQ, _S(3004, "ĳ���� ����"), _S(3007, "������ ĳ������ �Է��Ͽ� �ֽʽÿ�." ), UMBS_OKCANCEL | UMBS_INPUTBOX );
					return WMSG_SUCCESS;
				}
				else if( m_btnInsert.MouseMessage( pMsg ) != WMSG_FAIL )
				{
					Message( MSGCMD_MESSENGER_ADD_REQ, _S(2993, "ģ���߰�"), _S( 1620, "����� ģ���� �̸��� �Է��Ͽ� �ֽʽÿ�." ), UMBS_OKCANCEL | UMBS_INPUTBOX );

					return WMSG_SUCCESS;
				}
				else if( m_btnDelete.MouseMessage( pMsg ) != WMSG_FAIL )
				{
					Message( MSGCMD_MESSENGER_DEL_REQ, _S(2999, "ĳ���� ����"), _S(3040, "������ ĳ���͸��� �Է��Ͽ� �ֽʽÿ�." ), UMBS_OKCANCEL | UMBS_INPUTBOX );
					return WMSG_SUCCESS;
				}
				else if( m_sbMemberList.MouseMessage( pMsg ) != WMSG_FAIL )
					return WMSG_SUCCESS;
	...
	case MSGCMD_MESSENGER_ADD_REQ:
		{
			if( bOK )
			{
				...
				{
					Message( MSGCMD_MESSENGER_ERROR, _S(2997, "ģ�� �߰� ����"), _S(2995, "���ܵ� ĳ���Դϴ�. ���� �������� �� ģ���� �߰��Ͻñ�ٶ��ϴ�." ), UMBS_OK );		
	...
	case MSGCMD_MESSENGER_BLOCK_DEL_REQ:
		{
			if( bOK )
			{
					...
					(miDelMember = FindBlockMember(strInput)).m_nCharIndex != -1)
				{
					...
					{
						strMessage.PrintF(_S(3000,"%s���� �����Ͻðڽ��ϱ�?"), miDelMember.m_strName);

						if (nCommandCode == MSGCMD_MESSENGER_DEL_REQ)
							Message( MSGCMD_MESSENGER_DEL, _S(2999, "ĳ���� ����"), strMessage, UMBS_YESNO );
						else
							Message( MSGCMD_MESSENGER_BLOCK_DEL, _S(2999, "ĳ���� ����"), strMessage, UMBS_YESNO );
					}
					else
					{
						strMessage.PrintF(_S(3014, "%s���� ���������Ͻðڽ��ϱ�?"), miDelMember.m_strName);
						Message( MSGCMD_MESSENGER_UNBLOCK, _S(3013, "���� ����"), strMessage, UMBS_YESNO );						
					}
				}
				else
				{
					...
					if( strInput.Length() < MIN_NAME_SIZE || strInput.Length() > MAX_MY_NAME_SIZE_EX )
						Message( MSGCMD_MESSENGER_ERROR, _S(3003, "ģ�� ���� ����"), _S(3002, "�����ϰ��� �ϴ� ģ�� �̸��� �ùٸ��� �ʽ��ϴ�." ), UMBS_OK );		
					else
						Message( MSGCMD_MESSENGER_ERROR, _S(3003, "ģ�� ���� ����"), _S(781, "��� ĳ���Ͱ� �������� �ʽ��ϴ�."), UMBS_OK);
				}
			}
		}		
		break;
	...
	case MSGCMD_MESSENGER_BLOCK_REQ:
		{
			if( bOK )
			{
				...
				{
					Message( MSGCMD_MESSENGER_ERROR, _S(3008, "���� ����"), _S(3009, "�����ϰ��� �ϴ� �̸��� �ùٸ��� �ʽ��ϴ�"), UMBS_OK);		
					return;
				}
				if( FindBlockMember(strInput).m_nCharIndex >=0 )
				{
					Message( MSGCMD_MESSENGER_ERROR, _S(3008, "���� ����"), _S(3011, "�̹� ���ܵ� ĳ���Դϴ�."), UMBS_OK);
					return;
				}
				...
				for(int i=0; i<10; ++i)
				{
					...
					{
						Message( MSGCMD_MESSENGER_ERROR, _S(3008, "���� ����"), _S(3012, "��ȭ���� ���� ������ �� �����ϴ�."), UMBS_OK);
						return;
					}
				}
				...
				CTString strMessage;
				strMessage.PrintF(_S(3005, "%s���� �����Ͻðڽ��ϱ�?"), strInput);
				Message( MSGCMD_MESSENGER_BLOCK, _S(3004, "ĳ���� ����"), strMessage, UMBS_YESNO );		
			}
		}
		break;
	...
	case MSGCMD_MESSENGER_GROUP_ADD:
		{
			if( bOK )
			{
				if( FindGroup(strInput) >=0 )
					Message( MSGCMD_MESSENGER_ERROR, _S(3020, "�׷��߰� ����"), _S(3021, "���� �̸��� ���� �׷��� �����ϰ� �ֽ��ϴ�."), UMBS_OK);
	...
	case MSGCMD_MESSENGER_GROUP_DEL:
		{
			if( bOK )
			{
				...
				if( m_vecGroup[m_nSelectGroup]->m_vecMember.empty() )
				{
					...
					Message( MSGCMD_MESSENGER_ERROR, _S(3025, "�׷� ���� ����"), _S(3027, "�׷쿡 ��ȭ ��밡 �������� ������ �Ұ����մϴ�.\n�׷��� �����Ϸ��� ��ȭ��븦 �ٸ� �׷����� �ű�ž� �����մϴ�."), UMBS_OK);
			}
		}
		break;
	...
	case MSGCMD_MESSENGER_GROUP_RENAME:
		{
			if( bOK )
			{
				...
				if( FindGroup(strInput) >=0 )
					Message( MSGCMD_MESSENGER_ERROR, _S(3030, "�׷�� ���� ����"), _S(3021, "���� �̸��� ���� �׷��� �����ϰ� �ֽ��ϴ�."), UMBS_OK);
				else if( UTIL_HELP()->IsSpecialChar(strInput.str_String) == true )
				{
					CUIMsgBox_Info MsgBoxInfo;
					MsgBoxInfo.SetMsgBoxInfo(_S(191,"Ȯ��"), UMBS_OK,UI_NONE, MSGCMD_NULL);
...
void CUIMessenger::RegistMemberReq( CTString strName )
{
	...
	{
		Message( MSGCMD_MESSENGER_ERROR, _S(2997, "ģ�� �߰� ����"), _S( 1937, "����ϰ��� �ϴ� ģ�� �̸��� �ùٸ��� �ʽ��ϴ�." ), UMBS_OK );		
		return;
	}

	if( strcmp(strName,  m_MyInfo.m_strName) == 0 )	
	{
		Message( MSGCMD_MESSENGER_ERROR, _S(2997, "ģ�� �߰� ����"), _S( 1938, "�ڽ� �ڽ��� ����� �� �����ϴ�." ), UMBS_OK );	
		return;
	}
	...
	{
		Message( MSGCMD_MESSENGER_ERROR, _S(2997, "ģ�� �߰� ����"), _S(2998, "�̹� ��ϵǾ� �ִ� ĳ���Դϴ�." ), UMBS_OK );	
		return;
	}
...
void CUIMessenger::RegistMemberReq( int nCharIndex, CTString strName )
{
	...
	CTString strMessage;
	strMessage.PrintF( _S( 1623, "%s���� ģ����� ��û�� �ϼ̽��ϴ�. ���� �Ͻðڽ��ϱ�?" ), strName );
	Message( MSGCMD_MESSENGER_ADD_REP, _S(2996, "ģ����� ��û"), strMessage, UMBS_YESNO );
}
...
{
	/********
	// Date : 2005-06-08(���� 1:57:15), By Lee Ki-hwan : �ƹ��� ���õ� �ȵ� ���¿��� ������ 
	if ( m_miTarget.m_nCharIndex == -1 ) return; 

	CTString strMessage;
	strMessage.PrintF( _S( 1624, "������ %s���� ��ȭ��뿡�� �����Ͻðڽ��ϱ�?" ), m_miTarget.m_strName );
	Message( MSGCMD_MESSENGER_DEL, _s("ģ�� ����"), strMessage, UMBS_OKCANCEL );
	*********/
	...
	CTString strMessage;
	strMessage.PrintF(_S(3001, "%s���� �����߽��ϴ�."), strName);
	Message( MSGCMD_MESSENGER_ERROR, _S(2999, "ĳ���� ����"), strMessage, UMBS_OK );		
}
...
void CUIMessenger::AddFriendList( int nCharIndex, int nGroupIndex, CTString strName, eJob nJob, eCondition eState, bool bBlock )
{
	...
	{
		CTString strMessage;
		strMessage.PrintF( _S( 1626, "%s���� ģ�� ��Ͽ� �߰������ϴ�." ), strName  );
...
void CUIMessenger::AddFriendList( CMemberInfo miMember,  bool bBlock )
{
	...
	{
		CTString strMessage;
		strMessage.PrintF( _S( 1626, "%s���� ģ�� ��Ͽ� �߰������ϴ�." ), miMember.m_strName  );
...
void CUIMessenger::ErrorMsg( int nError )
{
	switch( nError )
	{
		...
		case MSG_FRIEND_ERROR_PACKET:    
			{
				ErrorMessage( _S( 1627, "�˼� ���� ������ �Դϴ�." ) );
			}
			break;
		case MSG_FRIEND_ERROR_NOT_EXIST: //�������� �ʴ´�.
			{
				ErrorMessage( _S( 1628, "�ش� �ɸ��Ͱ� ���� ���� �ʰų� [��������] �����Դϴ�." ) );
			}
			break;
		case 	MSG_FRIEND_ERROR_FULLMEMBER: //ģ�� ���� �ʰ�.
			{
				ErrorMessage( _S( 1629, "�� �̻� ����� �� �����ϴ�." ) );
			}
			break;
		case MSG_FRIEND_ERROR_ALREADY_EXIST: //�̹� ģ������Ʈ�� ����.
			{
				ErrorMessage( _S( 1630, "�̹� ��� �Ǿ� �ֽ��ϴ�." ) );
			}
			break;
		case MSG_FRIEND_ERROR_WAIT_OTHER: //�ٸ� ������κ��� ģ������� ��û���� ����..
			{
				ErrorMessage( _S( 1631, "�̹� ��� ��û�� ���� �����Դϴ�." ) );
			}
			break;
		case MSG_FRIEND_ERROR_NOT_MATCH_INDEX: //���ΰ� ģ�� ��û:���� ���̵� ��ġ���� �ʴ´�.
			{
				ErrorMessage( _S( 1632, "���̵� ��ġ���� �ʽ��ϴ�." ) );
			}
			break;
		case MSG_FRIEND_ERROR_HELPER_SERVER: //���ۼ������� ���ӿ���.
			{
				ErrorMessage( _S( 1633, "�˼� ���� ������ �Դϴ�." ) );
			}
			break;
		case MSG_FRIEND_ERROR_GAMESERVER:
			{
				ErrorMessage( _S( 1634, "�˼� ���� ������ �Դϴ�." ) );
			}
			break;
		case MSG_FRIEND_ERROR_REGIST_REFUSE_PVP:
			{
				ErrorMessage( _S( 3070, "PVP���� ĳ���ʹ� ģ������� �� �����ϴ�.") );
			}
			break;
	}
}
...
void CUIMessenger::DeleteMemberRep()
{
	...
	if( CUIManager::getSingleton()->DoesMessageBoxExist( MSGCMD_MESSENGER_ADD_WAIT ) ) // �߰� ��û���̶�� 
	{
		strMessage.PrintF( _S( 1635, "ģ���� ���� �߰��Ǿ����ϴ�." ) );
	}
	else
	{	
		strMessage.PrintF( _S( 1636, "ģ���� ���� �Ǿ����ϴ�." ) );
	}
}
	...
	for( int i =0; i < MAX_TALK; ++i )
	{
		...
		if( g_bUseChat[i] )
		{
			...
			if( UITalk->IsExistTarget(miMemberInfo.m_strName) )
			{
				...
				{
					TalkErrorMessage( _S( 1637, "��ȭ��밡 [��������] �����Դϴ�." ) );
					return false;
				}
				else
				{
					...
					{
						//Message�� ���� �����ϴ� (������ ����) [12/8/2006 KwonYongDae]
						//Message( MSGCMD_MESSENGER_ERROR, _S(3033, "��ȭ�ϱ� ����"), _S(3038, "�̹� ��ȭ���� ����Դϴ�."), UMBS_OK);
						return false;
					}
				}
			}
...
void CUIMessenger::RevOneVsOneTalk( int nSenderIndex, int nResiverIndex, CTString strSendName, CTString strChat, int nColIndex )
{
	...
	for( i =0; i < MAX_TALK; ++i )
	{
		...
		if( g_bUseChat[i] && 
			...
			&& UITalk->IsExistTarget( Sender.m_strName ) )// �������â, ������ ���� ����â ��������� �ִ� â
		{
			UITalk->AddTalkListString( strSendName, strChat, false, GetSafeChatColor(nColIndex));
			bExistTarget =true;
			break;
		}
	}
	...
	{			
		...
		if( ReadyOpenTalk( Sender ) )
		{
			...
			pUIManager->GetTalk(nUIIndex)->AddTalkListString( strSendName, strChat, false, GetSafeChatColor(nColIndex));
		}
	}	
}
...
void CUIMessenger::RevTalk( int nMakeCharIndex, int nChatIndex, CTString strSendName, CTString strChat, int nColIndex )
{
	...
	{
		CTalkUI* UITalk = pUIManager->GetTalk(i+UI_MESSENGER_TALK_START);
		if( g_bUseChat[i] && g_nChatIndex[i]==nChatIndex)// ���� ��ȭ���� ��밡 ���� �ɾ� �Գ׿�~
		{
...
//	extern INDEX g_iCountry;
	...
	pDrawPort->PutTextEx( _S(2984, "��ϵ� ģ����"), nX + Tab_MEMBER_X,
										nY_Friend, m_bFriendTab ? 0xE1B300FF :  0xA2A2A2FF);

	pDrawPort->PutTextEx( _S(2985, "���ܵ� ĳ����"), nX + Tab_BLOCK_X + nBlock_ExtraX,
...
void CUIMessenger::SetGroup(enum eGroup eState, int nSubMenu)
{
	...
	{
	case GROUP_INSERT:
		Message( MSGCMD_MESSENGER_GROUP_ADD, _S(3018, "���׷� �߰�"), _S(3019, "���ο� �׷���� �Է��Ͽ� �ֽʽÿ�." ), UMBS_OKCANCEL | UMBS_INPUTBOX );
		break;
	case GROUP_DELETE:
		{
			...
			if( m_nSelectGroup !=0 )
			{
				...
				CTString strMessage;
				strMessage.PrintF( _S(3024, "%s �׷��� �����Ͻðڽ��ϱ�?" ), m_vecGroup[nSubMenu]->m_strGroupName );
				Message( MSGCMD_MESSENGER_GROUP_DEL, _S(3023, "�׷� ����"), strMessage, UMBS_OKCANCEL );
			}
			else
				Message( MSGCMD_MESSENGER_ERROR, _S(3025, "�׷���� ����"), _S(3026, "�⺻ �׷��� ������ �� �����ϴ�."), UMBS_OK);
			break;
		}
	case GROUP_RENAME:
		m_nSelectGroup =nSubMenu;
		if( m_nSelectGroup !=0 )
		{
			...
			Message( MSGCMD_MESSENGER_GROUP_RENAME, _S(3028, "�׷�� ����"), _S(3029, "������ �׷���� �Է��Ͽ� �ֽʽÿ�." ), UMBS_OKCANCEL | UMBS_INPUTBOX );
		}
		else
		{
			Message( MSGCMD_MESSENGER_ERROR, _S(3030, "�׷�� ���� ����"), _S(3031, "�⺻ �׷��� �̸��� ������ �� �����ϴ�."), UMBS_OK);
		}
...
void CUIMessenger::AddGroup(int nGroupIndex, CTString strName)
{
	...
	{
		Message( MSGCMD_MESSENGER_ERROR, _S(3020, "�׷��߰� ����"), _S(3022, "���̻� �׷��� ������ �� �����ϴ�."), UMBS_OK);		
		return;
	}

	if( nGroupIndex ==-1)
	{
		Message( MSGCMD_MESSENGER_ERROR, _S(3020, "�׷��߰� ����"), _S(3, "�߸��� ���ڰ� ���ԵǾ��ֽ��ϴ�."), UMBS_OK);		
		return;
	}

	if( nGroupIndex ==0)
		strName =_S(2986, "��Ʈī����ģ��");
...
void CUIMessenger::RenameGroup(int nGroupIndex, CTString strNewName)
{
	if( nGroupIndex ==-1 )
	{
		Message( MSGCMD_MESSENGER_ERROR, _S(3030, "�׷�� ���� ����"), _S(3, "�߸��� ���ڰ� ���ԵǾ��ֽ��ϴ�."), UMBS_OK);
		return;
	}
...
void CUIMessenger::SetChat(int nMakeCharIndex, int nChatIndex, CTString strName)
{
	...
	if( bNewChat )		// ��â���� �����ϱ� ���� 1:1 ��ȭ���� ��밡 �ʴ�� ��밡 �ִ� ���� �˻��Ѵ�.
	{
		for( i =0; i < MAX_TALK; ++i )
		{
			// �巡�׵� â�� ���� â�� ã�ƶ�
...
void CUIMessenger::UnBlock(BYTE cError, int nCharIndex, CTString strName)
{
	...
	*****/
	...
	case MSG_EX_MESSENGER_BLOCK_NOTCHAR:
		Message( MSGCMD_MESSENGER_ERROR, _S(3016, "�������� ����"), _S(11, "�������� �ʴ� ĳ�����Դϴ�."), UMBS_OK);		
		return;
	case MSG_EX_MESSENGER_BLOCK_INVALIDNAME:
		Message( MSGCMD_MESSENGER_ERROR, _S(3016, "�������� ����"), _S(3017, "���������ϰ��� �ϴ� �̸��� �ùٸ��� �ʽ��ϴ�"), UMBS_OK);		
		return;
	}
	...
	CTString strMessage;
	strMessage.PrintF( _S( 3015, "%s���� ���������Ǿ����ϴ�." ), strName );
	Message( MSGCMD_MESSENGER_ERROR, _S(3013, "���� ����"), strMessage, UMBS_OK );
}
...
void CUIMessenger::Block(BYTE cError, int nCharIndex, CTString strName)
{
	...
	{
	case MSG_EX_MESSENGER_BLOCK_NOTCHAR:
		Message( MSGCMD_MESSENGER_ERROR, _S(3008, "���� ����"), _S(11, "�������� �ʴ� ĳ�����Դϴ�."), UMBS_OK);		
		return;
	case MSG_EX_MESSENGER_BLOCK_INVALIDNAME:
		Message( MSGCMD_MESSENGER_ERROR, _S(3008, "���� ����"), _S(3009, "�����ϰ��� �ϴ� �̸��� �ùٸ��� �ʽ��ϴ�"), UMBS_OK);		
		return;
	case MSG_EX_MESSENGER_NOMORE_BLOCK:
		Message( MSGCMD_MESSENGER_ERROR, _S(3008, "���� ����"), _S(3010, "���̻� ������ �� �����ϴ�."), UMBS_OK);		
		return;
	...
	****/
	...
	CTString strMessage;
	strMessage.PrintF(_S(3006, "%s���� ���ܵǾ����ϴ�."), strName);
	Message( MSGCMD_MESSENGER_ERROR, _S(3004, "ĳ���� ����"), strMessage, UMBS_OK );
}
