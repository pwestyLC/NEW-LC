	...
	: m_tbCharCount(nullptr)
{
	...
	setType(eUI_CONTROL_MULTI_EDIT);
}
...
void CUIMultiEditBox::Create()
{
	...
	// Sctroll Bar
	m_sbScrollBar.Create(this, GetWidth() - 6, 0, 9, GetHeight() - 3);
	...
	m_sbScrollBar.SetWheelRect(-GetWidth(), -1, GetWidth(), GetHeight());
	// UP Button
	...
	// Edit Box
	m_ebEditBox.Create(this, m_nFontWidth / 2, 0, GetWidth(), 15, m_nMaxChar);
	...
	// Blank Space
	m_nBlankSpaceTop = (m_ebEditBox.GetHeight() - _pUIFontTexMgr->GetFontHeight()) / 2;
	m_nBlankSpaceLeft = m_nFontWidth / 2;
}

void CUIMultiEditBox::InitXmlInfo(int nLineHeight)
{
	m_nLineHeight = (GetHeight() / m_nFontHeight);
}

void CUIMultiEditBox::InitFont()
{
	m_nFontHeight = _pUIFontTexMgr->GetFontHeight() + _pUIFontTexMgr->GetLineSpacing();
	m_nFontWidth = _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing();
}
...
//		if( nMaxLine >= m_nCurrentLine )
	...
	if (m_tbCharCount != NULL)
	{
		CTString strCharCount;
		strCharCount.PrintF("%d/%d", GetCharCount(), m_nMaxChar);
		m_tbCharCount->SetText(strCharCount);
	}
...
void CUIMultiEditBox::OnRender(CDrawPort* pDraw)
{
	int nX, nY;
	int nScrollBarPos = m_sbScrollBar.GetScrollPos();
	int nEndLine = m_strTexts.size();
	...
	if (nScrollBarPos <= m_nCurrentLine && nScrollBarPos + m_nLineHeight > m_nCurrentLine)
	{
		m_ebEditBox.OnRender(pDrawPort);
	}
	...
	// Render ScrollBar
	m_sbScrollBar.OnRender(pDrawPort);

	// Reading window
	if (m_ebEditBox.IsFocused())
	{
		// Reading window
		m_ebEditBox.RenderReadingWindow();
	}
...
{ 
	...
	{
		char* str = m_ebEditBox.GetString();
		int strLen = strlen(str);
		if (strLen * _pUIFontTexMgr->GetFontWidth() >= (m_nWidth - 20))
		{
			m_nCurrentLine++;
			m_strTexts.insert ( m_strTexts.begin() + m_nCurrentLine, "" );
...
WMSG_RESULT CUIMultiEditBox::OnLButtonDown(UINT16 x, UINT16 y)
{
	if (IsInside(x, y))
	{
		if (m_sbScrollBar.OnLButtonDown(x, y) != WMSG_FAIL)
		{
			m_ebEditBox.SetPos(m_ebEditBox.GetPosX(), (m_nCurrentLine - m_sbScrollBar.GetScrollPos()) * m_nFontHeight);
		}

		m_ebEditBox.OnLButtonDown(x, y);
...
WMSG_RESULT CUIMultiEditBox::OnMouseMove(UINT16 x, UINT16 y, MSG* pMsg)
{
	if (IsInside(x, y))
...
WMSG_RESULT CUIMultiEditBox::OnMouseWheel(UINT16 x, UINT16 y, int wheel)
{
	if (IsInside(x, y))
	{
		if (m_sbScrollBar.OnMouseWheel(x, y, wheel) != WMSG_FAIL)
		{
			m_ebEditBox.SetPos(m_ebEditBox.GetPosX(), (m_nCurrentLine - m_sbScrollBar.GetScrollPos()) * m_nFontHeight);
			return WMSG_SUCCESS;
		}
...
WMSG_RESULT CUIMultiEditBox::OnLButtonUp(UINT16 x, UINT16 y)
{
	if (m_sbScrollBar.OnLButtonUp(x, y) != WMSG_FAIL)
		return WMSG_SUCCESS;

	return WMSG_FAIL;
}

WMSG_RESULT CUIMultiEditBox::OnCharMessage(MSG* pMsg)
{
	if (!m_ebEditBox.IsFocused()) return WMSG_FAIL;
	return CharMessage(pMsg);
}

WMSG_RESULT CUIMultiEditBox::OnKeyMessage(MSG* pMsg)
{
	if (!m_ebEditBox.IsFocused()) return WMSG_FAIL;
	return KeyMessage(pMsg);
}

WMSG_RESULT CUIMultiEditBox::OnCloseProc()
{
	SetFocus(FALSE);
	return WMSG_FAIL;
}
...
void CUIMultiEditBox::SplitLine( int nIndex )
{
	...
	if( nIndex == -1 ) // MaxCharactor에서 자른다.
	{
		...
		if (g_iCountry == RUSSIA)
			...
			nSplitPos = UTIL_HELP()->CheckNoFixedLengthUsa(strTemp.str_String, m_nWidth - 20);
	}
	else
	{
		nSplitPos = nIndex;
...
void CUIMultiEditBox::SetMaxCharCount(int count)
{
	if (count >= 1)
	{
		m_nMaxChar = count;
		m_ebEditBox.SetMaxChar(count);
	}
}

void CUIMultiEditBox::SetMaxLineCount(int count)
{
	if (count >= 1)
		m_nMaxLine = count;
}

void CUIMultiEditBox::SetCursorMove(BOOL bMove)
{
	m_ebEditBox.SetCursorMove(bMove);
}

void CUIMultiEditBox::SetOnlyNumMode(BOOL bOnlyNum)
{
	m_ebEditBox.SetOnlyIntegerMode(bOnlyNum);
}

void CUIMultiEditBox::SetMaxCharTb(CUIText* m_tbText)
{
	m_tbCharCount = m_tbText;
}

int CUIMultiEditBox::GetCharCount()
{
	int nCharCount = 0;
	std::vector<CTString>::iterator begin = m_strTexts.begin();
	std::vector<CTString>::iterator end = m_strTexts.end();

	for (; begin != end; ++begin)
		nCharCount += (*begin).Length();
