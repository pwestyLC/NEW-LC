...
void CUIPlayerInfo::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
	...
	#ifdef PREMIUM_CHAR
	m_btnCharPremium.Create( this, CTString( "" ), 3, 39, 18, 18 );
	m_btnCharPremium.SetUV( UBS_IDLE, 733, 738, 760, 765, fTexWidth, fTexHeight );
	m_btnCharPremium.SetUV( UBS_ON, 761, 766, 788, 793, fTexWidth, fTexHeight );
	m_btnCharPremium.SetUV( UBS_CLICK, 761, 738, 788, 765, fTexWidth, fTexHeight );
	m_btnCharPremium.SetUV( UBS_DISABLE, 733, 766, 760, 793, fTexWidth, fTexHeight );
	#else
	m_btnCalendar.Create( this, CTString( "" ), 7, 27, 18, 18 );
	m_btnCalendar.SetUV( UBS_IDLE, 142, 14, 160, 32, fTexWidth, fTexHeight );
	m_btnCalendar.SetUV( UBS_CLICK, 161, 14, 179, 32, fTexWidth, fTexHeight );
	m_btnCalendar.CopyUV( UBS_IDLE, UBS_ON );
	m_btnCalendar.CopyUV( UBS_IDLE, UBS_DISABLE );
	#endif
	// Messanger button
	...
	#ifdef PREMIUM_CHAR
	m_btnCharPremium.SetEnable(FALSE);
	#else
	m_btnCalendar.SetEnable(FALSE);
	#endif
	...
	for(int i=0; i<QUICKMENU_BTN_MAX; ++i)
	{
		...
		#ifdef PREMIUM_CHAR
		if (i == QUICKMENU_BTN_PREMIUM_CHAR) {
			m_btnQuickMenu[i].SetUV( UBS_IDLE, 733, 738, 760, 765, fTexWidth, fTexHeight );
			m_btnQuickMenu[i].SetUV( UBS_ON, 761, 766, 788, 793, fTexWidth, fTexHeight );
		} else 
		#endif
...
void CUIPlayerInfo::ShowToolTip( BOOL bShow, int nToolTipID )
{
	...
	if( nOldToolTipID != nToolTipID )
	{
		...
		case 0:		// Web board
			...
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 6328, "Premium Char" ), "(Info)" );
				else
					m_strToolTip.PrintF( "%s %s", _S( 6328, "Premium Char" ), "(Info)" );

				m_btnCharPremium.GetAbsPos( nInfoX, nInfoY );
				nWidth = m_btnCharPremium.GetWidth();
			}
			#else
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 6005, "Calendar" ), "(Info)" );
				else
					m_strToolTip.PrintF( "%s %s", _S( 6005, "Calendar" ), "(Info)" );

				m_btnCalendar.GetAbsPos( nInfoX, nInfoY );
				nWidth = m_btnCalendar.GetWidth();
			}
			#endif
			break;
	...
	case WM_MOUSEMOVE:
		{
			...
			// Web board button
			#ifdef PREMIUM_CHAR
			if( m_btnCharPremium.MouseMessage( pMsg ) != WMSG_FAIL )
			{
				ShowToolTip( TRUE, 0 );
				return WMSG_SUCCESS;
			}
			#else
			if( m_btnCalendar.MouseMessage( pMsg ) != WMSG_FAIL )
			{
				ShowToolTip( TRUE, 0 );
				return WMSG_SUCCESS;
			}
...
BOOL CUIPlayerInfo::ProcessShortCut( MSG *pMsg )
{
	...
	else
	{
		...
		// Q - Open Loot Filter Menu (no Alt required)
		if( iScanCode == 16 )  // Q key
		{
			CUILootFilterMenu* pMenu = pUIManager->GetLootFilterMenu();
			if (pMenu != NULL)
			{
				pMenu->ToggleVisible();
			}
		...
		else if( _abKeysPressed[KID_RSHIFT] && !pUIManager->IsFocusAllEditBox())
		{
			if( g_iEnterChat && !pUIManager->IsFocusAllEditBox() )
			{
				switch( iScanCode )
				...
				case 13:
					{
						if( pUIManager->GetQuickSlot()->GetPage() == 3 || pUIManager->GetUI(UI_QUICKSLOT4)->IsVisible() )
						{
							pUIManager->GetQuickSlot()->UseQuickSlot( iScanCode - 2 , 3);
							break;
						}
					}
				default:
		...
		else if( _abKeysPressed[KID_LSHIFT] && !pUIManager->IsFocusAllEditBox())
		{
			...
			case 47:  // 'v'
				...
				break;       
			case 48:  // 'b'
				//pUIManager->GetSummonSecond()->UseSkill();// test
			{
				CUIManager* pUIManager = CUIManager::getSingleton();
#ifdef QUICK_PANEL
		...
		if( (_abKeysPressed[KID_LALT] || g_iEnterChat) && !pUIManager->IsFocusAllEditBox())
		{
			...
			case 44:		// 'z' - system menu
				...
				break;
#ifdef RANKING_PVP  //pwesty going to new Prestige system.
			case 48:		// 'b' - web board
...
void CUIPlayerInfo::ShowQuickMenuToolTip(BOOL bShow, int nToolTipID)
{
	...
	if( nOldToolTipID != nToolTipID )
	{
		...
		case QUICKMENU_BTN_MESSENGER:		// Messanger
			...
			break;
		case QUICKMENU_BTN_PREMIUM_CHAR:		// Web board
			{
				#ifdef PREMIUM_CHAR
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 6328, "Premium Char" ), "(Info)" );
