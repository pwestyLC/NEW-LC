...
#define PRESTIGE_BAR_PROGRESS_WIDTH 232

#define PS_PAGE_LEFT 	0
#define PS_PAGE_RIGHT 	1
#define PS_PAGE_MAX 	10

//#define SHOW_BATTLE_PASS_INFO
void CUIPrestige::SetHasNext(BOOL next) {
	m_bHasNext = next;
}

BOOL CUIPrestige::HasNext() const {
	return m_bHasNext;
}
class CmdPrestigeClose : public Command
{
public :
	CmdPrestigeClose() : m_pWnd(NULL)	{}
	void setData( CUIPrestige* pWnd )	{ m_pWnd = pWnd;	}
	void execute()
	{
		if (m_pWnd)
			m_pWnd->CloseUI();
...
class CmdPrestigeExchangeSend : public Command
{
public :
	CmdPrestigeExchangeSend() : m_pWnd(NULL)	{}
	void setData( CUIPrestige* pWnd )	{ m_pWnd = pWnd;	}
	void execute()
	{
		if (m_pWnd)
...
class CmdPrestigePageLeft : public Command
{
public:
	CmdPrestigePageLeft() : m_pWnd(NULL) {}
	void setData(CUIPrestige* pWnd) { m_pWnd = pWnd; }
	void execute()
	{
		if (m_pWnd)
			m_pWnd->SendPageChanged(PS_PAGE_LEFT);
	}
...
class CmdPrestigePageRight : public Command
{
public:
	CmdPrestigePageRight() : m_pWnd(NULL) {}
	void setData(CUIPrestige* pWnd) { m_pWnd = pWnd; }
	void execute()
	{
		if (m_pWnd)
			m_pWnd->SendPageChanged(PS_PAGE_RIGHT);
	}

private:
	CUIPrestige* m_pWnd;
};
CUIPrestige::CUIPrestige()
	: m_pBtnOk(NULL)
	, m_pDragArea(NULL)
	, m_pTextTitle(NULL)
...
void CUIPrestige::BackPage()
{

	SendPageChanged(PS_PAGE_LEFT);
}

void CUIPrestige::ForwardPage()
{

	SendPageChanged(PS_PAGE_RIGHT);
}
...
void CUIPrestige::initialize()
{
	...
	if (pClose)
	{
		pClose->SetCommandFUp(boost::bind(&CUIPrestige::CloseUI, this));
	}
	...
	if (m_pBtnOk)
	{
		m_pBtnOk->SetCommandFUp(boost::bind(&CUIPrestige::SendExchangeExp, this));
	}

	m_pBtnPageLeft = (CUIButton*)findUI("btn_page_left");
	if (m_pBtnPageLeft)
	{
		m_pBtnPageLeft->SetCommandFUp(boost::bind(&CUIPrestige::BackPage, this));
	}

	m_pBtnPageRight = (CUIButton*)findUI("btn_page_right");
	if (m_pBtnPageRight)
	{
		m_pBtnPageRight->SetCommandFUp(boost::bind(&CUIPrestige::ForwardPage, this));
	}
...
void CUIPrestige::OpenUI()
{
	...
	UIMGR()->GetInventory()->Lock(TRUE, TRUE, LOCK_PRESTIGE);
	...
	UIMGR()->RearrangeOrder(UI_PRESTIGE, TRUE);
}

void CUIPrestige::CloseUI()
{
	Hide(TRUE);
	SetVisible(FALSE);
	...
	UIMGR()->GetInventory()->Lock(FALSE, FALSE, LOCK_PRESTIGE);
	UIMGR()->RearrangeOrder(UI_PRESTIGE, FALSE);
}

void CUIPrestige::UpdateInfo(int premium, int points, int progress, int level, int season)
{
	m_nPoints = points;
	m_nPremium = premium;
	m_nLevel = level;
...
void CUIPrestige::UpdateListItem()
{
	CTString strTmp;
	int i = 0;
	...
	if (m_pTextProgress)
	{
		...
		if (m_nLevel < 10)
			...
			strTmp.PrintF("Achieved the max prestige");
	...
	{
		strTmp.PrintF("Prestige LvL.%d", m_nLevel);
	...
	{
		CTString strString;
		strString.PrintF(_S(9369, "season info."), m_nSeason);
	...
	{
		CTString strString = _S(9370, "reward info");
		pTextSeason->SetText(strString);
	}
	//if (!m_pBtnPageLeft || !m_pBtnPageRight)
		//return;
...
void CUIPrestige::SendExchangeExp()
{
	...
	if (m_nLevel == 10)//Cloud add
	{
		...
		strMessage.PrintF(_S(9371, "Prestige"));
		MsgBoxInfo.SetMsgBoxInfo(strMessage, UMBS_OK, UI_NONE, MSGCMD_NULL);
		strMessage.PrintF(_S(9372, "Sorry, you've already achieved the max prestige"));
	...
	_pNetwork->SendPrestigeReq(lNpcVIndex);

	CloseUI();
}

WMSG_RESULT CUIPrestige::OnMouseMove( UINT16 x, UINT16 y, MSG* pMsg )
{
	if (m_bHide)
		return WMSG_FAIL;
...
WMSG_RESULT CUIPrestige::OnLButtonDown( UINT16 x, UINT16 y )
{
	if (m_bHide)
		return WMSG_FAIL;
...
WMSG_RESULT CUIPrestige::OnLButtonUp( UINT16 x, UINT16 y )
{
	if (m_bHide)
		return WMSG_FAIL;
...
WMSG_RESULT CUIPrestige::OnLButtonDBLClick( UINT16 x, UINT16 y )
{
	if (m_bHide)
		return WMSG_FAIL;
...
void CUIPrestige::ResetPosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
	SetPos( ( pixMaxI + pixMinI - GetWidth() ) / 2, ( pixMaxJ + pixMinJ - GetHeight() ) / 2 );
}

void CUIPrestige::AdjustPosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
...
void CUIPrestige::ResetSavePosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
	ResetPosition( pixMinI, pixMinJ, pixMaxI, pixMaxJ );
...
void CUIPrestige::AddItemFree(PrestigeItem item)
{
	m_items_free.push_back(item);
}

void CUIPrestige::AddItemPremium(PrestigeItem item)
{
	m_items_premium.push_back(item);
}

void CUIPrestige::SendPageChanged(int page)
{
	if (page == PS_PAGE_LEFT)
	{
		if (m_nCurrentPage - 1 < 1)
			return;

		m_nCurrentPage -= 1;
		_pNetwork->SendPrestigeInfoReq(-1, m_nCurrentPage);
	}
	else if (page == PS_PAGE_RIGHT)
	{
		if (m_nCurrentPage + 1 > PS_PAGE_MAX)
			return;
