	...
	, m_nNewMailCount(0)
{
	...
	m_ptdMapTexture = NULL;	// [2012/10/11 : Sora] ����� ����
	m_ptdMapObjTexture = NULL;	// [2012/10/11 : Sora] ����� ����
	// trophy texture init. [5/31/2011 rumist]
	m_ptdTrophyTexture = NULL;
	m_bRenderTrophy = FALSE;
	m_nZone = -1;	// [2012/10/11 : Sora] ����� ����
	m_nLayer = -1;	// [2012/10/11 : Sora] ����� ����
	m_fZoomRate = 3.0f;	// [2012/10/11 : Sora] ����� ����
	m_nSelectedIcon = -1; // [2012/10/11 : Sora] ����� ����
...
CUIRadar::~CUIRadar()
{
	...
	STOCK_RELEASE( m_ptdMapTexture );		// [2012/10/11 : Sora] ����� ����
	STOCK_RELEASE( m_ptdMapObjTexture );	// [2012/10/11 : Sora] ����� ����
...
void CUIRadar::initialize()
{
	...
	//pBtn = (CUIButton*)findUI("btn_battlegift");
	//pBtn = (CUIButton*)findUI("btn_battlegift");
	
	/*if (m_pSprAniBP != NULL)
	{
		m_pSprAniBP->SetCommandFUp(boost::bind(&CUIBattlePass::CmdBattlePassExchangeSend, this));
		m_pSprAniBP->Hide(TRUE);
	}*/
	...
	m_liArenaEndTime = 0;
	m_pImgArrArenaTime = (CUIImageArray*)findUI("imgarr_arena");
 	if (m_pImgArrArenaTime != NULL)
	{
		m_pImgArrArenaTime->setUpdateTooltipF(boost::bind(&CUIRadar::UpdateArenaTime, this));
		m_pImgArrArenaTime->Hide(TRUE);
	}
	#endif
...
void CUIRadar::OnRender( CDrawPort* pDraw )
{
	if (IsRadarUse() == TRUE)
	{
		...
		RenderSignal();
		RenderMyPosition();		
		CTString expressTooltip;
		expressTooltip.PrintF(_S(8434,"Not found new letters"));
		if (m_bNotice)
		{
			...
			expressTooltip.PrintF(_S(8435,"You have %d new letters"), m_nNewMailCount);
		}
		m_pBtnExpress->clearTooltipCont();
		m_pBtnExpress->setTooltip(expressTooltip);
		if (m_bNoticeRenderFlag)
		{
			m_pBtnExpress->SetBtnState(UBS_ON);
		}
		else
		{
			m_pBtnExpress->SetBtnState(UBS_IDLE);
		}
	}
...
void CUIRadar::RenderObjectLocation()
{
	...
	for (i = 0; i < md->vecSubZone.size(); i++)
	{
		...
		{
			strTemp.PrintF( _S( 549, "%s(������)" ), CZoneInfo::getSingleton()->GetExtraName( md->World.nZoneIndex, md->vecSubZone[i].nSubZoneIndex ) );
		}
		else
		{
	...
	if( m_pMapOption->IsCheck(eNT_PARTY) == true )
	{
		...
		{
			if(pParty->GetLevel( iParty ) == 0)	// ������ 0�� ����� �������� �����̹Ƿ� �׸��� �ʴ´�.
				continue;

			if (pParty->GetMemberZone(iParty) != _pNetwork->MyCharacterInfo.zoneNo) // ��Ƽ���� �ٸ����� ���� ���
				continue;
	...
	if( m_pMapOption->IsCheck(eNT_FELLOWSHIP) == true )
	{
		...
		for(int i=0; i<EXPEDITION_GROUP_MAX; i++)
		{
			for( int j = 0; j < EXPEDITION_MEMBER_PER_GROUP; j++ )
			{
				...
				{
					if(!pParty->GetExpeditionMemberOnline(i,j))	// �¶��� ����� �ƴϸ� �׸��� �ʴ´�.
						continue;
					
					if(pParty->GetExpeditionMemberIndex(i,j) == _pNetwork->MyCharacterInfo.index)	// [090729 sora] ������ �������Ʈ�� �ڽŵ� ���ԵǾ������Ƿ� �ڽ��� ǥ������ �ʴ´�
						continue;

					if (pParty->GetExpeditionMemberZone(i, j) != _pNetwork->MyCharacterInfo.zoneNo) // ������ ����� ���� ���� �ƴҰ��
						continue;
...
#define SIGNAL_TIME				5.0f								// �ñ׳� ǥ�� �ð� 
#define SIGNAL_COUNT			5.0f								// �ñ׳� ǥ�� Ƚ�� 
#define SIGNAL_UNITTIME			( SIGNAL_TIME / SIGNAL_COUNT )		// �ѹ� ǥ�ð� �Ҷ� �ɸ��� �ð�
...
{
	static FLOAT fAngleOffSet = 270.0f; // ���� ǥ�ñⰡ ^�� ���ϱ� �ֵ��� �ϱ����ؼ� 270���� �����ش�.
	MapData* md;
	...
	// �ɸ����� X���� �߽����� �ñ׳��� ���� ���ϱ�
	FLOAT	angle = GetAngle( fX, fZ, 1.0f, 0.0f );	 // X �� �߽�
	angle *= RADIAN;
	...
	angle -= fAngleOffSet;  // ���� ǥ�ñⰡ ^�� ���ϱ� �ֵ��� �ϱ����ؼ� 90���� �����ش�.
...
bool CUIRadar::RadarToWorld( float& fX, float& fZ )
{
	...
	double	fSin = Sin( -_pNetwork->MyCharacterInfo.camera_angle ); // ���� �ɸ��� ������ �� ȸ�� ����
	double	fCos = Cos( -_pNetwork->MyCharacterInfo.camera_angle );
	
	fX -= m_nPosX + RADAR_CENTER_OFFSETX; // ���̴� �߽� ��ǥ�� ����( ������ ���� �ɸ����� ��ġ )
	fZ -= m_nPosY + RADAR_CENTER_OFFSETY;

	fX /= fRatio;	// ��ô 
	fZ /= fRatio;

	nTempX = ( fX * fCos - fZ * fSin ); // ȸ�� 
...
//------------------------------------------------------------------------------
//  CUIRadar::KeyMessage
// Explain: CtrlŰ�� ������ �� �ñ׳��� ���� �� �ֵ��� KeyMessage�� �޴´� 
// * �Ϲ����� UIWindow�� KEYUP �޼����� ���� �ʴ´�. ( ����Ű�� ����ϴ� ���� ����)
// *  UIMgr���� MAP�� Rar�� ���� ó���ؼ� KEYUP�޼����� �޵��� �Ͽ���
	...
	case MSGLCMD_ROYALRUMBLE_MANAGER:
		{
			...
			else if( ROYALRUMBLE_REJECT == nResult )
			{
				...
				MsgBoxInfo.SetMsgBoxInfo( _S(5405, "�ξⷳ�� ������û ���"), UMBS_YESNO, UI_RADAR, MSGCMD_ROYALRUMBLE_REJECT_REQ);
											
				MsgBoxInfo.AddString( _S(5406, "�ξⷳ�� ������û�� ����Ͻø� ������û ����� �������� �� �����ϴ�. �ξⷳ�� ������û�� ����Ͻðڽ��ϱ�?") );
...
void CUIRadar::RoyalRumbleJoinReqMsgBox()
{
	...
	strFee1 = pUIMgr->IntegerToCommaString(DEF_ROYALRUMBLE_JOIN_FEE_BEGINNER);
	strFee2 = pUIMgr->IntegerToCommaString(DEF_ROYALRUMBLE_JOIN_FEE_SPECIALTY);
	strFee3 = pUIMgr->IntegerToCommaString(DEF_ROYALRUMBLE_JOIN_FEE_ADVANCED);

	MsgBoxInfo.SetMsgBoxInfo( _S(5407, "�ξⷳ�� ������û"), UMBS_YESNO, UI_RADAR, MSGCMD_ROYALRUMBLE_JOIN_REQ);
	
	strTemp.PrintF(_S(5408, "�ȳ��Ͻʴϱ�? �ξⷳ�� ����ȸ�Դϴ�. ���� ������ %s, ���� ������ %s, ���� ������ %s"), strFee1, strFee2, strFee3);
...
void CUIRadar::OpenRoyalrumbleMgrMenu( const INDEX iMobIndex )
{
	...
	pUIManager->CreateMessageBoxL( _S( 5404, "�ξⷳ��" ), UI_RADAR, MSGLCMD_ROYALRUMBLE_MANAGER );
...
void CUIRadar::_showWinnerMsgBox()
{
	...
	if( pUIManager->DoesMessageBoxExist( MSGCMD_ROYALRUMBLE_WINNER_OUT ) )
		return;
	CUIMsgBox_Info	MsgBoxInfo;
	MsgBoxInfo.SetMsgBoxInfo( _S(5424, "���� �̵�"), UMBS_OK|UMBS_USE_TIMER, UI_RADAR, MSGCMD_ROYALRUMBLE_WINNER_OUT);
									
	MsgBoxInfo.AddString( _S(5426,	"����� ���ϵ帳�ϴ�."
									"��������� �̵��մϴ�.") );
...
void CUIRadar::OnExpressNotice(int newMailsCount)
{
	m_bNotice = true;
	m_nNewMailCount = newMailsCount;
...
void CUIRadar::updateServerTime()
{
	...
	time_t tmt;
	#ifdef RADAR_SERVER_TIME
	tmt = (ULONG)time(NULL) - _pNetwork->slServerTimeGap;
	#else
	tmt =  _pNetwork->slServerTime;
	#endif
...
void CUIRadar::setBtnPremiumTooltip()
{
	...
	if (pChar != NULL && pChar->getType() != PREMIUM_CHAR_TYPE_NONE)
	{
		...
		{
			str = _S(6328, "�����̾� ĳ���� ���� ���� ��" );
			str_time.PrintF( _S( 6070, "���� : %d��%d��%d��%d��%d��"), pTimeEnd->tm_year + 1900, pTimeEnd->tm_mon + 1, pTimeEnd->tm_mday, pTimeEnd->tm_hour, pTimeEnd->tm_min );
			color = 0x00FA00FF;
		}
	}
	else
...
#ifdef ARENA_TIME_SYSTEM
void CUIRadar::UpdateArenaTime()
{
	if (m_pImgArrArenaTime == NULL)
		return;

	if (m_liArenaEndTime > 0)
	{
		CTString strTitle = _S(8527, "Arena Time Left" );
		CTString strTime;
		
		__int64 curTime = _pTimer->GetHighPrecisionTimer().GetMilliseconds();
		int LeftTime = (m_liArenaEndTime - curTime) / 1000;
		if (LeftTime > 0)
		{
			strTime.PrintF(_s("%d : %02d"), LeftTime / 60, LeftTime % 60);
			m_pImgArrArenaTime->setTooltip(strTitle, 0xFF8000FF);
			m_pImgArrArenaTime->addTooltip(strTime, 0xFF8000FF, eALIGN_H_CENTER);
		}
		else
		{
			ResetArenaTime();
			_pNetwork->SendPvPBuff();
		}
	}
}

void CUIRadar::SetArenaTime( const INDEX remainTime /*= 0 */ )
{
	m_liArenaEndTime = _pTimer->GetHighPrecisionTimer().GetMilliseconds() + remainTime * 1000;

	if (m_pImgArrArenaTime == NULL)
		return;
	
	m_pImgArrArenaTime->Hide(FALSE);
	m_pImgArrArenaTime->SetRenderIdx(1);
}

void CUIRadar::ResetArenaTime()
{
	m_liArenaEndTime = 0;
