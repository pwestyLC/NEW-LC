...
// ----------------------------------------------------------------------------
// Name : CUIRankingSummon()
// Desc : 
// ----------------------------------------------------------------------------
CUIRankingSummon::CUIRankingSummon()
{
}
...
// Desc :
// ----------------------------------------------------------------------------
void CUIRankingSummon::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
...
// Desc :
// ----------------------------------------------------------------------------
void CUIRankingSummon::ResetPosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
...
// Desc :
// ----------------------------------------------------------------------------
void CUIRankingSummon::AdjustPosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
...
//------------------------------------------------------------------------------
// CUIRankingSummon::SetFocus
// Explain:  
// Date : 2005-03-10(���� 1:02:39) Lee Ki-hwan
//------------------------------------------------------------------------------
void CUIRankingSummon::SetFocus( BOOL bVisible )
{
...
// Desc : 
// ----------------------------------------------------------------------------
void CUIRankingSummon::OpenRankingList( )
{	
	if( IsVisible() )
	{		
		return;
	...
	CUIManager::getSingleton()->RearrangeOrder( UI_RANKING_SUMMON, TRUE );
}
...
// Desc : 
// ----------------------------------------------------------------------------
void CUIRankingSummon::ResetRanking()
{		
	m_lbRankingList.ResetAllStrings();
	m_sTotal.Clear();
	...
	CUIManager::getSingleton()->RearrangeOrder( UI_RANKING_SUMMON, FALSE );
}
...
// Desc : ��� ����� Ŭ�����մϴ�.
// ----------------------------------------------------------------------------
void CUIRankingSummon::ClearRankingList()
{
	if(!m_vectorRankingList.empty())
	{
		m_vectorRankingList.clear();
...
// Desc : Ȯ�� ��ư�� ������ ���� ó��.
// ----------------------------------------------------------------------------
void CUIRankingSummon::PressOKBtn()
{	
}
...
// Desc : ��� ��Ͽ� �߰�
// ----------------------------------------------------------------------------
void CUIRankingSummon::AddToRankingList( const CTString& strName, INDEX iNumOfDeadMonster, INDEX iJob )
{	
...
// Desc : ����� �����մϴ�.
// ----------------------------------------------------------------------------
void CUIRankingSummon::RefreshRankingList( BOOL bInit )
{
	if( !bInit && !IsVisible() )
		return;
...
// Desc :
// ----------------------------------------------------------------------------
void CUIRankingSummon::Render(void)
{
	CDrawPort* pDrawPort = CUIManager::getSingleton()->GetDrawPort();
	...
	m_sTotal.PrintF(_S( 8375, "Total Mobs" ));
	pDrawPort->PutTextEx( m_sTotal, m_nPosX + 230,
		m_nPosY + 29, 0xFFFFFFFF );	

	// Text in guild
	pDrawPort->PutTextEx( _S( 8371, "Ranking Summon" ), m_nPosX + RANKING_TITLE_TEXT_OFFSETX,
...
// Desc :
// ----------------------------------------------------------------------------
WMSG_RESULT	CUIRankingSummon::MouseMessage( MSG *pMsg )
{
	if (!IsVisible())
	{
		return WMSG_FAIL;
