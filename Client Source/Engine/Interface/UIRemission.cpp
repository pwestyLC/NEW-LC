...
void CUIRemission::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
	...
	// Buy button
	m_btnOK.Create( this, _S( 600, "����" ), 70, 372, 63, 21 );		
...
BOOL CUIRemission::InitRemission(  )
{
	...
	if(_pNetwork->MyCharacterInfo.hpcount > 0)		// HP �г�Ƽ ī��Ʈ�� �ִٸ�...
	{
	...
	if(_pNetwork->MyCharacterInfo.mpcount > 0)		// MP �г�Ƽ ī��Ʈ�� �ִٸ�...
	{
		TempUIButtonEx.Create( this, 0, 0, BTN_SIZE, BTN_SIZE, UI_REMISSION, UBET_REMISSION ); 
	...
	for( i = 0; i < ITEM_COUNT_IN_INVENTORY_NORMAL; i++ )
	{
		...
		if (pItemData != NULL &&
			((pItemData->GetType() == CItemData::ITEM_WEAPON) ||		//���� ���� �ƴ� �����۵� ���� ������ �� �ְ�
...
void CUIRemission::OpenRemission( int iMobIndex, BOOL bHasQuest, FLOAT fX, FLOAT fZ )
{
	...
	if(MD->IsRemission())		// FIXME : ���˺� NPC ���� Ȯ��.
	{
		// Create remission message box
		pUIManager->CreateMessageBoxL( _S( 601, "���˺�" ), UI_REMISSION, MSGLCMD_REMISSION_REQ );		
		...
		pUIManager->AddMessageBoxLString( MSGLCMD_REMISSION_REQ, TRUE, _S( 602, "�״��� ����� ������ �ִ� ���� ���ſ� ���̴±���." ), -1, 0xA3A1A3FF );		
		pUIManager->AddMessageBoxLString( MSGLCMD_REMISSION_REQ, TRUE, _S( 603, "�״밡 �ƹ��� ū �˸� �����ٰ� �ϴ���, �Ƶ��� ����ϴ� ���� ������ �׺��� �� ���� �����ôٿ�! �����̶� �Ų� �뼭�� ���Ͻÿ�." ), -1, 0xA3A1A3FF );		
		pUIManager->AddMessageBoxLString( MSGLCMD_REMISSION_REQ, TRUE, _S( 604, "�� ����� �״븦 ���Ͽ�, �� �����̳��� ������ ��� �帮�ڼ�." ), -1, 0xA3A1A3FF );		
		
		pUIManager->AddMessageBoxLString( MSGLCMD_REMISSION_REQ, FALSE, _S( 605, "�����Ѵ�." ), REMISSION_OK  );		
		pUIManager->AddMessageBoxLString( MSGLCMD_REMISSION_REQ, FALSE, _S( 606, "���ư���." ) );		
		...
		if(MD->IsEvent())
		{
			pUIManager->AddMessageBoxLString( MSGLCMD_REMISSION_REQ, FALSE, _S( 100, "�̺�Ʈ" ), REMISSION_EVENT  );		
		}
	}
...
void CUIRemission::Render()
{
	...
	// Text in remission
	pDrawPort->PutTextEx( _S( 601, "���˺�" ), m_nPosX + REMISSION_TITLE_TEXT_OFFSETX,	
		m_nPosY + REMISSION_TITLE_TEXT_OFFSETY, 0xFFFFFFFF );
	
	pDrawPort->PutTextExCX( _S( 601, "���˺�" ), m_nPosX + REMISSION_TAB_CX, m_nPosY + REMISSION_TAB_SY,		
		0x6B6B6BFF );
	
	pDrawPort->PutTextEx( _S( 609, "���� ����" ), m_nPosX + REMISSION_CURSP_SX,	
...
void CUIRemission::GetRemissionDesc( int iRemissionType, int nIndex, SWORD nIdx )
{
	...
	case REMISSION_HP:
		{
			strTemp.PrintF( _S( 610, "��ü�� ���˺�\n\n" ) );	
			AddRemissionDescString( strTemp, 0xFFC672FF );
			strTemp.PrintF( _S( 611, "HP�г�Ƽ ���� �� : %dȸ\n\n" ), _pNetwork->MyCharacterInfo.hpcount);	
			AddRemissionDescString( strTemp, 0xFFC672FF );
		}
		break;
	case REMISSION_MP:
		{
			strTemp.PrintF( _S( 612, "��ȥ�� ���˺�\n\n" ) );	
			AddRemissionDescString( strTemp, 0xFFC672FF );
			strTemp.PrintF( _S( 613, "MP�г�Ƽ ���� �� : %dȸ\n\n" ), _pNetwork->MyCharacterInfo.mpcount);	
			AddRemissionDescString( strTemp, 0xFFC672FF );
		}
		break;
	case REMISSION_ITEM:
		{		
			strTemp.PrintF( _S( 614, "���ε� ����\n\n" ) );							
			AddRemissionDescString( strTemp, 0xFFC672FF );
	...
	strTemp.PrintF( _S( 615, "���˺� ���� : %I64d ����" ), CalculatePrice(iRemissionType, nIdx) );	
	AddRemissionDescString( strTemp, 0xFFC672FF );
}
...
void CUIRemission::RenderRemissionBtns()
{
	...
	for( iRow = iRowS; iRow < iRowE; iRow++, nY += REMISSION_SLOT_OFFSETY )
	{
		...
		{
		case REMISSION_HP:
			m_strShortDesc.PrintF( _S( 616, "��ü�� ���˺�" ) );		
			pDrawPort->PutTextExCX( m_strShortDesc, m_nPosX + REMISSION_NAME_CX, m_nPosY + nY,	0xFFC672FF);
			break;
		case REMISSION_MP:
			m_strShortDesc.PrintF( _S( 617, "��ȥ�� ���˺�" ) );		
			pDrawPort->PutTextExCX( m_strShortDesc, m_nPosX + REMISSION_NAME_CX, m_nPosY + nY,	0xFFC672FF);
			break;
		case REMISSION_ITEM:
			{				
				m_strShortDesc.PrintF( _S( 618, "����� ���˺�" ) );		
				pDrawPort->PutTextExCX( m_strShortDesc, m_nPosX + REMISSION_NAME_CX, m_nPosY + nY,	0xFFC672FF);
	...
	case MSGLCMD_REMISSION_REQ:
		if( nResult == REMISSION_OK )
		{
			...
			else
			{
				...
				MsgBoxInfo.SetMsgBoxInfo( _S( 601, "���˺�" ), UMBS_OK,			
					UI_REMISSION, MSGCMD_REMISSION_NOTIFY );
				MsgBoxInfo.AddString( _S( 619, "���˹��� ���� ������ �����ϴ�." ) );	
...
void CUIRemission::PressOK()
{
	...
	CTString	strMessage;
	CUIMsgBox_Info	MsgBoxInfo;	
	MsgBoxInfo.SetMsgBoxInfo( _S( 601, "���˺�" ), UMBS_OKCANCEL, UI_REMISSION, MSGCMD_REMISSION_BUY);		
	strMessage.PrintF( _S( 620, "���� �� ���˺θ� �����Ͻðڽ��ϱ�?" ) );	
