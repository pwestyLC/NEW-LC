...
void CUISiegeWarfare::OpenSiegeWarfare()
{
	...
	CTString strMessage;        // Main message
	CTString strNecessity[4];   // Necessity strings
	CTString strSelection[5];   // Selection strings 
	CTString strName;
	int i;
	...
	// [2010/11/01 : Sora] Initialize name string
	strName = _S(1982, "Unknown NPC");
	if (npcIndex == 1155)
	{
		strName = CMobData::getData(npcIndex)->GetName();

		strMessage = _S(5163, "Welcome to the siege warfare!!\nYou are about to enter a time-limited battle. Please prepare accordingly.\nProceed when ready.");
	}
	else
	{
		strMessage = _S(1983, "Are you sure you want to start the siege warfare?\nOnly those who are fully prepared should apply.\nMake sure your guild is ready for this challenge.");

		strNecessity[0] = _S(1984, "Required Level: 5");
		strNecessity[1] = _S(1985, "Required Item: Siege Token");
		strNecessity[2] = _S(1986, "Required Gold: 20,000");
		strNecessity[3] = _S(1987, "Required Time: 10 minutes preparation");
	}
	...
	// Display necessity strings if necessary
	if (npcIndex == 219) // Siege Warfare NPC
	{
		for (i = 0; i < 4; i++)
		{
	...
	strSelection[0] = _S(1988, "Apply for Siege.");


	
		// Default selections for other NPCs
		if (npcIndex == 387)
		{
			strSelection[1] = _S(1998, "Modify Siege Application.");
		}
		else
		{
			strSelection[1] = _S(1989, "Apply for Battle.");
		}

		strSelection[2] = _S(1990, "View Siege Schedule.");
		strSelection[3] = _S(1991, "Check Siege Status.");
		strSelection[4] = _S(880, "Cancel.");
	

	CTString strLordCostume = _S(5115, "Lord Costume");
	...
	if (!pUIManager->GetSiegeWarfareNew()->GetWarState() && !_pUISWDoc->IsWar())
	{
		...
		// Check for guild conditions [1/21/2011 ldy1978220]
		if (npcIndex == 1155 && _pNetwork->MyCharacterInfo.lGuildPosition != GUILD_MEMBER_BOSS && _pNetwork->MyCharacterInfo.ubGuildNameColor != 0)
		{
			pUIManager->AddMessageBoxLString(MSGLCMD_SIEGE_WARFARE, FALSE, _S(2169, "Request Guild Costume"), SEL_GUILD_COSTUME_REQ);
			pUIManager->AddMessageBoxLString(MSGLCMD_SIEGE_WARFARE, FALSE, _S(2175, "Request Guild Title"), SEL_GUILD_TITLE_REQ);
		}
	}

	for (i = 0; i < 5; i++)
	{
		if (npcIndex == 1155 && i < 2)
			continue;
