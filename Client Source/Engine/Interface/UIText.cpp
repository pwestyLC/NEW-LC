...
CUIText::CUIText()
	...
	, m_color_move(DEF_UI_FONT_COLOR)
{
	...
	if (m_vecData.size() <= 0)
	{
		if (m_bVerticality == TRUE)
		{
			...
			for(int con = 0; con < nLength; con++)
			{
				...
				pDraw->PutTextEx(temStr, outX, nY + (nFontGapCnt * countryFontGapOffset), !m_isShowTooltip ? m_color[m_eChildItemState] : m_color_move, m_fZ, m_bShadow, m_colShadow);
				nFontGapCnt++;
			}
		}
		else
		{
			...
			pDraw->PutTextEx(m_strView, outX, nY, !m_isShowTooltip ? m_color[m_eChildItemState] : m_color_move, m_fZ, m_bShadow, m_colShadow);
		}		
	}
	else
	{
...
WMSG_RESULT	CUIText::MouseMove(UINT16 x, UINT16 y, MSG* pMsg)
{
	if (!this->IsInside(x, y))
	{
		if (m_isShowTooltip == TRUE)
		{
	...
	if (m_bItemToolTip != NULL && !m_isShowTooltip)
	{
		m_isShowTooltip = TRUE;
		CUITooltipMgr::getSingleton()->setData(this, m_bItemToolTip);
		return WMSG_SUCCESS;
	}
	return WMSG_FAIL;
}
