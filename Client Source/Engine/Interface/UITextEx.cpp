...
void CUITextEx::AddText(const char* str, DWORD dwCol /*=DEF_UI_COLOR_WHITE*/, int pos_x /*=-1*/, CItems* item, DWORD dwColMove)
{
 	CUIText* pText = new CUIText;
	CUIIcon* pIcon = new CUIIcon;
 	
 	pText->setFontColor(dwCol);
	pText->setFonrColorMove(dwColMove);
 	pText->SetText((CTString)str);
	pText->setItemToolTip(item);
	...
	{
		pIcon->SetPosX(pos_x);
		pIcon->setData(item, true, true);
		pIcon->SetWidth(13);
		pIcon->SetHeight(13);
		pIcon->SetCustomTooltip(TRUE);
		pIcon->setCount(0);
		addChild((CUIBase*)pIcon);
		m_vecUICont.push_back((CUIBase*)pIcon);
		pos_x += 15;
	}
