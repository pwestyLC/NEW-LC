...
void CUIFontTextureManager::Create()
{
	...
	strDirectory += DefHelp::getNationPostfix(DefHelp::getNationByLang(g_language), true);
	strDirectory += "\\";
...
void CUIButtonTextureManager::Create()
{
	...
	{
		vecTexData& vec_ = m_aptdButton[UBET_PASSIVE];

		for (iTex = 0; iTex < TEXCT_PASSIVE; iTex++)
		{
			strFileName.PrintF("PassiveBtn%d.tex", iTex);
			strFullPath = strDirectory + strFileName;
			tmp = _pTextureStock->Obtain_t(strFullPath);
			vec_.push_back(tmp);
		}
