...
void CUITooltipMgr::setData(CUIBase* pUI, CItems* items)
{
	clearTooltip();
	...
	else if (pUI->getType() == eUI_CONTROL_TEXT)
	{
		bool bRet = false;
		CUIText* text = static_cast<CUIText*>(pUI);
		if (items == NULL)
			return;
		m_nWearPos = GetWearPos(items);
		if (m_nWearPos >= 0)
			bRet = m_pTooltip[eTOOLTIP_FIRST]->SetItemData(&_pNetwork->MyWearItem[m_nWearPos], eTOOLTIP_FIRST, m_nWearPos);
		else
			bRet = m_pTooltip[eTOOLTIP_FIRST]->SetItemData(items, eTOOLTIP_FIRST, m_nWearPos);
		m_bSecond = m_pTooltip[eTOOLTIP_SECOND]->SetItemData(items, eTOOLTIP_SECOND, m_nWearPos);
		if (bRet == false)
			return;
		UpdateTooltipPos(UIRect(text->GetAbsPosX(), text->GetAbsPosY(), text->GetWidth(), text->GetHeight()), false);
	}
	else
	{
