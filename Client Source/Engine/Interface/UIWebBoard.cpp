...
BOOL CCommandOpen::ExecuteDelay(CUIWebBoard &rUIWebBoard, BOOL &bRender)
{
	...
	if(rUIWebBoard.m_web.Read(strContent, strError))
	{
		...
		MsgBoxInfo.SetMsgBoxInfo( _S( 385, "�Խ���" ), UMBS_OK, UI_WEBBOARD, MSGCMD_WEBBOARD_ERROR );
	...
	if(rUIWebBoard.m_nCurSection == UWS_FREEBOARD)
	{
		...
		if( rUIWebBoard.m_nWantModifyMode == UWM_DELETE )
		{
			sprintf(szURL, szTarget ,pWebAddress->get("freebbs_modify")
				, _pNetwork->m_iServerGroup						//server�� index
				, rUIWebBoard.m_nCurrentViewWriteIndex			//������ �ε���
				, _pNetwork->MyCharacterInfo.index				//ĳ���� �ε���
				, _pNetwork->MyCharacterInfo.userIndex			//���� �ε���
				, strUserID.c_str()								//���� ���̵�
			);	//HardCoding
		}
		else
...
void CUIWebBoard::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
	...
	// Search button
	m_btnSearch.Create( this, _S( 386, "�˻�" ), 379, 407, 63, 21 );
	...
	// List button
	m_btnList.Create( this, _S( 313, "���" ), 23, 407, 63, 21 );
	...
	// Write button
	m_btnWrite.Create( this, _S( 314, "����" ), 515, 407, 63, 21 );
	...
	// Reply button
	m_btnReply.Create( this, _S( 321, "���" ), 377, 407, 63, 21 );
	...
	// Delete button
	m_btnDelete.Create( this, _S( 338, "����" ), 446, 407, 63, 21 );
	...
	// Modify button
	m_btnModify.Create( this, _S( 339, "����" ), 515, 407, 63, 21 );
...
void CUIWebBoard::ToggleVisible()
{
	if (IsGamigo(g_iCountry) == TRUE)
	{
		...
		case SPAIN:
		#ifdef MALL_CLASSIC
			ShellExecute(NULL, "open", "https://www.lcnorth.com/", NULL, NULL, SW_SHOWNORMAL);
		#endif
			ShellExecute(NULL, "open", "https://www.darkagelc.com/", NULL, NULL, SW_SHOWNORMAL);
			break;
		case FRANCE:
...
void CUIWebBoard::RenderListCommon()
{
	...
	pDrawPort->PutTextEx( _S( 341, "����" ), m_nPosX + WBOARD_LIST_TITLE_SUBJECT_SX,
										m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
	if( m_nCurSection == UWS_HELP || m_nCurSection == UWS_NOTICE )
	{
		pDrawPort->PutTextEx( _S( 244, "�ۼ���" ), m_nPosX + WBOARD_LIST_TITLE_NOTICE_NAME_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
		pDrawPort->PutTextEx( _S( 398, "�ۼ���" ), m_nPosX + WBOARD_LIST_TITLE_NOTICE_DATE_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
	}
	else if( m_nCurSection == UWS_FREEBOARD || m_nCurSection == UWS_GUILD )
	{
		pDrawPort->PutTextEx( _S( 244, "�ۼ���" ), m_nPosX + WBOARD_LIST_TITLE_NAME_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
		pDrawPort->PutTextEx( _S( 398, "�ۼ���" ), m_nPosX + WBOARD_LIST_TITLE_DATE_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
		pDrawPort->PutTextEx( _S( 399, "��ȸ" ), m_nPosX + WBOARD_LIST_TITLE_HIT_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
	}
}
...
void CUIWebBoard::RenderWriteCommon()
{
	...
	pDrawPort->PutTextExCX( _S( 244, "�ۼ���" ), nX,
										m_nPosY + WBOARD_WRITE_TITLE_NAME_SY, 0xFFFFCAFF );
	pDrawPort->PutTextExCX( _S( 195, "��  ��" ), nX,
										m_nPosY + WBOARD_WRITE_TITLE_SUBJECT_SY, 0xFFFFCAFF );
	pDrawPort->PutTextExCX( _S( 418, "��  ��" ), nX,
...
void CUIWebBoard::SetListContent(std::string &strContent)
{
	...
	if( m_nCurSection == UWS_HELP || m_nCurSection == UWS_NOTICE)
	{
		...
		for( int i = 0; i < nCurrentListCount; i++ )
		{
			...
			//hit count
			temp = strContent.find(" ", tempOld);
			//m_lbListContent.AddString( 4, CTString( strContent.substr(tempOld, temp - tempOld).c_str() ), 0xE6E6E6FF );	// �������� ��ȸ���� �Ⱥ��� �ص���..
			tempOld = temp + 1;
			...
			//author
			m_lbListContent.AddString( 2, _S( 401, "���" ), 0xE6E6E6FF );	//HardCoding
		}
...
void CUIWebBoard::SetViewContent(std::string &strContent)
{
	...
	if(m_nCurSection == UWS_HELP || m_nCurSection == UWS_NOTICE)
	{
		...
		//author
		m_strReadName = _S( 401, "���" );	//HardCoding
	}
	else if(m_nCurSection == UWS_FREEBOARD || m_nCurSection == UWS_GUILD)
	{
		//bbs no
			...
			if( IsInside( nX, nY ) )
			{
				...
				else if( IsInsideRect( nX, nY, m_rcTab ) )
				{
					...
					{					
						...
						{
							pUIManager->GetChattingUI()->AddSysMessage( _S(3883, "��� ������ ��� �Խ����� �̿��� �ֽʽÿ�." ) );
						}
						else 
