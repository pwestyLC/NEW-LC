...
#include <Engine/Interface/UILootFilterMenu.h>
...
#include <Engine/Contents/function/UICraft.h>		// add 13.12.2016 //Craft system
...
#include "Contents/function/UIPromoKeyInputRST.h"
#include <Engine/Interface/UILootFilterMenu.h>
...
// Craft  //Craft system
	m_apUIs[UI_CRAFT] = new CUICraft;
	LoadXML("Craft_RST.xml", m_apUIs[UI_CRAFT]);

#ifdef PASSIVE_SYSTEM
	m_apUIs[UI_PASSIVE_SYSTEM_RST] = new CUIPassiveSystemRST;
	LoadXML("PassiveSystem_RST.xml", m_apUIs[UI_PASSIVE_SYSTEM_RST]);
#endif

	// Target information
	m_apUIs[UI_TARGETINFO] = new CTargetInfoUI;
	LoadXML( "TargetInfo.xml", m_apUIs[UI_TARGETINFO]);
	m_apUIs[UI_DONATION_TIER_MINI] = new CUIDonationTierMini;
	LoadXML("cashshop_loyalty_progress.xml", m_apUIs[UI_DONATION_TIER_MINI]);
	m_apUIs[UI_DONATION_TIER] = new CUIDonationTier;
	LoadXML("DonationTier.xml", m_apUIs[UI_DONATION_TIER]);
	...
	m_apUIs[UI_RANKING_SUMMON] = new CUIRankingSummon;
	m_apUIs[UI_RANKING_SUMMON]->Create( NULL, 0, 0, RANKING_WIDTH, RANKING_HEIGHT );

	m_apUIs[UI_RANKING_REBORN] = new CUIRankingReborn;
	m_apUIs[UI_RANKING_REBORN]->Create( NULL, 0, 0, RANKING_WIDTH, RANKING_HEIGHT );
...
#ifdef PRESTIGE_SYSTEM
	m_apUIs[UI_PRESTIGE] = new CUIPrestige;
	LoadXML("Prestige.xml", m_apUIs[UI_PRESTIGE]);
#endif
	// WareHouse
	...
	m_apUIs[UI_EXPEDITION_PARTY_DUNGEON] = new CUIExpeditionPartyDungeon;//Cloud Expedition_Party_Dungeon
	LoadXML("ExpeditionPartyDungeon.xml", m_apUIs[UI_EXPEDITION_PARTY_DUNGEON]);
#endif
	// party auto matching
...
//	m_apUIs[UI_NEWGUILDSTASH]->Create( NULL, 0, 0, 0, 0 );
...
#ifdef DAILY_LOGIN
	m_apUIs[UI_DAILY_LOGIN] = new DailyLogin;
	LoadXML("ReDLogin.xml", m_apUIs[UI_DAILY_LOGIN]);
#endif

	m_apUIs[UI_RLGL] = new RLGL;
	LoadXML("rlgl.xml", m_apUIs[UI_RLGL]);

#ifdef REZABOT
	m_apUIs[UI_REZABOT]= new ReZaBotUI;
	LoadXML("ReBot.xml",m_apUIs[UI_REZABOT]);
	...
	m_apUIs[UI_TRANSMOGRIFY] = new CTransmogrifyUI;
	LoadXML("Transmogrify.xml", m_apUIs[UI_TRANSMOGRIFY]);

	m_apUIs[UI_BOX_OPEN] = new CBoxOpenUI;
	LoadXML("Box_open.xml", m_apUIs[UI_BOX_OPEN]);

	m_apUIs[UI_SEAL_COMPOSE] = new CSealComposeUI;
	LoadXML("Seal_compose.xml", m_apUIs[UI_SEAL_COMPOSE]);

	m_apUIs[UI_PRS_REMOVE] = new CPRSRemoveUI;
	LoadXML("PRS_remove.xml", m_apUIs[UI_PRS_REMOVE]);

	m_apUIs[UI_EXTEND_MENU] = new CExtendMenuUI;
	LoadXML("Extend_menu.xml", m_apUIs[UI_EXTEND_MENU]);
	
	m_apUIs[UI_MAIL_BOX] = new CUIMailBox;
	LoadXML("MailBox.xml", m_apUIs[UI_MAIL_BOX]);
	m_apUIs[UI_LETTER] = new CUILetter;
	LoadXML("SendMail.xml", m_apUIs[UI_LETTER]);
	m_apUIs[UI_INVENTORY_OPTIONS] = new CUIInventoryOptions;
	LoadXML("InventoryOptions.xml", m_apUIs[UI_INVENTORY_OPTIONS]);
	#ifdef BATTLE_PASS_SYSTEM
	m_apUIs[UI_BATTLE_PASS] = new CUIBattlePass;
	LoadXML("BattlePass.xml", m_apUIs[UI_BATTLE_PASS]);
	#endif
//Val add auto igni/pickup to chat
	m_apUIs[UI_AUTOFUNC] = new CAutoFuncUI;
	LoadXML("autoselection.xml", m_apUIs[UI_AUTOFUNC]);

	m_apUIs[UI_PROMO_KEY_INPUT_RST] = new CUIPromoKeyInputRST;
	LoadXML("PromoKeyInputRST.xml", m_apUIs[UI_PROMO_KEY_INPUT_RST]);

	m_apUIs[UI_LOOT_FILTER_MENU] = new CUILootFilterMenu;
	LoadXML("UILootFilterMenu.xml", m_apUIs[UI_LOOT_FILTER_MENU]);
}
