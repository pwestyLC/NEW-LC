...
extern void CacheShadows(void)
{
	// mute all soundsf
	_pSound->Mute();
	//CWorld *pwo = (CWorld*)_pShell->GetINDEX("pwoCurrentWorld");  //why commented??
...
#ifdef NDEBUG
	, m_ubGMLevel(0) //GM level pwesty
#else
	, m_ubGMLevel(1)
#endif
	, ga_tvDemoTimerLastTime((__int64)0)
	, m_nSkillSendTime(0)  //pwesty
{
	// EDIT : BS : ��Ŷ ��ȣȭ
#ifdef CRYPT_NET_MSG
	...
	MyCharacterInfo.StatPointAddition = 0;
 ...
 */
#ifdef ANTICHEAT
typedef void(__thiscall* AntiCheatNHz)(); //a3 = always 0
AntiCheatNHz CheckThreadNHz = (AntiCheatNHz)GetProcAddress(LoadLibrary(XorStr<0xF4, 13, 0x4AE12F32>("\xBA\xBD\x8C\xB0\x8D\x98\x88\x9F\xD2\x99\x92\x93" + 0x4AE12F32).s), XorStr<0x99, 23, 0x4BD26CE0>("\xC6\xDB\xF5\xE8\xF4\xDC\xE6\xD0\xC0\xD1\xD0\xF0\xCD\xD4\xC2\xC9\xCD\xE4\xE3\xD6\xED\x9E" + 0x4BD26CE0).s);
#endif

void CheckTimer() // fix tickquantum skill hack getafix
{
	static const TIME expectedValue = TIME(1 / 20.0);
	static DWORD lastCheckTime = 0;

	DWORD currentTime = GetTickCount();

	if (currentTime - lastCheckTime >= 4000)
	{
		lastCheckTime = currentTime;

		if (CTimer::TickQuantum != expectedValue)
		{
...
void CNetworkLibrary::MainLoop(void)
{
	...
	CheckTimer(); //fix tickquantum skill hack getafix
...
int CNetworkLibrary::ReturnCCC(int iSCC)
{
	...
	case 16:
		iCCC = POLAND;
		break;
	case 17://FRANCE_SPAIN_CLOSEBETA_NA_20081124
		iCCC = SPAIN;
		break;
	case 19:
		iCCC = ITALY;
...
void CNetworkLibrary::RemovePlatinumStone(SWORD nTab, SWORD inven_idx)
{
	CNetworkMessage nmMessage;
	RequestClient::doItemRefin* packet = reinterpret_cast<RequestClient::doItemRefin*>(nmMessage.nm_pubMessage);
	packet->type = MSG_ITEM;
	packet->subType = MSG_ITEM_REMOVE_PRS_REQ;
	packet->tab = nTab;
...
// Desc : 
// ----------------------------------------------------------------------------
void CNetworkLibrary::SealComposeReq(SWORD nTab, SWORD inven_idx, ULONG count)
{
	CNetworkMessage nmMessage;
	RequestClient::doItemSealCompose* packet = reinterpret_cast<RequestClient::doItemSealCompose*>(nmMessage.nm_pubMessage);
	packet->type = MSG_ITEM;
	packet->subType = MSG_ITEM_SEAL_COMPOSE_REQ;
	packet->tab = nTab;
	packet->invenIndex = inven_idx;
	packet->count = count;
	nmMessage.setSize(sizeof(*packet));
...
// Desc : 
// ----------------------------------------------------------------------------
void CNetworkLibrary::TransmogrifyReq(SWORD nTab, SWORD inven_idx)
{
	CNetworkMessage nmMessage;
	RequestClient::doItemTransmogrify* packet = reinterpret_cast<RequestClient::doItemTransmogrify*>(nmMessage.nm_pubMessage);
	packet->type = MSG_ITEM;
	packet->subType = MSG_ITEM_TRANSMOGRIFY_REQ;
	packet->tab = nTab;
	packet->invenIndex = inven_idx;
	nmMessage.setSize(sizeof(*packet));
...
// Desc : 
// ----------------------------------------------------------------------------
void CNetworkLibrary::ExtendMenuReq(SWORD nTab, SWORD inven_idx, int days)
{
	CNetworkMessage nmMessage;
	RequestClient::doItemExtendMenu* packet = reinterpret_cast<RequestClient::doItemExtendMenu*>(nmMessage.nm_pubMessage);
	packet->type = MSG_ITEM;
	packet->subType = MSG_ITEM_EXTEND_MENU_REQ;
	packet->tab = nTab;
	packet->invenIndex = inven_idx;
	packet->days = days;
	nmMessage.setSize(sizeof(*packet));
...
void CNetworkLibrary::UseSlotItem(int tabId, int inven_idx, SBYTE sbWearType)
{
	CItems* pItems = &MySlotItem[tabId][inven_idx];
#ifdef REZABOT
	if (pItems->ItemData->getindex() != 0)
	{
		if (pItems->ItemData->type == CItemData::ITEM_ONCEUSE)
		{

			if (pItems->ItemData->subType == CItemData::ITEM_SUB_CASH_ITEM)
			{
				if (pItems->ItemData->num2 == 555)
				{
					CUIManager::getSingleton()->GetReZaBotUI()->OpenReZaBotUI(pItems->ItemData->getindex(), pItems->Item_UniIndex);
					return;
		...
		pItems->ItemData->GetType() == CItemData::ITEM_ACCESSORY)
	{
		...
		if (pItems->ItemData->GetFlag() & ITEM_FLAG_COSTUME2) {
			// �ڽ�Ƭ2 �ý��� ������
			if ((pItems->ItemData->GetSubType() == CItemData::ITEM_SHIELD_ONE_SUIT && //RomeoST
				pItems->ItemData->GetType() != CItemData::ITEM_WEAPON &&
				pItems->ItemData->GetType() == CItemData::ITEM_SHIELD))
			...
			!(pItems->ItemData->GetSubType() == CItemData::ACCESSORY_PET || pItems->ItemData->GetSubType() == CItemData::ACCESSORY_WILDPET))
		{
			...
			// newslot - alanssoares
			for (int iWearPos = WEAR_ACCESSORY_SPECIAL1; iWearPos <= WEAR_ACCESSORY_SPECIAL4; ++iWearPos)
			{
				if (pItems->ItemData->GetWearingPosition() >= WEAR_ACCESSORY_SPECIAL1 && pItems->ItemData->GetWearingPosition() <= WEAR_ACCESSORY_SPECIAL4)
				{
					if (!_pNetwork->MyWearItem[iWearPos].IsEmptyItem() == FALSE)
					{
						sbWear = iWearPos;
	...
	else if (pItems->ItemData->GetType() == CItemData::ITEM_ONCEUSE)
	{
		...
		case CItemData::ITEM_SUB_TARGET:
		{
			...
			if (pInfo->IsTargetActive(eTARGET) == TRUE) {
				...
				vDelta = penPlEntity->GetPlacement().pl_PositionVector - pEntity->GetPlacement().pl_PositionVector;
			}
...
void CNetworkLibrary::SendCraftItem(int idxCraft, int countItem)
{
	CNetworkMessage nmGM((UBYTE)MSG_CRAFT);
	nmGM << (SBYTE)MSG_CRAFT_ITEM;
	nmGM << (LONG)idxCraft;
	nmGM << (LONG)countItem;
...
void CNetworkLibrary::SendPassiveSystemList()
{
	CNetworkMessage nmGM((UBYTE)MSG_PASSIVE_SYSTEM);
	nmGM << (SBYTE)MSG_PASSIVE_SYSTEM_LIST;

	SendToServerNew(nmGM);
}

void CNetworkLibrary::SendPassiveSystemSlotList()
{
	CNetworkMessage nmGM((UBYTE)MSG_PASSIVE_SYSTEM);
	nmGM << (SBYTE)MSG_PASSIVE_SYSTEM_SLOT_LIST;

	SendToServerNew(nmGM);
}

void CNetworkLibrary::SendPassiveSystemEquip(int nIndex, int slot)
{
	CNetworkMessage nmGM((UBYTE)MSG_PASSIVE_SYSTEM);
	nmGM << (SBYTE)MSG_PASSIVE_SYSTEM_EQUIP_REQ << (INDEX)nIndex << (INDEX)slot;

	SendToServerNew(nmGM);
}

void CNetworkLibrary::SendPassiveSystemUnequip(int nIndex)
{
	CNetworkMessage nmGM((UBYTE)MSG_PASSIVE_SYSTEM);
	nmGM << (SBYTE)MSG_PASSIVE_SYSTEM_UNEQUIP_REQ << (INDEX)nIndex;
...
// Ranking ����
void CNetworkLibrary::RankingSummonReq()
{
	CNetworkMessage nmQuest((UBYTE)MSG_QUEST);
	nmQuest << (UBYTE)MSG_QUEST_RANKING_SUMMON_REQ;
	_pNetwork->SendToServerNew(nmQuest);
}

void CNetworkLibrary::RankingRebornReq()
{
	CNetworkMessage nmQuest((UBYTE)MSG_QUEST);
	nmQuest << (UBYTE)MSG_QUEST_RANKING_REBORN_REQ;
	_pNetwork->SendToServerNew(nmQuest);
}
...
void CNetworkLibrary::UsePetAllBuffs(SWORD nTab, SWORD inven_idx)
{
	CNetworkMessage nmpet((UBYTE)MSG_EXTEND);

	nmpet << (ULONG)MSG_EX_PET_USE_ALL_BUFF;
	nmpet << (INDEX)nTab;
	nmpet << (INDEX)inven_idx;
...
void CNetworkLibrary::SendMovingGuildWarArea()
{
	...
	SendToServerNew(nmWarp);
	if (MyCharacterInfo.lWarpZone != E_RLGL) {
...
std::string ReadRegValue(const std::string& location, const std::string& name) {
	HKEY key;
	TCHAR value[1024];
...
unsigned int atohex(const char* hex_str)
{
	...
	for (i = length - 1, j = 0; i >= 0; i--, j++)
	{
		...
		{
		case '0': dec = 0; break;
		case 'A': case 'a': dec = 10; break;
		case 'B': case 'b': dec = 11; break;
		case 'C': case 'c': dec = 12; break;
		case 'D': case 'd': dec = 13; break;
		case 'E': case 'e': dec = 14; break;
		case 'F': case 'f': dec = 15; break;
		case '1': case '2': case '3': case '4':
		case '5': case '6': case '7': case '8': case '9':
			dec = hex_str[i] - 48;
			break;
		default: result = 0; return 0;
		}
...
BOOL GetHWID()
{
	memset(_cmiComm.cci_iHid, 0, sizeof(char) * MAX_GUID_LENGTH + 1);  //pwesty hwid shit

	std::string key;
	key = ReadRegValue("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", "RegisteredOwner");
	

	char hwid[MAX_GUID_LENGTH + 1] = { 0 };
	unsigned short u16id = getVolumeHash();
	DWORD test = 0;
	test = MAKELPARAM(u16id, u16id);
	char strTest[9] = { 0 };
	hextoa(test, strTest);
	...
	for (int i = 0; i <= 32; i += 8)
	{
		...
		memcpy(&((byte*)(hwid))[i], strTest, 8);
	}
	
	((byte*)(hwid))[39] = 0x00;
	

	const char* tkey = key.c_str();
	char cur[3] = { 0 };
	...
	int i = 8;
	for (int j = 0; j < key.length() && i < (MAX_GUID_LENGTH + 1); j++)
	{
		_snprintf(cur, 3, "%02x", tkey[j]);
		memcpy(&((byte*)(hwid))[i], cur, 2);
		i += 2;
	}

	/*
	for (int i = 0; i < MAX_GUID_LENGTH; i++)  // if we need to force array to exist
	{
		if (((byte*)(hwid))[i] == 0x00) {
			((byte*)(hwid))[i] = (byte)'a';
		}
	...
	strncpy_s(_cmiComm.cci_iHid, MAX_GUID_LENGTH + 1, hwid, MAX_GUID_LENGTH + 1);  // this line causes isssue.
	...
	for (int i = 0; i < _countof(queries) && !found; i++) {
		...
		while (pEnumerator) {
			...
			if (SUCCEEDED(hr) && vtProp.vt == VT_BSTR && vtProp.bstrVal) {
				...
				// Convert UTF-16 (BSTR) to UTF-8
				int len = WideCharToMultiByte(CP_UTF8, 0, vtProp.bstrVal, -1, buffer, sizeof(buffer), NULL, NULL);
				if (len > 0) {
					...
					const char* bad_values[] = {
						...
						"00000000-0000-0000-0000" // Example from your data
					};

					bool is_bad = false;
					for (const char* bad : bad_values) {
						if (_stricmp(buffer, bad) == 0) {
							is_bad = true;
							break;
						}
					}

					// Skip bad values and very short strings (<4 chars)
					if (!is_bad && strlen(buffer) >= 4) {
...
*/
	...
	packet->mode = 0;
	packet->version = ulVersion;//ulVersion

#if	(_MSC_VER > 1200)
	strncpy_s(packet->id, MAX_ID_NAME_LENGTH + 1, strID.str_String, _TRUNCATE);
	strncpy_s(packet->pw, MAX_PWD_LENGTH + 1, strPW.str_String, _TRUNCATE);
#else
	...
	memcpy(packet->hwid, _cmiComm.cci_iHid, MAX_GUID_LENGTH + 1);
...
void CNetworkLibrary::MgrSetChatColor(int nColIndex)
{
	...
	nmMessage << (LONG)MSG_EX_MESSENGER;
	nmMessage << (BYTE)MSG_EX_MESSENGER_CHATCOLOR_CHANGE;
...
{
	CNetworkMessage nmMessage((UBYTE)MSG_EXTEND);
	nmMessage << (ULONG)MSG_QUEST_PBI_USE_REQ;
}
//============================================================================================================
...
void CNetworkLibrary::SendBuffNPCReq(UBYTE subType)
{
	CNetworkMessage nmMessage((UBYTE)MSG_EVENT);

	nmMessage << (UBYTE)MSG_BUFF_NPC_REQ;
	nmMessage << (UBYTE)subType;
...
#ifdef DEMIGOD
void CNetworkLibrary::SendDemiGodCharacterCondition()
{
	CNetworkMessage nmMessage((UBYTE)MSG_EVENT);

	nmMessage << (UBYTE)MSG_EVENT_DEMIGOD;
...
#ifdef DEMIGOD
void CNetworkLibrary::SendCreateDemiGodCharacter()
{
	CNetworkMessage nmMessage((UBYTE)MSG_EVENT);

	nmMessage << (UBYTE)MSG_EVENT_DEMIGOD_SUCCESS;
...
void CNetworkLibrary::Send_auto_quest(INDEX iQuestIndex)//Cloud auto quest
{
	CNetworkMessage nmMessage((UBYTE)MSG_EXTEND);
	nmMessage << (ULONG)MSG_EX_CLOUD_AUTO_QUEST;
	nmMessage << SLONG(iQuestIndex);
...
void CNetworkLibrary::ExpeditionDungeonDetailReq(SLONG Mode)
{
	CNetworkMessage	nm((UBYTE)MSG_EXPEDITION);
	nm << (SBYTE)MSG_PARTYDUNGEON_DETAIL;
	nm << Mode;
	SendToServerNew(nm);
}
#endif
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
//Cloud Expedition_Party_Dungeon
void CNetworkLibrary::ExpeditionTeamJoinReq(SLONG slIndex, SLONG sldifficulty, SLONG slpzone)
{
	CNetworkMessage	nm((UBYTE)MSG_EXPEDITION);
	nm << (SBYTE)MSG_PARTYDUNGEON_JOIN;
	nm << slIndex;
	nm << sldifficulty;
	nm << slpzone;
...
void CNetworkLibrary::ExpeditionTeamJoinMember()
{
	CNetworkMessage	nm((UBYTE)MSG_EXPEDITION);
	nm << (SBYTE)MSG_PARTYDUNGEON_JOIN_MEMBER;

	SendToServerNew(nm);
}

//Cloud Expedition_Party_Dungeon
void CNetworkLibrary::ExpeditionTeamRecallReq(SLONG slIndex)
{
	CNetworkMessage	nm((UBYTE)MSG_EXPEDITION);
	nm << (SBYTE)MSG_PARTYDUNGEON_RECALL_REQ;
	nm << slIndex;
...
//Cloud Expedition_Party_Dungeon
void CNetworkLibrary::ExpeditionTeamRewardReq(SLONG slIndex, SLONG slpzone)
{
	CNetworkMessage	nm((UBYTE)MSG_EXPEDITION);
	nm << (SBYTE)MSG_PARTYDUNGEON_REWARD;
	nm << slIndex;
	nm << slpzone;
...
// ----------------------------------------------------------------------------
//  Name 	: SendRemoveBuffReq()
//  Desc 	: Remove all buffs
// ----------------------------------------------------------------------------
...
void CNetworkLibrary::SendRebornReq(int nNpcVirIdx)
{
	CNetworkMessage nmReborn;
	RequestClient::reborn* packet = reinterpret_cast<RequestClient::reborn*>(nmReborn.nm_pubMessage);
	packet->type = MSG_REBORN;
	packet->subType = MSG_REBORN_REQ;
	packet->npcIndex = nNpcVirIdx;
	nmReborn.setSize(sizeof(*packet));

	SendToServerNew(nmReborn);
}


void CNetworkLibrary::SendBattleWarfareMove()
{
	CNetworkMessage nmBattleMove((UBYTE)MSG_EXTEND);
	nmBattleMove << (ULONG)MSG_EX_BATTLE_WARFARE_REQ;

	SendToServerNew(nmBattleMove);
}
...
#ifdef ARENA_TIME_SYSTEM
void CNetworkLibrary::SetPvpMode()
{
	m_bIsPvp = !m_bIsPvp;
	if (MyCharacterInfo.zoneNo == ZONE_START)
	{
		if (MyCharacterInfo.x < 1315 && MyCharacterInfo.x > 1270 &&
			MyCharacterInfo.z < 1030 && MyCharacterInfo.z > 920)
		{
			if (m_bIsPvp)
				UIMGR()->GetRadar()->SetArenaTime(60);
			else
				UIMGR()->GetRadar()->ResetArenaTime();
		}
		else
		{
			UIMGR()->GetRadar()->ResetArenaTime();
...
#ifdef PRESTIGE_SYSTEM
void CNetworkLibrary::SendPrestigeReq(int nNpcVirIdx)
{
	CNetworkMessage nmPrestige;
	RequestClient::prestige* packet = reinterpret_cast<RequestClient::prestige*>(nmPrestige.nm_pubMessage);
	packet->type = MSG_PRESTIGE;
	packet->subType = MSG_PRESTIGE_REQ;
	packet->npcIndex = nNpcVirIdx;
	nmPrestige.setSize(sizeof(*packet));

	SendToServerNew(nmPrestige);
}

void CNetworkLibrary::SendPrestigeInfoReq(int nNpcVirIdx, int page)
{
	CNetworkMessage nmPrestige;
	RequestClient::prestige* packet = reinterpret_cast<RequestClient::prestige*>(nmPrestige.nm_pubMessage);
	packet->type = MSG_PRESTIGE;
	packet->subType = MSG_PRESTIGE_INFO_REQ;
	packet->npcIndex = nNpcVirIdx;
	packet->page = page;
	nmPrestige.setSize(sizeof(*packet));
...
#endif
void CNetworkLibrary::SaveSettings(int autopickup, int autoigni, int autosell)    //[ADM]Dark & SyroX Save Auto System settings
{
	if (autopickup >= 0 && autopickup < 4 && autoigni >= 0 && autoigni < 6 && autosell >= 0 && autosell < 4)
	{
		CNetworkMessage msg((UBYTE)MSG_EXTEND);
		msg << (ULONG)MSG_EX_SETTINGS
			<< (double)autopickup
			<< (double)autoigni
			<< (double)autosell;
...
void CNetworkLibrary::SendPvPBuff()
{
	CNetworkMessage nm;
	RequestClient::skillPvPBuff* packet = reinterpret_cast<RequestClient::skillPvPBuff*>(nm.nm_pubMessage);
	packet->type = MSG_SKILL;
	packet->subType = MSG_SKILL_PVP_BUFF;
	nm.setSize(sizeof(*packet));
	SendToServerNew(nm);
}
#endif