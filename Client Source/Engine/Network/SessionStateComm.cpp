...
// Cash Exchange Packets
DECLARE_PACKET(CashExchangeListResponse);
DECLARE_PACKET(CashExchangeCreateListingResult);
DECLARE_PACKET(CashExchangeBuyListingResult);
...
void CSessionState::reg_packet_comm()
{
	...
	// Cash Exchange Packet Registration
	REG_PACKET(MSG_CASHEXCHANGE, MSG_CASHEXCHANGE_LIST_RESPONSE, CashExchangeListResponse);
	REG_PACKET(MSG_CASHEXCHANGE, MSG_CASHEXCHANGE_CREATE_LISTING_RESULT, CashExchangeCreateListingResult);
	REG_PACKET(MSG_CASHEXCHANGE, MSG_CASHEXCHANGE_BUY_LISTING_RESULT, CashExchangeBuyListingResult);
}
...
IMPLEMENT_MSG_UPDATE(GuildSkillLearn)
{
	...
	UIMGR()->GetGuild()->UpdateGuildSkillLearnMessage(*pPack); //pwesty 12-01-24
}
...
// Cash Exchange Packet Implementations
IMPLEMENT_PACKET(CashExchangeListResponse)
{
	CASH_DEBUG_LOG("[SessionStateComm] RecvCashExchangeListResponse called\n");
	CCashExchangeUI* pUI = (CCashExchangeUI*)UIMGR()->GetUI(UI_CASHEXCHANGE);
	if (pUI != NULL)
	{
		CASH_DEBUG_LOG("[SessionStateComm] CashExchangeUI found, calling HandleListingResponse\n");
		pUI->HandleListingResponse(*istr);
	}
	else
	{
		CASH_DEBUG_LOG("[SessionStateComm] ERROR - CashExchangeUI is NULL!\n");
	}
}

IMPLEMENT_PACKET(CashExchangeCreateListingResult)
{
	CCashExchangeUI* pUI = (CCashExchangeUI*)UIMGR()->GetUI(UI_CASHEXCHANGE);
	if (pUI != NULL)
	{
		unsigned char* pBuf = (unsigned char*)istr->GetBuffer();
		int nSuccess = pBuf[2];
		int nErrorCode = pBuf[3];
		pUI->HandleCreateListingResult(nSuccess, nErrorCode);
	}
}

IMPLEMENT_PACKET(CashExchangeBuyListingResult)
{
	// TODO: Implement buy listing result handling
	CCashExchangeUI* pUI = (CCashExchangeUI*)UIMGR()->GetUI(UI_CASHEXCHANGE);
	if (pUI != NULL)
	{
		// Handle buy result
