...
//안태훈 수정 끝	//(Easy Use World Editor)(0.1)
...

	{
		...
		INDEX iShadowDetail = g_iShadowDetail;

		if( iShadowDetail < 0 || iShadowDetail > 7) iShadowDetail = 0;

		if(iShadowDetail == SHADOW_PROJ_ALL || iShadowDetail == SHADOW_PROJ_ONLY)
...

{
	...
	// Safety check: ensure D3D device is available
	if (!_pGfx || !_pGfx->gl_pd3d9Device) return;
...

	{
...

		{
			...
			if(iShadowDetail == SHADOW_PROJ_ONLY)
			...
			_pGfx->gl_pd3d9Device->GetViewport(&vpOld);
			...
			_pGfx->gl_pd3d9Device->SetViewport(&vpNew);

			

			hr = _pGfx->gl_pd3d9Device->SetTextureStageState( 0, D3DTSS_TEXCOORDINDEX, 0);				D3D_CHECKERROR(hr);

			hr = _pGfx->gl_pd3d9Device->SetTextureStageState( 0, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_DISABLE);	D3D_CHECKERROR(hr);
			...
			_pGfx->gl_pd3d9Device->SetViewport(&vpOld);
			...
			hr = _pGfx->gl_pd3d9Device->SetTextureStageState( 0, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_CAMERASPACEPOSITION);	D3D_CHECKERROR(hr);

			hr = _pGfx->gl_pd3d9Device->SetTransform( D3DTS_TEXTURE0, (_D3DMATRIX*)&matShadowUV);						D3D_CHECKERROR(hr);
	...
	hr = _pGfx->gl_pd3d9Device->SetTextureStageState( 0, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_PASSTHRU );		D3D_CHECKERROR(hr);
...
//강동민 수정 끝		03.22
		...
		hr = _pGfx->gl_pd3d9Device->SetRenderState(D3DRS_STENCILENABLE, FALSE);				D3D_CHECKERROR(hr);

		hr = _pGfx->gl_pd3d9Device->SetRenderState(D3DRS_STENCILFUNC, D3DCMP_EQUAL);			D3D_CHECKERROR(hr);

		hr = _pGfx->gl_pd3d9Device->SetRenderState(D3DRS_STENCILREF,   0);					D3D_CHECKERROR(hr);

		hr = _pGfx->gl_pd3d9Device->SetRenderState(D3DRS_STENCILPASS, D3DSTENCILOP_INCR);	D3D_CHECKERROR(hr);

		

		// NOTE : Shadow Map을 생성 혹은...

		hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_ALPHAFUNC, D3DCMP_GREATEREQUAL);	D3D_CHECKERROR(hr);

		// NOTE : 필터링 적용후 외곽선 문제를 해결하기 위한...

		//hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_ALPHAREF,  1);

		hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_ALPHAREF,  0x40);					D3D_CHECKERROR(hr);
		...
		*/
...

		{
...

			{
				...
				if (!_pGfx || !_pGfx->gl_pd3d9Device) continue;
		...
		//hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_ALPHAFUNC, D3DCMP_GREATEREQUAL);

		//hr = _pGfx->gl_pd3d9Device->SetRenderState( D3DRS_ALPHAREF,  128);

		

		//////////////////////////////////////////////////////////////////////////

		//_pGfx->gl_pd3d9Device->SetRenderState(D3DRS_STENCILENABLE, FALSE);
