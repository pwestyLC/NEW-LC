...
// FIXME : NULL·Î ÃÊ±âÈ­ÇÏÁö ¾Ê°í »ç¿ëÇÏ°í ÀÖÀ½.
CRenderTexture				*_prtFilterTarget[2];
LPDIRECT3DVERTEXBUFFER9		_pFilterVertexBuffer = NULL;
LPDIRECT3DVERTEXBUFFER9		_pBackVertexBuffer[MAX_BACKVERTEX_WIDTH][MAX_BACKVERTEX_HEIGHT]; // 1280 X 1024 ±îÁö¸¸ Áö¿øµÇ´Â°É·Î °¡Á¤ÇÑ´Ù.
...
extern void ReleaseBloomTexture()
{
	...
	{
     _pGfx->gl_pd3d9Device->SetVertexShader(0);
		_pGfx->gl_pd3d9Device->SetVertexDeclaration(0);
		_dwTexCoord4OffsetVS_Shader->Release();
		_dwTexCoord4OffsetVS_Shader = NULL;
    }
	if(_dwTexCoord4OffsetVS_Declaration)
	{
		_dwTexCoord4OffsetVS_Declaration->Release();
...
HRESULT CRenderer::InitBloom()
{
	...
	{
		// Init Device
		// ºä, Åõ¿µµîÀÇ Çà·Ä°ª °è»êÇÏ´Â ºÎºÐ
		CalcFullCoverageMatrix(); // Vertex Shader¿¡ ³Ñ°ÜÁÙ ÀÎ¼ö¸¦ ¸¸µç´Ù. ÇÑ¹ø¸¸ ÇØÁÖ¸é µÈ´Ù.
...
*/
	m_nGaussianSize[0]	= g_iUseBloom*2; // ¿É¼ÇÀÇ ºí·³ ´Ü°è¸¦ ¹Ý¿µ // ½Å ÁÖ³ë¸ÊÀÇ Bloom È¿°ú ´Ü°è ´Ã¸²
