...
/************************ Block Encryption *************************/
void SeedEncrypt(BYTE *pbData, uint32_t *pdwRoundKey)
{
	uint32_t L0, L1, R0, R1, T0, T1, *K = pdwRoundKey;
    ...
    R1 = EndianChange(R1);
#endif
    ((uint32_t *)pbData)[0] = R0;
    ((uint32_t *)pbData)[1] = R1;
    ((uint32_t *)pbData)[2] = L0;
    ((uint32_t *)pbData)[3] = L1;
}


/* same as encrypt, except that round keys are applied in reverse order. */
void SeedDecrypt(BYTE *pbData, uint32_t *pdwRoundKey)
{
    uint32_t L0, L1, R0, R1, T0, T1, *K=pdwRoundKey;
...
void SeedRoundKey(uint32_t *pdwRoundKey, BYTE *pbUserKey)
{
  uint32_t A, B, C, D, E, F, G, H, T0, T1, *K=pdwRoundKey;

  A = ((uint32_t *)pbUserKey)[0];
  B = ((uint32_t *)pbUserKey)[1];
  C = ((uint32_t *)pbUserKey)[2];
  D = ((uint32_t *)pbUserKey)[3];
  E = ((uint32_t *)pbUserKey)[4];
  F = ((uint32_t *)pbUserKey)[5];
  G = ((uint32_t *)pbUserKey)[6];
  H = ((uint32_t *)pbUserKey)[7];
