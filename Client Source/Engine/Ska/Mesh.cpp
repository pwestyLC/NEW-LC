...
static void FillVertexWeightInfos(MeshLOD &mlod)
{
	...
	for( imsrf = 0; imsrf < ctmsrf; imsrf++ )
	{
		...
		else
		{
			...
			CPrintF("ǥ��� ���� ������ %d���Դϴ�. ǥ��� �������� 25�� ���Ͽ��� �մϴ�.", msrf.msrf_aubRelIndexTable.Count());
		}
	}
...
//������ ���� ��	//(For Performance)(0.1)
		...
		for(INDEX iSurface=0; iSurface<mlod.mlod_aSurfaces.Count(); ++iSurface) 
		{
			...
			else if(vtct == VTCT_TRIANGLE || vtct == VTCT_GROUP)	//�׻� �ϳ��� Tag�� �߰�.
		...
		if(mlod.mlod_aUVMaps.Count() > 0 && (bHaveTangentStream || _bSkaStudioApp))
		{
			...
			{
				ASSERT(FALSE && "Tagent ����� ���ؼ��� Vertex ��ġ�� �븻, �ؽ��� ��ǥ[0]�� ���� ��ġ�ؾ� �Ѵ�.");
			}
		}
...
static void FillWeightMapInfo(MeshLOD &mlod)
{
	...
	{
		mlod.mlod_aWeightMaps[iBone].mwm_aVertexWeight.Clear();	//�ϴ� WeightMap�� ������ �����.
	}
	...
	for(int iSurface=0; iSurface<surfaceCount; ++iSurface)
	{
		...
		for(INDEX iVertex=mlod.mlod_aSurfaces[iSurface].msrf_iFirstVertex; iVertex<vertexEnd; ++iVertex)
		{
			...
			{
				if(mlod.mlod_aVertexWeights[iVertex].mvwi_aubWeights[iBPV] < 1) continue;					//weight�� 0�� bone�� �ǹ̰� ����.
				
				MeshVertexWeight insertVal;	//WeightMap�� �߰��� ����
				insertVal.mww_iVertex = iVertex;
				insertVal.mww_fWeight = mlod.mlod_aVertexWeights[iVertex].mvwi_aubWeights[iBPV] / 255.0f;

				int relIndex = mlod.mlod_aVertexWeights[iVertex].mvwi_aubIndices[iBPV];				//vertex weight���� ���̴� ���� ������� �ε���
				int absIndex = mlod.mlod_aSurfaces[iSurface].msrf_aubRelIndexTable[relIndex];		//surface�� index table�� ���� �ε����� ����ִ�.GetRelativeWeightIndex()����

				int vertexWeightCount = mlod.mlod_aWeightMaps[absIndex].mwm_aVertexWeight.Count();
				mlod.mlod_aWeightMaps[absIndex].mwm_aVertexWeight.Expand( vertexWeightCount+1 );	//�ø��� �߰�.
...
static MeshTangent MakeMeshTangent(double *normal, double *tangent, double *binormal)
{
	...
	//transform for tangent space�� ��Ľ��� determinent�� �̿��ؾ� �Ѵ�.
/*
...
//��� �Ѱ��� �ﰢ���� �����ϰ� position, normal, �Ѱ��̻��� texcoord�� ������ �Ѵ�.
//texCoordForTangent�� ���� NormalMap�� � ��ǥ�� �°� �̾Ҵ����� ���� �޶������̴�.
//���� 0���� fix�س��� ���� �ٸ� ���� �־ �����ϴ�.(����� �ϵ��ڵ����� �ؾ���.
static void CalcVertexTangent(MeshLOD &mlod, int texCoordForTangent)
{
	if(mlod.mlod_aVertices.Count() < 3 || mlod.mlod_aNormals.Count() < 3 || mlod.mlod_aUVMaps.Count() < 1)
	{
		ASSERTALWAYS("�־��� MeshLOD�δ� TANGENT�� ���� �� �����ϴ�. �Է����ڰ� �����մϴ�.");
	}
