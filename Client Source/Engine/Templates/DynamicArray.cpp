...
void CDynamicArray<Type>::Clear(void) {
  ...
  if (da_Count!=0) {
    ...
    for (INDEX iPointer=0; iPointer<da_Count; iPointer++) {
      ...
      if (da_Pointers[iPointer]!=NULL) {
        ::Clear(*da_Pointers[iPointer]);
      } else {
        ASSERTALWAYS("CDynamicArray<>::Clear(): Null member pointer!");
      }
    }
...
void CDynamicArray<Type>::Delete(Type *ptMember) {
  ...
  if (ptMember==NULL || da_Pointers==NULL || da_Count<=0) {
    ASSERTALWAYS("CDynamicArray<>::Delete(): Invalid member or empty array!");
    return;
  }

  INDEX iMember = -1;
  for (INDEX i=0; i<da_Count; i++) {
    if (da_Pointers[i]==ptMember) {
      iMember = i;
      break;
    }
  }
  if (iMember<0) {
    ASSERTALWAYS("CDynamicArray<>::Delete(): Not a member of this array!");
    return;
  }
...
INDEX CDynamicArray<Type>::GetIndex(Type *ptMember) {
  ASSERT(this!=NULL);
  if (ptMember==NULL || da_Pointers==NULL || da_Count<=0) {
    ASSERTALWAYS("CDynamicArray<>::Index(): Invalid member or empty array!");
    return 0;
  }
  // slow !!!!
  // check all members
