...
CZoneInfo::CZoneInfo()
{
	...
	m_pZoneInfo[33].bRaidDungeon = TRUE;	// ��Ʈ ���� �����
	m_pZoneInfo[35].bRaidDungeon = TRUE;	// ��Ʈ ���� ������ ����
	m_pZoneInfo[36].bRaidDungeon = TRUE;	// ��ĭ ���
	m_pZoneInfo[41].bRaidDungeon = TRUE;	// ������ ���ϴ���
	m_pZoneInfo[45].bRaidDungeon = TRUE;	// Val add new raid
}
...
BOOL CZoneInfo::LoadObjInZoneData(void)
{
	...
	{
		{ CTString msg; msg.PrintF("File is not Exist.(Obj): %s", (const char*)strDataName); MessageBox(NULL, msg, "Load Error!", MB_OK); }
		return FALSE;
	}
		...
		// for each of the entities
		{
			FOREACHINDYNAMICCONTAINER(cenToDestroy, CEntity, iten) 
			{
				...
				iten->Destroy();    
					...
					CPrintF(TRANS("World::Clear nullptr entities! skip...\n"));
		}
		
			}
		}

// ### Need for correct termination of threads after clearing all entities
...
void CWorld::SearchSectorAroundEntity(CEntity* pen, const FLOAT3D &vEyesPos, CAnyProjection3D &prProjection)
{
	...
	while (!lhToAdd.IsEmpty())
	{
		...
		// ���⼭ Sector�ȿ� Entity�� list�� ��´�.
		//
		AddEntityZoningSector(pbsc); // ���Ⱑ �߿�
...
{
#ifndef DISABLE_TAB_TARGET
	CEntity* tmpPlayer = CEntity::GetPlayerEntity(0); // �ڱ� �߽�
