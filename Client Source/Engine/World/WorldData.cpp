...
#include <Engine/Entities/PassiveSystem.h>
...
void CWorld::LoadCraft()
{
	...
	if (m_bLoad == FALSE && bLoad && !_bWorldEditorApp)
	{
		CTString strTmp = "data\\craft.lod";

		fnTemp = strFullPath + strTmp;

		if (CCraft::loadEx(fnTemp) == false)
		{
			MessageBox(NULL, "Craft file is not correct", "Error!", MB_OK);
			_pGameState->Running() = FALSE;
			_pGameState->QuitScreen() = FALSE;
		}
		bLoad = FALSE;
...
void CWorld::LoadPassiveSystem()
{
	...
	if (m_bLoad == FALSE && bLoad && !_bWorldEditorApp)
	{
		CTString strTmp = "data\\passiveSystem.lod";

		fnTemp = strFullPath + strTmp;

		if (CPassiveSystem::loadEx(fnTemp) == false)
		{
			MessageBox(NULL, "Passive system file is not correct", "Error!", MB_OK);
			_pGameState->Running() = FALSE;
			_pGameState->QuitScreen() = FALSE;
		}

		LoadStringPassiveSystem();
