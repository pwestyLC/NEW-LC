...
void CWorld::LoadStringPassiveSystem()
{
	StringLoader* pLoader = StringLoader::getSingleton();
	CUIManager* pUIManager = CUIManager::getSingleton();
	...
	pLoader->LoadString(eSTRING_PASSIVE_SYSTEM, g_iCountry);

	int		nIdx;
	const char* strtmp = NULL;

	auto iter = CPassiveSystem::m_vecPassive.begin();
	auto eiter = CPassiveSystem::m_vecPassive.end();
	...
	for (; iter != eiter; ++iter)
	{
		...
		if (nIdx >= 0)
		{
			strtmp = pLoader->getData(eSTRING_PASSIVE_SYSTEM, nIdx, 0);
			if (strtmp != NULL)
				iter->name = strtmp;

			if (iter->name.c_str() == NULL)
				pUIManager->SetNoTranslationString(PASSIVE_SYSTEM_STRING, nIdx);

			strtmp = pLoader->getData(eSTRING_PASSIVE_SYSTEM, nIdx, 1);
			if (strtmp != NULL)
				iter->description = strtmp;

			if (iter->description.c_str() == NULL)
				pUIManager->SetNoTranslationString(PASSIVE_SYSTEM_STRING, nIdx);
		}
	}

	pLoader->release(eSTRING_PASSIVE_SYSTEM);
}
#endif
