...
extern ENGINE_API INDEX g_iNewWASD;
//#endif
...
static void DumpEnemies(void)
{
	...
	CWorld *pwo = _pShell->GetCurrentWorld(); //(CWorld *)_pShell->GetINDEX("pwoCurrentWorld");
...
extern INDEX hud_bShowLatency = TRUE;
...
ENGINE_API extern FLOAT g_fAvgLatency;
...
//	198 BOOL	m_nPlayFace = -1,
	...
	213 CTFileName m_fnMusic "Music"   'M' = CTFILENAME("data\\sounds\\game\\BGM\\GB_login.ogg"),
...
//		penWeapons->GetWeaponName(strWeaponName);
	...
	virtual void Read_net_StatusPC(CNetworkMessage *istr)
	{
		...
		if(pPack->charIndex == _pNetwork->MyCharacterInfo.index)
		{
			...
			//REZABOT 
			_pNetwork->MyCharacterInfo.char_botting = pPack->botting;
		...
		if (pObject != NULL)
		{
			...
			//REZABOT 
			pTarget->cha_botting = pPack->botting;
	...
	// FIXME: Except for when I am the attacker, all other cases can be grouped together.
		// Processing damage.
	virtual void Read_net_Damage_Character(CNetworkMessage *istr)
	{
		...
		if(pPack->charIndex == _pNetwork->MyCharacterInfo.index)
		{
				...
				if( _pNetwork->SearchEntityByNetworkID( pPack->targetIndex, pPack->targetType, penTargetEntity ) )
				{
					...
					if(pPack->targetHp <= 0)
					{
						...
						if (pUIManager->m_DamageLogInfo.GetStart())
						{
							...
							_UIAutoHelp->SetGMNotice ( "Exiting the mode for recording damage.", 0xFFAA33FF );
						}
					}
					return;
				}
			...
			// When I am attacking any other entity.
			else
			{
				CEntity *penTargetEntity = NULL;
				if( _pNetwork->SearchEntityByNetworkID( pPack->targetIndex, pPack->targetType, penTargetEntity ) )
				{						
					((CUnit*)penTargetEntity)->m_enAttackerID = pPack->charIndex;
					// When attacking another character.
					if(pPack->targetType == MSG_CHAR_PC )// PvP penalties are now visible only to administrators.
					{ //dethunter12
						
							// Displaying the damage information of the target.
							ShowAttackResult(penTargetEntity, (SLONG)pPack->damage, pPack->flag, pPack->skillIndex);
		...
		// FIXME: Is it okay to have this overlap? -.- 
			...
			if( _pNetwork->SearchEntityByNetworkID( pPack->charIndex, MSG_CHAR_PC, penEntity ) )
			{
				...
				if(pPack->targetType == MSG_CHAR_PC)
				{
					...
					if(pPack->targetIndex == _pNetwork->MyCharacterInfo.index)
					{
						...
						_pNetwork->MyCharacterInfo.hp = pPack->targetHp; //dethunter12
						return;
					}
				}

				// When attacking something other than the player.						
				CEntity *penTargetEntity = NULL;
				if( _pNetwork->SearchEntityByNetworkID( pPack->targetIndex, pPack->targetType, penTargetEntity ) )
				{
					((CUnit*)penTargetEntity)->m_enAttackerID = pPack->charIndex;

					// When the target is my summoned creature.
					...
					if(!_pNetwork->m_bSingleMode)
					{
						if(!((CUnit*)penEntity)->m_bSkilling && pPack->skillIndex == -1)
						{
							...
							// If a party member is attacking, save their target...
				...
				return; //The victim does not exist on the screen.
			}
			// Even if the attacker is not on the screen, if an NPC or PC's HP reaches 0 and they need to die...
			else
			{
				// Characters with 0 HP should die even if the attacker is not on the screen..
				if(pPack->targetType == MSG_CHAR_PC)
				{
					//In case of PVP�
					if(pPack->targetIndex == _pNetwork->MyCharacterInfo.index)
					{
						if( ((CPlayerEntity*)CEntity::GetPlayerEntity(0))->IsSitting() && pPack->flag != HITTYPE_MISS )
						{// If sitting, triggered upon being attacked
							pUIManager->GetCharacterInfo()->UseAction( 3 );
						}
		...
		{
			if( ((CPlayerEntity*)CEntity::GetPlayerEntity(0))->IsSitting() && pPack->flag != HITTYPE_MISS )
			{// Happens when being attacked while sitting.
				pUIManager->GetCharacterInfo()->UseAction( 3 );
			}
		...
		//Excluding the case where I am being attacked, for all other cases...
		CEntity *penTargetEntity = NULL;
		if( _pNetwork->SearchEntityByNetworkID( pPack->targetIndex, pPack->targetType, penTargetEntity ) )
		{
			...
			// // When the target is any other entity.
			else
			{
	...
	virtual void Read_net_Damage( SBYTE sbType, CNetworkMessage *istr)
	{
		...
		// alanssoares
		if (_pGameState->IsRestartGame() == TRUE && bMe == true && sbType == MSG_CHAR_PC)
		{
			pUIManager->GetSystemMenu()->CancelRestart();
		}
		...
		if( _pNetwork->SearchEntityByNetworkID( pPack->charIndex, sbType, penEntity ) )
		{
			...
			// When attacking something other than my player.
			CEntity *penTargetEntity = NULL;
			if( _pNetwork->SearchEntityByNetworkID( pPack->targetIndex, pPack->targetType, penTargetEntity ) )
			{
				((CUnit*)penTargetEntity)->m_enAttackerID = pPack->charIndex;

				// NOTE: Even if the synchronization is not perfect, I had to handle it this way because there are 
				// often times when the damage is not displayed when my summoned creature attacks!
				// In cases where the damage needs to be shown (when my summoned creature attacks or is attacked).
				BOOL bShowDamage = FALSE;  //PWESTY

				// When the attacker is my summoned creature.
				...
				{
					if (pInfo->GetMyApetInfo() != NULL && pInfo->GetMyApetInfo()->m_pEntity != NULL)
					{ // Registering the creature attacking my wild pet.
				...
				else
				{
					...
					// NOTE: Even if the synchronization is off, there are more instances where 
					// damage isn't displayed when my summoned creature attacks, so I had to handle it this way!
					if( bShowDamage )
					{
		...
		// Even if the attacker is not on the screen, characters with 0 HP should die...
		if(pPack->targetType == MSG_CHAR_PC)
		{
			//PVP�� ���
			if(pPack->targetIndex == _pNetwork->MyCharacterInfo.index)
		...
		// Deal damage to the target if the attacker is not visible.
		CEntity *penTargetEntity = NULL;
		if( _pNetwork->SearchEntityByNetworkID( pPack->targetIndex, pPack->targetType, penTargetEntity ) )
		{
			// When the target is my summoned creature.
...
//������ ���� ��	//(Open beta)(2004-12-14)
	...
	BOOL CheckAttackCharacter( CEntity *penEnt, FLOAT fLength )
	{
		...
		if ((!GAMEDATAMGR()->GetPartyInfo()->IsPartyMember(pTarget->GetCIndex())) && ( !GAMEDATAMGR()->GetPartyInfo()->IsExpedetionMember(pTarget->GetCIndex()) ) 
		&& (_pNetwork->MyCharacterInfo.zoneNo == ZONE_WARFARE_BATTLE || _pNetwork->MyCharacterInfo.zoneNo == ZONE_ROYAL_RUMBLE))
		{
			return TRUE;
		}
	...
	void SetCharacterHairChange(INDEX nSIdx, SBYTE sbType, BOOL IsMe)
	{
		...
		if (nCurCosIndex > 0)
		{
			...
			if (pCurCosItem != NULL)
			{
				...
				{
					if (nCurCosIndex == ITEM_TREASURE_HELMET || nCurCosIndex == ITEM_TREASURE_HELMET2)
					{
	...
	virtual	void SetChaWearing(CCharacterTarget* pTarget, CEntity* penEntity, CNetworkMessage *istr) //msg_appear�϶� ����.
	{
		...
		for(i = 0; i < WEAR_COUNT; ++i)//1021//1105
		{
			...
			if ( cos_index >= 0 )
			{
				...
				if (_pNetwork->GetItemData(cos_index)->IsFlag(ITEM_FLAG_INVISIBLE_COSTUME) == false)
				{
					...
					{
						if (cos_index == ITEM_TREASURE_HELMET || cos_index == ITEM_TREASURE_HELMET2)
						{
		...
		for (i = WEAR_ACCESSORY_1; i < WEAR_ACCESSORY_MAX; i++)
		{
			if ( pPack->accessory[i] > -1 )
			{
				...
				CItemData * pAccItem  = _pNetwork->GetItemData(pTarget->cha_itemAccessory[i]);

				CTString strEffectName  = CTString (pAccItem->GetArmorEffectName());
	...
	virtual void WearingCostumeArmor( CNetworkMessage *istr )
	{
		...
		if (pObject != NULL)
		{
			...
			if (item_index > 0) // �Ա�(�ڽ�Ƭ)
			{
				...
				{
					if (item_index == ITEM_TREASURE_HELMET || item_index == ITEM_TREASURE_HELMET2)
					{
...
// 		}
	...
	// November 9, 2012: Decrease count when taking damage, handling here
	// If the damage taken from a skill is 0, prevent the attacker's skill attack. Handle message output.
	...
	void ShowAttackResult(CEntity* penTargetEntity, ULONG targetDamage, SBYTE targetFlag, SLONG	skillIndex)
	{
		...
		// FIXME: Do we really need to check targetFlag???
		switch(targetFlag)
		{
		case HITTYPE_MISS:
					...
					switch(pPack->actionvalue)
					{
						case ACTION_NUM_PK:
							...
							else if(pPack->state & PLAYER_STATE_PKMODE)
							{
								...
								strSysMessage.PrintF( _S( 9388, "[%s]Áö¿ª [%s]°ø¼ºÀÌ %d¿ù %dÀÏ %d½Ã·Î ¿¹Á¤µÇ¾ú½À´Ï´Ù." ));
...
//������ ���� ����	//(Open beta)(2004-12-14)
	...
	void UpdateLatency(FLOAT tmLatencyNow)
	{
		...
		if (m_tmLatency > 0) {
			g_fAvgLatency = m_tmLatency * 1000;
		}
...
//������ ���� �� Ŭ���� �غ� �۾�		08.10
	...
	{
		BOOL bShowExtras = (ULONG64(pvUserData)&GRV_SHOWEXTRAS);
...
//������ ���� ��	//(Add & Modify SSSE Effect)(0.1)
		...
		if(IsAnimationPlaying(idBodyAnim))
		{
			...
			*/
			...
			//pwesty fucking with shit
			const int arraySize = sizeof(idPlayerWhole_Animation) / sizeof(idPlayerWhole_Animation[0]); // Or set explicitly if it's known

			for (int nIsCount = 0; nIsCount < 4; ++nIsCount)
			{
				// Check standard attack animations
				int animID = ANIM_ATTACK_1 + nIsCount;

				if (idPlayerWhole_Animation == nullptr) {
					std::cerr << "Error: Animation array is not initialized!" << std::endl;
					return; // Handle error appropriately
				}
				if (animID >= arraySize || animID < 0) {
					continue; // Skip invalid indices
				}

				if (IsAnimationPlaying(idPlayerWhole_Animation[animID])) {
					iCurrentIsAnim = animID;
					break;
				}

				// Check extended attack animations
				animID = ANIM_EXT_ATTACK_1 + nIsCount;
...
*/		
	...
	virtual void moveto(FLOAT3D vDelta)
	{
		...
		m_vDesiredPosition  = vDelta ;
		vDelta  = m_vDesiredPosition  - GetPlacement  () . pl_PositionVector ;
		g_bIsRotateEnd = FALSE;							
		g_bFirstRotate = TRUE;
		g_bTargetDirection = vDelta;
		
		GetPlayerWeapons()->m_vRayHitTmp = vDelta;

		// get the world pointer
		CWorld *pwo = (CWorld *)_pShell->GetINDEX("pwoCurrentWorld");
		// if there is no current world
		
		//GetPlayerWeapons()->m_penRayHitTmp = pwo->GetBackgroundViewer();
		...
		// for each entity in the world
		{
			...
			FOREACHINDYNAMICCONTAINER(pwo->wo_cenEntities, CEntity, iten) 
			{
				...
				{
					GetPlayerWeapons()->m_penRayHitTmp = pen;
					break;
				}
				else 
				{
		...
		//GetPlayerWeapons()->m_penRayHitTmp = GetEntity();//GetTerrain()->tr_penEntity;
		m_bReserveMove = FALSE;
	}
 //new2022	
...
virtual BOOL BOTSearchItem(int X) {
	...
	ObjectBase* pObject = ACTORMGR()->GetXObjectByDistance(GetPlacement().pl_PositionVector, X);
...
{
    return (m_bForward || m_bKeyMove) ? TRUE : FALSE;
}

virtual BOOL IsInputMovingKey()
{
    // consider input active when any of the movement controls has positive value
    return (
        _pInput->GetInputDevice(1)->id_aicControls[KID_W].ic_fValue > 0.0f ||
...
{
    // rotate input is considered active when left/right controls are pressed
    // and forward/back inputs are not active
    const FLOAT fW = _pInput->GetInputDevice(1)->id_aicControls[KID_W].ic_fValue;
    const FLOAT fS = _pInput->GetInputDevice(1)->id_aicControls[KID_S].ic_fValue;
    const FLOAT fA = _pInput->GetInputDevice(1)->id_aicControls[KID_A].ic_fValue;
    const FLOAT fD = _pInput->GetInputDevice(1)->id_aicControls[KID_D].ic_fValue;

    return (fW <= 0.0f && fS <= 0.0f && (fA > 0.0f || fD > 0.0f)) ? TRUE : FALSE;
}
...
//	vSide = vMove * vUp; // ���� (����)
	...
	if ((!m_bHold || !m_bStuned))
	{
		...
		else if (g_iNewWASD && _pInput->GetInputDevice(1)->id_aicControls[KID_A].ic_fValue == 1.0f)//Cloud wasd
		{
			s_bButtonPressedType = 3;
		}
		else if (g_iNewWASD && _pInput->GetInputDevice(1)->id_aicControls[KID_D].ic_fValue == 1.0f)//Cloud wasd
		{

			s_bButtonPressedType = 4;
		}
		else 
		{
			if( !m_bSendStopMessage )
	...
	if( s_bButtonPressedType )
	{
		if( ! ( IsAttacking() || IsSkilling() ) )
		{
			...
			else if(g_iNewWASD && s_bButtonPressedType == 3 )// A //Cloud wasd
			{
				m_vDesiredPosition = en_plPlacement.pl_PositionVector - (vRight * (plr_fSpeed * 1.5f));
				bButtonAction = TRUE;
				bMove = TRUE;
			}
			else if(g_iNewWASD && s_bButtonPressedType == 4 )// D //Cloud wasd
			{
				m_vDesiredPosition = en_plPlacement.pl_PositionVector + (vRight * (plr_fSpeed * 1.5f));
				bButtonAction = TRUE;
				bMove = TRUE;
...
/* 	else if (_pInput->GetInputDevice(1)->id_aicControls[KID_Q].ic_fValue == 1.0f)
	...
	if(g_iNewWASD)
	{
	if (_pInput->GetInputDevice(1)->id_aicControls[KID_W].ic_fValue == 1.0f && _pInput->GetInputDevice(1)->id_aicControls[KID_A].ic_fValue == 1.0f
	|| _pInput->GetInputDevice(1)->id_aicControls[KID_A].ic_fValue == 1.0f && _pInput->GetInputDevice(1)->id_aicControls[KID_W].ic_fValue == 1.0f
	)
	{
		m_vDesiredPosition = en_plPlacement.pl_PositionVector - ((vFront+vRight) / 2 * (plr_fSpeed * 1.5f));
		bButtonAction = TRUE;
		bMove = TRUE;
	}
	
	if (_pInput->GetInputDevice(1)->id_aicControls[KID_W].ic_fValue == 1.0f && _pInput->GetInputDevice(1)->id_aicControls[KID_D].ic_fValue == 1.0f
	|| _pInput->GetInputDevice(1)->id_aicControls[KID_D].ic_fValue == 1.0f && _pInput->GetInputDevice(1)->id_aicControls[KID_W].ic_fValue == 1.0f
	)
	{
	...
	if (_pInput->GetInputDevice(1)->id_aicControls[KID_S].ic_fValue == 1.0f && _pInput->GetInputDevice(1)->id_aicControls[KID_A].ic_fValue == 1.0f
	|| _pInput->GetInputDevice(1)->id_aicControls[KID_A].ic_fValue == 1.0f && _pInput->GetInputDevice(1)->id_aicControls[KID_S].ic_fValue == 1.0f
	)
	{
		m_vDesiredPosition = en_plPlacement.pl_PositionVector + ((vFront-vRight) / 2 * (plr_fSpeed * 1.5f));
		bButtonAction = TRUE;
