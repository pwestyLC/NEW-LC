...
void PingDB()
{
	...
	mysql_ping(&gserver.m_catalDB);
	mysql_ping(&gserver.m_dbuser);

	//CDBCmd cmd;
	//cmd.Init(&gserver.m_dbauth);
	//if( gserver.m_usebg_users )
	//	cmd.SetQuery("SELECT user_code FROM bg_user limit 0, 1 ");
	//else
	//	cmd.SetQuery("SELECT user_index FROM bill_user limit 0, 1 ");

	//if(cmd.Open())
	//{
	//	cmd.Init(&gserver.m_catalDB);
	//	cmd.SetQuery("select a_ctid from t_catalog limit 0, 1 ");
	//	if(!cmd.Open())
	//	{
	//		GAMELOG << init("BILL")	<< "FAILED OPEN t_catalog DB" << end;
	//	}
	//}
	//else
	//{
	//	if( gserver.m_usebg_users )
	//		GAMELOG << init("BILL") << "FAILED OPEN bg_user DB" << end;
	//	else
	//		GAMELOG << init("BILL")	<< "FAILED OPEN bill_user DB" << end;
	//}
}
