...
public:
	...
	bool Open(const char* sql)
	{
		...
		CCommand<CDynamicAccessor, ATL::CRowset, CMultipleResults> command;
		m_hr = command.Open(m_session, temp);
	...
	long Update(const char* sql)
	{
		...
		CCommand<CDynamicAccessor, ATL::CRowset, CMultipleResults> command;
		if( (m_hr = command.Open(m_session, temp, NULL, &baffected, DBGUID_DEFAULT, false)) == S_OK )
		{
			return baffected;
		}
		return 0;
	...
	bool Get(char* fname, char* val)
	{
		...
		// 유니코드 일때를 위함 
		WideCharToMultiByte(LOCALE, WC_COMPOSITECHECK, (LPCWSTR) p, -1, val, len, NULL, NULL);
