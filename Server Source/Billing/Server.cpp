...
CServer::CServer()
{
	...
	m_usebg_users = false;
	m_enableOnlinecash = false;
	m_onlinecashbalance = 0;
	m_onlinecashtime = 1;
	m_pulseOnlineCash = 0;
...
char* CServer::GetServerPath()
{
	...
	// Force wide API explicitly
	DWORD n = GetModuleFileNameW(GetModuleHandleW(NULL), szBuffer, 1000);
	if (n == 0 || n >= 1000)
		return nullptr;

	int path_len = (int)wcslen(szBuffer);
	int i;
	...
	// Convert wide -> multibyte (UTF-8)
	int bytes = WideCharToMultiByte(CP_UTF8, 0, szBuffer, -1, NULL, 0, NULL, NULL);
	if (bytes <= 0)
		return nullptr;

	char* ret = new char[bytes];
	if (WideCharToMultiByte(CP_UTF8, 0, szBuffer, -1, ret, bytes, NULL, NULL) <= 0) {
		delete[] ret;
		return nullptr;
	}
	return ret;
...
bool CServer::LoadSettingFile()
{
	...
	if( strcmp(gserver.m_config.Find("cash", "USEBGUSERS"), "TRUE") == 0)
		gserver.m_usebg_users = true;
		
	if( strcmp(gserver.m_config.Find("cash", "ENABLEONLINECASH"), "TRUE") == 0)
	{
		gserver.m_enableOnlinecash = true;
		gserver.m_onlinecashbalance = atoi(gserver.m_config.Find("cash", "ONLINECASHBALANCE"));
		gserver.m_onlinecashtime = atoi(gserver.m_config.Find("cash", "ONLINECASHTIME"));
	}
...
bool CServer::ConnectDB()
{
	...
	if (!mysql_real_connect(
		...
		0, NULL, 0))
	{
		GAMELOG << init("Auth DB CONN ERROR") 
				<< end;
#ifdef _WIN64
		Sleep(2500);
#else
	...
	if (!mysql_real_connect(
		...
		0, NULL, 0))
	{
		GAMELOG << init("User DB CONN ERROR") 
				<< end;
