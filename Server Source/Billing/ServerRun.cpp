...
void CServer::OnlineCash()
{
	if (!m_enableOnlinecash)
		return;

	if( m_pulseOnlineCash + (PULSE_REAL_MIN * m_onlinecashtime) <= m_pulse )
	{
		m_pulseOnlineCash = m_pulse;
		// do add cash
		do_onlineCash(m_onlinecashbalance);
	}
}
