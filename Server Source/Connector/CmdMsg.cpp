...
void PlayerRepMsg(CNetMsg::SP& msg)
{
	...
	{
		//Reza Check to force External ip in login server list
		//force ex start
		CLCString strForceEx(10);
		strForceEx.Format("ForceEx");
		int dwForceEx = 0;
		if (strcmp(gserver.m_config.Find("GameServer", (const char*)strForceEx), "") != 0)
		{

			dwForceEx = atoi(gserver.m_config.Find("GameServer", (const char*)strForceEx));
		}
		//force ex stop

		RefMsg(msg) << i;
		if (result[i - 1] && !dwForceEx)// if gameserver%d (i) is connected null if not //reza
		{
			RefMsg(msg) << gserver.m_userList[result[i - 1]->m_subno - 1].m_count;;

#ifdef SETTING_IF_INNER_IP_NEW // ¿ÜºÎ ¾ÆÀÌÇÇ¿Í ³»ºÎ ¾ÆÀÌÇÇ°¡ ³ª´µ¸é »ç¿ë, »õ·Î¿î ¹öÁ¯ ...yhj
			CLCString ex_ip(100);
		...
		else
		{
			...
			char  buffstrip[100] = { 0 };
			char  buffstrport[100] = { 0 };
			snprintf(buffstrip, 100, "EX_IP_%d", i);
			snprintf(buffstrport, 100, "EX_PORT_%d", i);

			if (strcmp(gserver.m_config.Find("GameServer", (const char*)buffstrip), "") != 0)
			{
				char buff[100] = { 0 };

				snprintf(buff, 100, "%s", gserver.m_config.Find("GameServer", (const char*)buffstrip));

				int tempint = 0;
				tempint = atoi(gserver.m_config.Find("GameServer", (const char*)buffstrport));
