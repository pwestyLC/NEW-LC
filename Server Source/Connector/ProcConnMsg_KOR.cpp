...
void ConnCashItemGift(CDescriptor* d, CNetMsg::SP& msg)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		// Step 1: Verify the item exists and is unused
		sql.Format("SELECT a_use_char_idx FROM t_purchase0%d "
			"WHERE a_user_idx = %d AND a_index = %d AND a_ctid = %d AND a_server = %d FOR UPDATE",
			sendUserIndex % 10, sendUserIndex, idx[i], ctid[i], gserver.m_serverno);
		...
		int useCharIdx = 0;
		cmd.GetRec("a_use_char_idx", useCharIdx);
		if (useCharIdx != 0)
		{
			bSuccess = false;
			break;
		...
		// Step 2: Update purchase record to mark as used by sender
		sql.Format("UPDATE t_purchase0%d "
			"SET a_use_char_idx = %d, a_use_date = NOW() "
			"WHERE a_user_idx = %d AND a_use_char_idx = 0 "
			"AND a_index = %d AND a_ctid = %d AND a_server = %d",
		...
		// Step 4: Insert into t_gift table
		sql.Format("INSERT INTO t_gift0%d "
			"(a_server, a_send_user_idx, a_send_char_name, a_send_msg, "
			"a_recv_user_idx, a_recv_char_name, a_send_date, a_ctid) "
			"VALUES (%d, %d, '%s', '%s', %d, '%s', NOW(), %d)",
			recvUserIndex % 10, gserver.m_serverno,
			sendUserIndex, escape_sendName, escape_sendMsg,
			recvUserIndex, escape_recvName, ctid[i]);
