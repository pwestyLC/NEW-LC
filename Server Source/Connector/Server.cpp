...
bool CServer::LoadSettingFile()
{
	...
	strTemp.Format("%s%s", m_serverpath, "connector.txt");
...
bool CServer::ConnectDB()
{
	if (!mysql_real_connect(
				...
				atoi(m_config.Find("Server", "DBPort")), NULL, 0))
	{
				...
				atoi(m_config.Find("Server", "DBPort")), NULL, 0))
	{
		...
		if (!mysql_real_connect(
				&m_dbDB,
				m_config.Find("DB", "IP"),
				m_config.Find("DB", "User"),
				m_config.Find("DB", "Password"),
				m_config.Find("DB", "DBName"),
				atoi(m_config.Find("Server", "DBPort")), NULL, 0))
	{
		LOG_ERROR("can't connect to db : ip[%s] user[%s] password[%s] dbname[%s] error[%s]",
				  m_config.Find("DB", "IP"),
...
bool CServer::InitGame()
{
	...
	if (!cmd.Open())
	{
		GAMELOG << init("SYSTEM_ERROR") << "Failed to open t_ct_limit" << end;
		return false;
	}

	if (cmd.GetRecordCount() < 1)
	{
		GAMELOG << init("SYSTEM") << "t_ct_limit has no records, continuing..." << end;
	}
	else
	{
		int ctid = 0;
		...
		// safely iterate over rows
		while (cmd.MoveNext())
		{
			if (!cmd.GetRec("a_ct_id", ctid) || !cmd.GetRec("a_sell", sell))
			{
			...
			CLimitSell* sellList = new CLimitSell(ctid, sell, false);
			gserver.m_limitSell[ctid] = sellList;
		}
	}
...
void CServer::TimerToGameServer()
{
	...
	if( m_nEventXmasTreeNoticeTime > 0 )
	{
		...
		if( m_bEventXmasTimerStart && m_nEventXmasTreeNoticeTime%6 == 1 )
		{
			...
			CNetMsg::SP rmsg(new CNetMsg);
			ConnEventXmas2007Msg( rmsg , MSG_CONN_EVENT_XMASTREE_ITEMDROP) ;
			RefMsg(rmsg) << (int) 0;		// 0�� ������ MSG ITEM DROP
			gserver.SendAllGameServer(rmsg);
