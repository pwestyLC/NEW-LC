...
bool CAffinityNPC::LoadRewardItem()
{
	...
	m_totalreward = cmd.GetRecordCount();
	m_reward = new REWARDITEM[cmd.GetRecordCount()];
	int k = 0;
	while(cmd.MoveNext())
	{
...
bool CAffinityProto::LoadAffinityWork()
{
	...
	m_npcWorkCount = cmd.GetRecordCount();
	this->m_npcWork = new AFFINITYWORK[cmd.GetRecordCount()];
	...
	m_itemWorkCount = cmd.GetRecordCount();
	this->m_itemWork = new AFFINITYWORK[cmd.GetRecordCount()];
	...
	m_questWorkCount = cmd.GetRecordCount();
	this->m_questWork = new AFFINITYWORK[cmd.GetRecordCount()];
...
bool CAffinityProto::LoadAffinityNPC()
{
	...
	this->m_affinityNPCCount = cmd.GetRecordCount();
	m_affinityNPC = new CAffinityNPC[cmd.GetRecordCount()];
...
bool CAffinityProtoList::LoadAffinity()
{
	...
	m_affinity = new CAffinityProto[cmd.GetRecordCount()];
	m_AffinityTotal = cmd.GetRecordCount();
...
int CAffinity::AddPoint(int point, CPC* pc, int bonus)
{
	...
	if (pc->m_assist.m_avAddition.hcSepAffinityMob) {
		bBoostered = true;
	}
	...
	{
		for(k = 0; k < m_proto->m_affinityNPC[i].m_totalreward; ++k)	// ���� Count
		{
