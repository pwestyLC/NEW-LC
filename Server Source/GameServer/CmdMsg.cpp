	...
	case MSG_CHAR_PC:
		{
			...
			packet->a_honor = ch->m_honor;
			packet->reborn = ch->m_reborn;
			packet->color_name = ch->m_color_name;

			bool isInvisibleHelmet = ch->m_inventory.m_inventoryOptions.GetOptionValue<bool>(INVENTORY_OPTION_INVENTORY_OPTION);
			for (int i = COSTUME2_WEARING_START, c = 0; i < COSTUME2_WEARING_START+MAX_COSTUME_WEARING; ++i, ++c)
			{
				if (isInvisibleHelmet && i == COSTUME2_WEARING_HELMET)
					packet->costume2[c] = -1;
				else if (ch->m_wearInventory.wearItemInfo[i])
			...
			{
				if (isInvisibleHelmet && i == WEARING_HELMET)
				{
			...
			// newslot - alanssoares
			for (int i = WEARING_SPECIAL_ACCESSORY1, c = 0; i <= WEARING_SPECIAL_ACCESSORY4; ++i, ++c)
			{
				if (ch->m_wearInventory.wearItemInfo[i])
					packet->accessory[c] = ch->m_wearInventory.wearItemInfo[i]->getDBIndex();
				else
			...
			if (ch->m_guildInfo && ch->m_guildInfo->guild())
			{
				...
				CDratanCastle * pCastleDratan = CDratanCastle::CreateInstance();
				CWarCastle * pCastleMerac = CWarCastle::GetCastleObject(ZONE_MERAC);
				if (ch->m_guildInfo->guild()->index() == pCastleDratan->GetOwnerGuildIndex())
					packet->guildinfo.guildMark = MARK_GUILD_DRATAN;
				else if (ch->m_guildInfo->guild()->index() == pCastleMerac->GetOwnerGuildIndex())
					packet->guildinfo.guildMark = MARK_GUILD_MERAC;
				else
					packet->guildinfo.guildMark = MARK_GUILD_RANKING;
	...
	case MSG_CHAR_NPC:
		{
			...
			{
				LOG_INFO("AppearMsg: skipping appear for NPC npcIndex=%d dbIndex=%d disableTime=%d", ch->m_index, ch->m_idNum, ch->m_disableTime);
				msg->Init();
				return;
			}
...
// NPC ���ݷ� ���� ǥ�ô� ������ �մϴ�.
	...
	if (ch->m_pZone->m_index == ZONE_START && posX < 1315 && posX > 1270 && posZ < 1030 && posZ > 920)
	{
		...
		if (ch->m_skillCooltime < 100)
		{
			ch->m_skillCooltime = 100;
		}
	}
	...
	if (ch->m_wearInventory.wearItemInfo[WEARING_PET])
	{
		...
		if (apet) 
		{
			if (apet->IsMount())
			{
				walkSpeed = apet->m_walkSpeed;
				runSpeed = apet->GetRunSpeed();
			}
		}
	...
	packet->level = ch->m_level;
	packet->exp = ch->m_exp;
	packet->levelupExp = ch->GetRequiredLevelExp();
	packet->hp = ch->m_hp;
	packet->maxHp = ch->m_maxHP;
	...
	else
	{
		...
		packet->run_or_fly_speed_org = ch->m_runSpeed_org; // (�߰��Ǵ°� �� ��)
	}

	packet->attackSpeed = ch->m_attackSpeed;
	packet->attackSpeedOrg = ch->m_attackSpeed_org;  // (�߰��Ǵ°� �� ��)
	...
	{
		packet->m_cloud_PartyDGCompleteData[i] = ch->m_PartyDGCompleteData[i];
		packet->m_cloud_PartyDGTitleRewardData[i] = ch->m_PartyDGTitleRewardData[i];
	}
#endif
	...
	packet->a_botting = ch->m_botting;
#endif
	// Skill cooldown is now controlled by player state (PK mode, RA mode, Change state, or zone type)
	packet->prob = gserver->GetSkillCooldownValue(ch->m_opDecreaseSkillDelay, ch->m_skillCooltime, ch);
	...
	packet->autosell = ch->m_autosell;
	packet->stats_addition = ch->m_stats_addition;
...
// Variant of AtMsg for offline vendors with no descriptor
void AtMsgOffline(CNetMsg::SP& msg, CPC* ch)
{
	if (!ch || !ch->m_pZone || !ch->m_pArea)
	{
		LOG_ERROR("AtMsgOffline: Invalid character, zone, or area pointer");
		return;
	}
	...
	packet->charIndex = ch->m_index;
	const char* charName = ch->GetName();
	if (charName)
	{
		size_t nameLen = strlen(charName);
		size_t copyLen = (nameLen < (size_t)(MAX_CHAR_NAME_LENGTH - 1)) ? nameLen : (size_t)(MAX_CHAR_NAME_LENGTH - 1);
		memcpy(packet->charName, charName, copyLen);
		packet->charName[copyLen] = '\0';
	}
	else
		packet->charName[0] = '\0';
	...
	packet->r = GET_R(ch);
	packet->yLayer = GET_YLAYER(ch);
	packet->userIndex = 0;  // No descriptor, use 0
	packet->guildindate = ch->m_guild_in_date;
	packet->plusEffect = ch->m_plusEffect;
	packet->mapAttr = ch->GetMapAttr();
...
void MenuMsg(CNetMsg::SP& msg, MSG_MENU_TYPE menutype)
{
	msg->Init(MSG_MENU);
	RefMsg(msg) << (unsigned char)menutype;
}
...
void ItemDropMsg(CNetMsg::SP& msg, CCharacter* ch, CItem* item)
{
	...
	// Add item grade for loot filter color system
	int itemGrade = -1;  // Default to -1 for normal items
	if (item->IsRareItem())
	{
		// Item can have rare options
		if (item->m_pRareOptionProto)
		{
			// Item is already identified - use actual rare option grade (mapped to client enum)
			itemGrade = item->m_pRareOptionProto->GetClientGrade();
			LOG_DEBUG("ItemDropMsg: Rare item IDENTIFIED - DBIndex=%d, DBGrade=%d, ClientGrade=%d (identified)", item->getDBIndex(), item->m_pRareOptionProto->GetGrade(), itemGrade);
		}
		else if (item->m_nRareOptionIndex > 0)
		{
			// Item is unidentified but we know which rare option was assigned - use its grade
			const CRareOptionProto* pRareProto = gserver->m_rareOptionList.Find(item->m_nRareOptionIndex);
			if (pRareProto)
			{
				itemGrade = pRareProto->GetClientGrade();
				LOG_DEBUG("ItemDropMsg: Rare item UNIDENTIFIED - DBIndex=%d, RareIndex=%d, DBGrade=%d, ClientGrade=%d (from m_nRareOptionIndex)", item->getDBIndex(), item->m_nRareOptionIndex, pRareProto->GetGrade(), itemGrade);
			}
			else
			{
				LOG_DEBUG("ItemDropMsg: Rare item UNIDENTIFIED - DBIndex=%d, RareIndex=%d, pRareProto=NULL", item->getDBIndex(), item->m_nRareOptionIndex);
			}
		}
		else if (item->m_itemProto)
		{
			// Fallback: Item can have rare options but no index was assigned - peek at first possible
			// DEBUG: Log all rare indices for this item proto
			LOG_DEBUG("ItemDropMsg: UNIDENTIFIED ITEM FALLBACK - DBIndex=%d, m_nRareOptionIndex=%d; Item proto rare indices:", item->getDBIndex(), item->m_nRareOptionIndex);
			for (int i = 0; i < MAX_RARE_OPTION_ITEM; i++)
			{
				int rareIdx = item->m_itemProto->getItemRareIndex(i);
				int rareProb = item->m_itemProto->getItemRareProb(i);
				if (rareIdx > 0)
				{
					LOG_DEBUG("  [%d] RareIdx=%d, RareProb=%d", i, rareIdx, rareProb);
				}
			}
			int nRareIndex = item->m_itemProto->getItemRareIndex(0);
			if (nRareIndex > 0)
			{
				const CRareOptionProto* pRareProto = gserver->m_rareOptionList.Find(nRareIndex);
				if (pRareProto)
				{
					itemGrade = pRareProto->GetClientGrade();
					LOG_DEBUG("ItemDropMsg: Rare item UNIDENTIFIED - DBIndex=%d, RareIndex=%d, DBGrade=%d, ClientGrade=%d (peeked first)", item->getDBIndex(), nRareIndex, pRareProto->GetGrade(), itemGrade);
				}
				else
				{
					LOG_DEBUG("ItemDropMsg: Rare item UNIDENTIFIED - DBIndex=%d, RareIndex=%d, pRareProto=NULL", item->getDBIndex(), nRareIndex);
				}
			}
			else
			{
				LOG_DEBUG("ItemDropMsg: Rare item UNIDENTIFIED - DBIndex=%d, nRareIndex<=0", item->getDBIndex());
			}
		}
	}
	else
	{
		LOG_DEBUG("ItemDropMsg: Normal item - DBIndex=%d, Grade=-1 (normal)", item->getDBIndex());
	}
	// Normal items (non-rare) always stay NORMAL (-1/gray) - don't use database grade
	RefMsg(msg) << (int)itemGrade;
...
void ItemAppearMsg(CNetMsg::SP& msg, CItem* item)
{
	...
	RefMsg(msg) << (unsigned char)MSG_ITEM_APPEAR
				...
				// Add item grade for loot filter color system
				int itemGrade = eITEM_GRADE_NORMAL;  // Default to normal/gray
				if (item->IsRareItem())
				{
					// Item can have rare options
					if (item->m_pRareOptionProto)
					{
						// Item is already identified - use actual rare option grade (mapped to client enum)
						itemGrade = item->m_pRareOptionProto->GetClientGrade();
						LOG_DEBUG("ItemAppearMsg: Rare item IDENTIFIED - DBIndex=%d, DBGrade=%d, ClientGrade=%d (identified)", item->getDBIndex(), item->m_pRareOptionProto->GetGrade(), itemGrade);
					}
					else if (item->m_nRareOptionIndex > 0)
					{
						// Item is unidentified but we know which rare option was assigned - use its grade
						...
						{
							itemGrade = pRareProto->GetClientGrade();
							LOG_DEBUG("ItemAppearMsg: Rare item UNIDENTIFIED - DBIndex=%d, RareIndex=%d, DBGrade=%d, ClientGrade=%d (from m_nRareOptionIndex)", item->getDBIndex(), item->m_nRareOptionIndex, pRareProto->GetGrade(), itemGrade);
						}
						else
						{
							LOG_DEBUG("ItemAppearMsg: Rare item UNIDENTIFIED - DBIndex=%d, RareIndex=%d, pRareProto=NULL", item->getDBIndex(), item->m_nRareOptionIndex);
						}
					}
					else if (item->m_itemProto)
					{
						...
						if (nRareIndex > 0)
						{
							...
							{
								itemGrade = pRareProto->GetClientGrade();
								LOG_DEBUG("ItemAppearMsg: Rare item UNIDENTIFIED - DBIndex=%d, RareIndex=%d, DBGrade=%d, ClientGrade=%d (peeked first)", item->getDBIndex(), nRareIndex, pRareProto->GetGrade(), itemGrade);
							}
							else
							{
								LOG_DEBUG("ItemAppearMsg: Rare item UNIDENTIFIED - DBIndex=%d, RareIndex=%d, pRareProto=NULL", item->getDBIndex(), nRareIndex);
							}
						}
						else
						{
							LOG_DEBUG("ItemAppearMsg: Rare item UNIDENTIFIED - DBIndex=%d, nRareIndex<=0", item->getDBIndex());
						}
					}
				}
				else
				{
					LOG_DEBUG("ItemAppearMsg: Normal item - DBIndex=%d, Grade=-1 (normal)", item->getDBIndex());
				}
				// Normal items (non-rare) always stay NORMAL (-1/gray) - don't use database grade
				RefMsg(msg) << (int)itemGrade;
}
...
void ItemSealComposeRepMsg(CNetMsg::SP& msg, char result, int total, CLCString name)
{
	msg->Init(MSG_ITEM);
	RefMsg(msg) << (unsigned char)MSG_ITEM_SEAL_COMPOSE_REP
				<< result
				<< total
				<< name;
}

void QuestPbiUseRepMsg(CNetMsg::SP& msg, char result)
{
	msg->Init(MSG_ITEM);
	RefMsg(msg) << (unsigned char)MSG_QUEST_PBI_USE_REQ
				<< result;
				
}

void ItemTransmogrifyRepMsg(CNetMsg::SP& msg, char result)
{
	msg->Init(MSG_ITEM);

	RefMsg(msg) << (unsigned char)MSG_ITEM_TRANSMOGRIFY_REP
				<< result;
}

void ItemExtendMenuRepMsg(CNetMsg::SP& msg, char result)
{
	msg->Init(MSG_ITEM);

	RefMsg(msg) << (unsigned char)MSG_ITEM_EXTEND_MENU_REP
				<< result;
}
...
void MsgWindowLog(CNetMsg::SP& msg, MSG_EX_STRING_Output_TYPE Type, const char* message)
{
	...
	RefMsg(msg) << MSG_EX_STRING
		<< (unsigned char)Type
		<< 8501 //the string that sends messages!
		<< 1
		<< 3
		<< (const char*)message;
}
void MsgWindowLog2(CNetMsg::SP& msg, MSG_EX_STRING_Output_TYPE Type, const char* message)
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_STRING
		<< (unsigned char)Type
		<< 8110 //the string that sends messages!
		<< 1
		<< 3
		<< (const char*)message;
}
...
void ExpSPMsg(CNetMsg::SP& msg, LONGLONG exp, LONGLONG sp)
{
	msg->Init(MSG_EXP_SP);
...
void HonorMsg(CNetMsg::SP& msg, LONGLONG honor)
{
	msg->Init(MSG_HONOR);
...
void QuestNPCListMsg(CNetMsg::SP& msg, int total, int* flag, int* index, int npcIndex)
{
	msg->Init(MSG_QUEST);
...
void PersonalShopSellListMsgOffline(CNetMsg::SP& msg, CPersonalShop::SP offlineShop, int charindex)
{
	LOG_INFO("PersonalShopSellListMsgOffline: ENTER - charindex=%d", charindex);

	if (!offlineShop)
	{
		LOG_ERROR("PersonalShopSellListMsgOffline: offlineShop is NULL");
		msg->Init();
		return;
	}

	char normalcount = offlineShop->GetNormalCount();
	LOG_INFO("PersonalShopSellListMsgOffline: normalcount=%d", normalcount);
	...
	if (normalcount)
	{
		...
		for (i = 0; i < normalcount; i++)
		{
			LOG_INFO("PersonalShopSellListMsgOffline: Processing normal item %d/%d", i+1, normalcount);

			pos = offlineShop->GetNextNormalItem(pos);
			if (pos == -1)
			{
				LOG_ERROR("PersonalShopSellListMsgOffline: GetNextNormalItem returned -1 at iteration %d", i);
				msg->Init();
				return;
			}

			int itemindex = offlineShop->GetNormalItemIndex(pos);
			int itemDBIndex = offlineShop->GetNormalItemDBIndex(pos);
			const PSItemFidelity* fidelity = offlineShop->GetNormalItemFidelity(pos);

			LOG_INFO("PersonalShopSellListMsgOffline: Item pos=%d, index=%d, dbindex=%d, fidelity=%p", 
				pos, itemindex, itemDBIndex, fidelity);

			if (!fidelity || !fidelity->valid)
			{
				LOG_ERROR("PersonalShopSellListMsgOffline: Invalid fidelity - fidelity=%p, valid=%d", 
					fidelity, fidelity ? fidelity->valid : -1);
				msg->Init();
				return;
			}

			LONGLONG count = offlineShop->GetNormalItemCount(pos);
			LONGLONG price = offlineShop->GetNormalItemPrice(pos);

			LOG_INFO("PersonalShopSellListMsgOffline: Creating item from fidelity - dbindex=%d, count=%lld", 
				itemDBIndex, count);

			CItem* tempItem = CreateItemFromOfflineShopFidelity(fidelity, itemDBIndex, count);
			if (!tempItem)
			{
				LOG_ERROR("PersonalShopSellListMsgOffline: CreateItemFromOfflineShopFidelity returned NULL");
				msg->Init();
				return;
			}

			LOG_INFO("PersonalShopSellListMsgOffline: Item created successfully, serializing packet data");

			RefMsg(msg) << itemindex
						<< tempItem->getDBIndex()
						<< tempItem->getPlus()
						<< tempItem->getFlag()
						<< tempItem->getNowDurability()
						<< tempItem->getMaxDurability();

			if (tempItem->IsOriginItem())
			{
				RefMsg(msg) << (char)tempItem->GetOriginOptionCnt();
				for (int j = 0; j < MAX_ORIGIN_OPTION; j++)
				{
					if (tempItem->GetOriginIndex(j) > -1)
					{
						RefMsg(msg) << (char)tempItem->GetOriginIndex(j)
									<< (int)tempItem->GetOriginLevel(j);
						RefMsg(msg) << tempItem->getOriginVar(j);
					}
				}
			}
			else
			{
				RefMsg(msg) << (char)tempItem->m_nOption;
				int j;
				for (j = 0; j < tempItem->m_nOption; j++)
				{
					RefMsg(msg) << (char)tempItem->m_option[j].m_type
								<< (int)tempItem->m_option[j].m_level;
				}
			}
			...
			if (tempItem->IsOriginItem())
			{
				RefMsg(msg) << (char)tempItem->GetBelongType()
							<< (char)tempItem->GetOriginSkillCnt();
				for (int j = MAX_ORIGIN_OPTION; j < MAX_ORIGIN; j++)
				{
					if (tempItem->GetOriginIndex(j) > -1)
					{
						RefMsg(msg) << tempItem->GetOriginIndex(j)
									<< (char)tempItem->GetOriginLevel(j);
					}
				}
			}

			ItemPublicMsg(msg, tempItem, false);
			LOG_INFO("PersonalShopSellListMsgOffline: ItemPublicMsg done");

			ItemSocketMsg(msg, tempItem);
			LOG_INFO("PersonalShopSellListMsgOffline: ItemSocketMsg done");

			delete tempItem;
			LOG_INFO("PersonalShopSellListMsgOffline: Item cleaned up");
		}
		LOG_INFO("PersonalShopSellListMsgOffline: Finished processing all normal items");
	}

	LOG_INFO("PersonalShopSellListMsgOffline: Checking for package items, type=%d", offlineShop->GetType());

	if (offlineShop->GetType() & PST_PACKAGE)
	{
		char packagecount = offlineShop->GetPackageCount();
		LOG_INFO("PersonalShopSellListMsgOffline: packagecount=%d", packagecount);
		RefMsg(msg) << offlineShop->GetPackagePrice()
					<< packagecount;
		...
		if (packagecount > 0)
		{
			...
			{
				pos = offlineShop->GetNextPackageItem(pos);
				if (pos == -1)
				{
					msg->Init();
					return;
				...
				int itemindex = offlineShop->GetPackageItemIndex(pos);
				int itemDBIndex = offlineShop->GetPackageItemDBIndex(pos);
				...
				LONGLONG count = offlineShop->GetPackageItemCount(pos);
				LONGLONG price = offlineShop->GetPackagePrice();
	...
	LOG_INFO("PersonalShopSellListMsgOffline: COMPLETE - msg size=%d bytes", msg->m_size);
}
...
// RomeoST CraftSystem //Craft system
void MsgCraftResult(CNetMsg::SP & msg, MSG_Craft_TYPE typeError)
{
	msg->Init(MSG_CRAFT);
	RefMsg(msg) << (unsigned char)typeError;
}
...
void GuildPointInfo(CNetMsg::SP& msg, LONGLONG guildpoint)
{
	msg->Init(MSG_GUILD);
...
void HelperSaveGuildPointMsg(CNetMsg::SP& msg, int guildindex, LONGLONG guildpoint )
{
	msg->Init( MSG_HELPER_COMMAND );
...
void HelperNewGuildPointUpdate( CNetMsg::SP& msg, int charindex, int guildindex, LONGLONG point )
{
	msg->Init( MSG_HELPER_COMMAND );
...
void CastleCristalRespondWaitMsg(CNetMsg::SP& msg, CPC * ch)
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_CASTLE_WAR
				<< (unsigned char)MSG_CASTLE_CRISTAL_RESPOND_WAIT
				<< ch->m_index;
}
...
void SendBuffNPCReqMsg( CNetMsg::SP& msg, MSG_BUFF_NPC_2021_TYPE type, unsigned char error )
{
	msg->Init(MSG_EVENT);
	RefMsg(msg) << (unsigned char)MSG_BUFF_NPC_REQ
				<< (unsigned char)type
				<< error;
}

void SendBuffNPCReqMsg( CNetMsg::SP& msg, MSG_BUFF_NPC_2021_TYPE type )
{
	msg->Init(MSG_EVENT);
	RefMsg(msg) << (unsigned char)MSG_BUFF_NPC_REQ
				<< (unsigned char)type;
...
#ifdef DEMIGOD
void HelperEventDemiGodMsg(CNetMsg::SP& msg, int nUserIdx, int nCharIdx)
{
	msg->Init(MSG_HELPER_COMMAND);
	RefMsg(msg) << MSG_HELPER_EVENT_DEMIGOD
		<< nUserIdx
		<< nCharIdx;
}
#endif
...
#ifdef DEMIGOD
void EventDemiGodMsg(CNetMsg::SP& msg, MSG_EVENT_DEMIGOD_ERROR_TYPE eError)
{
	msg->Init(MSG_EVENT);
	RefMsg(msg) << (unsigned char)MSG_EVENT_DEMIGOD
		<< (unsigned char)eError;
}
#endif
...
#ifdef DEMIGOD
void EventDemiGod_SuccessMsg(CNetMsg::SP& msg)
{
	msg->Init(MSG_EVENT);
	RefMsg(msg) << (unsigned char)MSG_EVENT_DEMIGOD_SUCCESS;
}
#endif
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
void ExpedPartyDungeon_CallMembersMsg(CNetMsg::SP& msg, CPC* ch, int zone)
{
	//if(ch == NULL)
		//return;
	...
	msg->Init(MSG_EXPEDITION);
	RefMsg(msg) << MSG_PARTYDUNGEON_RECALL_RECV
		<< ch->GetName()
		<< zone;
}
#ifdef CLOUD_EXPEDITION_REWORK
bool ExpedDungeonDetail_Msg(CNetMsg::SP& msg, CPC* ch, int Mode)
{
	int nCount = gserver->m_PartyConfig.GetZoneCount(Mode);

	msg->Init(MSG_EXPEDITION);
	RefMsg(msg) << MSG_PARTYDUNGEON_SEND
		<< nCount;
	for (int i = 0; i < gserver->m_PartyConfig.m_PartyExpeditions.size(); i++)
	{

		if (gserver->m_PartyConfig.m_PartyExpeditions[i].difficulty == Mode)
		{
			RefMsg(msg) << (int)gserver->m_PartyConfig.m_PartyExpeditions[i].zoneid
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].difficulty
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].level
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].cp
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].needitem
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].needcount
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].partymember
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].time
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].rewardindex
				//<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].count
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].rewardindex1
				//<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].count1
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].rewardindex2
				//<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].count2
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].rewardindex3
				//<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].count3
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].rewardindex4
				//<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].count4
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].title
				<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].box;
			//<< (int)gserver->m_PartyConfig.m_PartyExpeditions[i].boxcount;
		}
...
//PASSIVE_SYSTEM
//new 16-08-2022
void PassiveSystemListMsg(CNetMsg::SP& msg, CPC* ch)
{
	if (!ch)
		return;

	CPassiveSystemList::map_t& list = ch->m_passiveSystemList.getList();
	CPassiveSystemList::map_t::iterator it = list.begin();
	CPassiveSystemList::map_t::iterator endit = list.end();

	for (; it != endit; ++it)
	{
		CPassiveSystemList::PassiveSystem p = it->second;
		RefMsg(msg) << p->m_index;
	}
}

void PassiveSystemSlotListMsg(CNetMsg::SP& msg, CPC* ch)
{
	if (!ch)
		return;

	CPassiveSystemList::map_t& list = ch->m_passiveSystemSlot.getList();
	CPassiveSystemList::map_t::iterator it = list.begin();
	CPassiveSystemList::map_t::iterator endit = list.end();
...
void WearingInvisibleMsg(CNetMsg::SP& msg, CPC* ch, int isVisible)
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_INVENTORY_OPTION
		<< (unsigned char)INVENTORY_OPTION_INVENTORY_OPTION
		<< isVisible;
}
...
void CostumeWearingSuitMsg(CNetMsg::SP& msg, CPC* ch)
{
	...
	for( int i = 0; i < COSTUME2_SUIT_MAX_WEARING; i++ )
	{
		...
		if (isInvisibleHelmet && pos == COSTUME2_WEARING_HELMET)
		{
			RefMsg(msg) << -1;
		}
		else if( ch->m_wearInventory.wearItemInfo[pos] )
		{
...
void SysMsgBossRegen(CNetMsg::SP& msg, CLCString name, CLCString zone, int posX, int posZ)
{
	msg->Init(MSG_SYS);
	RefMsg(msg) << (unsigned char)MSG_SYS_BOSS_REGEN
				<< name 
				<< zone
				<< posX << posZ;
}
...
void SysMsgBossDead(CNetMsg::SP& msg, CLCString of, CLCString df, int hour, int min, int sec)
{
	msg->Init(MSG_SYS);
	RefMsg(msg) << (unsigned char)MSG_SYS_BOSS_DEAD
				<< of << df
				<< hour << min << sec;
}
...
void SysMsgFlagDead(CNetMsg::SP& msg, CLCString of, int posX, int posZ)
{
	msg->Init(MSG_SYS);
	RefMsg(msg) << (unsigned char)MSG_SYS_FLAG_DEAD
				<< of 
				<< posX << posZ;
}

void SysMsgFlagDC(CNetMsg::SP& msg, CLCString of, int posX, int posZ)
{
	msg->Init(MSG_SYS);
	RefMsg(msg) << (unsigned char)MSG_SYS_FLAG_DC
		<< of
		<< posX << posZ;
}

void SysMsgFlagPickup(CNetMsg::SP& msg, CLCString df, int posX, int posZ)
{
	msg->Init(MSG_SYS);
	RefMsg(msg) << (unsigned char)MSG_SYS_FLAG_PICKUP
				<< df 
				<< posX << posZ;
}
...
void RoyalRumbleWinner(CNetMsg::SP& msg, int leveltype, int charindex, CLCString nick, int zoneidx)
{
...
void RoyalRumbleTimeReq(CNetMsg::SP& msg, int time, int minute)
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_ROYAL_RUMBLE
				<< (unsigned char)MSG_EX_ROYAL_RUMBLE_TIME_REP
...
void MsgrRoyalRumbleSettingReq(CNetMsg::SP& msg, const int min, const int max, const int startHour, const int startMinute, const int charindex)
{
	msg->Init(MSG_MSGR_REQ);
	RefMsg(msg) << -1
				<< -1
...
void SubHelperUpdateDonationEventStatus(CNetMsg::SP& msg, bool isActive, time_t startTime, time_t endTime)
{
	msg->Init(MSG_SUBHELPER_COMMAND);
	RefMsg(msg) << MSG_SUBHELPER_DONATION_EVENT_UPDATE_STATUS
		<< isActive
		<< startTime 
		<< endTime;
}

void MoveBattleWarfareMsg(CNetMsg::SP& msg, int index)
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_BATTLE_WARFARE_REP
				<< index;
}
void SubHelperUpdateDonationEventReward(CNetMsg::SP& msg, int playerId)
{
	msg->Init(MSG_SUBHELPER_COMMAND);
	RefMsg(msg) << MSG_SUBHELPER_DONATION_EVENT_UPDATE_REWARD << playerId;
}