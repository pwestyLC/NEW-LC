...
void DBManager::_SaveCharaterQuery(std::vector<std::string> & vec, CPC* pChar, int table_no)
{
	...
	updateQuery += boost::str(boost::format(", a_exp_original = %1%") % pChar->m_exp_original);
	...
	updateQuery += boost::str(boost::format(", a_honor=%d") % pChar->m_honor);
	updateQuery += boost::str(boost::format(", a_reborn=%d") % pChar->m_reborn);
	updateQuery += boost::str(boost::format(", a_stats_addition=%d") % pChar->m_stats_addition);
	updateQuery += boost::str(boost::format(", a_event_summon_count=%d") % pChar->m_eventSummonCount);

#ifdef DAILY_LOGIN
	updateQuery += boost::str(boost::format(", a_day_counter=%d") % 	pChar->m_DLdaycount );
	
	char wbuffer[255] ={0};

	for(int i = 0 ; i < 35 ; i ++) {
		char cbuffer[255] = {0};
		snprintf(cbuffer,255,"%s",wbuffer);
		snprintf(wbuffer,255,"%s %d", cbuffer, pChar->m_DLddaycheck[i] <= 0 ? 0 : 1);
		//LOG_INFO("HIT DAILY LOGIN Loop ::%s::", wbuffer);
	}
	char* pwbuffer = &wbuffer[0];
	updateQuery += boost::str(boost::format(", a_weeks_concurrent= '%s'") % pwbuffer);
	//LOG_INFO("HIT DAILY LOGIN ::%s::", pwbuffer);
	...
	updateQuery += boost::str(boost::format(", a_level_original=%d") % pChar->m_level_original);
	...
	//updateQuery += boost::str(boost::format(", a_prestige = %1%") % pChar->m_prestige);
	updateQuery += boost::str(boost::format(", a_prestige_level = %1%") % pChar->m_prestigeLevel);
	updateQuery += boost::str(boost::format(", a_prestige_points = %1%") % pChar->m_prestigePoints);
	updateQuery += boost::str(boost::format(", a_prestige_premium = %1%") % pChar->m_prestigePremium);
#endif
...
#ifdef DEMIGOD
	updateQuery += boost::str(boost::format(", a_demigod_active=%d") % pChar->m_DemiGod);
#endif
...
//PASSIVE_SYSTEM
/*void DBManager::_SaveCharacterPassiveQuery(std::vector<std::string>& vec, CPC* pChar)
{
	std::string deleteQuery = boost::str(boost::format("DELETE FROM t_characters_passive WHERE a_char_index = %d") % pChar->m_index);
	vec.push_back(deleteQuery);

	if (pChar->m_passiveSystemSlot.GetCount() <= 0 && pChar->m_passiveSystemList.GetCount() <= 0)
		return;

	std::string insertQuery;
	insertQuery.reserve(MAX_DB_QUERY);
	insertQuery = boost::str(boost::format("INSERT INTO t_characters_passive (a_char_index, a_passive_slot_pos, a_passive_slot_id, a_passive_list) VALUES (%d,'") % pChar->m_index);

	// slots pos
	CPassiveSystemList::map_t& list = pChar->m_passiveSystemSlot.getList();
	...
	for (; it != endit; ++it)
	{
		...
		insertQuery += boost::str(boost::format(" %d") % p->m_slot);
	}
	insertQuery += "', '";

	// slots id
	list = pChar->m_passiveSystemSlot.getList();
	it = list.begin();
	endit = list.end();
	for (; it != endit; ++it)
	{
		CPassiveSystemList::PassiveSystem p = it->second;
		insertQuery += boost::str(boost::format(" %d") % (p->m_slot != -1 ? p->m_index : -1));
	}
	insertQuery += "', '";

	// list
	list = pChar->m_passiveSystemList.getList();
	...
	{
		CPassiveSystemList::PassiveSystem p = it->second;
		insertQuery += boost::str(boost::format(" %d") % p->m_index);
	}
	insertQuery += "')";
...
void DBManager::_SaveCharacterPassiveQuery(std::vector<std::string>& vec, CPC* pChar)
{
	// Step 1: Initialize default values for slots
	std::vector<int> slotPositions(3, -1); // Default: -1 for unfilled slots
	std::vector<int> slotIds(3, -1);       // Default: -1 for unfilled slots
	std::set<int> uniqueBooks;            // Track unique passive books

	// Step 2: Parse existing data
	std::vector<PassiveData> existingData = DBProcess::GetCurrentCharacterPassiveData(pChar->m_index);
	for (const auto& entry : existingData)
	{
		if (entry.slotPos >= 0 && entry.slotPos < 3 && entry.slotId >= 1 && entry.slotId <= 12)
		{
			slotPositions[entry.slotPos] = entry.slotPos;
			slotIds[entry.slotPos] = entry.slotId;
		}

		// Parse passive list
		std::istringstream listStream(entry.passiveList);
		int bookId;
		while (listStream >> bookId)
		{
			if (bookId >= 1 && bookId <= 12)
			{
				uniqueBooks.insert(bookId);
			}
		}
	}

	// Step 3: Update slots and books from in-memory data
	for (const auto& [key, passiveSystem] : pChar->m_passiveSystemSlot.getList())
	{
		if (passiveSystem && passiveSystem->m_index >= 1 && passiveSystem->m_index <= 12)
		{
			// Update slot position and ID
			if (passiveSystem->m_slot >= 0 && passiveSystem->m_slot < 3)
			{
				slotPositions[passiveSystem->m_slot] = passiveSystem->m_slot;
				slotIds[passiveSystem->m_slot] = passiveSystem->m_index;
			}

			// Add to passive list
			uniqueBooks.insert(passiveSystem->m_index);
		}
	}

	for (const auto& [key, passiveSystem] : pChar->m_passiveSystemList.getList())
	{
		if (passiveSystem && passiveSystem->m_index >= 1 && passiveSystem->m_index <= 12)
		{
			uniqueBooks.insert(passiveSystem->m_index);
	...
	// Step 4: Reset unequipped slots explicitly
	for (int i = 0; i < 3; ++i)
	{
		// If no passive is equipped in this slot, reset to -1
		if (slotIds[i] == -1 || pChar->m_passiveSystemSlot.Find(slotIds[i]) == nullptr)
		{
			slotPositions[i] = -1;
			slotIds[i] = -1;
		}
	...
	// Step 5: Generate strings for a_passive_slot_pos, a_passive_slot_id, and a_passive_list
	std::ostringstream posStream, idStream, listStream;
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		posStream << slotPositions[i];
		idStream << slotIds[i];
	}

	for (int bookId : uniqueBooks)
	{
		listStream << bookId << " ";
	}

	std::string passiveSlotPos = posStream.str();
	std::string passiveSlotId = idStream.str();
	std::string passiveList = listStream.str();
	if (!passiveList.empty())
	{
		passiveList.pop_back(); // Remove trailing space
	}

	// Debugging Logs
	LOG_INFO("Constructed a_passive_slot_pos: %s", passiveSlotPos.c_str());
	LOG_INFO("Constructed a_passive_slot_id: %s", passiveSlotId.c_str());
	LOG_INFO("Constructed a_passive_list: %s", passiveList.c_str());

	// Step 6: Build the SQL query
	std::ostringstream queryStream;
	queryStream << "INSERT INTO t_characters_passive (a_char_index, a_passive_slot_pos, a_passive_slot_id, a_passive_list) VALUES "
		<< boost::format("(%d, '%s', '%s', '%s')") % pChar->m_index % passiveSlotPos % passiveSlotId % passiveList
		<< " ON DUPLICATE KEY UPDATE "
		<< "a_passive_slot_pos = VALUES(a_passive_slot_pos), "
		<< "a_passive_slot_id = VALUES(a_passive_slot_id), "
		<< "a_passive_list = VALUES(a_passive_list)";

	// Debugging Log for Query
	LOG_INFO("Generated Query: %s", queryStream.str().c_str());

	vec.push_back(queryStream.str());
}



void DBManager::_SaveCharacterInventoryOptions(std::vector<std::string>& vec, CPC* ch)
{
	std::string deleteQuery = boost::str(boost::format("DELETE FROM t_inventory_options WHERE a_char_idx = %d") % ch->m_index);
	vec.push_back(deleteQuery);
	std::string insertQuery = "INSERT INTO t_inventory_options (a_char_idx, a_type, a_value) VALUES ";
	for (int i = 0; i < INVENTORY_OPTION_MAX; i++)
	{
		int value = ch->m_inventory.m_inventoryOptions.GetOptionValue<bool>(i);
		insertQuery += boost::str(boost::format("(%d, %d, %d)") % ch->m_index % i % value);
		if (i != INVENTORY_OPTION_MAX - 1)
		{
			insertQuery += ", ";
		}
	}
...
void DBManager::SaveCharacterInfo(CDescriptor* d, bool disconnect)
{
	...
	// On disconnect, force save everything; otherwise only save dirty subsystems
	unsigned int dirtyFlags = disconnect ? SAVE_DIRTY_ALL : pChar->m_saveDirtyFlags;

	// If nothing is dirty and not disconnecting, skip the save entirely
	if (dirtyFlags == 0)
	{
		delete pushdata;
		return;
