...
void DBProcess::CreateChar(boost::any& argv)
{
	...
	try
	{
...

		{
			...
			if (char_db_.getrowcount() >= maxCharCount)	// ĳ���� ���� ����
		...
		if (pChar->m_hairstyle > 5 || pChar->m_hairstyle < 1)
			pChar->m_hairstyle = 1;
		if (pChar->m_facestyle > 5 || pChar->m_facestyle < 1)
			pChar->m_facestyle = 1;	
...
#ifdef HANARO_EVENT
		{
			...
			else if (pChar->m_hanaroCode == "SK" || pChar->m_hanaroCode == "sk")
			{
				...
				nItemCount = nSKCount;
				nItemUsed = gserver->getNowSecond() + 30 * 24 * 60 * 60;	// 1����
				switch (pChar->m_job)
				{
				case JOB_TITAN:
			...
			else
			{
				...
				{
				#ifdef DEFAULT_STARTER_ITEMS
					{ 882,  883, 4966, 6595 }, // IDs
					{ 1000, 1000,   5,    5 }  // Counts
				#else
					//NOT DEFAULT
					{ 0, 0, 0, 0 },
					{ 0, 0, 0, 0 }
...
#ifdef ENABLE_DEFAULT_HANARO_ITEMS
				nItemCount = 4;
#else
				nItemCount = 0;
#endif

				// Create items and set belong flag
				for (int i = 0; i < nItemCount; ++i)
				{
					CItem* pItem = gserver->m_itemProtoList.CreateItem(
						nItemList[0][i], // Item ID
						-1,
						nItemList[1][i], // Count
						0, 0
					);
				...
				// Use the same nItemList arrays in SQL loop
				for (int i = 0; i < nItemCount; i++)
				{
					CItemProto* itemproto = gserver->m_itemProtoList.FindIndex(nItemList[0][i]);
					if (!itemproto)
						continue;
					...
					sql4 += boost::str(boost::format(", %1%, %2%, %3%, %4%, '%5%', %6%, %7%, %8%")
						% nItemList[0][i] % 0 % -1 % FLAG_ITEM_BELONG % serial % nItemList[1][i] % nItemUsed % 0);

					icol++;

					// When icol hits 4, flush query as one full row
					if (icol == 4)
					{
						query = sql1 + sql3 + sql2 + sql4 + ")";
						if (pChar->m_job != JOB_NIGHTSHADOW)
				...
				// Final flush for any remaining items (<4)
				if (icol != 0)
				{
					query = sql1 + sql3 + sql2 + sql4 + ")";
					if (pChar->m_job != JOB_NIGHTSHADOW)
					...
					// No need to increment irow here since it's last usage
				}
			}
		}
		...
		if (pChar->m_job != JOB_NIGHTSHADOW)
		{
			irow = 3;			//3¹øºÎÅÍ ÃÊ¹Ý Áö±Þ ¾ÆÀÌÅÛ È®Àå
			icol = 0;
