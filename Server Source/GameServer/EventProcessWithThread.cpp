...
#ifdef OFFLINE_SHOPS
	// Process offline shop settlement payouts when character logs in
	ProcessOfflineShopSettlementOnLogin(desc->m_pChar);

	// Restore owner's offline shop if they have one
	RestoreOfflineShopOnLogin(desc->m_pChar);
#endif
