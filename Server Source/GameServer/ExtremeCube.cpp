...
{
	return (gserver->m_subno == EXTREME_CUBE_SUBSERVER || gserver->m_subno == 3);
}
...
bool CExtremeCube::LoadCubeData()
{
	...
	m_mobCount = cmd.GetRecordCount();
	m_mobidx = new int[m_mobCount];
	memset(m_mobidx, 0, sizeof(int) * cmd.GetRecordCount());
	...
	m_bossCount = cmd.GetRecordCount();
	m_bossidx = new int[m_bossCount];
	memset(m_bossidx, 0, sizeof(int) * cmd.GetRecordCount());
	...
	m_regencount = cmd.GetRecordCount();

	m_regenPoint = new CUBEREGENPOINT[cmd.GetRecordCount()];
	...
	m_regcount = cmd.GetRecordCount();
	m_regdate = new REGDATE[cmd.GetRecordCount()];
