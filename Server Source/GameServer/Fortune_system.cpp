...
int Fortune_head::Get_item_index() const //pwesty c++17
{
	return Item_index;
}
...
Fortune_head* Fortune_proto_list::Get_fortune_head_proto(int Item_index)  //pwesty c++17
{ 
	auto compare_function = [&](const Fortune_head* fh) { return fh->Get_item_index() == Item_index; };
	auto it = std::find_if(fh.begin(), fh.end(), compare_function);
	if (it == fh.end()) {
