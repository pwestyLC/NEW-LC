...
Inventory::~Inventory()  //pwesty fix memory leak
{
...
void Inventory::Init(CPC* owner, unsigned int tab, unsigned int size, bool time_flag, bool money_flag)
{
	...
	// Initialize vectors directly with the right size and default values
	buffer_ = std::vector<CItem*>(size, nullptr);   // size elements, all nullptr
	virtual_count_ = std::vector<int>(size, 0);     // size elements, all 0
}
...
// itemIndex�� �ش��ϴ� ��� �������� ��ġ�� ������ ���ؼ� vector�� �ִ´�.
// Helper lambda to push a search result
auto push_pair = [&](size_t i, CItem* pItem, item_search_t& vec, unsigned int tab) {
	item_search_pair_t pair;
	pair.tab = tab;
	pair.invenIndex = static_cast<int>(i);
	pair.pItem = pItem;
	vec.push_back(pair);
	};
...
// 1. Search by DB Index (all items)
int Inventory::searchItemByDBIndex(unsigned int itemIndex, item_search_t& vec)
{
	...
	for (size_t i = 0; i < buffer_.size(); ++i)
	{
		...
		if (!pItem || pItem->m_itemProto->getItemIndex() != itemIndex)
			continue;

		total += pItem->Count();
		push_pair(i, pItem, vec, tab_);
	}
	return total;
}

// 2. Search by DB Index excluding worn items
int Inventory::searchItemByDBIndexWithoutWearItem(unsigned int itemIndex, item_search_t& vec)
{
	...
	{
		CItem* pItem = buffer_[i];
		if (!pItem || pItem->m_itemProto->getItemIndex() != itemIndex || pItem->m_wearPos != WEARING_NONE)
			continue;
...
// 3. Search by Condition (index, plus, flag)
int Inventory::searchItemByCondition(unsigned int itemIndex, unsigned int plus, unsigned int flag, item_search_t& vec)
{
	...
	{
		CItem* pItem = buffer_[i];
		if (!pItem || pItem->m_itemProto->getItemIndex() != itemIndex || pItem->getPlus() != plus || pItem->getFlag() != flag)
			continue;
...
int Inventory::searchFlagByItemProto(unsigned int flag, item_search_t& vec)
{
	...
	{
		CItem* pItem = buffer_[i];
		if (!pItem || (pItem->m_itemProto->getItemFlag() & flag) == 0)
			continue;
...
void Inventory::searchWearItem(item_search_t& vec)
{
	...
	{
		CItem* pItem = buffer_[i];
		if (!pItem || pItem->getWearPos() == WEARING_NONE)
			continue;
...
bool Inventory::addItem(CItem* pitem, addItemResult& retVal)
{
	...
	{
		item_search_t vec;
		searchItemByCondition(pitem->m_itemProto->getItemIndex(), pitem->getPlus(), pitem->getFlag(), vec);

		for (auto& p : vec)
		{
			if ((pitem->Count() + p.pItem->Count()) > p.pItem->m_itemProto->getStackCount())
				continue;
	...
	CNetMsg::SP rmsg(new CNetMsg);
	auto* packet = reinterpret_cast<UpdateClient::addItemInfo*>(rmsg->m_buf);
...
bool Inventory::increaseCountItem( unsigned int index, unsigned int count )
{
	...
	LONGLONG newCount = pItem->m_count + (LONGLONG)count;
	if (newCount > pItem->m_itemProto->getStackCount() || newCount < 0)
		return false;
...
bool Inventory::decreaseCountItem( unsigned int index, unsigned int count, bool& deleteFlag )
{
	CItem* pItem = buffer_[index];
	LONGLONG newCount = pItem->m_count - (LONGLONG)count;
	if (pItem == NULL || pItem->m_count < 1 || count < 1) // fix crash
	{
		newCount = 0;
		deleteFlag = true;//reza itemdrop hack
		LOG_ERROR("not enought item count. now count[%d], delete count[%d]", pItem->Count(), count);
		return deleteItem(index);//reza itemdrop hack
...
void Inventory::sort()
{
	...
	{
		// ������ ó�� (���� �������� ������ �ִٸ� ���Ӱ� ó������)
		for (size_t page = 0; page < QUICKSLOT_PAGE_NUM; ++page)
		{
			for (size_t slot = 0; slot < QUICKSLOT_MAXSLOT; ++slot)
			{
				if (owner_->m_quickSlot[page].m_slotType[slot] != QUICKSLOT_TYPE_ITEM)
					continue;

				CItem* item = owner_->m_quickSlot[page].m_item[slot];
				if (!item)
					continue;
...
bool InventoryManager::inserItemInStash(CItem* pItem)
{
	...
	for (size_t i = 0; i < pInven.size_; ++i)
	{
		if (pInven[i])  // nullptr check
			continue;
...
CItem * const InventoryManager::getItemOld( unsigned int row, unsigned int col)
{
	...
	if (static_cast<size_t>(invenIndex) >= pInven.size_)
		return nullptr;
...
bool InventoryManager::addItem( CItem* pitem )
{
	...
	owner_->SetSaveDirty(SAVE_DIRTY_INVEN);
...
bool InventoryManager::addItemInStash(CItem* pitem)
{
	...
	owner_->SetSaveDirty(SAVE_DIRTY_STASH);
...
bool InventoryManager::decreaseItemCount(unsigned int tab, unsigned int index, unsigned int count)
{
	...
	Inventory& pInven = inven_[tab];
	bool delete_flag = false;
	bool result = pInven.decreaseCountItem(index, count, delete_flag);
	if (result)
		owner_->SetSaveDirty(SAVE_DIRTY_INVEN);
	return result;
}
...
LONGLONG InventoryManager::increaseMoney( LONGLONG nas, LONGLONG bonus )
{
	...
	owner_->SetSaveDirty(SAVE_DIRTY_CHAR);
...
{
	//if( NULL != owner_->m_wearInventory.wearing_cost_set ) // [110208:selo] �� �� �ǻ�
	//{
	//	CNetMsg::SP rmsg(new CNetMsg);
	...
	//{
	//	CNetMsg::SP rmsg(new CNetMsg);
	//	PreInventoryMsg(rmsg, owner_);	// Inventory ������ �ѱ�� ���� �ڽ�Ƭ ������ ���� �ѱ��.
	//	SEND_Q(rmsg, owner_->m_desc);
	//}
		...
		for (int j = 0; j < loopSize; ++j)
		{
					...
					&& pItem->m_itemProto->getItemSubTypeIdx() == IETC_MONSTER_MERCENARY_CARD)
			{
				if( pItem->getUsed() != -1 && pItem->getUsed() < gserver->getNowSecond() )
				{
					...
					if( gift1 )
					{
						if (owner_->GiveItem(6259, 0,0,1, true ) == false)  // 6259 ���� �뺴ī�� ����1
						{
					...
					if( gift2 )
					{
						if (owner_->GiveItem(6260, 0,0,1, true ) == false) // 6260 ���� �뺴ī�� ����2
						{
							GAMELOG << init("MONSTER MERCENARY FAIL :: Give item", owner_)
									<< "item index : " << 6260 << end;
						}
            ...
            #if defined (REZABOT) || defined(PET_BUFF_ALL_SYSTEM)
			else if (pItem->m_itemProto->getItemTypeIdx() == ITYPE_ONCE && 
					(pItem->m_itemProto->getItemFlag() & ITEM_FLAG_ABS) &&
					(pItem->getUsed() < gserver->getNowSecond()))
			{
					GAMELOG << init("STUFF EXPIRE", owner_)
							<< itemlog(pItem)
							<< end;
			...
			else if( pItem->IsUsedPlatinumSpecial() )
			{
				if (pItem->getUsed() < gserver->getNowSecond())
				{
					...
					bool isInvisibleHelmet = owner_->m_inventory.m_inventoryOptions.GetOptionValue<bool>(INVENTORY_OPTION_INVENTORY_OPTION);
...
{
	item_search_t vec;
	this->searchFlagByItem(FLAG_ITEM_COMPOSITION, vec);
	...
	for (; it != endit; ++it)
	{
		...
		if (pitem->m_itemProto->getItemFlag() & ITEM_FLAG_COMPOSITE ||
		pitem->getFlag() & FLAG_ITEM_TRANSMOGRIFY)
		{
			// �ռ��� �ǻ��� �ش� ��� ã�ƺ���
...
{
	item_search_t vec;
	this->searchFlagByItemProto(ITEM_FLAG_CASTLLAN, vec);
	item_search_t::iterator it = vec.begin();
	item_search_t::iterator endit = vec.end();
	for (; it != endit; ++it)
	{
...
bool InventoryManager::isPossibleAddInStash( std::vector<possible_search_t>& search_vec )
{
	...
	{
		CItem* item = p.first;
		if (item->m_itemProto->getItemFlag() & ITEM_FLAG_COUNT)		// ��ġ�Ⱑ �����ϴٸ�
		{
			item_search_t vec;
