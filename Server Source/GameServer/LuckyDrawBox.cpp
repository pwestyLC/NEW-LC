...
bool CLuckyDrawBox::Load()
{
	...
	if( dbQuest.GetRecordCount() <= 0)
		return false;

	m_nCount = dbQuest.GetRecordCount();
	LuckyDrawBoxData * td = new LuckyDrawBoxData[dbQuest.GetRecordCount()];
	int j=0;
	do
	{
		...
		dbQuest1.Open();
		if(dbQuest1.GetRecordCount() > 0 )
		{
			td[j].nNeedCount = dbQuest1.GetRecordCount();
			LuckyDrawBoxNeed * ntd = new LuckyDrawBoxNeed[dbQuest1.GetRecordCount()];
			int i=0;
...
//					dbQuest1.GetRec("a_Luckydraw_idx",	ntd[i].lucky_idx);
		...
		if(dbQuest1.GetRecordCount() > 0 )
		{
			...
			td[j].nResultCount = dbQuest1.GetRecordCount();
			LuckyDrawBoxResult * rtd = new LuckyDrawBoxResult[dbQuest1.GetRecordCount()];
			int i=0;
			do
			{
//				dbQuest1.GetRec("a_index",			rtd[i].index);
