...
bool CMissionCaseProto::LoadProto()
{
	...
	m_nRegenCount = cmd.GetRecordCount();
	m_npcRegen = new MOBREGEN[m_nRegenCount]; // constructor initializes step=0, typecount=0, mobtype=nullptr
	...
	while (cmd.MoveNext())
	{
		...
		CLCString sql2(1024);
		sql2.Format("SELECT * FROM t_npc_regen_combo WHERE a_step = %d AND a_missioncase_idx = %d AND a_enable = 1",
			step, m_nIndex);
		...
		if (cmd2.GetRecordCount() == 0)
			return false;

		m_npcRegen[i].step = step;
		m_npcRegen[i].typecount = cmd2.GetRecordCount();
		m_npcRegen[i].mobtype = new MOBTYPE[cmd2.GetRecordCount()](); // value-initialized
...
bool CMIssionCaseList::LoadList()
{
	...
	m_nCount = cmd.GetRecordCount();
	m_proto = new CMissionCaseProto[cmd.GetRecordCount()];
...
bool CGroundEffect2::Init(int zone, CArea* area)
{
	...
	for (i = 0; i < MAX_GROUND_EFFECT2_NPC_NUM; ++i)
	{
		...
		else if (m_area->m_zone->m_index == ZONE_PARTY_DUNGEON0 || m_area->m_zone->m_index == ZONE_PARTY_DUNGEON1 || m_area->m_zone->m_index == ZONE_PARTY_DUNGEON2 || m_area->m_zone->m_index == ZONE_PARTY_DUNGEON3 || m_area->m_zone->m_index == ZONE_PARTY_DUNGEON4)
		{

			if (i < MAX_GROUND_EFFECT2_NPC_NUM - 12)
			{
				GET_X(npc) = (m_area->m_zone->m_zonePos[1][1] + m_area->m_zone->m_zonePos[1][3]) / 4;
				GET_Z(npc) = (m_area->m_zone->m_zonePos[1][2] + m_area->m_zone->m_zonePos[1][4]) / 4;
			}
			else if (i < MAX_GROUND_EFFECT2_NPC_NUM - 9)
			{
				GET_X(npc) = (m_area->m_zone->m_zonePos[3][1] + m_area->m_zone->m_zonePos[3][3]) / 4;
				GET_Z(npc) = (m_area->m_zone->m_zonePos[3][2] + m_area->m_zone->m_zonePos[3][4]) / 4;
			}
			else if (i < MAX_GROUND_EFFECT2_NPC_NUM - 5)
			{
				GET_X(npc) = (m_area->m_zone->m_zonePos[4][1] + m_area->m_zone->m_zonePos[4][3]) / 4;
				GET_Z(npc) = (m_area->m_zone->m_zonePos[4][2] + m_area->m_zone->m_zonePos[4][4]) / 4;
			}
			else if (i < MAX_GROUND_EFFECT2_NPC_NUM)
			{
				GET_X(npc) = (m_area->m_zone->m_zonePos[2][1] + m_area->m_zone->m_zonePos[2][3]) / 4;
				GET_Z(npc) = (m_area->m_zone->m_zonePos[2][2] + m_area->m_zone->m_zonePos[2][4]) / 4;
			}
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
	else if (m_zonenum == ZONE_PARTY_DUNGEON0 || m_zonenum == ZONE_PARTY_DUNGEON1 || m_zonenum == ZONE_PARTY_DUNGEON2 || m_zonenum == ZONE_PARTY_DUNGEON3 || m_zonenum == ZONE_PARTY_DUNGEON4)
	{
		m_pulseFirst = gserver->m_pulse + PULSE_REAL_SEC * 20;
		m_pulseSecond = m_pulseFirst + PULSE_REAL_SEC * 2;
	}
#endif
	else
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
	else if (m_zonenum == ZONE_PARTY_DUNGEON0)
	{
		effectCount = MAX_GROUND_EFFECT2_NPC_NUM;
		effectKind = 1;// fogo
	}
	else if (m_zonenum == ZONE_PARTY_DUNGEON1)
	{
		effectCount = MAX_GROUND_EFFECT2_NPC_NUM;
		effectKind = 3;// raio
	}
	else if (m_zonenum == ZONE_PARTY_DUNGEON2)
	{
		effectCount = MAX_GROUND_EFFECT2_NPC_NUM;
		effectKind = 4;//vento
	}
	else if (m_zonenum == ZONE_PARTY_DUNGEON3)
	{
		effectCount = MAX_GROUND_EFFECT2_NPC_NUM;
		effectKind = 5;// gelo
	}
	else if (m_zonenum == ZONE_PARTY_DUNGEON4)
	{
		effectCount = MAX_GROUND_EFFECT2_NPC_NUM;
		effectKind = 2;// terra
	}
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
		else if (m_zonenum == ZONE_PARTY_DUNGEON0 || m_zonenum == ZONE_PARTY_DUNGEON1 || m_zonenum == ZONE_PARTY_DUNGEON2 || m_zonenum == ZONE_PARTY_DUNGEON3 || m_zonenum == ZONE_PARTY_DUNGEON4)// || m_zonenum == ZONE_PK_TOURNAMENT)// || m_zonenum == ZONE_START)
		{
			CArea* pArea = NULL;
			CPC* FindPC = NULL;
			pArea = GetArea();
			if (pArea)
			{
				...
				int cell_ext = CELL_EXT;

#ifdef INCREASECELLRENDER

				getNewCell_ext(m_zonenum, cell_ext);
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
			|| m_zonenum == ZONE_PARTY_DUNGEON0
			|| m_zonenum == ZONE_PARTY_DUNGEON1
			|| m_zonenum == ZONE_PARTY_DUNGEON2
			|| m_zonenum == ZONE_PARTY_DUNGEON3
			|| m_zonenum == ZONE_PARTY_DUNGEON4
#endif		  
			)
		{
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
									else if ((ch->m_pZone->m_index == ZONE_PARTY_DUNGEON0
										|| ch->m_pZone->m_index == ZONE_PARTY_DUNGEON1
										|| ch->m_pZone->m_index == ZONE_PARTY_DUNGEON2
										|| ch->m_pZone->m_index == ZONE_PARTY_DUNGEON3
										|| ch->m_pZone->m_index == ZONE_PARTY_DUNGEON4
