...
bool CMonsterMercenary::LoadData()
{
	...
	for(i=0; i<m_count; i++)
	{
		...
		mmd[i].nNpcCount = cmd.GetRecordCount();
		...
		if( m_Data[i].nClassIdx == classIdx)
		{
			...
			else
			{
				...
				for(j=0; j<m_Data[i].nNpcCount; j++)
				{
					...
					{
						pCData->nWalkSpeed	= m_Data[i].npcInfo[j].walkSpeed * 2;
						pCData->nRunSpeed	= m_Data[i].npcInfo[j].runSpeed * 2;
...
bool CMonsterMercenary::SetMercenaryData( CItem* pItem, int classIdx, int npcIndex)
{
	...
	pItem->setPlus_2(pItem->getPlus_2() + (att << 8)); // HMM maybe the plus will be too high?
