...
void CNPC::DeleteNPC()
{
	if (m_bScriptedMovement && gserver)
	{
		gserver->RemoveMovementTasksForNPC(this);
	}
	DelAttackList(this);
	delete this;
}
...
void CNPC::ResetStat()
{
	...
	m_opMeleeHitRate = m_proto->m_nHit;
	m_opMagicHitRate = m_proto->m_nHit;
	...
	if( GetOwner() && this == GetOwner()->GetSummonNpc(SUMMON_NPC_TYPE_MERCENARY) && GetMercenaryClassData() )
	{
		...
		m_maxMP = m_maxHP = GetOwner()->m_level * GetMercenaryClassData()->nHpMp + (GetOwner()->m_maxHP /  2);
		m_recoverMP = m_recoverHP = GetOwner()->m_level * 5;

		m_eqMelee	= GetOwner()->m_level * GetMercenaryClassData()->nAttDef + GetMercenaryClassData()->nBonusAtt;
		m_eqMelee += GetOwner()->m_eqMelee + (GetOwner()->m_eqMelee * 5.00); 
		m_eqMagic	= GetOwner()->m_level * GetMercenaryClassData()->nAttDef + GetMercenaryClassData()->nBonusAtt;
		m_eqMagic += GetOwner()->m_eqMagic + (GetOwner()->m_eqMagic * 5.00);
		m_eqDefense	= GetOwner()->m_level * GetMercenaryClassData()->nAttDef * 2 + GetMercenaryClassData()->nBonusDef;
		m_eqDefense += GetOwner()->m_eqDefense / 2;
		m_eqResist	= GetOwner()->m_level + GetMercenaryClassData()->nBonusDef;
		m_eqResist += GetOwner()->m_eqResist / 2;
		m_opMeleeHitRate += GetOwner()->m_opMeleeHitRate + (GetOwner()->m_opMeleeHitRate *.3);
		m_opMagicHitRate += GetOwner()->m_opMagicHitRate + (GetOwner()->m_opMagicHitRate * .3);
		

#ifdef SYSTEM_MONSTER_MERCENARY_CARD_EX
		m_walkSpeed	= GetMercenaryClassData()->nWalkSpeed * 2;
		//m_runSpeed	= GetMercenaryClassData()->nRunSpeed;
		SetRunSpeed(GetMercenaryClassData()->nRunSpeed * 2);
