...
#endif
#include "CmdMsg.h"  //pwesty npctalk
///////////////////
// ANPCPtoro member
...
int CNPCProto::getSkillPointForPremiumChar( int premium_type )
{
	...
	{
	case PREMIUM_CHAR_TYPE_FIRST:
		skillpoint = (LONGLONG)(ori_skillpoint * 1.1f);
		break;
...
bool CNPCProtoList::Load()
{
	...
	while (dbNpc.MoveNext())
	{
		...
		dbNpc.GetRec("a_flag1",			npcflag1);		//Ȯ�� �÷���(�߰�)
		...
		m_npcProtoList[i].m_flag1 = npcflag1;		//Ȯ�� �÷���(�߰�)
		...
		if (gserver->m_subno !=1 && gserver->m_subno != 3 && npcIndex == 1705)
		{
			continue;  //pvp flag carrier npc disable on non pk channels pwesty
		}
...
CNPC* CNPCProtoList::Create(int npcindex, CNPCRegenInfo* regenInfo)
{
	...
	npc->m_regenTime = gserver->getNowSecond(); // ���� �ð��� ���� �Ѵ�.
#endif
...
bool CNPCProto::IsBossInList()
{
    if (std::find(gserver->m_bossList.begin(), gserver->m_bossList.end(), this->m_index) != gserver->m_bossList.end()) {
        return true;
    }
    return false;
}