...
bool Notice::SendNoticeList(int charIndex, CNetMsg::SP& msg)
{
	if (!msg || !msg->m_buf)
	{
		LOG_ERROR("SendNoticeList: Invalid msg or msg->m_buf is null");
		return false;
	}
	...
	if (!pc)
	{
		LOG_ERROR("SendNoticeList: No player found for charIndex %d", charIndex);
		return false;
	}
	if (!pc->m_desc)
	{
		LOG_ERROR("SendNoticeList: Player descriptor is null");
		return false;
	}

	auto* p = reinterpret_cast<RequestClient::NoticeList*>(msg->m_buf);
	int pageNum = p->pageNum;

	const size_t vecSize = _vec.size();
	if (vecSize == 0)
	{
		LOG_ERROR("SendNoticeList: Notice vector is empty");
		return false;
	}

	constexpr int MAX_NOTICE_COUNT = 10;
	size_t startIndex = pageNum * MAX_NOTICE_COUNT;
	if (startIndex >= vecSize)
	{
		LOG_ERROR("SendNoticeList: startIndex (%zu) >= vecSize (%zu)", startIndex, vecSize);
		return false;
	}

	size_t remaining = vecSize - startIndex;
	int count = static_cast<int>((remaining >= MAX_NOTICE_COUNT) ? MAX_NOTICE_COUNT : remaining);

	CNetMsg::SP rmsg(new CNetMsg);
	auto* packet = reinterpret_cast<ResponseClient::NoticeList*>(rmsg->m_buf);
	packet->type = MSG_EXTEND;
	packet->subType = MSG_EX_NOTICE;
	packet->thirdType = MSG_EX_NOTICE_LIST;
	packet->count = count;
	packet->isNextPage = (vecSize > startIndex + count);

	for (int j = 0; j < count; ++j)
	{
		NoticeStr* notice = _vec[startIndex + j];
		if (!notice)
		{
			LOG_ERROR("SendNoticeList: Null notice pointer at index %zu", startIndex + j);
			continue;
		}

		packet->data[j].noticeRev = notice->index;
		strncpy(packet->data[j].noticeString, notice->notice_string, sizeof(packet->data[j].noticeString) - 1);
		packet->data[j].noticeString[sizeof(packet->data[j].noticeString) - 1] = '\0';
		packet->data[j].date = notice->date;
	}

	// ✅ FIXED: Don't overcalculate size — struct already contains data[10]
	rmsg->setSize(sizeof(ResponseClient::NoticeList));
...
void Notice::load()
{
	...
	if (!dbcmd.Open())
	{
		LOG_INFO("Notice Date Load Error. mysql_error(%s)", mysql_error(dbcmd.GetDbConn()));
		return ;
	}
	...
	while(dbcmd.MoveNext())
	{
		...
		const char* db_str = dbcmd.GetRec("a_notice_string");
		if (db_str)
		{
			strncpy(data->notice_string, db_str, sizeof(data->notice_string) - 1);
			data->notice_string[sizeof(data->notice_string) - 1] = '\0';
		}
		else
		{
			data->notice_string[0] = '\0';
		}
...
void PopupNotice::load()
{
	...
	if (!dbcmd.Open())
	{
		LOG_INFO("PopupNotice Date Load Error. mysql_error(%s)", mysql_error(dbcmd.GetDbConn()));
		return ;
	}
...
void PopupNotice::save_db(PopupNoticeData* data)
{
	...
	if (!dbcmd.Update())
	{
		LOG_INFO("PopupNotice Date Save Error. mysql_error(%s)", mysql_error(dbcmd.GetDbConn()));
		return ;
	}
}
