	...
	: m_name(PS_MAX_SHOPNAME + 1)
{
	...
	memset(m_normalitemdbindex, -1, sizeof(int) * PS_MAX_NORMAL_ITEM);
	memset(m_normalitemid, 0, sizeof(int) * PS_MAX_NORMAL_ITEM);
	memset(m_normalCount, 0, sizeof(LONGLONG) * PS_MAX_NORMAL_ITEM);
	memset(m_normalPrice, 0, sizeof(LONGLONG) * PS_MAX_NORMAL_ITEM);
#ifdef OFFLINE_SHOPS
	memset(m_normalFidelity, 0, sizeof(m_normalFidelity));
#endif
	memset(m_packageitemindex, -1, sizeof(int) * PS_MAX_PACKAGE_ITEM);
	memset(m_packageitemdbindex, -1, sizeof(int) * PS_MAX_PACKAGE_ITEM);
	memset(m_packageitemid, 0, sizeof(int) * PS_MAX_PACKAGE_ITEM);
	memset(m_packageCount, 0, sizeof(LONGLONG) * PS_MAX_PACKAGE_ITEM);
#ifdef OFFLINE_SHOPS
	memset(m_packageFidelity, 0, sizeof(m_packageFidelity));
#endif
	m_packagePrice = 0;

#ifdef OFFLINE_SHOPS
	// Initialize offline mode and persistence fields
...
bool CPersonalShop::AddItem(bool bNormal, int itemindex, int itemdbindex, LONGLONG count, LONGLONG price)
{
	...
	if (bNormal)
	{
		...
		m_normalitemdbindex[i] = itemdbindex;
	...
	else
	{
		...
		m_packageitemdbindex[i] = itemdbindex;
