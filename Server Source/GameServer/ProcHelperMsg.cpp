...
#ifdef DEMIGOD
void ProcHelperEventDemiGod( CNetMsg::SP& msg);
#endif
	...
	case MSG_HELPER_COMMAND:
		{
			...
			case MSG_HELPER_GUILD_MEMBER_ADD_NTF:		// ��� ��� �߰� �˸�
...
void ProcHelperPD4RewardRep(CNetMsg::SP& msg)
{
	...
	//MSG_QUEST_PD4_REWARD_REP,					// ���� ��û : charindex(n)
}
...
void ProcHelperGuildMemberList(CNetMsg::SP& msg)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		case MSG_GUILD_POSITION_MEMBER:				// �Ϲ� ����
			member->pos(pos);
			break;
		}
...
void ProcHelperGuildLevelUpRep(CNetMsg::SP& msg)
{
	...
	if (errcode == MSG_GUILD_ERROR_OK)
	{
		...
		else if( guild->level() >= GUILD_EXTEND_BASE_LEVEL )
		{
			...
			int	gap = guild->level() - GUILD_EXTEND_BASE_LEVEL;  // �̷̹����� ����
			if( gap >= 0 )
			{
	...
	for (int i = 0; i < GUILD_MAX_MEMBER; i++)
	{
		if (guild->m_member[i] != NULL)
		{
			if (guild->m_member[i]->GetPC() != NULL
				&& guild->m_member[i]->online() == 1				// �¶��� ������ ���
				&& guild->m_member[i]->GetPC()->m_desc != NULL)	// ��ũ���Ͱ� ���� �ϴ� ���
			{
...
void ProcHelperGuildMemberOutNotify(CNetMsg::SP& msg)
{
	...
	if(member->GetPC())
	{
		...
		member->GetPC()->m_inventory.CastllanItemRemove(-1, false, true); // �ʸ� �����.
	}
...
void ProcHelperGuildMemberKickNotify(CNetMsg::SP& msg)
{
	...
	if (pc)
	{
		...
		time_t t;
		pc->m_guild_in_date = time(&t) / 60 / 60 / 24;
#endif
...
// >>
	...
	if( pDratanCastle->GetOwnerCharIndex() == current )
	{
		...
		if( pc )
		{
			...
			pc->m_inventory.CastllanItemRemove(-1, false, true); // �ʸ� �����.
		}
	}
	...
	if( cast && cast->GetOwnerGuildIndex() == guild->index() )
	{
		...
		if( preboss )
		{
			...
			preboss->m_inventory.CastllanItemRemove(-1, false, true); // �ʸ� �����.
		}
	...
	//						}
			...
			boss->m_inventory.CastllanItemRemove(-1, false, true); // �ʸ� �����.
		}
	}
...
void ProcHelper_FriendAddRep(CNetMsg::SP& msg)
{
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		ResponseClient::FriendErrorMsg(rmsg, (MSG_FRIEND_ERROR_TYPE)packet->errorCode);//��û�ڿ��� �������θ� �˸���.
	...
	else
	{
		if (reqPc)//���ÿ� ������ �־�� �Ѵ�.
		{
			...
			{
				CNetMsg::SP rmsg(new CNetMsg);
				ResponseClient::FriendAddNotify(rmsg, appPc->m_index, appPc->GetName(), appPc->m_job, appPc->m_nCondition); //�³��ڿ��� ����� �߰����� �˸�
				SEND_Q(rmsg, reqPc->m_desc);
			}
		}
		...
		if(appPc)
		{
			...
			CNetMsg::SP rmsg(new CNetMsg);
			ResponseClient::FriendAddNotify(rmsg, reqPc->m_index, reqPc->GetName(), reqPc->m_job, reqPc->m_nCondition); //��û�ڿ��� ����� �߰����� �˸�.
...
void ProcHelper_GiftRecvChar(CNetMsg::SP& msg) //pwesty dupe
{
	int count = 0;
	...
	if (count < 1 || count > 10)
	{
		...
		GAMELOG << init("GIFT RECV ERROR: Invalid count", ch)
			<< "count: " << count
			<< end;
...
void ProcHelperTeacherLoadRep(CNetMsg::SP& msg)
{
	...
	if( d && d->m_pChar && d->m_pChar->m_index )
	{
		...
		else if( d->m_pChar->m_teachType == MSG_TEACH_STUDENT_TYPE ) // �л�
		{
			CLCString teachName(MAX_CHAR_NAME_LENGTH + 1);
...
void ProcHelperFameupRep(CNetMsg::SP& msg)
{
	...
	if(tch)
	{
		...
		if( tch->m_displayCanSstone
			&& ((tch->m_fame >= 1000  && tch->m_superstone < 1 )
			|| (tch->m_fame >= 2000 && tch->m_superstone < 2 )
			|| (tch->m_fame >= 3000 && tch->m_superstone < 3)) )
		{
			CNetMsg::SP rmsg(new CNetMsg);
...
void ProcHelperPartyInviteReq(CNetMsg::SP& msg)
{
	...
	if (pTargetPC->m_Exped || pBossPC->m_Exped )	// ������ ���¿��� ��Ƽ�ʴ� �Ұ�
		return;
...
void ProcHelperEventPhoenix( CNetMsg::SP& msg )
{
	...
	LOG_INFO("Phoenix msg type result: %d ", (int)type);
	// �ɸ��� ��񿡼� �˻����� �� �Ǵн� �̺�Ʈ�� �����ϴٴ� ������ ���� ���
...
void ProcHelperEventDemiGod(CNetMsg::SP& msg)
{
	...
	LOG_INFO("DemiGod msg type result: %d ", (int)type);
	// �ɸ��� ��񿡼� �˻����� �� �Ǵн� �̺�Ʈ�� �����ϴٴ� ������ ���� ���
	if ((MSG_HELPER_EVENT_DEMIGOD_ERROR_TYPE)type == MSG_HELPER_EVENT_DEMIGOD_OK)
	{
		CNetMsg::SP rmsg(new CNetMsg);
		EventDemiGodMsg(rmsg, MSG_EVENT_DEMIGOD_OK);
		SEND_Q(rmsg, ch->m_desc);
		return;
	}

	// �ɸ��� ��񿡼� �˻����� �� ������ 100�� �Ѵ� ���� ��� �Ǵн� �̺�Ʈ �ڰ��� �ȵȴٴ� ����
	else if ((MSG_HELPER_EVENT_DEMIGOD_ERROR_TYPE)type == MSG_HELPER_EVENT_DEMIGOD_ERROR_REQUIREMENT)
	{
		CNetMsg::SP rmsg(new CNetMsg);
...
void ProcHelperExped_CreateRep(CNetMsg::SP& msg)
{
	...
	int* pCharIndex		 = new int[nMemberCount];			// ĳ���� �ε���
	CLCString* pCharName = new CLCString[nMemberCount];		// ĳ���� �̸�
	int* pGroupType		 = new int[nMemberCount];			// �׷� Ÿ��
	int* pMemberType	 = new int[nMemberCount];			// ��� Ÿ��
	int* pSetLabelType	 = new int[nMemberCount];			// ǥ��
	int* pQuitType		 = new int[nMemberCount];			// ���� ,������
...
void ProcHelperExped_KickRep(CNetMsg::SP& msg)
{
	...
	pExped->DeleteMember(nTargetIndex);
	if(pTargetPC)
		pTargetPC->SetExpedLabel(-1); // ���� �ʱ�ȭ ���ش�.
...
void ProcHelperInzoneGetRoomnoRep( CNetMsg::SP& msg)
{
	...
	CZone* pZone=NULL;
	//int nCurrentZoneNo = ch->m_nJoinInzone_ZoneNo; 34�������� 33�������� �̵� �� �Ա��� �̵���Ŵ.
	int nCurrentZoneNo = ch->m_pZone->m_index;

	if((nZoneNo==ZONE_ALTER_OF_DARK || nZoneNo==ZONE_AKAN_TEMPLE
			|| nZoneNo == ZONE_TARIAN_DUNGEON || nZoneNo == ZONE_ENCHANTED_DUNGEON
		) && nRoomNo != nBossRoomNo)
	{
		//����: �ͼӵ� ���� ������ �������� �޶� ���� �Ұ�
		CNetMsg::SP rmsg(new CNetMsg);
	...
	if(pZone && pZone->m_area && pZone->m_zonePos)
	{
		...
		if (ch->IsParty() && ch->m_index == nBossIndex)
		{
			ch->m_party->m_nDifficulty = nDifficulty;
		}
#endif
		int extra = 0;
		...
		if(ch->IsExped() && pZone->IsExpedRaidZone())
		{
			...
			if( (area->m_zone->m_index == ZONE_AKAN_TEMPLE
					...
					|| area->m_zone->m_index == ZONE_ENCHANTED_DUNGEON
				)
				&& area->GetPCCount() >= 8)		
			{
				CNetMsg::SP rmsg(new CNetMsg);
...
void ProcHelperWorldcup2010(CNetMsg::SP& msg)
{
	...
	case MSG_HELPER_WORLDCUP2010_TOTO_GIFT_REP:
		switch(nErrType)
		{
		case MSG_HELPER_WORLDCUP2010_TOTO_GIFT_ERROR_SUC: // ������ �ְ� ��~
			{
				int nTableCount = 7;
				int giftTable[7][2]=
				{	// item_idx,  count
				...
				if( ch->m_inventory.getEmptyCount() < nTableCount ) // �κ� �˻�
				{
	...
	case MSG_HELPER_WORLDCUP2010_ATTENDANCE_REP:
		switch(nErrType)
		{
		case MSG_HELPER_WORLDCUP2010_ATTENDANCE_ERROR_SUC: // ���� ī�带 ��������
			{
				int supportIndex = 19;
	...
	case MSG_HELPER_WORLDCUP2010_KOREA_REP:
		{
			int item[7][5] = {
				{5819, 5820, 5821, 5822, 5823},		// Ÿ��ź
				{5824, 5825, 5826, 5827, 5828},		// ����Ʈ
				{5804, 5805, 5806, 5807, 5808},		// ����
				{5809, 5810, 5811, 5812, 5813},		// ������
				{5814, 5815, 5816, 5817, 5818},		// �α�
				{5829, 5830, 5831, 5832, 5833},		// �Ҽ���
				{5834, 5835, 5836, 5837, 5838}		// ����Ʈ������
			};
			int i, j;
			...
			case MSG_HELPER_WORLDCUP2010_KOREA_ERROR_EMPTY:
				{
					...
					HelperWorldcup2010KoreaRepMsg(rmsg, ch->m_index, 1);	// ���� �Ϸ� ��û
					SEND_Q(rmsg, gserver->m_helper);
					return;
				}
				break;
			case MSG_HELPER_WORLDCUP2010_KOREA_ERROR_SUC:
				{
					...
					for( i = 0; i < 5; i++ )
					{
						if( ch->m_inventory.addItem(pItem[i]) == false)
						{
							...
							HelperWorldcup2010KoreaRepMsg(rmsg, ch->m_index, 2);	// ���� ���� ��û
							SEND_Q(rmsg, gserver->m_helper);
							return;
						}
						else
	...
	case MSG_HELPER_TEACH_RENEWER_GIFT_SUC:
		{
			int nCount;
			RefMsg(msg) >> nCount;
			int itemIndex = GUARDIAN_ITEM_REWARD; // �İ����� ��ǥ
			...
			case MSG_EVENT_CHAR_BIRTHDAY_ERROR_SUC:
				{
					...
					if( years > 10 ) // 10���� �Ѿ�� 10�� ¥�� ���������� �ش�.
						years = 10;

					int giftindex= 6717 + years; // ����ǰ�� 6718 ~ 6727 (1�ֳ���� 10�ֳ����)
...
void ProcHelperGuildMarkExpire(CNetMsg::SP& msg)
{
	// helper���� ��帶ũ �ð��� ����ϰ� ���ᰡ �Ǿ����� ���Ӽ����� �޾Ƽ� Ŭ���̾�Ʈ���� �ѷ��ش�.
...
void ProcHelperCastleStateCync(CNetMsg::SP& msg)
{
	...
	if( pMercWar )
	{
		if( gserver->m_subno != WAR_CASTLE_SUBNUMBER_MERAC) // ���� ä���� �������� �ʿ䰡 ����.
	...
	if( pDratanWar )
	{
		if( gserver->m_subno != WAR_CASTLE_SUBNUMBER_DRATAN) // ���� ä���� �������� �ʿ䰡 ����.
...
void ProcHelperGuildStashKeep(CNetMsg::SP& msg)
{
	...
	// Helper ���� �� error ó��
	// 1,2,3 �ΰ�� ���� Helper �� Unlock �ϰ�, 4 �ΰ��� ������ ���� // error 1,2,3,4 �� Ŭ�� ����
	switch ( error )
	{
	case 0:		// ����
		break;
	case 1: case 2: case 3:
		{
			CNetMsg::SP rmsg(new CNetMsg);
			HelperGuildStashKeepErrorMsg( rmsg, charIndex, guildIndex, 1 );			// 1. ����
			SEND_Q( rmsg, gserver->m_helper );
		}
	case 4:		// �ٸ� ������ �����
	case 9:		// ��� â�� ���� ����
...
void ProcHelperGuildStashTake(CNetMsg::SP& msg)
{
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildStashKeepErrorMsg( rmsg, charIndex, guildIndex, 1 );				// Helper �� UnLock ��û
		SEND_Q( rmsg, gserver->m_helper );
		return;
	}
	...
	LOG_INFO("Error guild take %d", error);
	
	// Helper ���� �� error ó��
	// 1,2,3,5 �ΰ�� ���� Helper �� Unlock �ϰ�, 4 �ΰ��� ������ ���� // error 1,2,3,4 �� Ŭ�� ����
	switch ( error )
	{
	case 0:		// ����
		break;
	...
	if( pc->m_inventory.getEmptyCount() < itemCount )	// 2: �κ� ����
	{
		{
			CNetMsg::SP rmsg(new CNetMsg);
	...
	if( bGiveGuildItem )	// ������ �˻��ߴµ� �� ������...
	{
		...
		{
			CNetMsg::SP rmsg(new CNetMsg);
			HelperGuildStashTakeErrorMsg( rmsg, charIndex, guildIndex, 0, takeMoney );			// 0: ����ó��
			SEND_Q( rmsg , gserver->m_helper );
		}

		{
			CNetMsg::SP rmsg(new CNetMsg);
			GuildStashErrorMsg( rmsg, MSG_NEW_GUILD_STASH_TAKE , 0 );	 // 0: ����ó��
			SEND_Q( rmsg, pc->m_desc );
		}
	}
	else 	// ���� �սô�.
	{
		for( int delIdx=0 ; delIdx<errorIndex ; ++delIdx )
			delete ppGuildItem[delIdx];		// Create �� Item ����

		{
			CNetMsg::SP rmsg(new CNetMsg);
			HelperGuildStashTakeErrorMsg( rmsg, charIndex, guildIndex, 3 );			// 3: ���� ���� ( ���� �ʰ� )
			SEND_Q( rmsg , gserver->m_helper );
		}

		{
			CNetMsg::SP rmsg(new CNetMsg);
			GuildStashErrorMsg( rmsg, MSG_NEW_GUILD_STASH_TAKE , 3 );	 // 3: �κ� ���� �ʰ�
			SEND_Q( rmsg, pc->m_desc );
		}
	}
...
void ProcHelperGuildStashError(CNetMsg::SP& msg)
{
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		GuildStashErrorMsg( rmsg, MSG_NEW_GUILD_STASH_ERROR, error );	 // ���ۿ��� ���� �״�� ��������
		SEND_Q( rmsg, pc->m_desc );
	}
}
#endif // DEV_GUILD_STASH
