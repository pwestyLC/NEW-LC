	...
	{
		{
			// �����ڰ� ���� ��
			CNetMsg::SP rmsg(new CNetMsg);
			ChatGMWhisperMsg(rmsg, sidx, /*sname*/ "GM", receiver->GetName(), chat);		// ���� �������� ������ �޽��� Ŭ���̾�Ʈ�� ����
			SEND_Q(rmsg, receiver->m_desc);
		}
...
void ProcessMessengerMessage_Winner(CNetMsg::SP& msg)
{
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		RoyalRumbleWinner(rmsg, leveltype, charindex, nick, 38);
		PCManager::instance()->sendToAll(rmsg);
	}
...
void ProcessMsgrEventActiveUpdate(CNetMsg::SP& msg)
{
	...
	case MSG_EX_EVENT_AUTOMATION_ADD:
		...
		// �̺�Ʈ�� ���� �̺�Ʈ ���̺� �ʱ�ȭ �� ��.
		// ������ �̺�Ʈ(35)		CharDB.t_event_gomdori_2007		UserDB.t_event_gomdori_count
		// �ҷ��� �̺�Ʈ(49)		CharDB.t_event_halloween2007
		// ũ�������� �̺�Ʈ(50)	UserDB.t_event_xmastree_point
	...
	case MSG_EX_EVENT_AUTOMATION_REMOVE:
		...
		{
			// ũ�������� �̺�Ʈ CharDB.t_pet.a_pet_turnto_npc ������Ʈ �ʿ�
			CNetMsg::SP rmsg(new CNetMsg);
			rmsg->Init(MSG_HELPER_COMMAND);
			RefMsg(rmsg) << MSG_HELPER_EVENT_TABLE_INIT << eventIndex;
			SEND_Q(rmsg, gserver->m_helper);
...
void ProcessMsgrRoyalRumbleSetting(CNetMsg::SP& msg)
{
	int min = 0, max = 0, startHour = 0, startMinute = 0, charindex = 0;
	RefMsg(msg) >> min >> max >> startHour >> startMinute >> charindex;
	CLCString str(1024);
	...
	else
	{
		...
		gserver->m_RoyalRumble.setStartMinute(startMinute);
		CDBCmd cmd;
		cmd.Init(&gserver->m_dbdata);
		CLCString sql(1024);
		sql.Format("UPDATE t_royalrumble SET a_minPlayer = %d, a_maxPlayer = %d, a_startHour = %d, a_startMinute = %d", min, max, startHour, startMinute);
		cmd.SetQuery(sql);
		if(!cmd.Update())
		{
		...
		{
			str.Format("Royal rumble is set completed. Minimum Player Count:%d, Maximum Player Count:%d, Start Hour:%d Minute: %d", min, max, startHour, startMinute);
			CNetMsg::SP rmsg(new CNetMsg);
			MsgrSendMsgToCharacterReq(rmsg, charindex, str);
			SEND_Q(rmsg, gserver->m_messenger);
		}
