...
bool CRaidInfo::Load()
{
	if(!m_RaidObjectDataProtoList.Load())	// m_RaidDataProtoList�� Load �ϱ��� m_RaidObjectDataProtoList���� Load �Ͽ��� �Ѵ�.
		return false;
...
int CAreaRaidInfoList::LoadSavePointRaidInfo(int ZoneIndex, int roomindex)
{
	...
	int nSaveInfo;
	cmd.GetRec("a_save_info", nSaveInfo);	// Save�� DB�� index
...
bool CRaidObjectList::Load(int ZoneIndex)
{
	...
	{
		m_nCountRow = cmd.GetRecordCount();
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////
				if(m_pArea->m_zone->m_index == ZONE_AKAN_TEMPLE && m_RaidData[i].GetTodo()->GetProto()->GetObjectType() == TOUCH_FIELD)
				{
					...
					if(bTrigger1 && bTrigger2)
					{
						...
						if(nPCCount == 0)
						{
							m_pArea->m_nDelayOpenGate = gserver->getNowSecond()+5;	//5�� �� ������.
						}
					}
				}
////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	int i, j;
		if (ch->m_pZone->m_index != ZONE_DUNGEON4  &&  ch->m_pZone->m_index != ZONE_CAPPELLA_1 && ch->m_pZone->m_index != ZONE_ALTER_OF_DARK && ch->m_pZone->m_index != ZONE_AKAN_TEMPLE && 
			ch->m_pZone->m_index != ZONE_TARIAN_DUNGEON && ch->m_pZone->m_index != ZONE_ENCHANTED_DUNGEON)  //pwesty new raid
  {
    GAMELOG << init("Hack Raid REGEN error") <<  "name" << delim << ch->m_name << delim << "Zone" << delim <<  ch->m_pZone->m_index << end;
   ch->m_desc->Close("HACK ATTEMPT");
   return;
  }
	switch(type)
	{
	...
	if (!m_pArea->m_zone->m_CNpcRegenRaidList) //Raid regen fix
				return;
	...
	case SUMMON_DISTRICT:
		{
			...
			if (!m_pArea->m_zone->m_CNpcRegenRaidList) //Raid Regen fix
				return;
			CNPC* npc;
		//	int i, RegenCount; 	//commented out for crash fix
			int RegenCount = 0; //crash fix (dethunter12+reza)
			int i; //crash fix
			if (RegenCount == -1) //crash fix
			return;	//crash fix
			if (!m_pArea->m_zone->m_CNpcRegenRaidList)
				return;
