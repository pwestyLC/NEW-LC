...
{
	if (user == NULL || m_rewardIdx < 0 || user->m_RockPaperScissorsInfo.GetGameCount() < 1 || user->m_RockPaperScissorsInfo.GetWinCount() <1)
	{
		return false;
	}
	...
	LOG_INFO("Reward IDx :%d : Win Count %d", m_rewardIdx, winCount);
...
void CRockPaperScissorsGame::SetProbTable(void)
{
	...
	m_totalStageCount = cmd.GetRecordCount();
