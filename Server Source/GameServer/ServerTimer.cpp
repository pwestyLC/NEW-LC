...
void ServerTimerPerSecond::operate( rnSocketIOService* service )
{
    ...
    gserver->m_RLGL.Tick(); //RLGL

	gserver->m_autoEventScheduler.Tick(); //Auto Event Scheduler

	CArea::checkLifeTime();
	gserver->m_donationTierManager.Tick();
	gserver->m_eventAutomationMgr.checkEventTime();
...
void ServerTimerPerHundredMSecond::operate( rnSocketIOService* service )
{
    ...
    // advance scripted NPC movements (100ms tick)
    gserver->UpdateScriptedMovements();
}
...
void ServerTimerPerMinute::operate( rnSocketIOService* service )
{
	...
	gserver->m_buffSystem.TickSystem(); //ken buff system
	//LOG_INFO("DEBUG_FUNC : START");
	gserver->PingDB();
...
void ServerTimerPerMidnight::Init()
{
	...
	time_t newtime = mktime(&nowtm);	// ���� 0�� 0�� 0��
	newtime += 86400;					// ���� 0�� 0�� 0��
