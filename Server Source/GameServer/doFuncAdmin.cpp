...
{
	int time = gserver->m_buffSystem.SetTime(0);
 CNetMsg::SP rmsg(new CNetMsg);
 if(vec.size() > 0) {
  if(vec[0] == "on") {
     if(vec.size()> 1) 
	 time = gserver->m_buffSystem.SetTime(atoi(vec[1].c_str()));
	 gserver->m_buffSystem.TurnOn();
	 gserver->m_buffSystem.TickSystem();
	 char buff[255] = {0};
	 sprintf(buff, "Auto Buffs: ON : Timer : %d mins" , time/60);
	 SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", buff);
  }


  else if(vec[0] == "off") {
   gserver->m_buffSystem.TurnOff();
   SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", "Auto Buffs: OFF");
  }
  else if(vec[0] == "reload") {
   gserver->m_buffSystem.ReloadTable();
   gserver->m_buffSystem.TickSystem();
   SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", "Auto Buffs: Reloaded");
  }
  else 
	{
	  return;
...
void do_GMReloadItem(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	if (!gserver)
		return;

	bool ok = false;
	if (gserver->m_itemProtoList.m_nCount > 0)
	{
		ok = gserver->m_itemProtoList.ReloadSafe();
	}
	else
	{
		ok = gserver->m_itemProtoList.Load();
	}

	CNetMsg::SP rmsg(new CNetMsg);
	if (ok)
	{
		// Post-reload: purge orphaned items from all online players and resend inventories
		int removedCount = 0;
		...
		{
			CPC* pc = (*iter).pPlayer;
			if (pc == NULL || !pc->m_bPlaying || pc->m_desc == NULL)
				continue;

			// Remove orphaned worn items (proto no longer in DB)
			for (int w = 0; w < MAX_WEARING; ++w)
			{
				CItem* wornItem = pc->m_wearInventory.wearItemInfo[w];
				if (wornItem == NULL)
					continue;

				if (gserver->m_itemProtoList.FindIndex(wornItem->m_itemProto->getItemIndex()) == NULL)
				{
					pc->m_wearInventory.RemoveItem(w);
					++removedCount;
				}
			}

			// Remove orphaned inventory items (proto no longer in DB)
			item_search_t allItems;
			pc->m_inventory.searchAllItem(allItems);

			for (int i = (int)allItems.size() - 1; i >= 0; --i)
			{
				CItem* pItem = allItems[i].pItem;
				if (pItem == NULL)
					continue;

				if (gserver->m_itemProtoList.FindIndex(pItem->m_itemProto->getItemIndex()) == NULL)
				{
					pc->m_inventory.deleteItemByItem(pItem);
					++removedCount;
				}
			}

			// Resend full inventory + worn items so clients see updated proto data
				pc->m_inventory.sendInfoToClient();
				pc->m_wearInventory.sendWearInfoToClient();
				pc->CalcStatus(true);
			}

			// Post-reload: purge orphaned items from the ground across all zones/areas
			int groundRemovedCount = 0;
			for (int zi = 0; zi < gserver->m_numZone; ++zi)
			{
				CZone* zone = &gserver->m_zones[zi];
				if (zone == NULL || zone->m_area == NULL)
					continue;

				for (int ai = 0; ai < zone->m_countArea; ++ai)
				{
					CArea* area = &zone->m_area[ai];
					if (area == NULL || area->m_cell == NULL)
						continue;

					for (int cx = 0; cx < area->m_size[0]; ++cx)
					{
						for (int cz = 0; cz < area->m_size[1]; ++cz)
						{
							CItem* item = NULL;
							CItem* itemNext = area->m_cell[cx][cz].m_listItem;
							while ((item = itemNext))
							{
								...
								if (gserver->m_itemProtoList.FindIndex(item->m_itemProto->getItemIndex()) == NULL)
								{
									...
									ItemDisappearMsg(disappmsg, item->getVIndex());
									area->SendToCell(disappmsg, GET_YLAYER(item), cx, cz);

									area->ItemFromCell(item);
									item->m_pArea = NULL;
									delete item;
									++groundRemovedCount;
								}
			...
			std::string msg = boost::str(boost::format("Item reload complete. Removed %d orphaned items from players, %d from ground.") % removedCount % groundRemovedCount);
		SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", msg.c_str());
	}
	else
		SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", "Item reload failed.");
	SEND_Q(rmsg, ch->m_desc);
}

// reload quest prototypes
void do_GMReloadQuest(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
    if (!gserver)
        return;

    bool ok = gserver->m_questProtoList.Load();

    CNetMsg::SP rmsg(new CNetMsg);
    char buf[128];
    if (ok)
        sprintf(buf, "Quest reload complete. Loaded %d quests.", gserver->m_questProtoList.m_nCount);
    else
        sprintf(buf, "Quest reload failed.");

    SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", buf);
    SEND_Q(rmsg, ch->m_desc);
}

// reload zones (calls LoadZone on each zone index)
void do_GMReloadZone(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
    if (!gserver)
        return;

    int reloaded = 0;
    int total = gserver->m_numZone;
    ...
    CNetMsg::SP rmsg(new CNetMsg);
    char buf[128];
    sprintf(buf, "Zone reload complete. Reloaded %d/%d zones.", reloaded, total);
    SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", buf);
    SEND_Q(rmsg, ch->m_desc);
}

// reload NPCs for all zones (calls LoadNPC on every zone)
void do_GMReloadNPC(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
    if (!gserver)
        return;
    ...
    CNetMsg::SP rmsg(new CNetMsg);
    char buf[128];
    sprintf(buf, "NPC reload complete. Reloaded NPCs for %d/%d zones.", reloaded, total);
    SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", buf);
    SEND_Q(rmsg, ch->m_desc);
}

// reload shops for all zones
void do_GMReloadShop(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
    if (!gserver)
        return;
	...
	CNetMsg::SP rmsg(new CNetMsg);
	char buf[128];
	sprintf(buf, "Shop reload complete. Reloaded %d zones.", reloaded);
	SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", buf);
	SEND_Q(rmsg, ch->m_desc);
}

void do_GMReloadScriptedMovements(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	if (!gserver)
		return;

	gserver->ReloadScriptedMovements();

	CNetMsg::SP rmsg(new CNetMsg);
	char buf[256];
	sprintf(buf, "Scripted NPC movements reloaded. %d config(s), %d active task(s).",
		(int)gserver->m_scriptedMovementConfigs.size(), (int)gserver->m_movementTasks.size());
	SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", buf);
	SEND_Q(rmsg, ch->m_desc);

	GAMELOG << init("GM COMMAND", ch) << "reload_scripted_movements" << end;
}
	...
	*tmpBuf = '\0';		// argument for arg
...

	{
		...
		case 2:
			...
			//DB�� ����
			{
				std::string query = boost::str(boost::format("INSERT INTO t_notice_all (a_index, a_notice_string, a_date) values (%d, '%s', now())") % Notice::instance()->getLastRev() % arg);
...
void do_RLGLCMD(CPC* ch, const char* arg, std::vector<std::string>& vec) { //RLGL

    if (!gserver)
        return;

    gserver->m_RLGL.RLGLCMD(ch, arg,vec);


}
void do_WATCH(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	CNetMsg::SP rmsg(new CNetMsg);
	SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "",
		"", "Skill watch list reloaded.");
	SEND_Q(rmsg, ch->m_desc);
}
		...
		for (z = sz; z <= ez; z++)
		{
			...
			while ((tch = tchNext))
			{
				...
				if (IS_NPC(tch))
				{
					...
					if( tnpc->m_proto->m_index == 491 ||				// �ٴڵ����� ���� �Ұ�
							( tnpc->m_flag & NPC_WARCASTLE ) )				// ���� NPC ���� �Ұ�
						continue;
...
void do_GMExpUp(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	if (exppercent < 0 || exppercent > 100)
		return ;
	LONGLONG exp = ch->GetRequiredLevelExp();
...
{
#ifdef EVENT_DOUBLE
	int target = -1;			// ��� ���� �� ��ȣ: -1: all, 1 ~ n: server group
	int	zonenum;			// ��� �� ��ȣ: -1: all, 0 ~ : zonenum
	int subno = -1;
	zonenum = -1;
	char tmpBuf[MAX_MESSAGE_SIZE] = {0,};
...
{
	// ���� �����ϱ�	: /event gball gift <team1 score> <team2 score> <year> <month> <day> <hour> <min>
	//					: ������� ���� �����Ͽ� ������ �Ͻÿ� ���� ����

	// 1. ���� ���� �˻� : GOLDENBALL_STATUS_VOTE_END
	// 2. ���ɾ�� �� ������ �Ľ�
	// 3. ���� �ð��� �Է� �ð� ��
	// 4. �޽����� ����

	// 1. ���� ���� �˻� : GOLDENBALL_STATUS_VOTE_END OR GOLDENBALL_STATUS_GIFT
...
{
	// ���� �����ϱ�	: /event gball vote <team1> <team2> <year> <month> <day> <hour> <min>
	//					: ������� ���� �����Ͽ� ������ �Ͻÿ� ���� ����

	// 1. ���� ���� �˻� : GOLDENBALL_STATUS_NOTHING
	// 2. ���ɾ�� �� ������ �Ľ�
	// 3. ���� �ð��� �Է� �ð� ��
	// 4. �޽����� ����
...
{
	// �����ϱ�		: oxevent start <year> <mon> <day> <hour> <min>
	//				: �ٷ� ���� �����ϸ� min���� �̺�Ʈ ����, �ش� ���� ä�ο��� ����
	// �����ϱ�		: oxevent end
	//				: oxevent �� �ٷ� ����Ǹ�, ������ �������� ��� �ֳ븶���� �̵��Ѵ�., ��ü ������ ����
	// ���� ����	: oxevent quiz <num> <sec>
	//				: num�� �ش��ϴ� ���� ����, ���ѽð� sec ��, sec�� ������ �⺻20��, ��ü ������ ����
	char tmpBuf[MAX_MESSAGE_SIZE] = {0,};
	arg = AnyOneArg(arg, tmpBuf);
	CLCString strSubCommand = tmpBuf;
...
#ifdef EVENT_ITEMDROP_AUTO // /MsgrEventReqMsg �Լ��� �ٸ� �������� ����ϹǷ� �ܺο��� rmsg ������.
		int i;
		for(i = 0; i < 6; i++)
		{
...
void do_GMSilenceMulti(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	if(distance <= 0 || time == nullptr) //pwesty c++17
		return ;
...
void do_GMBattleStop(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	if (guildindex1 < 0 || guildindex2 < 0 || time == nullptr) //pwesty c++17
		return;
...
void do_GMResetRaid(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	// ������ DB ������ �ʱ�ȭ
	{
		...
		{
			CNetMsg::SP rmsg(new CNetMsg);
			HelperRaidInzoneGameDataClearReqMsg(rmsg, ZONE_ENCHANTED_DUNGEON);
			SEND_Q(rmsg, gserver->m_helper);
		}
...
bool ParsingTime(const char*& arg, int start_dt[], int end_dt[])
{
	...
	{
		if(start_dt[0]/4 == 0 && (start_dt[0] % 100 != 0 || start_dt[0] % 400 == 0))		//����
		{
	...
	{
		if(end_dt[0]/4 == 0 && (end_dt[0] % 100 != 0 || end_dt[0] % 400 == 0))		//����
		{
...
void do_GMUpdateEvent(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	gserver->m_event_summon.Load();
	gserver->m_donationTierManager.Load();
	gserver->LoadOXQuiz();
	gserver->LoadPetConfig();
}

void do_GMReloadBattleWarfare(CPC* ch, const char* arg, std::vector<std::string>& vec) 
{
	gserver->m_BattleWarfare.Load();
}


void do_GMServerConfig(CPC* ch, const char* arg, std::vector<std::string>& vec) 
{
	gserver->LoadServerConfig();
}
		...
		case 3888:
			{
				...
				else
				{
					...
					nUpProb[0] = 75 - (nNowPlusValue * 4 ) + nLevelDiff;	// ����
					if( nUpProb[0] < 1 )
						nUpProb[0] = 1;
					nUpProb[1] = nNowPlusValue;							// �϶�
					nUpProb[2] = 25 - nNowPlusValue;						// ����
		...
		case 3889:
			{
				...
				else
				{
					...
					nUpProb[0] = 95 - (nNowPlusValue * 5 ) + nLevelDiff;	// ����
					if( nUpProb[0] < 1 )
						nUpProb[0] = 1;
					nUpProb[1] = 0;											// �϶�
					nUpProb[2] = 0;											// ����
...
void do_GMFunc(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	else if(strcmp(tmpBuf, "raidtodo") == 0)
	{
		if( ch->m_pArea->m_zone->m_index != ZONE_AKAN_TEMPLE
				...
				&& ch->m_pArea->m_zone->m_index != ZONE_ENCHANTED_DUNGEON
		  )
			return ;
	...
	else if(strcmp(tmpBuf, "addgp") == 0)
	{
		...
		arg = AnyOneArg(arg, tmpBuf);
		gp = atoi(tmpBuf);
		if(gp < 0 || gp > 100000) // 0���� �۰ų�, 100,000���� ũ�� return
			return ;
		if( ch
				&& NULL != ch->m_guildInfo
	...
	arg = AnyOneArg(arg, tmpBuf);
	addbp = atoi(tmpBuf);
	if (addbp < 0 || addbp > 1000000) // 0º¸´Ù ÀÛ°Å³ª, 1,000,000º¸´Ù Å©¸é return
		return;
	...
	else if (strcmp(tmpBuf, "reset_ps") == 0)
	{
	int season = 0;//Cloud add Season
	CPrestige* ps = gserver->FindPrestigeByLevel(1);
	if (ps)
	{
		season = ps->m_season;
	}
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Prestige Reseted");
		SEND_Q(rmsg, ch->m_desc);
	}
	}

	else if (strcmp(tmpBuf, "reload_ps") == 0)
	{
	gserver->LoadPrestige();
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Prestige Reloaded");
		SEND_Q(rmsg, ch->m_desc);
	}
	}
	else if (strcmp(tmpBuf, "addps") == 0)
	{
	int addps = 0;
	arg = AnyOneArg(arg, tmpBuf);
	addps = atoi(tmpBuf);
	if (addps < 0 || addps > 1000000) // 0���� �۰ų�, 1,000,000���� ũ�� return
		return;

	ch->m_prestigePoints += addps;
	{
		CNetMsg::SP rmsg(new CNetMsg);
		rmsg->Init(MSG_EXTEND);
		RefMsg(rmsg) << MSG_EX_STRING
	...
	else if (strcmp(tmpBuf, "reload_partyconfig") == 0)
	{

	if (gserver->m_PartyConfig.Load())
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Expeditions Reloading...");
		SEND_Q(rmsg, ch->m_desc);
	}
	}
#endif
...
void do_GMEchoNpc(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	// npc index
	int npcIndex = 0;		// 0 �̸� �ֺ� ��� NPC, -1 �ֺ� ��ȭ, -2 �ֺ� ��ȭX, Ư�� index
	arg = AnyOneArg(arg, tmpBuf, true);
...
void do_GMAttendanceChange(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	int attentDay; // �⼮��
	int attentMax; // �ִ� �⼮��
	int rewardPoint; // ���������¥. ( 50 / 100 / 150 / 200 / 250 / 300 / 365 )
...
void do_GMJobChange(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	ch->m_statpt_int = 0;
	ch->m_statpt_con = 0;
	ch->m_statpt_remain = (ch->m_level - JOB_2ND_LEVEL) * 3 + (ch->m_level - JOB_2ND_LEVEL) / 2 + (JOB_2ND_LEVEL - 1);
	ch->m_statpt_remain += ch->m_stats_addition;

#ifdef DEMIGOD
	if ( ch->m_bPhoenix_Char == 1 || ch->m_bPremium_Phoenix == 1 || ch->m_DemiGod == 1)
#else
	if (ch->m_bPhoenix_Char == 1 || ch->m_bPremium_Phoenix == 1)
#endif
	{
		ch->m_statpt_remain += 30;
	}

	{
        ...
        CLCString sql(1024);
        char g_buf[8192];
        sql.Format("SELECT * FROM `t_skill` WHERE  `a_job` = %i AND `a_job2` = %i", ch->m_job, ch->m_job2);
        CDBCmd cmd;
        CDBCmd cmd1;
        CDBCmd cmd2;
        ...
        sql.Format("SELECT * FROM `t_skill` WHERE  `a_job` = %i AND `a_job2` = 0", ch->m_job);
        cmd1.SetQuery(sql);
		...
		int passiveSkills[21] = { 2019,2020,2021,2022,2023,2024,2025,2026,2027,2028,2029,2030,2031,2032,2033,2034,2035,2036,2037,2038,2039 };
		int i;
		bool hasPassiveSkill = false;
		int existingPassive = 0;
		int existingPassive2 = 0;
		int existingPassive3 = 0;
		for (i = 0; i < 21; i++)
		{
			if (ch->m_passiveSkillList.Find(passiveSkills[i]))
			{
				hasPassiveSkill = true;
				existingPassive = passiveSkills[i];
			}
		}
        ...
        {
            sql.Format("SELECT `a_level`, `a_learnItemIndex1`, `a_learnItemCount1` FROM `t_skillLevel` WHERE  `a_learnLevel` <= %i AND `a_index` = %i ORDER BY `a_level` DESC LIMIT 1", ch->m_level, Skill[i].index);
            cmd2.SetQuery(sql);
            ...
            else if (Skill[i].active == 0)
            {
				...
				if (learnItemIndex != -1)
				{
					...
					int sc = ch->m_inventory.searchItemByCondition(learnItemIndex, 0, 0, vec);
					if (sc == 0 || sc < learnItemCount)
					{
						continue;
					}
					ch->m_inventory.deleteItem(vec, learnItemCount);
				}
	...
	if (!dbChar.Update())
	{
		DBLOG	<< init("DB ERROR : UPDATE CHARACTER", ch)
				<< mysql_error(dbChar.GetDbConn()) << delim
				<< "JOB" << delim << ch->m_job
				<< end;
	}
...
void do_GMItemGetRange(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	{
		if (dbindex == NAS_ITEM_DB_INDEX) // nas�� �������� ����
			continue;
...
void doGMRoyalRumbleSetting(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	* ���� 2. max�� min ���� ū ����̾�� �Ѵ�.
	* ���� 3. max�� 40���� Ŭ �� ����.
	* ���� 4. startHour�� 0 ~ 23�� ����̾�� �Ѵ�.
	* ���ɾ� ��Ģ
	* /royalrumble <min> <max> <startHour> <startMinute>
	*/
	...
	if(CRoyalRumble::checkSettingValue(nParam[0], nParam[1], nParam[2], nParam[3]) != CRoyalRumble::kValueIsValid)
	{
		message.Format("RoyalRumble setting values is invalid.");
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", message);
	...
	*/
	{
		CNetMsg::SP rmsg(new CNetMsg);
		MsgrRoyalRumbleSettingReq(rmsg, nParam[0], nParam[1], nParam[2], nParam[3], ch->m_index);
		SEND_Q(rmsg, gserver->m_messenger);
	}
...
void do_GMJewelLoad(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	gserver->m_jewelDataList.Load();
}
...
{
	LONGLONG levelup_exp = ch->GetRequiredLevelExp();
...
void do_GoNextPC(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	if (cpc_vec.empty())
	{
		previous_char_index = 0; // ���� 1���� �ְ�, ������ ã�ư����� ���� ��� - �ٽ� ã�ư��� ���ؼ�
...
void do_VirtualReform(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	CItem* maginfireItem = ch->m_inventory.FindByDBIndex(1839); // Ȳ�� ������
...
void do_GM_guildPoint(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	LONGLONG guildpoint = (LONGLONG)ATOLL(vec[1].c_str());
...
void do_GM_apetAccExp(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	LONGLONG charLevelupExp = ch->GetRequiredLevelExp();
...
void do_GM_PremiumChar_Reset(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	LOG_INFO("PREMIUM_CHAR_RESET (by GM command) - charIndex : %d", ch->m_index);	// ���� �˻��� �����ϰ� �ϱ� ���� �۾�
...
void do_raid_changeMaxAreaCount(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	if(zone->m_index == ZONE_ALTER_OF_DARK || zone->m_index == ZONE_CAPPELLA_1 || zone->m_index == ZONE_AKAN_TEMPLE
		|| zone->m_index == ZONE_TARIAN_DUNGEON || zone->m_index == ZONE_ENCHANTED_DUNGEON //pwesty
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
#ifndef CLOUD_EXPEDITION_SOLO_DUNGEONS
		|| zone->m_index == ZONE_PARTY_DUNGEON0
		|| zone->m_index == ZONE_PARTY_DUNGEON1
		|| zone->m_index == ZONE_PARTY_DUNGEON2
		|| zone->m_index == ZONE_PARTY_DUNGEON3
		|| zone->m_index == ZONE_PARTY_DUNGEON4
#endif
#endif				
		)
	{
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
void do_spawnRAIDNPC(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	char tmpBuf[MAX_MESSAGE_SIZE] = { 0, };
	...
	*tmpBuf = '\0';
	...
	arg = AnyOneArg(arg, tmpBuf);
	int DifficultyNPC = atoi(tmpBuf);
	arg = AnyOneArg(arg, tmpBuf);
	int iLayer = atoi(tmpBuf);

	std::string query = boost::str(boost::format("INSERT INTO t_npc_regen_raid(a_zone_num, a_npc_idx, a_pos_x, a_pos_z, a_pos_h, a_pos_r, a_y_layer, a_store_middle) values(%d, %d, %f, %f, %f, %f, %d, %d)")
		% ch->m_pZone->m_index % IdNPC % ch->m_pos.m_x % ch->m_pos.m_z % ch->m_pos.m_h % ch->m_pos.m_r % iLayer % DifficultyNPC);
	DBManager::instance()->pushQueryForDataDB(0, query);

	GAMELOG << init("New-RAID NPC-Regen") << "USE GM COMMAND : CHAR_ID : "
...
void do_GM_donationEvent(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	const char* type;
	tm t;
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, ch->m_index, "", "", "Subhelper not running");
		SEND_Q(rmsg, ch->m_desc);
		return;
	}
	type = vec[0].c_str();
	if (strcmp(type, "start") == 0)
	{
		if (vec.size() < 11)
			goto UNKNOWN_FORMAT;
		if (gserver->m_donationTierManager.GetEventInfo().IsActive)
		{
			CNetMsg::SP rmsg(new CNetMsg);
			SayMsg(rmsg, MSG_CHAT_GM, ch->m_index, "", "", "Donation event already started");
			SEND_Q(rmsg, ch->m_desc);
			return;
		}

		t.tm_year = atoi(vec[1].c_str()) - 1900;
		t.tm_mon = atoi(vec[2].c_str()) - 1;
		t.tm_mday = atoi(vec[3].c_str());
		t.tm_hour = atoi(vec[4].c_str());
		t.tm_min = atoi(vec[5].c_str());
		t.tm_sec = 0;
		t.tm_isdst = -1;
		startTime = mktime(&t);
		t.tm_year = atoi(vec[6].c_str()) - 1900;
		t.tm_mon = atoi(vec[7].c_str()) - 1;
		t.tm_mday = atoi(vec[8].c_str());
		t.tm_hour = atoi(vec[9].c_str());
		t.tm_min = atoi(vec[10].c_str());
		t.tm_sec = 0;
		t.tm_isdst = -1;
		endTime = mktime(&t);
		if (startTime >= endTime)
			goto UNKNOWN_FORMAT;
		CNetMsg::SP rmsg(new CNetMsg);
		SubHelperUpdateDonationEventStatus(rmsg, true, startTime, endTime);
		SEND_Q(rmsg, gserver->m_subHelper);
	}
	else if (strcmp(type, "stop") == 0)
	{
		...
		{
			CNetMsg::SP rmsg(new CNetMsg);
			SayMsg(rmsg, MSG_CHAT_GM, ch->m_index, "", "", "Donation event not started");
			SEND_Q(rmsg, ch->m_desc);
			return;
		}
		CNetMsg::SP rmsg(new CNetMsg);
		time_t curTime = time(0);
		SubHelperUpdateDonationEventStatus(rmsg, false, curTime, curTime);
		SEND_Q(rmsg, gserver->m_subHelper);
	}
	else if (strcmp(type, "reload") == 0)
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SubHelperUpdateDonationEventReward(rmsg, ch->m_index);
		SEND_Q(rmsg, gserver->m_subHelper);
	}
	else
	{
...
UNKNOWN_FORMAT:
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, ch->m_index, "", "", "Unknown command format");
		SEND_Q(rmsg, ch->m_desc);
	}
}
...
void do_spawnSHOP(CPC* ch, const char* arg, std::vector<std::string>& vec) //dethunter12 spawn shop 
{
	char tmpBuf[MAX_MESSAGE_SIZE] = { 0, };
...
void do_banPlayer(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	std::string query;
	{
		...
		LOG_INFO("Found user %d", cmd.GetRecordCount());
...
void do_blockPlayer(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	if (arg == NULL)
		return;
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_NOTICE, 0, "", "", "can't block myself");
		SEND_Q(rmsg, ch->m_desc);
		return;
	}
...

	{
		...
		query = boost::str(boost::format("SELECT a_mac FROM t_mac_log WHERE a_user_code = %d ORDER BY a_index DESC LIMIT 1") % userCode);
		LOG_INFO("Query %s", query.c_str());
		cmd.SetQuery(query);
		...
		cmd.GetRec("a_mac", mac);
	}

	{
		query = boost::str(boost::format("INSERT INTO t_mac_block (a_mac, a_description) VALUES ('%s','%s') ") % mac.c_str() % kickedName.c_str());
		LOG_INFO("Query %s", query.c_str());
		CDBCmd cmd;
		cmd.Init(&gserver->m_dbauth);
		cmd.SetQuery(query);
...
#ifdef REZAPCHA
void do_rezap(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	if (vec.size() > 0)
	{
		if (vec[0] == "test")
		{
			...
			CLCString OtherPlayer(100);
			OtherPlayer = vec[1].c_str();
			for (int i = 2; i < vec.size(); i++) {

				OtherPlayer.Format("%s %s", OtherPlayer.GetBuffer(), vec[i].c_str());
			}

			//get name and pointer to target player

			CLCString name = "";
			CPC* pPC = PCManager::instance()->getPlayerByName(OtherPlayer, 1);
			if (pPC == NULL) {
				pPC = PCManager::instance()->getPlayerByName(OtherPlayer, 0);
			...
			//open test ui on the target player
			//gserver->m_Rezapcha.RezapchaOpen(pPC);

			//send command user confirmation 

			CLCString Msg(255);
			Msg.Format("Sent Test to player %s", pPC->m_name.GetBuffer());

			SendRezaMessage(ch, Msg.GetBuffer(), SystemMsgType::SYSMSG_CYAN);
		...
		else if (vec[0] == "jail")
		{
			...
			CLCString Msg(255);
			Msg.Format("%s Player: %s", (jail == 1 ? "Jailed" : "Unjailed"), pPC->m_name.GetBuffer());
		...
		else if (vec[0] == "lock")
		{
			...
			CLCString Msg(255);
			Msg.Format("%s Player: %s", (lock == 1 ? "Locked" : "Unlocked"), pPC->m_name);
