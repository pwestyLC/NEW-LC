...
void process_expressInputItem(CNetMsg::SP& msg)
{
	...
	if (packet->nas <= 0 && packet->item_index > 0) // ������ ������ ��� item index�� ��ȿ�� ����
	{
...
void process_use_unsearchable_stonestatue(CPC* ch, CNetMsg::SP& msg)
{
	...
	if (((sendItem->m_itemProto->getItemFlag() & ITEM_FLAG_COMPOSITE) || (sendItem->getFlag() & FLAG_ITEM_TRANSMOGRIFY)) && (sendItem->getFlag() & FLAG_ITEM_COMPOSITION))
	{
	...
	if (sendItem->Count() < packet->itemCount || packet->itemCount <= 0) //Fix Express System Exploit
	{
		CNetMsg::SP rmsg(new CNetMsg);
