...
// 		SEND_Q( rmsg, ch->m_desc );
	...
	if(ch->m_pZone && ch->m_pArea
					&& (ch->m_pZone->m_index != ZONE_GUILDROOM))
	{
...
void do_newGuildStashTake( CPC* ch, CNetMsg::SP& msg )
{
	...
	if (packet->takeMoney == 0 && (packet->listCount <= 0 || packet->listCount >5 ))
	{
		std::string str = boost::str(boost::format(
