...
void do_PremiumChar_Jump(CPC* ch, CNetMsg::SP& msg)
{
	...
	if (ch->CanWarp() != 0 || ch->IsInvisible())
	{
		CNetMsg::SP rmsg(new CNetMsg);
	...
	if (ch->m_pZone->m_index == ZONE_PK_TOURNAMENT || pToChar->m_pZone->m_index == ZONE_PK_TOURNAMENT
		|| ch->GetMapAttr() & MATT_FREEPKZONE || pToChar->GetMapAttr() & MATT_FREEPKZONE
		|| ch->GetMapAttr() & MATT_WAR || pToChar->GetMapAttr() & MATT_WAR || pToChar->m_admin == 10
		|| ch->m_pZone->isRVRZone() || pToChar->m_pZone->isRVRZone()
		|| ch->IsSetPlayerState(PLAYER_STATE_PKMODE | PLAYER_STATE_PKMODEDELAY | PLAYER_STATE_RAMODE) || pToChar->IsSetPlayerState(PLAYER_STATE_PKMODE | PLAYER_STATE_PKMODEDELAY | PLAYER_STATE_RAMODE)
