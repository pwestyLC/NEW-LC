...
void do_Raid_InzoneJoin(CPC* ch, CNetMsg::SP& msg)
{
	...
	if(nDifficulty == -1 && nZoneNo == ZONE_TARIAN_DUNGEON)
		nDifficulty = 0;
	if (nDifficulty == -1 && nZoneNo == ZONE_ENCHANTED_DUNGEON)
		nDifficulty = 0;
	...
	int MemberCount = ((int)gserver->m_PartyConfig.GetDungeonMember(nZoneNo, nDifficulty));
#ifndef CLOUD_EXPEDITION_SOLO_DUNGEONS	
	if (nZoneNo == ZONE_PARTY_DUNGEON0 && (!ch->IsParty() || ch->IsParty() && ch->m_party->GetMemberCount() < MemberCount))//-4 add para debug
	{
		//에러: 해당 존은 파티 존
		CNetMsg::SP rmsg(new CNetMsg);
		RaidErrorMsg(rmsg, MSG_RAID_ERROR_INZONE_JOIN_THISZONE_PARTYZONE);
	...
	if (nZoneNo == ZONE_PARTY_DUNGEON1 && (!ch->IsParty() || ch->IsParty() && ch->m_party->GetMemberCount() < MemberCount))
	{
		//에러: 해당 존은 파티 존
		CNetMsg::SP rmsg(new CNetMsg);
		RaidErrorMsg(rmsg, MSG_RAID_ERROR_INZONE_JOIN_THISZONE_PARTYZONE);
	...
	if (nZoneNo == ZONE_PARTY_DUNGEON2 && (!ch->IsParty() || ch->IsParty() && ch->m_party->GetMemberCount() < MemberCount))
	{
		//에러: 해당 존은 파티 존
		CNetMsg::SP rmsg(new CNetMsg);
		RaidErrorMsg(rmsg, MSG_RAID_ERROR_INZONE_JOIN_THISZONE_PARTYZONE);
	...
	if (nZoneNo == ZONE_PARTY_DUNGEON3 && (!ch->IsParty() || ch->IsParty() && ch->m_party->GetMemberCount() < MemberCount))
	{
		//에러: 해당 존은 파티 존
		CNetMsg::SP rmsg(new CNetMsg);
		RaidErrorMsg(rmsg, MSG_RAID_ERROR_INZONE_JOIN_THISZONE_PARTYZONE);
	...
	if (nZoneNo == ZONE_PARTY_DUNGEON4 && (!ch->IsParty() || ch->IsParty() && ch->m_party->GetMemberCount() < MemberCount))
	{
		//에러: 해당 존은 파티 존
		CNetMsg::SP rmsg(new CNetMsg);
		RaidErrorMsg(rmsg, MSG_RAID_ERROR_INZONE_JOIN_THISZONE_PARTYZONE);
	...
	if (ch->m_inventory.FindByDBIndex(4913, 0, 0))
	{
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
		if (((ch->m_level < (int)gserver->m_PartyConfig.GetDungeonLevel(nZoneNo, nDifficulty) - 10)))
		{
			CNetMsg::SP rmsg(new CNetMsg);
			RaidErrorMsg(rmsg, MSG_RAID_ERROR_INZONE_JOIN_INVALIDELEVEL);
			SEND_Q(rmsg, ch->m_desc);
		...
		if (nZoneNo == ZONE_ENCHANTED_DUNGEON	//Val add new raid, we keep ramslo level limit of 175
			&& (ch->m_level < RAID_JOIN_LEVEL_LOW_LIMIT_TARIAN_DUNGEON - 10)
			&& nDifficulty == DUNGEON_DIFFICULTY_NORMAL)
		{
			CNetMsg::SP rmsg(new CNetMsg);
		...
		else if (nZoneNo == ZONE_ENCHANTED_DUNGEON	//Val add new raid, we keep ramslo level limit of 175
			&& (ch->m_level < RAID_JOIN_LEVEL_LOW_LIMIT_TARIAN_DUNGEON)
			&& nDifficulty == DUNGEON_DIFFICULTY_NORMAL)
		{
			CNetMsg::SP rmsg(new CNetMsg);
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
		if (((ch->m_level < (int)gserver->m_PartyConfig.GetDungeonLevel(nZoneNo, nDifficulty))))
		{
			CNetMsg::SP rmsg(new CNetMsg);
			RaidErrorMsg(rmsg, MSG_RAID_ERROR_INZONE_JOIN_INVALIDELEVEL);
			SEND_Q(rmsg, ch->m_desc);
...
#ifndef CLOUD_EXPEDITION_SOLO_DUNGEONS	
	if (ch->IsParty() &&
		(nZoneNo == ZONE_PARTY_DUNGEON0
			|| nZoneNo == ZONE_PARTY_DUNGEON1
			|| nZoneNo == ZONE_PARTY_DUNGEON2
			|| nZoneNo == ZONE_PARTY_DUNGEON3
			|| nZoneNo == ZONE_PARTY_DUNGEON4
			)
		&&
		ch->m_party->m_nDifficulty > -1 &&
		ch->m_party->m_nDifficulty != nDifficulty)
	{
		GAMELOG << init("ERROR WORNG DIFFICULTY", ch)
...
#ifdef CLOUD_EXPEDITION_PARTY_DUNGEONS
		else if ((ch->m_nJoinInzone_ZoneNo == ZONE_PARTY_DUNGEON0
			|| ch->m_nJoinInzone_ZoneNo == ZONE_PARTY_DUNGEON1
			|| ch->m_nJoinInzone_ZoneNo == ZONE_PARTY_DUNGEON2
			|| ch->m_nJoinInzone_ZoneNo == ZONE_PARTY_DUNGEON3
			|| ch->m_nJoinInzone_ZoneNo == ZONE_PARTY_DUNGEON4) && nZoneNo == -1)	// 현재 존: 예배당 1, 2
		{
			nZoneNo = ZONE_START;
			if (nRebirth == 1)
			{
			...
			|| ch->m_nJoinInzone_ZoneNo == ZONE_PARTY_DUNGEON2
			|| ch->m_nJoinInzone_ZoneNo == ZONE_PARTY_DUNGEON3
			|| ch->m_nJoinInzone_ZoneNo == ZONE_PARTY_DUNGEON4) && nZoneNo != -1)	// 예배당에서 예배당 지하로 간다.
		{
			if (nZoneNo == ZONE_START)
				nExtra = 0;
			else
				goto ERROR_RETURN;
		}
...
//				nExtra = 0;
		...
		else if (ch->m_nJoinInzone_ZoneNo == ZONE_ENCHANTED_DUNGEON)	//Val add new raid return zone to 44 (enchanted forest)
		{
			if (nExtra == -1)
				nExtra = 1;
			// 아칸사원에서 존을 나갈 때 정보..
			if (nRebirth == 1)
			{
				nZoneNo = ZONE_MISTY_CANYON_EXTREME;
				//				nExtra = 0;
			}
			else if (nRebirth == 0)
			{
				nZoneNo = ZONE_MISTY_CANYON_EXTREME;		// 에게하로 튕겨버려!
//				nExtra = 0;
			}
			else
				goto ERROR_RETURN;
