...
void do_StashKeep(CPC* ch, CNetMsg::SP& msg)
{
	...
	for (int i = 0; i < packet->keepCount; ++i)
	{
		...
		item[i] = pitem;
		LONGLONG fee = (LONGLONG)packet->list[i].count * 100;
		if (fee < 0 || fee / 100 != packet->list[i].count)
		{
			LOG_ERROR("HACKING? : stash fee overflow. char_index[%d]", ch->m_index);
			ch->m_desc->Close("stash fee overflow");
			return;
		}
		needmoney += fee;
		if (needmoney < 0)
		{
			LOG_ERROR("HACKING? : needmoney overflow. char_index[%d]", ch->m_index);
			ch->m_desc->Close("needmoney overflow");
			return;
		}
	}
