...
void do_ExTitleSystemMake(CPC* ch, CNetMsg::SP& msg)
{
	...
	int total_seals = packet->total_seals;
	int* option_index = new int[MAX_TITLE_SEAL];
	int* option_level = new int[MAX_TITLE_SEAL];
	int option_count = 0;
	...
	if (total_seals > 0 && total_seals <= MAX_TITLE_SEAL) {

		LOG_INFO("Create a new custom title with seals: %d", total_seals);

		option_count = total_seals;

		int totalAbsSeals = 0;
		for (int i = 0; i < total_seals; i++) {
			CItem* pItem = ch->m_inventory.getItem(packet->sealsTab[i], packet->sealsInvenIdx[i]);
			if (pItem->getDBIndex() >= 11489 && pItem->getDBIndex() <= 11498) 
				totalAbsSeals++;
		}

		if (totalAbsSeals > 1) {
			LOG_INFO("Cannot duplicate seal %d", totalAbsSeals);
			CNetMsg::SP rmsg(new CNetMsg);
			TitleSystemMsg(rmsg, MSG_EX_TITLE_SYSTEM_CANNOT_DUPLICATE_SEAL, -1, ch->m_index, -1);
			SEND_Q(rmsg, ch->m_desc);
			return;
		}
		...
		for (int i = 0; i < total_seals; i++) {
			...
			if (pItem == NULL) {
				LOG_INFO("HACKING? invalid DBIndex. char_index[%d]", ch->m_index);
				ch->m_desc->Close("invalid DBIndex");
				return;
			}

			if (pItem->m_itemProto->getItemSubTypeIdx() == IONCE_SEAL) {
				option_index[i] =  pItem->m_itemProto->getItemNum0();
				option_level[i] = pItem->m_itemProto->getItemNum1();
				LOG_INFO("Make Option %d Level %d", option_index[i], option_level[i]);
...
void do_ExTitleSystemMake_return_DBThread(boost::any& any)
{
	...
	for(int i = 0; i < MAX_TITLE_SEAL; i++)
		LOG_INFO("Make OK Option %d Level %d", title->option_index[i], title->option_level[i]);
