...
bool CServer::LoadSettingFile()
{
	...
	strTemp.Format("%s%s", m_serverpath, "helper.txt");
...
//		const char* pland = guildland;
		...
		int landcount = Castle.GetRecordCount();
		...
		std::string select_guild_member_query = boost::str(boost::format(
				"SELECT g.*, eg.a_position_name, eg.a_contribute_exp, eg.a_contribute_fame, eg.a_contribute_exp_min as a_contribute_exp_min, eg.a_contribute_exp_max as a_contribute_exp_max, eg.a_contribute_fame_min as a_contribute_fame_min, eg.a_contribute_fame_max as a_contribute_fame_max, eg.a_point"
