...
// ��� ���� ������
	...
	case MSG_HELPER_TEACHER_LOAD_REQ:
		...
		break;
		//�޸��ɸ� �̺�Ʈ
	case MSG_HELPER_SEARCHFRIEND_ADD_REQ:			//�޸��ɸ� ����ϴ� �κ�
		do_CommandEventSearchFriendMemberAddReq(msg, dest);
		break;
	case MSG_HELPER_SEARCHFRIEND_SELECT_ADD_REQ:			// �ɸ� ��� ����
		do_CommandEventSearchFriendMemberSelectAddReq(msg, dest);
		break;
	case MSG_HELPER_SEARCHFRIEND_ONETIME_ADD_REQ:			// �ѽð� ���� üũ
		do_CommandEventSearchFriendMemberOneTimeCheckReq(msg, dest);
		break;
	case MSG_HELPER_SEARCHFRIEND_LISTGOOD_REQ:			// ��ϵ� ģ�� ���󰡴ɿ���
...
#ifdef DEMIGOD
	case MSG_HELPER_EVENT_DEMIGOD:
		do_CommandEventDemiGod(msg, dest);
		break;
#endif
...
void do_CommandTeacherGiveup(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	// ���⼭�� �л��� teach_type�� MSG_TEACH_NO_STUDENT_TYPE�� �����ϰ� do_TeachCancel�� ȣ���Ѵ�.
...
///////////////////////////////////////////
// Rank DB control in personal dungeon 4(������ ��) : bw
//
...
void do_CommandPD4RankInit(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	while(charjob < JOBCOUNT)
	{
		...
		do
		{
			...
			if(rewardCmd.Open() && rewardcmd.GetRecordCount() > 0)
				continue;
#endif
...
void do_CommandGuildCreateReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if (prev == NULL)
	{
		...
		if(checkcmd.Open() && checkcmd.GetRecordCount() > 0)
		{
			CNetMsg::SP rmsg(new CNetMsg);
...
void do_CommandGuildLevelUpReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if( guild->level() + 1 >= GUILD_EXTEND_BASE_LEVEL )
	{
		...
		std::string update_extend_guild = boost::str(boost::format(  // was -%d pwesty
...
void do_CommandGuildChangeBossReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildChangeBossRepMsg(rmsg, guildindex, current, change, MSG_GUILD_ERROR_NOTFOUNDMEMBER);  // ���ִ� �ȵ�
		SEND_Q(rmsg, dest);
		return ;
	}
	...
	if (!cmd.Update() || cmd.GetRecordCount() < 1)
	{
		cmd.Rollback();
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildChangeBossRepMsg(rmsg, guildindex, current, change, MSG_GUILD_ERROR_NOTFOUNDMEMBER);
...
void do_CommandGuildBattleStopReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if(isAccept == true)
	{
...

		{
			...
			if( itr != gserver.m_guildstash.end() )		// ã�Ҵ�
			{
...
void do_CommandEventMoonStoneUpdateReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if(gserver.m_nMoonStoneNas > maxNas)//1000�� �̻� �ȸ��̵��� �Ѵ�.
	...
	if (cmd.Open())
	{
		...
		if (cmd.GetRecordCount() > 0) // update
		{
...
void do_CommandFriendMemberAddReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	LOG_INFO("FriendAddReq: requesterindex=%d reqname=%s reqjob=%d | approvalindex=%d appname=%s appjob=%d",
		packet->requesterindex, (const char*)packet->reqname, packet->reqjob,
		packet->approvalindex, (const char*)packet->appname, packet->appjob);
	//DB�� �����ؼ� ó��..
...
void do_CommandBlockCharReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	cmd.SetQuery(select_characters_query);
	if( !cmd.Open() || cmd.GetRecordCount() < 1 || !cmd.MoveFirst())
	{
		CNetMsg::SP rmsg(new CNetMsg);
...
void do_CommandGiftCharReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if( !cmd.Open() || cmd.GetRecordCount() < 1 )
	{
		CNetMsg::SP rmsg(new CNetMsg);
...
void do_CommandPetCreateReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if( cmd.GetRecordCount() >= MAX_OWN_PET )
	{
		CNetMsg::SP rmsg(new CNetMsg);
...
void do_CommandGuildStashHistoryReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if (cmd.Open() && cmd.GetRecordCount())
	{
		int i = 0;
...
void do_CommandEventSearchFriendMemberAddReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if (!(cmd.Open() && cmd.MoveFirst()))
	{
		...
		{   //Ȥ�ö� t_event_searchfriend_date���� �� ��¥�� �ҷ����� ������ ���..����.
			GAMELOG << init("EVENT SEARCH FRIEND ERROR_GAMESERVER ")
					<< approval_nick << delim
					<< request_nick << delim
					<< request_datestamp << delim
...
void do_CommandPartyQuitReq(CNetMsg::SP& msg, CDescriptor* dest)
{
		...
		nBossIndex == nTargetIndex)
	{
		...
		for(i = 0; i < MAX_PARTY_MEMBER; i++) // ���� �ĺ� ã��
		{
			member = pParty->m_listMember[i];
			if(member)
			{
...
void do_CommandPartyChangeTypeReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if(cAllOneSet==MSG_PARTY_SETALLONE_ALL)	//��ǥ Ÿ�� ����
	{
		pParty->SetPartyTypeAll(cPartyType);
	}else if(cAllOneSet==MSG_PARTY_SETALLONE_ONE)	 //����ȭ 1�� ����
	{
...
void do_CommandPartyMatchInviteReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	// ��Ƽ Ÿ���� PARTY_TYPE_BATTLE�̸� tch�� ch�� ���� ���̸� �˻�
	// ���̰� +- 10�̻��̸� ERROR;
...
void do_CommandGuildMemberAdjust( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	if( gMember )
	{
		...
		{
			LONGLONG gp = gMember->guild()->GetGuildPoint();
			if(needGP >= gp)
				gp = 0;
			else
				gp = gp - needGP;
...
void do_CommandNewGuildMemberList( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	membercount = cmd.GetRecordCount();
	int i = 0;
...
void do_CommandCubeStateReq( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	RefMsg(rmsg) << (char)cmd.GetRecordCount();
	while(cmd.MoveNext())
	{
		cmd.GetRec("a_guild_index", guildidx);
		cmd.GetRec("a_name", strname);
...
void do_CommandCubeStateReq( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	rank = 0;
	nRemainSendCount = (char)cmd.GetRecordCount();
...
{
	int nUserIdx = 0, nCharIdx = 0;
	RefMsg(msg) >> nUserIdx >> nCharIdx;

	CDBCmd DB_cmd;
	DB_cmd.Init(&gserver.m_dbchar);

	bool hasLevel100 = false;
	bool hasMaxLevel = false;

	// Combined query: grab all characters, levels, phoenix state
	char query[1024] = { 0 };
	sprintf(query,
		"SELECT a_index, a_level, a_phoenix, a_premium_phoenix_active "
		"FROM t_characters WHERE a_user_index = %d",
		nUserIdx);
	DB_cmd.SetQuery(query);
	...
	{
		int idx = 0, level = 0, tmpPhoenix = 0, tmpPremPhoenix = 0;
		DB_cmd.GetRec("a_index", idx);
		DB_cmd.GetRec("a_level", level);
		DB_cmd.GetRec("a_phoenix", tmpPhoenix);
		DB_cmd.GetRec("a_premium_phoenix_active", tmpPremPhoenix);
	...
	if (!charFound)
	{
		LOG_INFO("Phoenix Event: Char not found: userIdx=%d, charIdx=%d", nUserIdx, nCharIdx);
		CNetMsg::SP rmsg(new CNetMsg);
		HelperEventPhoenixMsg(rmsg, MSG_HELPER_EVENT_PHOENIX_ERROR_REQUIREMENT, nCharIdx);
		SEND_Q(rmsg, dest);
		return;
	...
	LOG_INFO("Phoenix Event: Checking for charIdx=%d | phoenix=%d | premium=%d | hasLvl100=%d | hasMaxLvl=%d",
		nCharIdx, phoenix, premPhoenix, hasLevel100, hasMaxLevel);

	// Phoenix check
	if (phoenix == 0)
	{
		// Regular phoenix: needs at least one level 100+ char
		if (hasLevel100)
		{
			CNetMsg::SP rmsg(new CNetMsg);
	...
	else if (phoenix == 1 && premPhoenix == 0)
	{
		// Premium phoenix: needs at least one max level char
		if (hasMaxLevel)
		{
			CNetMsg::SP rmsg(new CNetMsg);
			HelperEventPhoenixMsg(rmsg, MSG_HELPER_EVENT_PHOENIX_OK, nCharIdx);
...
#ifdef DEMIGOD
void do_CommandEventDemiGod(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	// 100 �����̻��� ĳ���Ͱ� �ִٴ� ���� ǥ��
	bool level_200_up = false;

	char select_characters_query[1024] = { 0, };
	// �ڽ��� ���� ���� ��� �ɸ����� �ε����� �����´�.
	sprintf(select_characters_query, "SELECT a_index, a_demigod_active FROM t_characters WHERE a_user_index=%d ", nUserIdx);
	DB_cmd1.SetQuery(select_characters_query);
	if (!DB_cmd1.Open())
		return;

	query_count1 = DB_cmd1.GetRecordCount();
	if (query_count1)
	{
	...
	else
	{
		// ������ ĳ���͸� �ҷ����� ���ؼ� �ڰ� �̴���
		CNetMsg::SP rmsg(new CNetMsg);
		HelperEventDemiGodMsg(rmsg, MSG_HELPER_EVENT_DEMIGOD_ERROR_REQUIREMENT, nCharIdx);
		SEND_Q(rmsg, dest);
		return;
	}
	...
	if (level_200_up)
	{
		...
		sprintf(select_characters_query_2, "SELECT a_demigod_active FROM t_characters WHERE a_index=%d ", nCharIdx);
		...
		DB_cmd3.MoveFirst();
		DB_cmd3.GetRec("a_demigod_active", demigod);

		LOG_INFO("DemiGod %d", demigod);
		// �Ǵн� �̺�Ʈ ���� ������ ĳ���� �̶��
		if (demigod == 0)
		{
			// �ڰ��� ����
			CNetMsg::SP rmsg(new CNetMsg);
			HelperEventDemiGodMsg(rmsg, MSG_HELPER_EVENT_DEMIGOD_OK, nCharIdx);
			SEND_Q(rmsg, dest);
			return;
		}
		else
...
void do_Command_DisconnectHackCharacter( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	} else if( nHack_type == HACK_TYPE_MOVE_SPEED )
	{
		strType_hack.Format("move hack speed");
	} else if( nHack_type == HACK_TYPE_ATTACK_SPEED )
	{
		strType_hack.Format("attack hack speed");
	}
	else
		return;
...
void do_Command_Exped_CreateReq( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	if(it != gserver.m_listParty.end())
	{
		...
		if(pExped)
		{
			...
			{
				int nRealCount = pExped->GetMemberCount() - 1;	//�������� ����

				int* pCharIndex		 = new int[nRealCount];			// ĳ���� �ε���
				CLCString* pCharName = new CLCString[nRealCount];	// ĳ���� �̸�
				int*  pGroupType	 = new int[nRealCount];			// �׷� Ÿ��
				int*  pMemberType	 = new int[nRealCount];			// ��� Ÿ��
				int*  pSetLabelType	 = new int[nRealCount];			// ǥ��
				int*  pQuitType		 = new int[nRealCount];			// ����, ������
...
void do_Command_Raid_InzoneDataClearReq( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	if (nZoneNo == ZONE_ENCHANTED_DUNGEON)
	{
		sql.Format("DELETE FROM t_trigger_saveinfo WHERE a_zone_index = %d", nZoneNo);
		cmd.SetQuery(sql);
		if (!cmd.Update())
...
void do_CommandTriggerEvent( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	else if (DB_cmd.GetRecordCount() > 0)
	{
...
void do_Command_RaidInfo( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	// ����� ���̵��� ���� ������ ���Ѵ�.
	nRaidCount = DB_cmd.GetRecordCount();
...
void do_Worldcup2010Event(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	switch( nSubType )
	{
	case MSG_HELPER_WORLDCUP2010_TOTO_REQ: // ���� ����
...
void do_TeachRenewerLimitTime(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	for (i=0; i<cnt; i++) // ��ϵ� �İ��� �߿� �ð��� �������� ã�´�.
	{
		...
		cmd.MoveFirst();
		if( cmd.GetRecordCount() == 1 ) // 10���� �����ٸ� 1�� ���� �� �ۿ� ����.
		{
			CLCString buf(256);
			int tIDX, gCnt;
...
void do_CommandGuildStashList(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if( cmd.Open() && cmd.MoveFirst() )
	{
		RefMsg(rmsg) << cmd.GetRecordCount(); // item count
...
void do_CommandGuildStashKeep(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	GAMELOG << init("GUILD_STASH_SECURITY") << "CLIENT STASH INDEX : " << charindex << delim << "GuildIndex" << delim << guildindex << end;
	...
	if( itr != gserver.m_guildstash.end() )		// ã�Ҵ�
	{
		...
		if( ptmpGuildStash )
		{
			if( !ptmpGuildStash->GetLock(charindex) )
			{
				// �̹� ���ɸ�
				CNetMsg::SP rmsg(new CNetMsg);
				HelperGuildStashKeepMsg( rmsg, charindex, guildindex, 3, 0, 0 );  // ������Դϴ�.
				SEND_Q( rmsg, dest);
				return;
			}
		}
	...
	else		// ó�� �ε�
	{
		...
		pGuildStash->GetLock(charindex);		// ���θ��� 100% ����
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildStashKeepMsg( rmsg, charindex, guildindex, 1, 0, 0 );  // �׷���� �����ϴ�.
		SEND_Q( rmsg, dest);
	...
	if( !pGuildStash->CheckStashSpace(&gserver.m_dbchar) || !pGuildStash->KeepToDB(&gserver.m_dbchar) )		// ������ ���ų� ������Ʈ ����
	{
		pGuildStash->RemoveItem();// ������ ����
		if( !pGuildStash->UnLock(charindex) ) // Unlock ���� Log �����
		{
		}
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildStashKeepMsg( rmsg, charindex, guildindex, 9, 0, 0 ); // ��� â�� ���� ����
		SEND_Q( rmsg, dest);
		return;
	}

	// ���� �Ϸ� GameServer �� Message ���� ( ItemList ���� ), ������ Lock �ɸ� ����
	CNetMsg::SP rmsg(new CNetMsg);
...
void do_CommandGuildStashKeepError(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if( itr != gserver.m_guildstash.end() )		// ã�Ҵ�
	{
		...
		if( !pGuildStash->CanUnLock(charindex) )		// Lock �ǻ���� �ƴϸ�
			return;

		if( 0 != error )	// â���� ������ ������, ������ ���޿� ����
		{
			pGuildStash->TakeFromDB(&gserver.m_dbchar);		// DB �� �ðܳ��� �������� temp �������� �������� ����.
		}
		else
		{
			// �α��ѹ� ���� ���ô�.
...
void do_CommandGuildStashTake(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	LOG_INFO("Guildstash enable %d", nEnable);
	if( nEnable == 0 ) // â���� �̿��� �� �����ϴ�.
	{
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildStashErrorMsg( rmsg, charindex, 6 );
	...
	if( itr != gserver.m_guildstash.end() )		// ã�Ҵ�
	{
		...
		if( ptmpGuildStash )
		{
			if( !ptmpGuildStash->GetLock(charindex) )
			{
				// �̹� ���ɸ�
				CNetMsg::SP rmsg(new CNetMsg);
				HelperGuildStashTakeMsg( rmsg, charindex, guildindex, 4, 0, 0 );  // 4: �ٸ������� ������Դϴ�.
				SEND_Q( rmsg, dest);
				return;
			}
		}
	...
	LOG_INFO("Guildstash check end");

	if( itr == gserver.m_guildstash.end() )
	{
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildStashTakeMsg( rmsg, charindex, guildindex, 1, 0, 0 );  // �׷���� �����ϴ�.
		SEND_Q( rmsg, dest);
		return;
	}
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildStashTakeMsg( rmsg, charindex, guildindex, 1, 0, 0 );  // 1: �׷���� �����ϴ�.
		SEND_Q( rmsg, dest);
		return; // ������ ����
	}
...

	{
		...
		{
			CNetMsg::SP rmsg(new CNetMsg);
			HelperGuildStashTakeMsg( rmsg , charindex, guildindex, 5, 0, 0 ); // 5: ã���� ���� ������
			SEND_Q( rmsg, dest );
			return;
		}
			...
			if( cmd.Open() && cmd.MoveFirst() )
			{
				...
				else
				{
					...
					if( dbCount > 1 || stashItemCount > 1 )
						pGuildStash->SetItemProtoFlag( itemDBIndex , 1);			// Count�Ҽ� �ֵ��� �÷��� ����
				}
			}
			else	// ���� �������̳�..
			{
	...
	{
		// ���� �߸��Ǿ��� error !!
		pGuildStash->KeepToDB(&gserver.m_dbchar); // dB ����
		pGuildStash->RemoveItem();
		pGuildStash->UnLock(charindex);		//  UnLock
		return;
	}
...
void do_CommandGuildStashTakeError(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if( itr != gserver.m_guildstash.end() )		// ã�Ҵ�
	{
		...
		if( 0 != error )	// â���� ������ ������, ������ ���޿� ����
		{
			pGuildStash->KeepToDB(&gserver.m_dbchar);	// temp �� �ִ� ������ DB �� ����
		}
		else
		{
...
void do_CommandGuildStashLog(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		HelperGuildStashLogMsg( rmsg, charindex, guildindex, cmd.GetRecordCount() );
