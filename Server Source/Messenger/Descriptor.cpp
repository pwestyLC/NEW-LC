...
void CDescriptor::operate( rnSocketIOService* service )
{
	...
	// Log the raw type/subtype
	if (msg && msg->m_size > 1) {
		uint8_t type = msg->m_buf[0];
		uint8_t subType = msg->m_buf[1];
		LOG_INFO("GMTOOL MSG Received: len=%d, type=%u, subType=%u", msg->m_size, type, subType);
	}
