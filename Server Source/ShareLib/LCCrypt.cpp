...
uint32_t CNM_MakeSeedForClient(const char* strID, const char* strPW, uint32_t nTickCount)
{
	time_t timeDummy;
	time(&timeDummy);

	uint32_t nBitShuffle = LCDES3_GetRandom32Bit((uint32_t)timeDummy);
	uint32_t nMethod = LCDES3_GetRandom32Bit(nBitShuffle);
	uint32_t nID = (uint32_t)(strID[0]);
	uint32_t nPW = (uint32_t)(strPW[0]);
	...
	uint32_t nInitSeed = nMethod | nID | nPW | nTickCount;
	uint32_t nRet = nBitShuffle << 28;

	for (int i = 0; i < 28; i++)
	{
		if ((nInitSeed >> i) & 1)
		{
			nRet |= (1U << tableShuffle[nBitShuffle][i]);
		}
	}
...
uint32_t CNM_MakeSeedForServer(uint32_t nRandomValue, uint32_t nServerPulse)
{
	time_t timeDummy;
	time(&timeDummy);
	...
	uint32_t nInitSeed = nMethod | nRandomValue | nServerPulse;
	uint32_t nRet = nBitShuffle << 28;
