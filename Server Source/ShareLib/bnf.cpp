...
bnf::bnf() :io_service_(8), io_service_work_( new boost::asio::io_service::work( io_service_ ) )
	, stopped_(false)
	, run_flag_(false)
...
void bnf::Run( int worker_thread_count )
{
	...
	{
		m_threads.push_back(thread_group_.create_thread(boost::bind(&bnf::WorkerThread, this)));
	...
	catch( std::exception&  yurface) 
	{
		LOG_ERROR(yurface.what());
	}
	catch(boost::exception &e)
	{
		LOG_ERROR(boost::diagnostic_information(e, true).c_str());
	}
	LOG_INFO( "bnf - worker thread end." );

	io_service_.stop();

	// session process A取﹞aRun
...
session_handle bnf::CreateConnect( std::string& host, std::string& port, void* user_data )
{
	...
	LOG_INFO( "bnf [%s] CreateConnect - connected. fd: %d, handle: %d", pSession->ip().c_str(), pSession->Socket().native_handle(), pSession->GetHandle() );
