...
inline void updateSettings(CNetMsg::SP& msg, int autopickup, int autoigni, int autosell)
{
    settings* packet = reinterpret_cast<settings*>(msg->m_buf);
    packet->type = MSG_UPDATE_DATA_FOR_CLIENT;
    packet->subType = MSG_SUB_UPDATE_SETTINGS;
    packet->autopickup = autopickup;
    packet->autoigni = autoigni;
    packet->autosell = autosell;
    msg->setSize(sizeof(settings));
}
