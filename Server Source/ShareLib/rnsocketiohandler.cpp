...
bool ServiceHandler::connect(const std::string& host, tINT port)
{
    ...
    service_ = static_cast<rnSocketIOService*>(bnf::instance()->GetSessionPointer(chandle));
    return true;
}

bool ServiceHandler::asyncConnect(const std::string& host, tINT port)
{
    if (service_ != nullptr)
        return false;

    session_handle handle = bnf::instance()->CreateAsyncConnect(const_cast<std::string&>(host), port, this);
    ...
    service_ = static_cast<rnSocketIOService*>(bnf::instance()->GetSessionPointer(handle));
    return true;
}