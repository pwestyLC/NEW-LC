...
{
 #ifndef _WIN64
	int oldflags = fcntl(socket_.native_handle(), F_GETFD, 0);
	if (oldflags >= 0)
	{
		fcntl(socket_.native_handle(), F_SETFD, oldflags | FD_CLOEXEC);
	}
#endif
}
...
void rnSocketIOServiceTcp::_getIp()
{
	...
	if (error)
	{
		LOG_ERROR("bnf - socket_.remote_endpoint error. error[%s] / fd[%d] / handle[%X]",
			error.message().c_str(), socket_.native_handle(), GetHandle());
		return;
	}
...
void rnSocketIOServiceTcp::notusedSetReadWaitTimer()
{
...
void rnSocketIOServiceTcp::notusedSetWriteWaitTimer()
{
	if (this->io_service_.stopped())
		return;
...
//LOG_INFO( "TCP(RELIABLE) [%d] IP : %s :",packet_header_.reliable, ip().c_str());

	//possible fix to deny any logins not fitting the proper header criteria reza
...
void rnSocketIOServiceTcp::HandleReadBody(const boost::system::error_code& error)
{
	...
	// 	now_packet_->crc32_flag_ = true; // ?   ? ?(?)                   ?    ?       CRC32                

	now_packet_->m_ptr = 1;	//   ?  ?    u  ¡Æ     ?     m_type              ?                ?    ? .
	//////////////////////////////////////////////////////////////////////////
...
void rnSocketIOServiceTcp::ReadWaitTimerClose(/*const boost::system::error_code& error*/)
{
	...
	//read_wait_timer_.expires_from_now(boost::posix_time::seconds(10));  //pwesty
	read_wait_timer_.async_wait(boost_strand_.wrap(boost::bind(&rnSocketIOServiceTcp::ReadWaitTimerClose, this)));
	/* june
		if (error)
			return;
