...
bool ReservedGmCommandManager::Init()
{
	...
	std::string qry = boost::str(boost::format(
									 ...
									 " FROM t_reserved_gm_command WHERE a_server=%1% ORDER BY a_index")
								 % gserver.m_serverno);
	dbChar.SetQuery(qry.c_str());
	if (dbChar.Open() == false)
		return false;

	LOG_INFO("Total calendar %d", dbChar.GetRecordCount());
	...
	while (dbChar.MoveNext())
	{
		...
		// LOG_INFO("Calendar - %s %d %d %d %d", ele.a_title, ele.a_evnetType, ele.a_image_type, ele.a_image_x, ele.a_image_y);
...
{
	{
		...
		for (; it != endit; ++it)
		{
			...
			if (d.ele.a_evnetType == eEVENT_TYPE_INFO) // �ܼ� ���� ��¿��̸� �˻����� ����
				continue;
...
void ReservedGmCommandManager::sendListToClient( rnSocketIOService* service, int m_Index, int year, int month )
{
	...
	for (; it != endit; ++it)
	{
				...
				(d.end_year == packet->year && d.end_month == packet->month))
		{
			...
			memset(packet->ele[count].a_start_title, 0x00, sizeof(packet->ele[count].a_start_title));
			memset(packet->ele[count].a_end_title, 0x00, sizeof(packet->ele[count].a_end_title));

			// LOG_INFO("Calendar packet - %s %d %d %d %d", packet->ele[count].a_title, packet->ele[count].a_evnetType, packet->ele[count].a_image_type, packet->ele[count].a_image_x, packet->ele[count].a_image_y);
	...
	LOG_INFO("Calendar send packet %d", count);
