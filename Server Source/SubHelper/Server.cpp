...
bool CServer::LoadSettingFile()
{
	...
	strTemp.Format("%s%s", m_serverpath, "subhelper.txt");
...
bool CServer::LoadSettings()
{
	...
	GAMELOG << init("SYSTEM")
			<< "Load donation event settings..."
			<< end;
	if (!gserver.m_donationEvent.Load())
	{
		LOG_ERROR("Donation event settings loading failed");
		return false;
	}
...
bool CServer::CopyToTradeAgentItemInfo()
{
	...
	itemList.reserve(cmd.GetRecordCount());
