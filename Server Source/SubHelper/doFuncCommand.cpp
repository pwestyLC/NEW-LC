...
void do_Command(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	case MSG_SUBHELPER_DONATION_EVENT_UPDATE_STATUS:
		do_CommandUpdateDonationEventStatus(msg, dest);
		break;
	case MSG_SUBHELPER_DONATION_EVENT_UPDATE_REWARD:
		do_CommandUpdateDonationEventReward(msg, dest);
		break;
	default:
		break;
	}
...
////////////////////////////////////////
	...
	else
	{
		...
		{
			// [selo: 101109] errortype 2 �� ���� ����
			// ���� ����ÿ����� dest=NULL �� ���־� SEND_Q ���� ���׳� ������ ���� �κ��� ���ش�.
			// ��� ���� ������ �����Ѵ�.
...
void do_Command_LCBallinfo( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	int coinCount = cmd.GetRecordCount() ;
	...
	RefMsg(rmsg) << (char) cmd.GetRecordCount();		// �ڽ� ����
	while( cmd.MoveNext() )
	{
		char	curCourse =0;
		int		courseItem=0;
...
void do_Command_LCBalluse( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	COURSEITEM * pItem = new COURSEITEM [cmd.GetRecordCount()];
	...
	int getIdx = 0;
	for( idx=0; idx<cmd.GetRecordCount() ; idx++ )
	{
	...
	if (totalRemain >= 1)
		...
		sql.Format("UPDATE t_lcball SET a_item_remain = 0 WHERE a_tocken_index = %d and a_course_code = %d and a_order=%d and a_item_index = %d; ", coinIndex, courseIndex, getIdx, pItem[getIdx].itemIndex);
		  cmd.SetQuery( sql );
    	if (!cmd.Update() )
...
void do_CommandExtCharacterSlotItemUse(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	int _remainTime = 0;
	unsigned char _ret = 0; // 0: ����, 1: ����, 2: �ܿ��ð� ����
...
void do_CommandUpdateDonationEventStatus(CNetMsg::SP& msg, CDescriptor* dest)
{
	bool isActive;
	...
	{
		CDBCmd db;
		db.Init(&gserver.m_dbData);
		std::string sql = boost::str(boost::format("UPDATE t_donation_event SET a_start_date = FROM_UNIXTIME(%1%), "
			"a_finish_date = FROM_UNIXTIME(%2%), a_enable = 1;")
			% startTime % endTime);
		db.SetQuery(sql);
		db.Update();
	}
	gserver.m_donationEvent.UpdateStatus(isActive, startTime, endTime);
	if (!isActive)
	{
		gserver.m_donationEvent.FinishEvent();
	}
...
void do_CommandUpdateDonationEventReward(CNetMsg::SP& msg, CDescriptor* dest)
{
	gserver.SendToAllGameServer(msg);
