  ...
  internal class Smc
  {
    ...
    public static void MakeList()
    {
      for (int index = 0; index < Smc.SMCFile.Count<tSmcRecords>(); ++index)
        Smc.Menu.Add(Smc.SMCFile[index].ID.ToString() + " - ");
    }

    public static bool ReadFile(string FileName)
    {
      Smc.openedFile = FileName;
      Encoding encoding = Encoding.GetEncoding("windows-874");
      try
      {
        ...
        using (BinaryReader binaryReader = new BinaryReader((Stream) File.Open(FileName, FileMode.Open)))
        {
          ...
          while (binaryReader.BaseStream.Position < binaryReader.BaseStream.Length)
          {
            ...
            if (num1 != 0)
            {
              ...
              {
                ID = num1 - 1,
                Name = encoding.GetString(binaryReader.ReadBytes((int) binaryReader.ReadInt16())),
              ...
              for (int index1 = 0; index1 < num2; ++index1)
              {
                tModels tModels = new tModels()
                {
                  ...
                  ModelFile = encoding.GetString(binaryReader.ReadBytes((int) binaryReader.ReadInt16())),
                ...
                for (int index2 = 0; index2 < num3; ++index2)
                {
                  ...
                  {
                    TexID = index2 + 1,
                    TextureName = encoding.GetString(binaryReader.ReadBytes((int) binaryReader.ReadInt16())),
                    TextureFile = encoding.GetString(binaryReader.ReadBytes((int) binaryReader.ReadInt16()))
                  };
        ...
        using (BinaryWriter binaryWriter = new BinaryWriter((Stream) output))
        {
          ...
          for (int ItemID = Smc.SMCFile.Min<tSmcRecords>((Func<tSmcRecords, int>) (p => p.ID)); ItemID <= Smc.SMCFile.Max<tSmcRecords>((Func<tSmcRecords, int>) (p => p.ID)); ++ItemID)
          {
            ...
            if (index1 != -1)
            {
              ...
              binaryWriter.Write((short) encoding.GetBytes(tSmcRecords.Name).Length);
              binaryWriter.Write(encoding.GetBytes(tSmcRecords.Name));
              ...
              for (int index2 = 0; index2 < tSmcRecords.Models.Count<tModels>(); ++index2)
              {
                ...
                binaryWriter.Write((short) encoding.GetBytes(model.ModelFile).Length);
                binaryWriter.Write(encoding.GetBytes(model.ModelFile));
                ...
                for (int index3 = 0; index3 < model.Textures.Count<tTextures>(); ++index3)
                {
                  ...
                  binaryWriter.Write((short) encoding.GetBytes(texture.TextureName).Length);
                  binaryWriter.Write(encoding.GetBytes(texture.TextureName));
                  binaryWriter.Write((short) encoding.GetBytes(texture.TextureFile).Length);
                  binaryWriter.Write(encoding.GetBytes(texture.TextureFile));
                }
              }
            }
            else
