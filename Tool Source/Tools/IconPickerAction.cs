...
// Type: LcDevPack_TeamDamonA.Tools.IconPickerAction
...
{
  public class IconPickerAction : Form
  {
    ...
    public IconPickerAction() => this.InitializeComponent();
    ...
    private void IconPicker_Load(object sender, EventArgs e)
    {
      ...
      for (int index = 0; index < ((IEnumerable<string>) files).Count<string>(); ++index)
      {
        ...
        if (((IEnumerable<string>) source).Count<string>() == 2 && source[1].ToLower().Contains("actionbtn"))
          this.comboBox1.Items.Add((object) Path.GetFileNameWithoutExtension(files[index]));
      }
      if (this.comboBox1.Items.Count > 0)
        this.comboBox1.SelectedIndex = 0;
      this.comboBox1.SelectedItem = (object) ("ActionBtn" + this.OldItemBtnSelect.ToString());
    }
    ...
    private void pictureBox1_MouseClick(object sender, MouseEventArgs e)
    {
      int x = e.X;
      int y = e.Y;
      this.TexID = Convert.ToInt32(0);
      this.TexRow = (int) Math.Floor((double) y / 32.0);
      this.TexColumn = (int) Math.Floor((double) x / 32.0);
      this.SelectedIcon.Image = (Image) this.databaseHandle.IconAction(this.TexID, this.TexRow, this.TexColumn);
    }
    ...
    private void InitializeComponent()
    {
      ...
      this.ClientSize = new Size(521, 347);
      this.Controls.Add((Control) this.button2);
      this.Controls.Add((Control) this.button1);
      this.Controls.Add((Control) this.groupBox1);
      this.Controls.Add((Control) this.pictureBox1);
      this.Controls.Add((Control) this.comboBox1);
      this.Name = nameof (IconPickerAction);
      this.Text = nameof (IconPickerAction);
