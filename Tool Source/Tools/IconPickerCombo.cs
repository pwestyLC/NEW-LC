...
// Type: LcDevPack_TeamDamonA.Tools.IconPickerCombo
...
{
  public class IconPickerCombo : Form
  {
    ...
    public IconPickerCombo() => this.InitializeComponent();
    ...
    private void IconPicker_Load(object sender, EventArgs e)
    {
      ...
      for (int index = 0; index < ((IEnumerable<string>) files).Count<string>(); ++index)
      {
        ...
        if (((IEnumerable<string>) source).Count<string>() == 2 && source[1].ToLower().Contains("combobtn"))
          this.comboBox1.Items.Add((object) Path.GetFileNameWithoutExtension(files[index]));
      }
      if (this.comboBox1.Items.Count > 0)
        this.comboBox1.SelectedIndex = 0;
      this.comboBox1.SelectedItem = (object) ("ComboBtn" + this.OldItemBtnSelect.ToString());
    }
    ...
    private void pictureBox1_MouseClick(object sender, MouseEventArgs e)
    {
      ...
      this.TexRow = (int) Math.Floor((double) y / 50.0);
      this.TexColumn = (int) Math.Floor((double) x / 50.0);
      this.SelectedIcon.Image = (Image) this.databaseHandle.IconCombo(this.TexID, this.TexRow, this.TexColumn);
    }
    ...
    {
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (IconPickerCombo));
      ...
      this.SelectedIcon.Size = new Size(50, 50);
      ...
      this.Name = nameof (IconPickerCombo);
      this.Text = nameof (IconPickerCombo);
